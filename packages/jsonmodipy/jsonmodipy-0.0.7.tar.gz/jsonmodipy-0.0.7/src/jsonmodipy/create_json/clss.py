"""Creates a .json backup of a class in a .py file if it exists and is not yet
backed up."""
from typeguard import typechecked


@typechecked
def create_src_code_class_backup_json(filepath: str) -> None:
    """
    0. Verifies the target file exists.
    1. Verifies the target code is of valid Python syntax.
    1. Verifies the class exists within the Python syntax.
...
# TODO: verify class documentation makes sense.
@typechecked
def generate_json_from_py_class(
    *,
    filedir: str,
    filename: str,
    class_name: str,
    ...
    1. Makes the a json of the class named:
    <filedir>/<filename>_class_<class>_class_<iteration>.json
    2. Applies black formatting to the Json.
    3. Verifies the <filedir>/<filename>_class_<class>_<iteration>.json can
    ...
    file.
    """
    print("TODO: Generate Json code from class in code of Python file.")
    print(filedir)
    print(filename)
    print(class_name)
    print(iteration)
