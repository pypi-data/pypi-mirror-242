...
def list(organization, detail):
    with connect_to_ckan() as ckan:
        ...
        if detail:
            ...
            filtered_list = [key['title'] for key in r['results']]
            click.echo(filtered_list)
...
def download(id, path):
    with connect_to_ckan() as ckan:
        download_url = f'{ckan.address.rstrip("/")}/dataset/{id}/download_all'
        request_header={'Authorization': ckan.apikey}
        response = requests.get(download_url, headers=request_header, stream=True)
        ...
        if response.ok:
            ...
            click.echo(f'Download successfully. File path: {file_path}')
        else:
            click.echo(f'Download failed. Response: {response.status_code} {response.reason}')
...
def push(data):
    with connect_to_ckan() as ckan:
        data_dict = json.loads(data)
        r = ckan.action.resource_create(**data_dict)
        click.echo(r)
