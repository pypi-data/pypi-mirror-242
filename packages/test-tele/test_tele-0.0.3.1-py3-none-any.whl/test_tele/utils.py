...
from telethon.tl.types import messages, MessageEntityTextUrl
from telethon.tl.custom.message import Message

from test_tele import __version__
from test_tele import storage as st
# from test_tele.bot.bot_header import get_count_album, get_back_to_default, get_user_setting
...
# Download Module
# async def gallery_dl(recipient: EntityLike, tm: "TgcfMessage", **kwargs):
#     """Start subprocess gallery-dl"""

#     commands = await get_user_setting(recipient, **kwargs)
...
#                         category, link, link_list = await get_count_album(item)

#                         # if category:
#                         #     tm.text = f"[source]({link})"
#                         #     tm.text += f"\n#{category['category']}"
...
#                             n += 1
#                             if len(urls) % 10 == 0:
#                                 # tm.text += f'\n@ttloli_bot' if n == count else None
#                                 tm.new_file = urls
...
#                             for url in urls:
#                                 tm.text = url
#                                 await start_download(recipient, tm)
#                             return
#                         # tm.text += f'\n@ttloli_bot'
...
# async def start_download(recipient: EntityLike, tm: "TgcfMessage", **kwargs):
#     """Split task to download with gallery-dl"""
#     logging.warning(f"tes haloooo")
#     logging.warning(f"sampe sini bisa ? {kwargs}")
#     asyncio.create_task(gallery_dl(recipient, tm, **kwargs))
