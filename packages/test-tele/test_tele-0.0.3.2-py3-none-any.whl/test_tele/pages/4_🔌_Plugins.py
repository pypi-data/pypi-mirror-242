...
from test_tele.plugin_models import FileType, Replace, Style, Language, SendAs
from test_tele.password import check_password
from test_tele.utils_webui import get_list, get_string, hide_st
...
if check_password(st):
    ...
    with st.expander("Special Rizumiya"):
        CONFIG.plugins.special.check = st.checkbox(
            "Apply this plugin", value=CONFIG.plugins.special.check
        )
        CONFIG.plugins.special.download = st.checkbox(
            "Download images from url (send as album)", value=CONFIG.plugins.special.download
        )
        CONFIG.plugins.special.translate = st.checkbox(
            "Translate text", value=CONFIG.plugins.special.translate
        )
        language_type = [item.value for item in Language]
        CONFIG.plugins.special.lang = st.selectbox(
            "Choose language", language_type, index=language_type.index(CONFIG.plugins.special.lang)
        )
        send_as_Type = [item.value for item in SendAs]
        CONFIG.plugins.special.send_as = st.selectbox(
            "Choose message delivery format* [BETA - Working fine in past mode]", send_as_Type, index=send_as_Type.index(CONFIG.plugins.special.send_as)
        )
        st.caption("*) specifically, 'send as album' can only be done on media messages")
