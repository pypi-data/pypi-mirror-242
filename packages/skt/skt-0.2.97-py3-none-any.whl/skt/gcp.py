...
def get_bigquery_client(credentials=None, project_id=PROJECT_ID, location=LOCATION):
    ...
    job_config_labels = {
        "nes_client_id": str(os.environ.get("CLIENT_ID", "")).lower(),
        "jupyterhub_user": str(os.environ.get("JUPYTERHUB_USER", "")).lower(),
    }
...
def bq_table_to_parquet(
        dataset, table_name, output_dir, col_list="*", partition=None, where=None, mode="overwrite",
