...
def find_span_by_id(span_id):
    return spans_by_id.get(span_id)


def start_and_track_span(tracer, name):
    span = tracer.start_span(name)
    spans_by_id[span.get_span_context().span_id] = span
    return span
...
class LLMonitorCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(
        ...
        try:
            ...
            self.__tracer = llmonitor.tracer
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        if self.__has_valid_config is False:
            return
        try:
            start_and_track_span(self.__tracer, run_id)
            parent_run = find_span_by_id(parent_run_id)
            parent_run_id = getattr(parent_run, "span_id", parent_run_id)
