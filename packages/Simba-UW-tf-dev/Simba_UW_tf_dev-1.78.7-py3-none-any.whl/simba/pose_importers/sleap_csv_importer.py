...
class SLEAPImporterCSV(ConfigReader, PoseImporterMixin):
    ...
    def __init__(self,
        ...
        if self.animal_cnt > 1:
            ...
            self.data_and_videos_lk = dict([(get_fn_ext(file_path)[0], {'DATA': file_path, 'VIDEO': None}) for file_path in self.input_data_paths])
    ...
    def run(self):
        for file_cnt, (video_name, video_data) in enumerate(self.data_and_videos_lk.items()):
            ...
            if 'instance.score' in self.data_df.columns:
                self.data_df = self.data_df.drop('instance.score', axis=1)
