    ...
    async def __aiter__(self) -> AsyncIterator[dict]:
        ...
        try:
            while not self._manually_closed:
                try:
                    # Wait for a message with a 10-second timeout
                    message = await asyncio.wait_for(self.recv(), 10)
                    yield message
                except asyncio.TimeoutError:
                    # Break the loop if no message is received within 10 seconds
                    print("No message received for 10 seconds, exiting loop.")
                    break
