...
def open_model_file(gui=None):
    ...
    if file_path is not None:
        generator_model = Generator(in_channels=10, features=64, image_size=32, control_size=64)
...
def check_state_plan(gui=None):
    ...
    control =  torch.from_numpy(np.asarray([float(i) for i in plan_size_var.get().split(',')])).float()
    control = control.unsqueeze(0)

    full_control = torch.randn(1, 64).float()

    sdr_action_step_tensor = torch.argmax(generator_model(sdr_goal_tensor, full_control).view(1, 2, 32, 32).detach(), dim=1).view(1, 1, 32, 32)
...
def create_goal_idea():
    ...
    goal_idea = Idea(_id=0, name="Goal", value=[float(i) for i in plan_size_var.get().split(',')], _type=0)
    init_node_idea = Idea(_id=1, name="initialNode", value=float(init_node_var.get()), _type=1)
    intermidiate_idea = Idea(_id=2, name="intermediateGoal", value=[float(i) for i in intermidiate_var.get().split(',')], _type=1)
    final_goal_idea = Idea(_id=3, name="finalGoal", value=[float(i) for i in final_goal_var.get().split(',')], _type=1)
    context_idea = Idea(_id=4, name="lastActionSteps", value="", _type=0)
...
if __name__ == '__main__':
    ...
    plan_size_var = create_text_field(gui, 4, "Nodes:")    
