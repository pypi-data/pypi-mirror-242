...
def cb(chunk, message, verbose: bool = False):
    global tokens
        ...
        while True:
            ...
            if user_prompt:
                if not_called:
                    ...
                    messages, _ = ai(
                        user_prompt, stream_callback=partial(cb, verbose=verbose), is_question=True
                    )
                    not_called = False
                else:
                    ...
                    print(f"\n{bold}{red}AI{reset}{red}: {bold}{blue}Thinking...{reset}\n")
                    ai.try_call(
                        user_prompt, stream_callback=partial(cb, verbose=verbose), messages=messages
                    )
                user_prompt = None
            else:
                while True:
