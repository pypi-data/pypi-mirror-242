...
chill_dump_sql_file = Path("chill-dump.sql")
...
def _load_data(app):
    ...
    if chill_dump_sql_file.is_file() and chill_sql_dump_tables:
        chill_data_files.insert(0, str(chill_dump_sql_file))
    with app.app_context():
        ...
        with db:
            ...
            if chill_sql_dump_tables:
                chill_dump_sql_file.touch()
                tmp_db_file = tempfile.mkstemp()[1]

                tmp_db = sqlite3.connect(tmp_db_file)
                with tmp_db:
                    cur = tmp_db.cursor()
                    cur.executescript(chill_dump_sql_file.read_text())
                    tmp_db.commit()

                    other_tables = list(map(lambda x: x[0], cur.execute("""
                        SELECT name FROM sqlite_schema WHERE type ='table' AND name NOT LIKE 'sqlite_%' AND name LIKE :chill_sql_dump_tables;
                    """, dict(chill_sql_dump_tables=chill_sql_dump_tables)).fetchall()))
                    cur.close()

                os.unlink(tmp_db_file)
                if len(other_tables):
                    chill_dump_backup = str(chill_dump_sql_file) + ".bak"
                    app.logger.info(
                    ...
                    with open(chill_dump_backup, 'w') as f:
                    ...
                    # load
                    if chill_dump_sql_file.is_file():
                        cur = db.cursor()
                        cur.executescript(chill_dump_sql_file.read_text())
                        db.commit()
