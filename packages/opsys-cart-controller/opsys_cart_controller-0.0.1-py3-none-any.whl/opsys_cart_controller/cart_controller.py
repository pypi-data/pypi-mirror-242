...
class CartController(CartAbstract):
    ...
    def __init__(self):
        ...
        Args:
            laser_com (int): LRF COM port number
            ps_com (int): Motor PS COM port number
            deviation (int): Diff between measured and configured distance
            velocity (int): Cart motor velocity
            accel (int): Motor acceleration
            decel (int): Motor Deceleration
            timeout (int, optional): Movement step timeout. Defaults to 60.
        """
        self.laser_com = LaserConfigurations.COM
        self.ps_com = PowerSupplyConfigurations.COM
        self.deviation = CartConfigurations.DEVIATION
        self.velocity = CartConfigurations.VELOCITY
        self.accel = CartConfigurations.ACCEL
        self.decel = CartConfigurations.DECEL
        self.timeout = CartConfigurations.TIMEOUT
