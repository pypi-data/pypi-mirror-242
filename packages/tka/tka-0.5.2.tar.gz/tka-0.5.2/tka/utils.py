...
def _find_valid_moshkov_assays(auc_modality_filter):
    with importlib.resources.path(
        "tka.data", f"auc_modalities_{auc_modality_filter['version']}.csv"
    ) as file_path:
...
def load_mobc_ordered_feature_columns(version: str = "v4"):
    """
    ...
    """
    with importlib.resources.path(
        "tka.data", f"mobc_feature_columns_ordered_{version}.txt"
    ) as file_path:
