    ...
    if len(cycles) > 0:
        ...
        cycle = {
            **_create_cycle(cycle, include_matrix),
            ...
            'emissions': non_empty_list(map(_format_aggregate(_new_emission(primary_product, site_type)), emissions)),
...
def _sum_blank_nodes(blank_nodes: list):
    return {
        **blank_nodes[0],
        'value': [list_sum(flatten([n.get('value', []) for n in blank_nodes]))],
