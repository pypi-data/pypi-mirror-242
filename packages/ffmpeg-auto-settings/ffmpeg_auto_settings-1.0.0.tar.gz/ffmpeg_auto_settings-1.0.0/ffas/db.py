...
from .misc import print_err, singleton, FFAS_PATH
...
class Video(Base):
    ...
    __table_args__ = (
        ...
        + (UniqueConstraint('type', 'encoder', 'crf_e2', 'vmaf_e6', 'vmaf_harmonic_e6', 'variant', 'additional'),)
    )
    _valid_types = ['source', 'sample', 'sample-encode', 'hstack', 'full-encode']
    _valid_types_sql = ', '.join(f"'{type}'" for type in _valid_types)

    id: Mapped[str] = mapped_column(
        CheckConstraint(f'length(id) = {len(str(uuid.uuid4()))}'),
        primary_key=True,
