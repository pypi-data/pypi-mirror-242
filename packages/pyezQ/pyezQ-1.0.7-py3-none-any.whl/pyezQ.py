...
class Account():
        ...
        def wrapper(self, *args, **kwargs):
            ...
            while retries < max_retries:
                try:
                    ...
                    print(f'{func.__name__} execution failed, '
                    ...
                    if hasattr(e, 'code') and e.code == 10100101:
                        ...
                        sleep(retry_delay)
                    retries = retries + 1                   
            raise Exception(f"function:[{func.__name__}] Max retries exceeded. Attempt {max_retries} times failed. ")
        return wrapper
    ...
    def query_experiment(
        ...
        while time()-t0 < max_wait_time:
            try:
                ...
                status_code = res.status_code
                if status_code != 200:
                    raise Exception(f'查询实验失败, 请求接口失败, status_code:{status_code}')
                result = json.loads(res.text)
                code = result.get('code', -1)
    ...
    def set_machine(
        ...
        status_code = res.status_code
        if status_code != 200:
            raise Exception(f'下载实验参数失败, 请求接口失败, status_code:{status_code}')
        result = json.loads(res.text)
        code = result.get('code', -1)
    ...
    def get_experiment_circuit(
        ...
        status_code = res.status_code
        if status_code != 200:
            raise Exception(f'查询实验线路失败, 请求接口失败, status_code:{status_code}')
        result = json.loads(res.text)
        code = result.get('code', -1)
