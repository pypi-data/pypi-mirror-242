...
class Theis(GAnalyticalSolutionBase):
    def __init__(self):
        super().__init__()
    ...
    def calc(self, sample_coordinate, well_coordinate=np.array([0.0, 0.0, 0.0])):
        if not isinstance(sample_coordinate, np.ndarray) or not isinstance(well_coordinate, np.ndarray):
