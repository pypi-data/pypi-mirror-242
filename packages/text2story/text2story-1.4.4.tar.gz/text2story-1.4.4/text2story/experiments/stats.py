...
def build_stats_byelement(doc,  element_type="event", dataset_type="BRAT", **kwargs):
    # the kwargs is to filter accoding to attribute values

    items2filter = set(kwargs.keys())
    attr_count = {}
    nelements = 0
    ...
    for tok in doc:
        for ann_type, attr_map in tok.attr:
            ...
            # check if this annotation present some attribute
            # the it is going to be filtered
            attr_map_keys = set(attr_map.keys())
            inter_result = attr_map_keys.intersection(items2filter)
            ignore_annotation = False
            if len(inter_result) != 0:
                for attr in inter_result:
                    if attr_map[attr] != kwargs[attr]:
