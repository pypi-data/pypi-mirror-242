...
def get_verb_events(tok_lst) -> dict:
    """
    Get the events whose pos attribute is a verb

    @param tok_lst[TokenCorpus]: Given a document in the TokenCorpus format
    @return: A dictionary where keys are the id of annotation and values
    are TokenCopus objects.
    """
    event_lst = {}
    ...
    for i in range(len(tok_lst)):
        ...
        for attr_item in tok.attr:
            ...
            if "Pos" in attr_map and attr_map["Pos"] == "Verb":  # it is an event token
