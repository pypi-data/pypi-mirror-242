...
from dilithium.xoshiro128starstar import *
...
class Dilithium:
    ...
    def keygen(self, key_seed):
        # Random seed
        #zeta = self.random_bytes(32)
        
        # generate zeta from key_seed using xoshiro128**
        zeta = randombytes(key_seed, 32)
    ...
    def sign(self, sk_bytes, seed, message_length=32):
        """
        Dilithium sign algorithm
        Input:
            sk_bytes: secret key as output from keygen
            seed: seed from which we will generate a message using xoshiro128starstar randombytes
            message_length: length of the desired message to be generated from the seed, default is 32
        Output:
            sig: signature with the message concatenated at the end
        """
        # unpack the secret key
        rho, K, tr, s1, s2, t0 = self._unpack_sk(sk_bytes)
        
        # generate the message from the seed
        m = randombytes(seed, message_length)
        ...
        while True:
            ...
            return self._pack_sig(c_tilde, z, h), m
        
        
    def sign_with_input(self, sk_bytes, m):
        """
        Dilithium sign algorithm
        Input:
            sk_bytes: secret key as output from keygen
