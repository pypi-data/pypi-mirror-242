# coding: utf-8

"""
    Nomad Envoy

    This is the API descriptor for the Nomad Envoy API, responsible for order operation and product lists. Developed by [Samarkand Global](https://samarkand.global) in partnership with [Youzan](https://www.youzan.com/), [LittleRED](https://www.xiaohongshu.com/), [PDD](http://www.pinduoduo.com/), etc. Read the documentation online at [Nomad API Suite](https://api.samarkand.io/). - Install for node with `npm install nomad_envoy_cli` - Install for python with `pip install nomad-envoy-cli`  # noqa: E501

    The version of the OpenAPI document: 1.54.0
    Contact: paul@samarkand.global
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import nomad_envoy_cli
from nomad_envoy_cli.models.api_response_inventory import ApiResponseInventory  # noqa: E501
from nomad_envoy_cli.rest import ApiException

class TestApiResponseInventory(unittest.TestCase):
    """ApiResponseInventory unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test ApiResponseInventory
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = nomad_envoy_cli.models.api_response_inventory.ApiResponseInventory()  # noqa: E501
        if include_optional :
            return ApiResponseInventory(
                code = 56, 
                message = '0', 
                items = [
                    nomad_envoy_cli.models.inventory.Inventory(
                        product_id = '0', 
                        batch_id = '0', 
                        location_id = '0', 
                        sku_name = '0', 
                        barcode = '0', 
                        quantity = 1, )
                    ]
            )
        else :
            return ApiResponseInventory(
        )

    def testApiResponseInventory(self):
        """Test ApiResponseInventory"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
