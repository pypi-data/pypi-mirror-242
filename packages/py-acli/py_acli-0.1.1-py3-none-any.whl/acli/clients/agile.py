...
@dataclass
class AgileCommand(Command):
    pass

@dataclass
class AgileClient(RemoteClient):
    def execute(self, command: AgileCommand):
        return super().do_execute(command)

# Variants for addBoardAdministrators

@dataclass
class AddBoardAdministratorsGivenBoardAndUserId(AgileCommand):
...
@dataclass
class AddBoardAdministratorsGivenBoardAndGroup(AgileCommand):
    """Add users and groups as board administrators. Both userId and group can be specified as a comma separated list."""
    board: str
    """Board name or id."""
    group: str
...
@dataclass
class AddBoardColumn(AgileCommand):
...
@dataclass
class AddIssuesToSprint(AgileCommand):
...
@dataclass
class AddQuickFilter(AgileCommand):
...
@dataclass
class CompleteSprint(AgileCommand):
...
@dataclass
class CreateBoardGivenBoardAndTypeAndFilter(AgileCommand):
...
@dataclass
class CreateBoardGivenBoardAndTypeAndProject(AgileCommand):
    """Create a board. Type is any supported type like scrum, kanban, or diy. If a filter is not provided, a filter will be automatically created from the  project parameter representing a comma separated list of projects. For Cloud, the location of the board will be set to the first project provided in the project parameter if provided. That means the board will be located on the board list in the project UI."""
    board: str
    """Board name or id."""
    type: str
...
@dataclass
class CreateSprint(AgileCommand):
...
@dataclass
class DeleteBoard(AgileCommand):
...
@dataclass
class DeleteSprint(AgileCommand):
...
@dataclass
class GetBoardColumnList(AgileCommand):
...
@dataclass
class GetBoardList(AgileCommand):
...
@dataclass
class GetQuickFilter(AgileCommand):
...
@dataclass
class GetQuickFilterList(AgileCommand):
...
@dataclass
class GetServerInfo(AgileCommand):
...
@dataclass
class GetSprintList(AgileCommand):
...
@dataclass
class ReleaseVersion(AgileCommand):
...
@dataclass
class RemoveBoardColumn(AgileCommand):
...
@dataclass
class RemoveQuickFilter(AgileCommand):
...
@dataclass
class RenderRequest(AgileCommand):
...
@dataclass
class RunFromBoardListGivenInput(AgileCommand):
...
@dataclass
class RunFromBoardListGivenCommon(AgileCommand):
    """Run actions for each board matching selection criteria similar to getBoardList."""
    common: str
...
@dataclass
class RunFromBoardListGivenFile(AgileCommand):
    """Run actions for each board matching selection criteria similar to getBoardList."""
    file: str
    """Path to file based content or result output. For some action it may be a directory path. Use - for standard input. Use + for getting lines from 'input' parameters (deprecated)."""
    options: RunFromBoardListOptions | None = None
...
@dataclass
class RunFromQuickFilterListGivenBoardAndInput(AgileCommand):
...
@dataclass
class RunFromQuickFilterListGivenBoardAndCommon(AgileCommand):
    """Run actions for each board matching selection criteria similar to getQuickFilterList."""
    board: str
    """Board name or id."""
    common: str
...
@dataclass
class RunFromQuickFilterListGivenBoardAndFile(AgileCommand):
    """Run actions for each board matching selection criteria similar to getQuickFilterList."""
    board: str
    """Board name or id."""
    file: str
...
@dataclass
class RunFromSprintListGivenBoardAndInput(AgileCommand):
...
@dataclass
class RunFromSprintListGivenBoardAndCommon(AgileCommand):
    """Run actions for each sprint matching selection criteria similar to getSprintList."""
    board: str
    """Board name or id."""
    common: str
...
@dataclass
class RunFromSprintListGivenBoardAndFile(AgileCommand):
    """Run actions for each sprint matching selection criteria similar to getSprintList."""
    board: str
    """Board name or id."""
    file: str
...
@dataclass
class SetBoardAdministratorsGivenBoardAndUserId(AgileCommand):
...
@dataclass
class SetBoardAdministratorsGivenBoardAndGroup(AgileCommand):
    """Set users and groups as board administrators replacing any previously configured administrators. Both userId and group can be specified as a comma separated list."""
    board: str
    """Board name or id."""
    group: str
...
@dataclass
class StartSprint(AgileCommand):
...
@dataclass
class UpdateBoard(AgileCommand):
...
@dataclass
class UpdateBoardCard(AgileCommand):
...
@dataclass
class UpdateBoardColumn(AgileCommand):
...
@dataclass
class UpdateQuickFilter(AgileCommand):
...
@dataclass
class UpdateSprint(AgileCommand):
...
@dataclass
class ValidateLicense(AgileCommand):
