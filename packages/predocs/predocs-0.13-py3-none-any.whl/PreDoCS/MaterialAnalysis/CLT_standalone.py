...
class Laminate(object):
    ...
    def get_engineering_constants(self, method='with_poisson_effect', reference_surface=None):
        ...
        if method == 'with_poisson_effect':
            ...
            E_1 = 1. / (ABD_inv[idx(11)] * t)
            E_2 = 1. / (ABD_inv[idx(22)] * t)
            G = 1. / (ABD_inv[idx(33)] * t)
            return E_1, E_2, G
        elif method == 'without_poisson_effect':
            A, B, D, A_s = self.get_ABD_dict(reference_surface)
            E_1 = A[11] / t
            E_2 = A[22] / t
            G = A[66] / t
            return E_1, E_2, G
        elif method == 'song':
            A, D, B, A_s = self.get_ABD_dict(reference_surface)
            K_11 = A[22] - A[12]**2/A[11]
            #K_12 = A[26] - A[12]*A[16]/A[11]
            K_22 = A[66] - A[16]**2/A[11]
            return None, K_11 / t, K_22 / t
        elif method == 'wiedemann':
            ABD = self.get_ABD_matrix(reference_surface)
            ABD_inv = np.linalg.inv(ABD)
            E_1 = 1. / (ABD_inv[idx(11)] * t)
            E_2 = 1. / (ABD_inv[idx(22)] * t)
            G = 1. / ((ABD_inv[idx(33)]/2 + ABD_inv[idx(12)]) * t)
            return E_1, E_2, G
        else:
