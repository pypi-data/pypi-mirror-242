...
from nan_dev.raw.core import TLObject
...
class RPCError(Exception):
    ...
    def raise_it(rpc_error: "raw.types.RpcError", rpc_type: Type[TLObject]):
        ...
        if error_id not in exceptions[error_code]:
            raise getattr(import_module("nan_dev.errors"), exceptions[error_code]["_"])(
                value=f"[{error_code} {error_message}]",
                rpc_name=rpc_name,
                is_unknown=True,
                is_signed=is_signed,
        ...
        raise getattr(
            import_module("nan_dev.errors"), exceptions[error_code][error_id]
