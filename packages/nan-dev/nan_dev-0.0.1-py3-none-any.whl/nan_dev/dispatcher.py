...
import nan_dev
from nan_dev import utils
from nan_dev.handlers import (CallbackQueryHandler, ChatJoinRequestHandler,
...
from nan_dev.raw.types import (UpdateBotCallbackQuery,
...
class Dispatcher:
    ...
    def __init__(self, client: "nan_dev.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await nan_dev.types.Message._parse(
                    self.client,
                    update.message,
                    users,
                    chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await nan_dev.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler,
            )

        async def user_status_parser(update, users, chats):
            return (
                nan_dev.types.User._parse_user_status(self.client, update),
                UserStatusHandler,
            )

        async def inline_query_parser(update, users, chats):
            return (
                nan_dev.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler,
            )

        async def poll_parser(update, users, chats):
            return (nan_dev.types.Poll._parse_update(self.client, update), PollHandler)

        async def chosen_inline_result_parser(update, users, chats):
            return (
                nan_dev.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler,
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                nan_dev.types.ChatMemberUpdated._parse(
        ...
        async def chat_join_request_parser(update, users, chats):
            return (
                nan_dev.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except nan_dev.StopPropagation:
                                raise
                            except nan_dev.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
