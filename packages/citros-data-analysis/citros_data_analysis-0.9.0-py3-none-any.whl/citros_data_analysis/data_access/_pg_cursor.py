...
class _PgCursor:
    ...
    _connection_parameters = {'host': None,
                              'user': None,
                              'password': None,
                              'database': None,
                              'options': None,
    ...
    def __init__(self, host = None, port = None, user = None, password = None, database = None, 
                schema = None, batch = None,  sid = None, debug = False):
    ...
    def _make_connection_postgres(self):
        ...
        '''
        _PgCursor.pg_connection = psycopg2.connect(host = self._host, 
                                      user = self._user, 
    ...
    def _if_connection_parameters_changed(self):
        new_connection = {'host': self._host,
                        'user': self._user,
                        'password': self._password,
                        'database': self._database,
                        'options': "-c search_path=" + self._schema,
    ...
    def _execute_query(self, query, param_execute = None, check_batch = True):
        ...
        for j in range(2):
            ...
            except (psycopg2.errors.InFailedSqlTransaction, psycopg2.OperationalError) as e:
                if j == 0:
                    _PgCursor.pg_connection.close()
                    self._make_connection_postgres()
                else:
    ...
    def _get_general_info(self, filter_by):
        ...
        '''
        size, topic_list, sid_list, message_count, error_occurred, error_name = self._general_info_query(filter_by = filter_by)
        sid_list.sort()
    ...
    def _get_topic_info(self, filter_by):
        ...
        '''
        message_count, structure = self._topic_info_query(filter_by = filter_by, out_format = 'dict')
    ...
    def _data(self, data_names: list = None, additional_columns: list = None) -> pd.DataFrame:
        '''
        ...
        df, error_name = self._get_data(data_names, additional_columns = additional_columns, 
                            filter_by = filter_by, order_by = order_by, 
                            method = self._method, n_avg = self._n_avg, n_skip = self._n_skip)
    ...
    def _is_batch_in_database(self, batch_id):
        ...
        table_result = self._execute_query(query, {'schema': self._schema}, check_batch = False)['res']
    ...
    def _is_schema_exist(self):
        ...
        schema_list = self._execute_query(query, {'schema': self._schema}, check_batch = False)['res']
    ...
    def _topic_info_query(self, filter_by = None, out_format = 'dict'):
        ...
        FROM (
            ...
            FROM {table_name}""" + query_filter + """) AS t;
        """)
        
        q_result = self._execute_query(query.format(*param_sql, table_name = sql.Identifier(table_name)), param_execute, check_batch = False)['res']
    ...
    def _pg_get_data_structure(self, topic: str = None):
        ...
        structure, error_name = self._download_data_structure(filter_by = filter_by, out_format = 'str')
    ...
    def _sid_info_query(self, group_by = None, filter_by= None):
        ...
        result = self._execute_query(query.format(*param_sql), param_execute, check_batch = False)['res']
    ...
    def _get_unique_values(self, column_names: Optional[Union[str, list]], filter_by: dict = None) -> list:
        '''
    ...
    def _general_info_query(self, filter_by: dict = None) -> list:
        ...
        query = sql.SQL("""
                        SELECT 
                          ...
                          COUNT(t.*) AS n
                        FROM (
                            SELECT * FROM {table}""" + query_filter +""") AS t;""").format(
                                    *param_sql,
                                    table = sql.Identifier(table_name))
        query_result = self._execute_query(query, param_execute)
        data = query_result['res']
    ...
    def get_min_max_value(self, column_name: str, filter_by: dict = None, return_index: bool = False, mode = 'MAX'):
        ...
        if return_index:
            ...
            query = sql.SQL("""
                            ...
                            FROM (
                                SELECT """ + query_row + """ as col_x, sid, rid, 
                                       """+mode+"""(""" + query_row + """) OVER () AS extremum_x
                                FROM {table_name}""" + query_filter +""") AS t
                            WHERE col_x = extremum_x;""").format(*param_sql, table_name = sql.Identifier(table_name))
    ...
    def _get_counts(self, column_name: str = None, group_by: Optional[Union[str, list]] = None, filter_by: dict = None, 
    ...
    def _get_unique_counts(self, column_name: str = None, group_by: list = None, filter_by: dict = None, 
                          nan_exclude: bool = False) -> list:
        '''
    ...
    def _get_data(self, data_query = None, additional_columns = None, filter_by = None, order_by = None,
        ...
        if method == '':
            df, error_name = self._get_data_all(data_query = data_query, 
                                        additional_columns = additional_columns, 
                                        filter_by = filter_by, order_by = order_by)

        elif method == 'skip':
            df, error_name = self._skip_rows(data_query = data_query, additional_columns = additional_columns, 
                                    filter_by = filter_by, order_by = order_by, 
                                    n_skip = n_skip)
            
        elif method == 'move_avg': 
            df, error_name = self._moving_average(data_query = data_query, additional_columns = additional_columns, 
                                        filter_by = filter_by, order_by = order_by,
                                        n_avg = n_avg, n_skip = n_skip)
            
        elif method == 'avg':
            df, error_name = self._average(data_query = data_query, additional_columns = additional_columns, 
                                    filter_by = filter_by, order_by = order_by, 
                                    n_avg = n_avg)
        else:
            if self._debug:
    ...
    def _average(self, data_query = None, additional_columns = None, filter_by = None, order_by = None,  n_avg = 1):
        ...
        if query_filter_data != '':
            ...
            query = sql.SQL(query_avg + main_query + query_group_avg + query_order).format(*param_sql, table = sql.Identifier(table_name))        
        query_result = self._execute_query(query, param_execute)
        data = query_result['res']
        error_name = query_result['error']
        if data is not None:
    ...
    def _moving_average(self, data_query = None, additional_columns = None, filter_by = None, order_by = None,
                                 n_avg = 1, n_skip = 1):
        '''
    ...
    def _get_sid_tables(self, data_query, topic, additional_columns, filter_by, order_by, method, n_avg, n_skip):
        '''
        ...
        result_table, error_name = self._get_data(data_query = data_query, additional_columns = additional_columns, 
    ...
    def data_for_time_plot(self, topic_name, var_name, time_step, sids, remove_nan, inf_vals):
        ...
        **kwargs
        ...
        df, error_name = self.topic(topic_var).sid(sids).set_order({'sid': 'asc', 'rid': 'asc'})._data(data_columns)
