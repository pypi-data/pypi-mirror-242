...
def find_optimal_number_of_intervals(
    ...
    while True:
        ...
        while iter_indexes:
            ...
            df_temp = _merge_rows_with_similar_bad_rates(df_temp, index_sorted, indexes, dropped_indexes)
...
def _get_avg_consecutive_bad_rates(df: pd.DataFrame, indexes: list[int]) -> float:
    """
    Given a dataframe, this function returns the average of the consecutive bad rates
    :param df: Dataframe with the bad rates
    :param indexes: List with the two indexes that we are going to merge in the df
    :return: Average of the consecutive bad rates
    """
    return (
        (df.loc[indexes[0], 'bad_rate']*df.loc[indexes[0], 'count'] +
         df.loc[indexes[1], 'bad_rate']*df.loc[indexes[1], 'count']) /
            (df.loc[indexes[0], 'count'] + df.loc[indexes[1], 'count'])
        )
...
def _merge_rows_with_similar_bad_rates(
    ...
    :param candidate_indexes: Set with all the indexes were the bad rate is not monotonical
    :param dropped_indexes: Set with the indexes already dropped
    :return: Dataframe with the rows merged
    """
    df.loc[indexes_to_merge[0], 'bad_rate'] = _get_avg_consecutive_bad_rates(df, indexes_to_merge)
    df.loc[indexes_to_merge[0], 'count'] = df.loc[indexes_to_merge[0], 'count'] + df.loc[indexes_to_merge[1], 'count']
    df['quantile'] = df['quantile'].cat.rename_categories(
        {
            df.loc[indexes_to_merge[0], 'quantile']: pd.Interval(
                left=df.loc[indexes_to_merge[0], 'quantile'].left,
                right=df.loc[indexes_to_merge[1], 'quantile'].right,
                closed='right'
            )
        }
    )
    # If we are merging an index which is in the indexes_to_merge set, we must drop it
    # (it won't exist in future iterations)
    if indexes_to_merge[1] in candidate_indexes:
        dropped_indexes.add(indexes_to_merge[1])
    dropped_indexes.add(indexes_to_merge[1])
    df.drop(indexes_to_merge[1], inplace=True)
    return df
