...
def merge_intervals(df: pd.DataFrame, indexes: list[int]) -> pd.DataFrame:
    """
    Given a dataframe and a list of indexes, this function merges the rows in the indexes, averaging the bad rates
    and summing the counts. To use this function the df should be ordered by 'interval' column in ascending order
    and with the index of the df starting from 0 to len(df) - 1 (if not, you should reset the index first).

    :param df: Dataframe with the bad rates
    :param indexes: List with the two indexes we are going to merge
    :return: Dataframe with the rows merged
    """
    df.loc[indexes[0], 'bad_rate'] = _get_avg_consecutive_bad_rates(df, indexes)
