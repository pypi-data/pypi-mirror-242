...
def analyse(iv_array, depl_volt, module_sn, layer, ref=None, temp=None):
    ...
    Vdepl = 0
    if depl_volt is not None and depl_volt != 0:
        Vdepl = depl_volt
        log.info(f"Using manual input depletion voltage {Vdepl}V.")
    elif ref is not None:
        try:
            tmp_vfd = max([v["Vfd"] for v in ref["reference_IVs"]])
            if tmp_vfd != 0:
                Vdepl = tmp_vfd
                ...
                log.warning(
                    bcolors.WARNING
                    + f"Depletion voltage provided is not valid: {tmp_vfd}V. Proceed using default value!"
                    + bcolors.ENDC
                )
        except KeyError:
            depl_volt = None
    ...
    if Vdepl == 0 and (depl_volt is None or depl_volt == 0):
    ...
    if len(iv_array["sigma current"]) == 0:
        ...
        p11 = ax.errorbar(
            iv_array["voltage"][1:],
            [i for i in normalised_current[1:]],
            yerr=iv_array["sigma current"][1:],
            fmt=style_normdata,
            elinewidth=1,
            label="current (norm. 20$\\circ$C)",
        ...
        if not qchelper:
            ...
            try:
                ...
                if typer["depl_volt"] is not None:
                    ...
                    log.warning(
                        bcolors.WARNING
                        + f"No depletion voltage found, using manual input via --vdepl {typer['depl_volt']}"
                        + bcolors.ENDC
                    )
                else:
                    log.warning(
