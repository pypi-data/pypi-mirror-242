        ...
        for database_key in database_keys:
            ...
            try:
                ...
                if self.notifications.enabled:
                    msg = f"{settings.SYNC_ENV_PROJECT_NAME}:[{settings.ENVIRONMENT}] - {database['NAME']} database backup failed"
                    self.notifications.send_slack_message(msg)                
                raise CommandError(err) from err

            self.logger.info(f"Successfully backed up database: {database['NAME']}!", )
            if self.notifications.enabled:
                msg = f"{settings.SYNC_ENV_PROJECT_NAME}:[{settings.ENVIRONMENT}] - {database['NAME']} database backup was success"
                self.notifications.send_slack_message(msg)
