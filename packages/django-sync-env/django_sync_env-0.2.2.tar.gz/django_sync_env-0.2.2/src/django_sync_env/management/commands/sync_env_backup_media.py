...
class Command(BaseSyncBackupCommand):
    help = """Backup media files, gather all in a tarball and encrypt or compress."""
    content_type = "media"
    ...
    def handle(self, **options):
        ...
        try:
            ...
            if self.notifications.enabled:
                msg = f"{settings.SYNC_ENV_PROJECT_NAME}:[{settings.ENVIRONMENT}] - media assets backup was successful"
                self.notifications.send_slack_message(msg)                 
            ...
            if self.notifications.enabled:
                msg = f"{settings.SYNC_ENV_PROJECT_NAME}:[{settings.ENVIRONMENT}] - media assets backup failed"
                self.notifications.send_slack_message(msg)            
    ...
    def backup_mediafiles(self, media_storage, storage):
        ...
        self.logger.info("Backup size: %s", utils.handle_size(tarball))
        # Store backup
        tarball.seek(0)
        self.write_to_storage(storage, tarball, filename)
        self.logger.info("Media backup completed: %s", utils.handle_size(tarball))
