...
def update_historized_on_bulk_create(objs, transaction=None):
    ...
    historized_instances = []
    for obj in objs:
        # Create a new historized instance
        historized_instance = historized_model(
            on_txn=transaction,
            original=obj,
            off_txn=Transaction.get_sentinel(),  # Set off_txn to sentinel value
        )

        # Copy fields from the original instance
        for field in sender._meta.fields:
            if field.name not in ["id", "transaction", "pk", "transaction_id"]:
                setattr(historized_instance, field.name, getattr(obj, field.name))

        historized_instances.append(historized_instance)

    # Bulk create historized instances
    historized_model.objects.bulk_create(historized_instances)
