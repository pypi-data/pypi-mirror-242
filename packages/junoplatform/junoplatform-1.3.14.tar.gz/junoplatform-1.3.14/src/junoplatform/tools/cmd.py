...
@deploy.command()
@click.argument('package_id', required=False)
@click.option("-k", "--keep", help="keep edge config. true: edge first and merge package cfg. false: package overwrites edge", default=True, show_default=True)
@pass_base
@auth
def new(base, package_id, keep: str):
    '''deploy package
    '''
    if not package_id:
        try:
