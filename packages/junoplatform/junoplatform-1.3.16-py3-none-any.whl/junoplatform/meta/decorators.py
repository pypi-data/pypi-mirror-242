...
import sys
import requests
from junoplatform.io.data_clean import abnormal_detection_clean, DetectionResult, TagWithProperties, parse_config
...
class EntryPoint:
    ...
    def _data_analysis_clean(self):
        toggle = True
        local_key = f"system.ai.abnormal.confirm.{self.plant}.{self.module}"
        confirm = []
        data_config = {}
        tfmt = "%Y/%m/%d %H:%M:%S"
        last_siz = 0
            ...
            try:
                ...
                if hist_len == -1:
                    if toggle:
                        toggle = not toggle
                        logging.warning(
                            f"ab_detection will be disbled, since {tags}")
                    continue
                ...
                if not confirm:
                    ...
                    i = 0
                    while True:
                        uc = self.storage.local.io.lrange(local_key, i, i)
                        if uc:
                    ...
                    if i > 0:
                        self.storage.local.io.lpop(local_key, i)
                    uc = self.storage.local.io.lrange(local_key, 0, -1)
                    if uc:
                    ...
                    confirm = confirm[1:]
                    siz = self.storage.local.io.llen(local_key)
                    if siz != last_siz:
                        _u = self.storage.local.io.lindex(local_key, siz - 1)
                        u = json.loads(_u)
                        last_siz = siz
                        rec = []
                        for t in tags:
                ...
                errta = len(confirm) - hist_len
                if errta > 0:
                    confirm = confirm[errta:]
                elif errta < 0:
                    # should not happen
                    pass

                _confirm = np.array(confirm)

                logging.info(f"start of data clean: uc {_confirm.shape}")

                self._dateset = DataSet()
                d, t, _ = self._dateset.fetch(
                    tags=self.cfg_in['tags'], num=self.cfg_in['items'])

                _, ab = abnormal_detection_clean(d, tags, t, _confirm)
                if ab:
                    payload = {
                ...
                if confirms:
                    ...
                    self.storage.local.io.rpush(
                        local_key, json.dumps(uc).encode())
                ...
                siz = self.storage.local.io.llen(local_key)
                if siz > hist_len:
                    self.storage.local.io.lpop(local_key, siz - hist_len)
