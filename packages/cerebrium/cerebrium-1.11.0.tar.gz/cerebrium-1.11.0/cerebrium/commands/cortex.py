...
def setup_app(
    ...
    if "predict" in params:
        predict_data = (
            predict_data if predict_data is not None else params.get("predict")
        )
...
def build(
    ...
    predict: Union[str, None] = typer.Option(
        None,
        help="JSON string containing all the parameters that will be used to run your deployment's predict function on initialisation",
    ),
    include: str = typer.Option(
        "",
        help=(
