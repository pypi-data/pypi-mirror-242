...
def init(
    ...
    config = {
        ...
        "gpu_count": gpu_count,
        "predict_data": predict_data or '{"prompt": "Hello World"}',
        "min_replicas": 0,
    }
...
def setup_app(
    ...
    if disable_predict_data:
        ...
        if predict_data is None:
            utils.cerebriumLog(
                ...
                "Please provide predict data in json format using the `--predict-data`"
                " flag or add it to your config.yaml under `predict_data`.\n"
                "This data is used to run your predict function on build to ensure your "
                "new deployment will work before replacing your existing deployment.\n"
                "Otherwise, use the --disable-predict flag if you would like to risk it.",
                prefix="Argument Error:",
            )
    ...
    if disable_predict_data:
        print("🔮 Disable-predict enabled. Skipping predict function...")
