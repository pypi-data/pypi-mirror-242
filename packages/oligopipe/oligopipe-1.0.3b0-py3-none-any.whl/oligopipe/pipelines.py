...
class PredictionPipeline(Pipeline):
    def execute(self):
        ...
        varcopp_input, discarded_vars = self.preprocess_inputs(variants)
        if len(varcopp_input.gene_pairs) > 10000:
            logging.warning("There are a lot of gene pairs ({0}).".format(len(varcopp_input.gene_pairs)))
    ...
    def preprocess_inputs(self, variants):
        ...
        """
        varcopp_input, discarded_vars, db_schema = VarcoppPreprocessPipeline(variants, self.metadata["filters"],
                                                                             self.metadata["gene_panel"]) \
            .execute(self.metadata["patient_info"], self.metadata["genome_build"], self.metadata["database"])
...
class PrioritizationPipeline(Pipeline):
    ...
    def preprocess_inputs(self, variants):
        ...
        hop_input, discarded_vars, db_schema = HOPPreprocessPipeline(variants, seeds, self.metadata["filters"],
                                                                     self.metadata["top_N"]) \
            .execute(self.metadata["patient_info"], self.metadata["genome_build"], self.metadata["database"])
        self.metadata["db_schema"] = db_schema
