...
class MotionMagicDutyCycle:
    ...
    def __init__(self, position: rotation, enable_foc: bool = True, feed_forward: float = 0.0, slot: int = 0, override_brake_dur_neutral: bool = False):
    ...
    def _send_request(self, network: str, device_hash: int, cancel_other_requests: bool) -> StatusCode:
        ...
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlMotionMagicDutyCycle(ctypes.c_char_p(bytes(network, 'utf-8')), device_hash, self.update_freq_hz, cancel_other_requests, self.position, self.enable_foc, self.feed_forward, self.slot, self.override_brake_dur_neutral))
