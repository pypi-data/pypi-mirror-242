<template>
  <UFormWrapper :is-loading="isLoading" add-default-classes>
    <template #header>
    ...
    <template #content>
      ...
      <USelectWrapper
        ...
        :get-option-label="
          (item) =>
            `${item?.attributes?.manufacturer} ${item?.attributes?.model} (${item?.attributes?.designator})`
        "
      ...
      <div>
        <ULabel :required="false" label-text="Mission Number" />
        <div class="flex">
          <UInputWrapper
      ...
      <UInputWrapper v-if="0" label-text="Unit" placeholder="Please enter Unit" />
      <UInputWrapper v-model="formModel.apacs_number" label-text="APACS Number" placeholder="" />
      <UInputWrapper
