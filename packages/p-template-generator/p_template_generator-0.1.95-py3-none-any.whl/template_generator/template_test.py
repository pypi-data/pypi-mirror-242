...
def test(searchPath):
    ...
    updateRes(rootDir)
    if os.path.exists(os.path.join(rootDir, "res")):
        shutil.rmtree(os.path.join(rootDir, "res"))
    if os.path.exists(os.path.join(rootDir, "tp")):
        shutil.rmtree(os.path.join(rootDir, "tp"))
    test_template_dir = os.path.join(rootDir, "tp_20231121")
    ...
    img_start_pts = calendar.timegm(time.gmtime())
    config = {
        "input":[
    ...
    template.executeTemplate(config, searchPath, False, False, False)
    img_success = False
    if os.path.exists(img_output):
        img_success = True
    img_end_pts = calendar.timegm(time.gmtime())
        
    config["template"] = os.path.join(test_template_dir, "gen_video")
    config["output"] = video_output
    template.executeTemplate(config, searchPath, False, False, False)
    video_end_pts = calendar.timegm(time.gmtime())
    video_success = False
    ...
    template.executeTemplate(config, searchPath, False, False, False)
    tp_end_pts = calendar.timegm(time.gmtime())
    tp_success = False
    if os.path.exists(tp_output):
        tp_success = True

    if video_success and img_success and tp_success:
        print(f"[1024x1024                      Picture] Generate time is {img_end_pts-img_start_pts}s")
        print(f"[1024x1024, 16.7s, 30 fps         Video] Generate time is {video_end_pts-img_end_pts}s")
        print(f"[1024x1024, 9s             txt to Video] Generate time is {tp_end_pts-video_end_pts}s")
