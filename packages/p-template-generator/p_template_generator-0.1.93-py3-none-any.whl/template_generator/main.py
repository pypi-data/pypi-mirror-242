...
def main():
    ...
    process_begin_pts = calendar.timegm(time.gmtime())
    ...
    try:
        ...
        if module in module_func:
            module_func[module]()
            logging.info(f"================ success end ({calendar.timegm(time.gmtime()) - process_begin_pts}) ===================\n\n")
            sys.exit(0)
        else:
            print("Unknown command:", module)
            logging.info(f"================ fail end with [Unknown command] ({calendar.timegm(time.gmtime()) - process_begin_pts}) ===================\n\n")
            _funnyLogo()
            sys.exit(-1)
    except Exception as e:
        print(f"uncatch Exception:{e}")
        logging.info(e)
        logging.info(f"================ fail end with [uncatch Exception] ({calendar.timegm(time.gmtime()) - process_begin_pts}) ===================\n\n")
        _funnyLogo()
        sys.exit(-1)
