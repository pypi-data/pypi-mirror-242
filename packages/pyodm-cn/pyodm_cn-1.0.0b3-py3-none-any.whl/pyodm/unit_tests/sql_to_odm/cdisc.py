...
from pyodm.unit_tests.sql_to_odm.cdisc_element import CdiscElement
...
def cdisc_sql_factory(clazz, sql_path: Path, mapper_path: Path):
    ...
    setattr(clazz, "mapper", __mapper)
    return clazz


def __mapper(self, registry: CdiscRegistry):
    if self.isCdiscMapper:
        elements = XMLMapperReader(registry, self).read(XmlPathResource(self.mapper_file))
        return elements
    return []
