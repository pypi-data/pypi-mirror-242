...
class AtomPositionManager(FileManager):
    ...
    def export_as_PDB(self, file_location:str=None, bond_distance:float=None, save_to_file:str='w', verbose:bool=False) -> str:
        ...
        for A, position_A in enumerate(self.atomPositions):       #loop over different atoms
            for B, position_B in enumerate(self.atomPositions):
                AB_bond_distance = float(bond_distance) if bond_distance is not None else (self.covalent_radii[self.atomLabelsList[A]] + self.covalent_radii[self.atomLabelsList[B]]) * 1.4
    ...
    def read_configXYZ(self, file_location: str = None, lines: list = None, energy_key: str = 'DFT_energy', 
