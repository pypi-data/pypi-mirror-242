...
# Transpose function that works on matrices (2d tensors).
...
def window1d(input_array, size: int, shift: int = 1, stride: int = 1):
    """
    ...
    if not isinstance(input_array, (list, np.ndarray)):
        raise ValueError("input_array must be a list or a numpy array")
...
def convolution2d(
    ...
    assert (
        len(input_matrix.shape) == 2 and len(kernel.shape) == 2
    ), "Both input_matrix and kernel should be 2D numpy arrays"
    # Check if stride is an integer and greater than 0
    assert (
        isinstance(stride, int) and stride > 0
    ), "Stride should be an integer greater than 0"

    # Calculate the dimensions of the output matrix
    output_rows = (input_matrix.shape[0] - kernel.shape[0]) // stride + 1
    output_cols = (input_matrix.shape[1] - kernel.shape[1]) // stride + 1
