...
async def ssh_custom_forward(spawner, port_forward_info):
    ...
    body = {
        ...
        "svc_name": remote_svc_name,
        "svc_port": remote_svc_port,
        "target_node": local_svc_name,
        "target_port": local_svc_port,
    }
...
async def is_system_allowed(spawner, user_options):
    ...
    if not partition:
        ...
        systems_list = [*dropdown_list.get(service_option, {})]
    ...
    if partition in custom_config.get("systems", {}).get(system, {}).get(
        ...
        if (
            not partition
            in dropdown_list.get(service_option, {})
            .get(system, {})
            .get(account, {})
            .get(project, {})
            .keys()
        ...
        if reservation and reservation != "None":
            reservation_list = (
                dropdown_list.get(service_option, {})
                .get(system, {})
                .get(account, {})
                .get(project, {})
                .get(partition, ["None"])
    ...
    if service_limit:
        ...
        if current >= service_limit:
            reason = f'User {user.name} already has the maximum number of {service_limit} servers with configuration "service - {service}" running simultaneously. One must be stopped before a new server can be created'
