...
class BaseBHV(StoreBHV):
    ...
    def within_std(self, vs: list[Self], d: float, to_random: bool = False) -> list[int]:
        return self.within(vs, int(DIMENSION*(self.std_to_frac(d + self.EXPECTED_RAND_APART*to_random, to_random))))
    ...
    def std_apart(self, other: Self, to_random=False) -> float:
        return self.frac_to_std(self.bit_error_rate(other) - .5*to_random, to_random)
    ...
    def frac_to_std(frac, to_random=False):
        n = AbstractBHV.normal(.5*DIMENSION if to_random else 0)
        stdvs = n.zscore((frac*DIMENSION + .5*DIMENSION) if to_random else frac*DIMENSION)
        return stdvs
    ...
    def std_to_frac(std, to_random=False):
        ...
        return frac - .5 if to_random else frac
