...
def verify_ods_eq_code_params_ts(ods):
    """
    ...
    :param ods: ODS instance
    """
    assert isinstance(ods, (ODS, OMFITods)), "Input is not ods type."
    assert 'equilibrium' in ods, "ODS must contain equilibrium."
    assert 'code' in ods['equilibrium'], "Equilibrium ods must contain code."
    assert 'parameters' in ods['equilibrium']['code'], "Code ods must contain parameters."
    assert 'time_slice' in ods['equilibrium']['code']['parameters'], "Parameters ods must contain time_slice."
...
def multi_efit_to_omas(device, pulse, efitid, ods=None, minimal=False, aeqdsk_time_diff_tol=0.1, no_empty=False, **kw):
    ...
    native_cocos = gEQDSK_COCOS_identify(np.nanmean(eqi['bcentr']), np.nanmean(eqi['cpasma']))
