...
class GUI(object):
    ...
    def __init__(self, pythonFile, relativeLocations, **kw):
        ...
        top.withdraw()
        if OMFIT['MainSettings']['SETUP']['GUIs_on_top']:
...
def inputbox(prompt='Input box'):
    ...
    '''
    result = Dialog(entries={'Input': ''}, message=prompt, answers=['Submit', 'Cancel'], icon="question", title='Waiting for user input')
    if result[0] == 'Submit':
...
def Figure(toolbar=True, returnFigure=False, fillX=False, **kw):
    ...
    :param returnFigure: [False] function returns figure `f` or axis `f.use_subplot(111)`
