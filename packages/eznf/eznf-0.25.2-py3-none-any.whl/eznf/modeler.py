...
class Modeler:
    ...
    def add_kconstraint(self, bound, variables) -> None:
        k_constraint = KConstraint(bound, variables)
    ...
    def serialize_encoding(self, filename, clauses=None) -> None:
        ...
        with open(filename, 'w') as file:
            if self._max_sat:
                ...
                file.write("p knf {} {}\n".format(len(self._varmap), len(clauses) + len(knf_constraints)))
                for clause in clauses:
                    file.write(" ".join(map(str, clause)) + " 0\n")
...
def to_numerical(clause, modeler: Modeler) -> [int]:
    ...
    for slit in clause:
        if type(slit) == str:
            numerical_clause.append(modeler.v(slit))
        elif type(slit) == int:
...
class CAtMostOne:
    ...
    def to_clauses(self) -> [[int]]:
        if len(self.variables) <= 4:
            ...
            new_aux_var = self.modeler.add_var(f"__aux_atmostone_{self.modeler.n_vars()}", "auxiliary variable for at most one constraint")
            head = self.variables[:3] + [new_aux_var]
            tail = self.variables[3:] + [-new_aux_var]
            return CAtMostOne(head, self.modeler).to_clauses() + CAtMostOne(tail, self.modeler).to_clauses()
...
class KConstraint:
    def __init__(self, bound, variables) -> None:
        self.bound = bound
        self.variables = variables

    def to_str(self) -> str:
        lits = [self.bound] + self.variables
