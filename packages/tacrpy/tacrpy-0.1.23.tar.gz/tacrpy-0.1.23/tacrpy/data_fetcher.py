...
class Projects:
    ...
    :param projects: DataFrame načtených dat ze zdroje
    :type projects: DataFrame
    :param summary: DataFrame s agregovanými údaji na úrovni veřejných soutěží
    :type summary: DataFrame
    """

    def __init__(self, creds_or_projects: object):
        """ Kontstruktor, který načte data do DataFrame, očistí finanční hodnoty a vytvoří agregovanou tabulku.

        :param creds_or_projects: údaje, které slouží k authenizaci v rámci Google Colab nebo primo trida Projects
        """
        self.projects = None
        if isinstance(creds_or_projects, Projects):
            self.projects = creds_or_projects.projects.copy()
        else:
            self.projects = self._get_projects(creds_or_projects)
    ...
    def select_programme(self, *args: str):
