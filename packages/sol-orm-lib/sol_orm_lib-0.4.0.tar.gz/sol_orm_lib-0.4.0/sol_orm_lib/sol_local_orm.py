...
class MySQLClient:
    ...
    def select(self, statement) -> tuple:
        """
    ...
    def get_total_power_today_kwh(self):
        """Get total power today in the total_power_today table."""
        cursor = self.cnx.cursor()
        try:
            cursor.execute(
                f"""
            ...
            logger.error(f"Failed to retrieve total power today: {e}")
            return False   
        finally:
            cursor.close()    
    ...
    def insert_total_power_today_kwh(self, timestamp, total_power):
        """
        Insert total power today in the total_power_today table.
        """ 
        cursor = self.cnx.cursor()
        try:
            cursor.execute(
            ...
            logger.success(f"Total power today {total_power} inserted successfully in local database")
            return True 
        except mysql.connector.Error as e:
            logger.error(f"Failed to insert total power today {total_power}: {e}")
            return False 
        finally:
            cursor.close()
