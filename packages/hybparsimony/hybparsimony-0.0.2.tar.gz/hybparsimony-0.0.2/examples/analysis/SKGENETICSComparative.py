...
now = datetime.now()
NAME_FILE = 'sklearn_genetics_opt_' + now.strftime("%Y_%m_%d_%H_%M_%S_") + '.csv'
...
for type_prob in ['binary', 'regression', 'multiclass']:
    for nrow, row in res_basedata.iterrows():
        if row['type_prob']==type_prob:
            ...
            X_train_val_orig, X_test_orig, y_train_val, y_test = train_test_split(X.values, y.values,
            ...
            for run_total in range(NUM_RUNS):
                ...
                #     PBOUNDS = {'alpha': Continuous(1e-5, 1e5, distribution='log-uniform'),
                #                'gamma': Continuous(1e-5, 1e3, distribution='log-uniform')}
                # elif row['type_prob']=='binary':
                #     algorithm = 'LogisticRegression'
                #     scoring_gen = 'neg_log_loss'
                #     clf = LogisticRegression(max_iter=5000)
                #     PBOUNDS = {'C': Continuous(1e-5, 1e3, distribution='log-uniform')}
                # else:
                #     algorithm='LogisticRegression'
                #     scoring_gen = 'f1_macro'
                #     clf=LogisticRegression(max_iter=5000)
                ...
                try:
                    # 1. Search best hyperparameter with Bayesian Optimization
                    # --------------------------------------------------------
                    ...
                    # 2. Search the best features
                    # ---------------------------
                    cv = KFold(n_splits=5, random_state=1234*run_total, shuffle=True)
                    evolved_estimator = GAFeatureSelectionCV(estimator=model,
                                                    cv=cv,
                                                    scoring=scoring_gen,
                                                    population_size=15,
                                                    generations=NUM_GEN,
                                                    ...
                                                    keep_top_k=2,
                                                    elitism=True)
                    evolved_estimator.fit(X_train_val, y_train_val)
                    
                    # Extract Best Feats y select from X_train and X_test 
                    best_feats = input_names[evolved_estimator.support_]
                    best_feats_bool = evolved_estimator.support_ 
                    NFS = len(best_feats)
                    
                    X_train_val = X_train_val[:,best_feats_bool]
                    X_test = X_test[:,best_feats_bool]
                    
                    # 3. Search best hyperparameter with the selected features
                    # --------------------------------------------------------
                    optimizer = BayesianOptimization(f=optimize_fun_bayes,
                                                pbounds=PBOUNDS,
                                                random_state=1234*run_total,
