...
def waterlevels_run(
    ...
    for resource in resource_keys:
        ...
        if static:
            create_static_page(resource, db)
...
def step1_data_validation(resource, db, plot_figures=True):
    ...
    logger.info("Retrieving water level data...")
...
def step2_defining_recovered_wls(resource, db, plot_figures=True):
    ...
    logger.info(
        str(pd.merge(qc, cmp_df["all_conditions"], left_index=True, right_index=True))
    )
    ...
    data_quality = pd.merge(qc, cmp_df, left_index=True, right_index=True).reset_index()
...
def create_static_page(resource, db):
