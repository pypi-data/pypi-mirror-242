...
def plot_wl_historical_rankings(ranks, qc, title="", figsize=None):
    ...
    year_min = ranks.obs_date.dt.year.min()
    year_max = ranks.obs_date.dt.year.max()
    ...
    for w in unsorted_well_ids:
        ...
        years_data = len(ranks[ranks.well_id == w].obs_date.dt.year.unique())
        sort_records.append(
    ...
    for i, well_id in enumerate(well_ids):
        for j in range(ncols):
            ...
            ranking = None
            ranks_subset = ranks[
                (ranks.well_id == well_id) & (ranks.obs_date.dt.year == year)
            ]
            if len(ranks_subset):
