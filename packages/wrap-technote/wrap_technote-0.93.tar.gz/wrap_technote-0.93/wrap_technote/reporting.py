...
def load_reporting_period(key=CURRENT_RPERIOD, **kwargs):
    ...
    if key in reporting_period_paths:
        ...
        logger.debug(f"Found pre-existing definition hard-coded in source: key={key} path={path}")
        ...
        path = ANNUAL_WRA_PATH / f"Backcalc_{year1}_{start_year}-{end_year[2:]}" / "Code"
...
def rp_key_to_path(key):
    ...
    if key == "2019-20":
        ...
        end_year = str(int(year1) - 1)
        start_year = str(int(end_year) - 4)
        return ANNUAL_WRA_PATH / f"Backcalc_{year1}_{start_year}-{end_year[2:]}" / "Code"
    else:
...
class Report:
    ...
    def get_aggregate_resources_data(self, param, workbook_name, worksheet_name):
        ...
        for agg_res_key, sdf in aggs.groupby("aggregate_resource_key"):
            ...
            rdf = rdf[['resource_key'] + [c for c in rdf.columns if not c == 'resource_key']]
...
class Resource:
    ...
    def get_wl_ranking_qc_results(self, wls):
        ...
        qc = wls.groupby("well_id").apply(
            lambda df: calc_well_record_quality(
                df, years_to_extract, spans_to_count_in=spans_to_count_in
            )
        )
        indexers = []
        comparisons = []
...
class AggregateGroundwaterResource:
    ...
    def __init__(self, aggregate_resource_key, reporting_period=CURRENT_RPERIOD, param=None):
        if not isinstance(reporting_period, ReportingPeriod):
            reporting_period = load_reporting_period(reporting_period)
            self.reporting_period = reporting_period
        else:
    ...
    def read_data(self, workbook_name, worksheet_name=None, param=None):
        ...
        if param is None:
            logger.debug(f"param=None; self.param={self.param}; workbook_name={workbook_name} worksheet_name={worksheet_name}")
            if self.param:
                param = self.param
        ...
        for resource_key, df in dfs.items():
            ...
            df = df[['resource_key'] + [c for c in df.columns if not c == "resource_key"]]
            concat_dfs.append(df)
...
def read_data(
    ...
    for resource_key in set(resource_keys):
        ...
        try:
            ...
            logger.warning(f"Error reading data for resource={resource.resource_key} workbook_name={workbook_name} worksheet_name={worksheet_name}. Skipping.")
        else:
