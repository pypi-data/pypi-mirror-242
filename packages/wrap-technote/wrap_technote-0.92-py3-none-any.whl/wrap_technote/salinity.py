...
def calculate_salinity_indicator_summary_results(alldf, pct_diff, trend_pct):
    ...
    curr_pct_diffs = (
        alldf[incl_idx]
        .curr_tds_pct_diff_indicator.value_counts()
        .reindex(pct_diff["labels"])
    )
    curr_pct_diffs = (
        curr_pct_diffs[::-1]
        .reset_index()
        .rename(
            columns={
                "index": "curr_tds_pct_diff_indicator",
                "curr_tds_pct_diff_indicator": "n_wells",
            }
        )
    )
    ...
    tds_trend_pct_changes = (
        ...
        .reindex(trend_pct["labels"])
    )
    tds_trend_pct_changes = (
        tds_trend_pct_changes[::-1]
        .reset_index()
        .rename(
            columns={
                "index": "tds_trend_pct_change_indicator",
                "tds_trend_pct_change_indicator": "n_wells",
            }
        )
    )
    pct_wells = (
...
def collate_salinity_summary_data(resource, db=None):
    ...
    if len(tdstrends):
        ...
        tdstrends_mgl_summ_1 = pd.DataFrame()
        tdstrends_mgl_summ_2 = pd.DataFrame()
        tdstrends_5yr_summ_1 = pd.DataFrame()
        tdstrends_5yr_summ_2 = pd.DataFrame()
    ...
    if len(curr_pct_diff):
        ...
        curr_pct_diff_summ_1 = pd.DataFrame()
...
def construct_salinity_template_sentences(
    ...
    cat2 = data["trend_pct_summ_1"].loc["slope_pct_change_yr"]
