...
def rank_and_classify(s, param_name=""):
    ...
    new_df[percentile_col] = s.rank(pct=True) * 100
    new_df[bom_class_col] = percentile_to_bom_class(new_df[percentile_col])
    # new_dfbom_class_col] = new_df[percentile_col].transform(
    #     percentile_to_bom_class
    # )
    return new_df
...
def calc_well_record_quality(
    ...
    results = {
        "n_years_with_obs": len(df.groupby(df[dt_col].dt.year).first()),
