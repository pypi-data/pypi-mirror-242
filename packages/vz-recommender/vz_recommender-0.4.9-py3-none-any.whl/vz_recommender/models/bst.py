...
class BayouTaskTTwoNLP(nn.Module):
    def __init__(self, task_type_dim, user_deep_dims, user_deep_embed_dims, user_num_wide, user_wad_embed_dim,
        ...
        self.task_type_embedding = nn.Embedding(task_type_dim, seq_embed_dim)
        # user layers
        ...
        self.user_dense_0 = torch.nn.Linear(
            # task_out + aep_seq_out + user_ctx_out + svc_out + new_svc_out + svc_desc_dly_out
    ...
    def forward(self, task_type_in, user_deep_in, offer_deep_in, svc_in, new_svc_in, svc_desc_dly_in,
        ...
        user_out = torch.stack([task_type_out, aep_seq_out, user_ctx_out, svc_out, new_svc_out, svc_desc_dly_out], dim=1)
...
class BSTAudienceTaWTTwoNLP(nn.Module):
    def __init__(self, user_deep_dims, user_deep_embed_dims, user_num_wide, user_wad_embed_dim,
                 ...
                 svc_dim, svc_embed_dim, new_svc_dim, new_svc_embed_dim,
                 svc_desc_dly_dim, svc_desc_dly_embed_dim,
                 page_dim, item_dim, seq_embed_dim, user_moe_kwargs, sequence_transformer_kwargs=None):
        super().__init__()
        # user layers
        self.user_context_head = ContextHead(
            deep_dims=user_deep_dims,
...
class AudienceTTIHQMM(nn.Module):
    def __init__(self, user_deep_dims, user_deep_embed_dims, user_num_wide, user_wad_embed_dim,
                 offer_deep_dims, offer_deep_embed_dims, offer_num_wide, offer_wad_embed_dim,
                 svc_dim, svc_embed_dim, new_svc_dim, new_svc_embed_dim,
                 svc_desc_dly_dim, svc_desc_dly_embed_dim, ihq_dim,
        ...
        self.ihq_embedding = nn.Embedding(ihq_dim, seq_embed_dim // 2)
    ...
    def forward(self, user_deep_in, offer_deep_in, svc_in, new_svc_in, svc_desc_dly_in,
        ...
        ihq_out = self.ihq_embedding(ihq_in.long())
        ihq_out = self.mm_pooling(ihq_out)
...
class BayouTaskNLP(nn.Module):
    def __init__(self, task_type_dim, offer_deep_dims, offer_deep_embed_dims, offer_num_wide, offer_wad_embed_dim,
        ...
        self.user_dense_0 = torch.nn.Linear(
            # task_type_out + nlp_out 
            in_features=seq_embed_dim + seq_embed_dim,
            out_features=seq_embed_dim * 4
        )
        self.user_dense_1 = torch.nn.Linear(
            in_features=seq_embed_dim * 4,
    ...
    def forward(self, task_type_in, offer_deep_in, offer_wide_in, nlp_in):
        """
        Args:
            deep_in: list, a list of Tensor of shape [batch_size, deep_dims].
            seq_in: Tensor, shape [batch_size, seq_len].
...
#        nlp_out = self.nlp_encoder(**nlp_in).last_hidden_state[:, 0, :].to(dtype=torch.float32)
        ...
        user_out = torch.stack([task_type_out, nlp_out], dim=1)
        user_out = self.user_att_pooling(user_out)
        user_out, user_aux_loss = self.user_moe(user_out)
        user_out = torch.concat(user_out.unbind(1), dim=1)
        user_out = self.user_dense_0(user_out)
...
class BayouTaskNLPMultiClass(nn.Module):
    def __init__(self, task_type_dim, out_dim,
                 seq_embed_dim, user_moe_kwargs,
                 nlp_encoder_path, nlp_dim, sequence_transformer_kwargs=None):
        super().__init__()
    ...
    def forward(self, task_type_in, nlp_in):
        """
        Args:
            deep_in: list, a list of Tensor of shape [batch_size, deep_dims].
            seq_in: Tensor, shape [batch_size, seq_len].
