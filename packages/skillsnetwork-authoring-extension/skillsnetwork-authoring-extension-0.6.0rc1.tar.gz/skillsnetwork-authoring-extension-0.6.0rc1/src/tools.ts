 ...
 * @param context Notebook context
 */
export function getLabCommitID(
  labContent: LabContent,
) : string {

  // check if labContent has the expected structure
  if (
  ...
  // Attempt to retrive commitID from either metadata or content.metadata
...
export const openIndependentLab = async (awb_token: string, docManager: IDocumentManager, contentsManager: ContentsManager): Promise<NotebookPanel> => {
  ...
  if (prevFile) {
    ...
    if (currCommitID !== prevCommitID) {
      await convertFileToBackup(prevFile, contentsManager);
      await showDialog({
        title: 'Your Lab was Updated',
        body: `The newest published version of "${labFilename}" is loaded. Your previous version has been backed up under "${Globals.BACKUP_EXT}.ipynb"`,
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (axiosError.response && axiosError.response.status !== 404) {
        // If the error is not a 404 error, log it and throw it
      ...
      // If not an error object, throw it and handle appropriately outside
 ...
 */
async function convertFileToBackup(file: Contents.IModel, contentsManager: ContentsManager) : Promise<void> {
  const path = file.path;
  console.log("path: ", path);
  const fileExt = path.split('.').pop();

  // removes the last portion of the string, separated by the last dot
  const fileNameWithoutExt = path.replace(/\.[^/.]+$/, "");
  const newFilename = `${fileNameWithoutExt}${Globals.BACKUP_EXT}.${fileExt}`;
  const prevFile = await getFile(newFilename, contentsManager);
  
  if (prevFile) {
    try {
      await contentsManager.delete(newFilename);
    } catch (error) {
      console.error("Unexpected error occured: ", error);
    }
  }
  try {
    await contentsManager.rename(path, newFilename);
  } catch (error) {
    console.error("Unexpected error occured: ", error);
  }
}
