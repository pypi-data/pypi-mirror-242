...
def iauPas(al, ap, bl, bp):
    return lib.iauPas(al, ap, bl, bp)

def iauS2c(theta, phi):
...
#s = ps.d2dtf(scale, ndp, dj1, dj2)  #debugs for scale
print(v) 
...
#delat1 = ps.dat(iy, im, iday, fday)  #OK
print(iy, im, iday, delat)
#print(iy, im, iday, delat1)
line()
...
scale = bytes(scale).decode('utf-8','ignore')
print(scale, d1, d2)
line()
...
#a   =  np.array([3.0,   4.0,   0.0]).reshape(1,3)
an  =  sf.pymPm(a)
print(an)
a   =  np.array([0.3,   1.2,  -2.5]).reshape(1,3)
an  =  sf.pymPm_A(a)
...
phi = 0.3456789
r   = np.array([[2.0, 3.0, 2.0], [3.0, 2.0, 3.0], [3.0, 4.0, 5.0]]).reshape(3,3)
r1  = np.array([[2.0, 3.0, 2.0], [3.0, 2.0, 3.0], [3.0, 4.0, 5.0]]).reshape(3,3)
 
rx  = sf.pymRx(phi, r)
print(rx)
rx  = sf.pymRx_A(phi, r1)
print(rx)
