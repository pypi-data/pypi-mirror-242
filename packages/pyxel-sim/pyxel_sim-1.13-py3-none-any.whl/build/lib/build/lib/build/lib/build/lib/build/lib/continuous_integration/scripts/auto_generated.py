...
class ModelSceneGenerationLoadStarMapArguments(Mapping[str, Any]):
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('right_ascension', 'declination', 'fov_radius', 'band'))
...
class ModelPhotonCollectionSimpleApertureArguments(Mapping[str, Any]):
    pixel_scale: float = field(
        metadata=schema(
            title='pixel_scale'
            ,description='Pixel scale. Unit: arcsec/pixel.'
        )
    )
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('pixel_scale', 'aperture', 'wavelength_band'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
class DetailedDetectionPipeline(DetectionPipeline):
    ...
    photon_collection: Optional[
        Sequence[Union[ModelPhotonCollectionIllumination, ModelPhotonCollectionLoadImage, ModelPhotonCollectionLoadPsf, ModelPhotonCollectionOpticalPsf, ModelPhotonCollectionShotNoise, ModelPhotonCollectionSimpleAperture, ModelPhotonCollectionStripePattern, ModelPhotonCollectionWavelengthDependenceAirs, ModelLoadDetector, ModelSaveDetector, ModelFunction]]
