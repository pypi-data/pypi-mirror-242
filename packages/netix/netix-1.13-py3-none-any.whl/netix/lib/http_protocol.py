...
from ._content import exception_content
...
class HTTPProtocol(asyncio.Protocol):
    ...
    async def handle_exception(self, exc):
        if (
            ...
            or (self._response.headers_sent() and not self._request.upgraded)
        ):
            return

        self.print_exception(exc, quote(unquote(self._request.path.decode('latin-1'))))
        ...
        if self.options['debug']:
            ...
            formatted_traceback = "<br>".join(html.escape(line) for line in traceback_text.splitlines())

            error_type = traceback_text.splitlines()[-1].split(":")[0].strip()
            error_message = str(exc)
            traceback_lines = traceback_text.splitlines()
            underlined_line = 'No any critical exception found by handler!'
            for i, line in enumerate(traceback_lines):
                if "^" in line:
                    underlined_line = traceback_lines[i - 1]

            # Extract the file and line number from the traceback
            file_and_line = ""
            for line in traceback_lines:
                if "File " in line:
                    file_and_line = line.strip()

            html_response = exception_content(error_message, formatted_traceback, underlined_line, error_type, file_and_line)
        else:
            # Return a simple Internal Server Error response without traceback
            html_response = f"""
            <html>
            <head>
        ...
        html_response = html_response.encode(encoding)
