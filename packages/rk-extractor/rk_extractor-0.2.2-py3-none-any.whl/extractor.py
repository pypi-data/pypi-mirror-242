...
class extractor:
    ...
    def _add_ext_const(self, nll):
        ...
        log.debug('-' * 80)
        log.debug(f'{"Parameter":<40}{"Value":<20}{"Error":<20}')
        log.debug('-' * 80)
        for par, val, var in zip(l_par, l_val, l_var):
            name = par.name
            err  = math.sqrt(var)
            log.debug(f'{name:<40}{val:<20.3e}{err:<20.3e}')
        log.debug('-' * 80)

        cns_ext = zfit.constraint.GaussianConstraint(params = l_par,
                                                     observation= l_val,
                                                     uncertainty= cov)

        nll = nll.create_new(constraints=cns_ext)
    ...
    def get_fit_result(self):
        ...
        log.info('Adding CK constraints')
        nll = self._add_const(nll)

        log.info('Adding extra constraints')
        nll = self._add_ext_const(nll)
