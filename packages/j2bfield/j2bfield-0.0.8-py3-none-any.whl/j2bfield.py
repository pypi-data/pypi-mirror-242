...
def generate_current_plane(plane_size=(1000, 1000), J = 1., print_allowed = False):
    ...
    for i in range(plane_size[0]):
        for j in range(plane_size[1]):
            current_tensor[i, j] = J*np.array(cur2tensor[plane[i, j]])
...
def generate_current_field(current_field_size=(10, 10, 10), *args, **kwargs):
    ...
    for arg in args:
        if isinstance(arg, tuple) and len(arg) == 2:      
            J_field[:,:,arg[1],:2] = generate_current_plane((current_field_size[0],current_field_size[1]), arg[0])
        elif isinstance(arg, int):
            J_field[:,:,arg,:2] = generate_current_plane((current_field_size[0],current_field_size[1]))
        else:
            print("J平面要求的格式错误")
    return J_field
   
def calculate_magnetic_plane(J_plane, relactive_z, dx = 1, dy = 1):
    
    
    mu0 = 4 * np.pi * 1e-7  # 真空中的磁导率

    # 获取电流密度张量的大小
    nx, ny, _ = J_plane.shape
    
    J = np.zeros((nx, ny, 3))
    
    J[:,:,:2] = J_plane
    
    B_plane = np.zeros_like(J)
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            r_mag = np.sqrt(r_vec_x**2 + r_vec_y**2 + relactive_z**2)
            if relactive_z == 0:
                r_mag[i, j] = np.inf  # 自身点距离设为无穷大，忽略自身对磁场的贡献
            
            dJ = J[i, j, :][np.newaxis, np.newaxis, :]
            
            # 利用比奥-萨伐尔定律计算磁场，利用广播规则来避免循环计算
            
            cross_prod = np.cross(dJ, np.concatenate((np.stack((r_vec_x, r_vec_y), axis=-1), np.full(r_vec_x.shape + (1,), relactive_z)), axis=2))
            dB = cross_prod * mu0 / (4 * np.pi * r_mag[..., np.newaxis]**2)
...
def plot_vectors_plane(J = None, J_z=5, B = None, B_z=5):
    ...
    mul_factor = 4 * abs(J_z - B_z)
    mu0 = 4 * np.pi * 1e-7 
    add_times = mul_factor/mu0
        ...
        if len(J.shape) == 4:
            for i in range(J.shape[0]):
                for j in range(J.shape[1]):
                    if np.linalg.norm(J[i,j]) != 0:
                        tongyi = np.linalg.norm(J[i,j,J_z])
                    else:
                        tongyi = 1
                    ax.quiver(i, j, J_z, J[i, j, J_z, 0]/tongyi, J[i, j, J_z, 1]/tongyi, J[i, j, J_z, 2]/tongyi, color="black", lw = 2)
        elif len(J.shape) == 3:
            for i in range(J.shape[0]):
                for j in range(J.shape[1]):
                    if np.linalg.norm(J[i,j]) != 0:
                        tongyi = np.linalg.norm(J[i,j])
                    else:
                        tongyi = 1
                    ax.quiver(i, j, J_z, J[i, j, 0]/tongyi, J[i, j, 1]/tongyi, 0, color="black", lw = 2)
    ...
    try:
        if len(B.shape) == 4:
            ...
            B_colors = B_norm / np.max(B_norm)
            for i in range(B.shape[0]):
                for j in range(B.shape[1]):
                    ax.quiver(i, j, B_z, B[i, j, B_z, 0], B[i, j, B_z, 1], B[i, j, B_z, 2], length=add_times, color=plt.cm.jet(B_colors[i,j,B_z]), lw = 1)
        elif len(B.shape) == 3:
            B_norm = np.linalg.norm(B, axis=-1)
            B_colors = B_norm / np.max(B_norm)
            for i in range(B.shape[0]):
                for j in range(B.shape[1]):
                    ax.quiver(i, j, B_z, B[i, j, 0], B[i, j, 1], B[i, j, 2], length=add_times, color=plt.cm.jet(B_colors[i,j]), lw = 1)
    except AttributeError :
        pass
...
def plot_tensor(J = None, B = None):
    ...
    try:
        ...
        J_colors = J_norm / np.max(J_norm)
        for i in range(J.shape[0]):
            for j in range(J.shape[1]):
                for k in range(J.shape[2]):
                    if np.linalg.norm(J[i,j,k]) != 0:
                        tongyi = np.linalg.norm(J[i,j,k])
                    else:
                        tongyi = 1
                    ax.quiver(
                        i, j, k, J[i,j,k,0]/tongyi, J[i,j,k,1]/tongyi, J[i,j,k,2]/tongyi,lw = 2,
                        color="black")
        ...
        for i in range(B.shape[0]):
            for j in range(B.shape[1]):
                for k in range(B.shape[2]):
                    ax.quiver(
                        i, j, k, B[i,j,k,0], B[i,j,k,1], B[i,j,k,2],lw = 1,
                        length=add_times, color=plt.cm.jet(B_colors[i,j,k])
                    )
    except AttributeError:
        pass
