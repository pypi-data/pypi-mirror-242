...
def style():
    for i in range(5):
        style = wf(task='gen_style' ,style=i) # iris act 
        result = style.inference(data="ele.jpg" ,img_type='cv2')
        style.show(result)
        style.save(result,"ele_{}.jpg".format(i))
    #     result,img = style.inference(data="ele.jpg" ,img_type='cv2')
    #     style.show(result)
    #     result,img = style.inference(data="ele.jpg" ,img_type='pil')
    #     style.show(result)
    #     result = style.inference(data="ele.jpg" ,img_type='pil')
    #     style.show(result)
        # style.format_output(lang="zh")
        # cv2.imshow("img",img)
        # cv2.imshow("result",result)
        # cv2.waitKey(0)
        # style.show(img)
    # styles = [ wf(task='gen_style',style=i) for i in range(5) ]
    # # result,img = style.inference(data="ele.jpg",img_type='pil')
    # import random
    # # style.show(result)
    # # 打开摄像头，实时风格迁移
...
def drive_demo():
    from XEdu.hub import Workflow as wf
    drp = wf(task='drive_perception')# ,checkpoint="checkpoints/yolopv2_post_Nx3x192x320.onnx") 
    # result = drp.inference(data="demo.jpg")

    result,image = drp.inference(data="cat1.jpg",img_type='pil')
    drp.format_output(lang="zh")
