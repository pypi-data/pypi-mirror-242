...
class SquadExample(object):
    ...
    def __repr__(self):
        s = []
        s.append("qas_id: %s" % (printable_text(self.qas_id)))
        s.append("question_text: %s" % (printable_text(self.question_text)))
...
def read_squad_examples_context(input_data):
    """Read a SQuAD context into a list of SquadExample."""
...
def read_squad_examples_qa(input_data):
    """Read a SQuAD qa into a list of SquadExample."""
    ...
    return qas_id, question_text

def squad_example(qas_id, question_text, doc_tokens, orig_answer_text=None, start_position=None, end_position=None):
...
def read_squad_examples_file(input_file):
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if not nbest:
            nbest.append(
                _NbestPrediction(text="None", start_logit=0.0, end_logit=0.0))
...
def main():
    ...
    eval_examples = read_squad_examples_file(input_file=args.predict_file)
