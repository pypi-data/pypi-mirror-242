...
def lint(session: nox.Session) -> None:
    session.install("black[colorama] >= 23.11.0, < 24")
    session.install("flake8 >= 6.1.0, < 7")
    ...
    session.install("mypy >= 1.7.0, < 2")
    session.install("nox >= 2023.4.22, < 2024")
    session.install("pep8-naming >= 0.13.3, < 1")
    session.install("pyright >= 1.1.335, < 2")
    session.install("reuse >= 2.1.0, < 3")

    session.install("types-Pillow == 10.0.0.3, < 11")
    ...
    if not fix:
        ...
        session.run("pyright", "--warnings", *paths, silent=True)
...
def coverage(session: nox.Session) -> None:
    session.install("coverage[toml] >= 7.3.0, < 8")
...
if os.name == "posix":
    coverage = nox.session(python="3.10")(coverage)
...
def freeze(session: nox.Session) -> None:  # noqa: C901
    ...
    session.install("pyinstaller >= 6.2.0, < 7")
    ...
    shutil.rmtree(dst_dir / "imageio/resources/images", ignore_errors=True)
    ...
    if os.name == "nt":
        for spdx_license_id in ["WIN10SDK-RTM-AUG_2018", "mlt687465"]:
            shutil.copy2(f"LICENSES/LicenseRef-{spdx_license_id}.txt", licenses_dst_dir)

    session.install("pip-licenses >= 4.3.3, < 5")
    session.install("pipdeptree >= 2.13.1, < 3")
    session.install("reuse >= 2.1.0, < 3")
    ...
    session.run(
        ...
        f"--output-file={licenses_dst_dir / 'LicenseRef-no_vtf.txt'}",
        "--packages",
    ...
    def with_common_options(
        ...
        def decorator(run: _TestCaseRun[_TC]) -> _TestCaseRun[_TC]:
            def inner(self: _TC, args: Sequence[object], opts: dict[str, object]) -> None:
                ...
                no_progress_value = ... if no_progress is True else None
