...
from convergence.dto.base_dto import ApiResponse, ApiResponseBody
...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    def __build_convergence_unified_bad_request(self, error_response, http_request):
        ...
        response.header.code = errors.INVALID_DATA
        response.body = self.__convert_pydantic_error_to_convergence(error_response)
    ...
    def __convert_pydantic_error_to_convergence(self, error_response) -> ApiResponseBody:
        result = BadRequestDetails()

        errors = {}
        for entry in error_response['detail']:
            map_id = '.'.join(entry['loc'])
            error_entry = None
            if map_id in errors:
                ...
                error_entry.field = '.'.join(entry['loc'][1:])
    ...
    def __invalid_json_input(self, http_request):
        ...
        response.header.code = errors.UNPARSEABLE_INPUT
