...
if load_dotenv(root_env):
    log.info(f"loading environment from .env config file {root_env}")
    with open(root_env, "r") as f:
        for line in f:
            log.info(line)
else:
    log.info(f"no {root_env} file found, no environment loaded")
...
class Configuration:
    ...
    def __init__(self) -> None:
        ...
        self.runner_image = _default_runner_image()
