...
def app(
    ...
    def decorator_apps(func: Callable[[Any], Any]) -> Callable[[Any, Any], Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            if id is None:
                # # If no name was assigned, name the app after the function
                set_id = func.__name__.replace("_", "-")  # Fix unsupported chars
            else:
                set_id = id
        ...
        app = App(func=wrapper, id=id, type=type, parameters=parameters)
        context.add_app(app)
        return wrapper
