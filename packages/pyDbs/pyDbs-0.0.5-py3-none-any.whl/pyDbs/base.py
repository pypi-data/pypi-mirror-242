...
class SymMaps:
	...
	def getLagFromSol(self, x, symbol, lags, dropna=False, **kwargs):
		""" Return a shifted symbol like self.addLaggedSym, but without adding it to the compilation stage."""
	...
	def getLag(self, x, lags, name = None, dropna = False, **kwargs):
		...
		m = self.lagMaps(adj.rc_pd(noneInit(name, self[x.name]), **kwargs), lags)
	...
	def getRoll(self, x, rolls, name = None, dropna = False, **kwargs):
		""" Return a shifted symbol like getShiftFromSol, but where x is the "non-shifted" symbol (pd.Series) instead of the global vector"""
		m = self.rollMaps(adj.rc_pd(noneInit(name, self[x.name]), **kwargs), rolls)
		return pd.Series(x[m.values].values, index = x.index)
	...
	def getShift(self, x, shifts, name = None, dropna = False, opt = None, **kwargs):
		""" Return a shifted symbol like getShiftFromSol, but where x is the "non-shifted" symbol (pd.Series) instead of the global vector"""
		m = self.shiftMaps(adj.rc_pd(noneInit(name, self[x.name]), **kwargs), shifts, **noneInit(opt, {}))
		return pd.Series(x[m.values].values, index = x.index)
