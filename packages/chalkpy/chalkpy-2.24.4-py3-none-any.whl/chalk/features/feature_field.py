...
"""
    ...
    def _validate_join(self) -> Filter:
        ...
        if not (self.is_has_many or self.is_has_one):
            raise TypeError(
                f"Feature {self.root_fqn} has a join filter ({join}) but its type annotation is not a feature class or dataframe ({self.typ})."
            )
        self._validate_filter(join)
        return join
    ...
    def _validate_filter(self, filter: Filter):
        ...
        if filter.operation in ("is_near_ip", "is_near_l2", "is_near_cos"):
            # Synthesize a distance pseudofature on the foreign feature set
