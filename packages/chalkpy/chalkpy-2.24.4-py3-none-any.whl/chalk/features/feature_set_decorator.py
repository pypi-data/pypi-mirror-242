...
from chalk.features.tag import Tags
from typing import Type, TypeVar, overload, Optional, Union, Callable
...
def features(
    ...
    *,
    ...
    def wrap(c: Type[T]) -> Type[T]:
        ...
        try:
            class_source = _get_class_source(c)
        ...
        error_builder = FeatureClassErrorBuilder(
            filename=source_info.filename if source_info is not None else "__main__",
            namespace=namespace,
        ...
        if name is not None and re.sub(r"[^a-z_0-9]", "", namespace) != namespace:
            ...
            error_builder.add_decorator_kwarg_value_error(
                kwarg="name",
                code=code,
                message=f"duplicate namespace {name}",
            )
        ...
        if isinstance(max_staleness, str):
            try:
                ...
                error_builder.add_decorator_kwarg_value_error(
                    kwarg="max_staleness",
                    code="Max-staleness must be a valid duration string, e.g. '30m' or '1h 30m'.",
                    message=f"invalid duration {max_staleness}",
                )
                raise
...
def _parse_annotation_comments(source_info: ClassSource, cls_annotations: Dict[str, Any]) -> Mapping[str, str]:
    ...
    source_has_decorator = source_lines[0].lstrip().startswith("@")
    """python 3.8 does not include decorator in getsource return"""

    for stmt in source_info.tree.body:
        if isinstance(stmt, ast.AnnAssign) and isinstance(stmt.target, ast.Name):
            line = stmt.lineno - source_info.tree.lineno
            line = line if source_has_decorator else line - 1
            comments: List[str] = []
...
def _process_class(
    ...
    for name, annotation in raw_cls_annotations.items():
        if name in ("features", "namespace", "items", "is_near"):
            detail = f"Feature '{name}' on class '{cls.__name__}' uses a reserved name."
            error_builder.add_property_error(
                feature_name=name,
                message=f"reserved name '{name}'",
                code=detail,
            )
        ...
        if isinstance(annotation, Windowed):
            ...
            if wind is None or not isinstance(wind, Windowed):
                ...
                error_builder.add_annotation_error(
                    feature_name=name,
                    message="missing = windowed('10m', ...)",
                    code=f"Windowed feature '{namespace}.{name}' is missing a windowed(...) call.",
                )
                raise TypeError(
                    (
    ...
    for name, member in inspect.getmembers(cls):
        ...
        if isinstance(member, Windowed) and not isinstance(cls_annotations.get(name), Windowed):
            ...
            error_builder.add_annotation_error(
                feature_name=name,
                message="missing Windowed[...]",
                code=message,
            ) or error_builder.add_property_error(
                feature_name=name,
                message="missing Windowed[...]",
                code=message,
            )
    ...
    def __chalk_observed_at__(cls: Type[Features]) -> FeatureWrapper:
        ...
        if ts_feature is not None:
            if ts_feature.attribute_name != GENERATED_OBSERVED_AT_NAME:
                m = f"Object {cls.__name__} has no attribute '{GENERATED_OBSERVED_AT_NAME}"
                error_builder.add_property_error(
                    ts_feature.attribute_name,
                    code=m,
    ...
    for f in tuple(cls_fields):
        # error_builder.add_property_value_error(f.attribute_name, message="nope", code="hello")
        if singleton and f.primary and f.name != "__chalk_singleton_id__":
            ...
            # error_builder.remove_feature_action(
...
def _class_setattr(cls: Type[Features], key: str, value: Any):
    ...
    if isinstance(value, Underscore):
        ...
        raise TypeError(
            f"In order to define feature '{cls.namespace}.{key}', please set it equal to an underscore expression. For example, `{cls.__name__}.{key}: int = _.a + _.b`."
        )
    if f is not None:
