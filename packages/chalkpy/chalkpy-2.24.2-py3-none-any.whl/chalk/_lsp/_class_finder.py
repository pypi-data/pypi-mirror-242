...
def get_resolver_ast(fn) -> ast.FunctionDef:
    ...
    source = "".join(lines)
    tree = ast.parse(source)
    resolver_finder = _ChalkFunctionFinder(name)
    try:
        resolver_finder.visit(tree)
...
def get_source_filename(object) -> Optional[str]:
    """Modified in first line from inspect.getsource(object)"""
    try:
        filename = sys.modules.get(object.__module__).__file__
    except:
