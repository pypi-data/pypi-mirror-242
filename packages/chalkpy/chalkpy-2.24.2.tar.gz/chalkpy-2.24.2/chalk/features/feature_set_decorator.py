...
def _init_fn(
    ...
    body_lines.append(f"for __chalk_kwarg__ in __chalk_kwargs__:")
    body_lines.append(f"    if __chalk_kwarg__ not in __chalk_feature_names__:")
    body_lines.append(
...
def _process_class(
    ...
    try:
        ...
        dedent_source = textwrap.dedent(class_source)
        try:
            tree = get_class_ast(cls)
        except:
            tree = None
    ...
    for name, annotation in raw_cls_annotations.items():
        if name in ("features", "namespace", "items", "is_near"):
            message = f"Feature name '{name}' in '{namespace}' is a reserved name. Please rename this feature."
    ...
    for name, member in inspect.getmembers(cls):
        ...
        if name not in cls_annotations and isinstance(member, Feature):
            ...
            if member._is_feature_time:
                ...
                error_builder.add_annotation_error(name, message)
                raise TypeError(message)
        ...
        if isinstance(member, Windowed) and not isinstance(cls_annotations.get(name), Windowed):
            ...
            error_builder.add_annotation_error(name, message) or error_builder.add_property_error(name, message)
            raise TypeError(message)
    ...
    if singleton:
        for c in cls_fields:
            if c.primary:
                ...
                error_builder.add_property_error(c.attribute_name, message)
                raise ValueError(message)
    ...
    def __chalk_observed_at__(cls: Type[Features]) -> FeatureWrapper:
        ...
        if ts_feature is not None:
            if ts_feature.attribute_name != GENERATED_OBSERVED_AT_NAME:
                ...
                error_builder.add_property_error(ts_feature.attribute_name, m)
    ...
    for f in tuple(cls_fields):
        ...
        for i in range(1, f.version.maximum + 1):
            ...
            if f_i.attribute_name in cls.__annotations__:
                ...
                error_builder.add_property_error(f_i.attribute_name, m)
                raise ValueError(m)
    ...
    set_new_attribute(cls=cls, name="__chalk_diagnostics__", value=error_builder.diagnostics)
    return cls
