...
class Messages:
    ...
    # @param    styled      <b>(Optional)</b> A boolean value that says wether the message contains styled text or not. This argument defaults to <tt>False</tt>
    # @return A @p dict in json format
    # @author Felix Hune
    # @version 1.1
    # @date 2023-11-23
    def send(self, message: str, recipients: list[str], attachments: list[str]=[], styled: bool = False) -> dict:
        if not styled:
            ...
            r = requests.post(f"{self.address}:{self.port}/v2/send", headers={'Content-Type': 'application/json'}, json={"base64_attachments": attachments, "message": message, "number": self.number, "recipients": recipients, "text_mode": "styled"})
        r = r.json()
        return r
