...
def event_study(factor_data, benchmark_adj_close, stock_adj_close, sample_period=(-45, -20), estimation_period=(-5, 20)):
    ...
    return pd.DataFrame(ret, columns=range(*estimation_period))
...
def create_factor_data(factor:pd.DataFrame or Dict[str, pd.DataFrame], adj_close:pd.DataFrame,
    ...
    dates = adj_close.index.intersection(ref.index)

    ret = {}
    for name, f in factor.items():
        reashaped_f = f.reindex(dates, columns=sids)
    ...
    total_index = None
    reshaped_adj_close = adj_close.reindex(dates, columns=sids)
    for d in days:
        temp = (reshaped_adj_close.shift(-d-1) / reshaped_adj_close.shift(-1) - 1).unstack()
        ret[f"{d}D"] = temp.values
        total_index = temp.index

    if event is not None:
        reshaped_event = event.reindex(index=dates, columns=sids)
