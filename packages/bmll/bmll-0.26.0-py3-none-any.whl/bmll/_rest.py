...
from requests import request, exceptions as requests_exc

from bmll.exceptions import QUOTA_EXCEEDED_ERROR, AuthenticationError, LoginError, QuotaReachedError, RequestTooLarge
...
class Session:
    ...
    def _new_auth_status_check(self):
        """ Hit the status check endpoint of the new auth service.
            If the response is not a 200, revert to using the old lab services
        """
        # don't use the built-in request bc that will get stuck in a loop on login.
        use_new_services = True
        try:
            resp = request('get', self.SERVICE_URLS['lab-auth'] + '/status-check')
        ...
        if not use_new_services:
            compute_url = self.SERVICE_URLS
            if 'dev' in compute_url:
                base_url = 'https://{service}-dev.dev.bmlldev.com'
            elif 'staging' in compute_url:
                base_url = 'https://{service}-staging.staging.bmlltech.com'
            else:
                base_url = 'https://{service}.bmlltech.com'
            self.SERVICE_URLS['compute'] = base_url.format(service='compute')
            self.SERVICE_URLS['lab-auth'] = base_url.format(service='lab-auth')
