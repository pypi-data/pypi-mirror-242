...
__all__ = ('listing_trades', 'listing_orders', 'instrument_market_state', 'instrument_cbbo', 'MarketDataClient')
MAX_POLL_TIME = 60*5
...
class MarketDataClient:
    ...
    def listing_trades(self, object_id, date):
        """Retrieve trades data for a listing on a given day.
        ...
        """
        return self._get_data(f'/listing/trades/{object_id}/{date}')

    def listing_orders(self, object_id, date):
        """Retrieve order book data for a listing on a given day.
        ...
        Returns:
            :class:`pandas.DataFrame`
        """
        return self._get_data(f'/listing/orders/{object_id}/{date}')
    ...
    @backoff.on_predicate(lambda: backoff.expo(base=2, factor=0.1, max_value=30), _is_wait, max_time=MAX_POLL_TIME)
...
listing_trades = _DEFAULT_CLIENT.listing_trades
listing_orders = _DEFAULT_CLIENT.listing_orders
