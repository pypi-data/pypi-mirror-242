...
impl FromKlvm for NftMetadata {
    fn from_klvm(a: &Allocator, node: NodePtr) -> Result<Self> {
        let items: Vec<(String, NodePtr)> = FromKlvm::from_klvm(a, node)?;
        let mut metadata = Self::default();

        for (key, value_ptr) in items {
            match key.as_str() {
                "sn" => metadata.edition_number = FromKlvm::from_klvm(a, value_ptr)?,
                "st" => metadata.edition_total = FromKlvm::from_klvm(a, value_ptr)?,
                "u" => metadata.data_uris = FromKlvm::from_klvm(a, value_ptr)?,
                "h" => metadata.data_hash = FromKlvm::from_klvm(a, value_ptr)?,
                "mu" => metadata.metadata_uris = FromKlvm::from_klvm(a, value_ptr)?,
                "mh" => metadata.metadata_hash = FromKlvm::from_klvm(a, value_ptr)?,
                "lu" => metadata.license_uris = FromKlvm::from_klvm(a, value_ptr)?,
                "lh" => metadata.license_hash = FromKlvm::from_klvm(a, value_ptr)?,
                _ => (),
            }
        }
...
impl ToKlvm for NftMetadata {
    fn to_klvm(&self, a: &mut Allocator) -> Result<NodePtr> {
        let mut items: Vec<(&str, NodePtr)> = Vec::new();

        if !self.data_uris.is_empty() {
            items.push(("u", self.data_uris.to_klvm(a)?));
        }

        if let Some(hash) = self.data_hash {
            items.push(("h", hash.to_klvm(a)?));
        }

        if !self.metadata_uris.is_empty() {
            items.push(("mu", self.metadata_uris.to_klvm(a)?));
        }

        if let Some(hash) = self.metadata_hash {
            items.push(("mh", hash.to_klvm(a)?));
        }

        if !self.license_uris.is_empty() {
            items.push(("lu", self.license_uris.to_klvm(a)?));
        }

        if let Some(hash) = self.license_hash {
            items.push(("lh", hash.to_klvm(a)?));
        }

        items.extend(vec![
            ("sn", self.edition_number.to_klvm(a)?),
            ("st", self.edition_total.to_klvm(a)?),
        ]);
