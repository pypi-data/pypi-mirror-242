use hex_literal::hex;
use klvm_traits::{klvm_list, match_list, match_tuple, Error, FromKlvm, Result, ToKlvm};
...
impl FromKlvm for DidSolution {
    fn from_klvm(a: &Allocator, node: NodePtr) -> Result<Self> {
        let (mode, args) = <match_tuple!(u8, NodePtr)>::from_klvm(a, node)?;

        match mode {
            1 => Ok(Self::InnerSpend(
                <match_list!(NodePtr)>::from_klvm(a, args)?.0,
            )),
...
impl ToKlvm for DidSolution {
    fn to_klvm(&self, a: &mut Allocator) -> Result<NodePtr> {
        match self {
            Self::InnerSpend(solution) => klvm_list!(1, solution).to_klvm(a),
        }
    }
}
