...
def sort_portfolios(
    ...
    if(rebalance_freq == 'A'):
        fin = dfin.merge(rebalance_df[['permno', 'ffyear', 'port_name']], how = 'left', on = ['permno', 'ffyear'])
    else:
        fin = rebalance_df
    ...
    rets = _util_funcs.gorup_avg(df = fin, 
                                 ...
                                 vr = return_col, 
                                 wt = weight_col)
    firm = fin.groupby([date_col, 'port_name'])['permno'].count().reset_index().rename(columns = {'permno': 'num_firms'})
