...
from drf_emails.user import serializers, models
from drf_emails.user.typing import Code, Kwargs


USER = get_user_model()


class ActionCodeVerifyView(APIView):
...
class ActionVerifyView(APIView):
    permission_classes = (AllowAny,)
...
class SignupVerify(ActionVerifyView):
    ...
    def handle_action_code(self, action_code, **kwargs) -> None:
        # Verify user
...
class PasswordReset(APIView):
    ...
    def post(self, request, format=None):
        ...
        return Response({
            'details': _('Password reset not allowed.'),
        }, status=status.HTTP_400_BAD_REQUEST)
...
class EmailChange(APIView):
    ...
    def post(self, request, format=None):
        ...
        if user_with_new_email and user_with_new_email.is_verified:
            return Response({
                'details': _('Email address already taken.'),
            }, status=status.HTTP_400_BAD_REQUEST)
...
class PasswordChange(APIView):
    ...
    def post(self, request, format=None):
        ...
        return Response({
            'details': _('Password has been changed.'),
...
class Login(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not user.is_verified:
            return Response({
                'details': _('User account not verified.'),
            }, status=status.HTTP_401_UNAUTHORIZED)
