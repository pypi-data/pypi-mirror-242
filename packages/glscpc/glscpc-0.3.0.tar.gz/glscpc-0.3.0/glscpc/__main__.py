...
def main(
    files: Tuple[Tuple[BinaryIO]], verbose: int, cmd: str, includes: IncludeMode, check_args_for_files: bool, wrap: int, args: Tuple[str]
):
    ...
    click.echo(LOGO)
    log.debug(
        "Arguments: files=%r, verbose=%r cmd=%r includes=%r, check_args_for_files=%r, wrap=%r, args=%r",
        files,
        verbose,
        cmd,
        includes,
        check_args_for_files,
        wrap,
        args,
    )
    ...
    for file in files:
        ...
        try:
            checker = Checker(cmd, includes, wrap)
    ...
    if codes:
        ...
        for code in sorted(codes):
            click.echo(f"• [SC{code}](https://www.shellcheck.net/wiki/SC{code})")
