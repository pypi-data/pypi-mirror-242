...
def init_prompt():
    ...
    print_options("Import source options:", UPLOAD_METHODS)
    ...
    print_options("Framework options:", FRAMEWORKS)
    import_framework_type = prompt(
        "Select framework (press tab to autocomplete): ",
    ...
    print_options("Deployment types:", DEPLOYMENT_TYPE)
    deployment_type = prompt(
        "Select deployment type (press tab to autocomplete) : ",
        completer=get_deployment_types(),
        complete_while_typing=True,
        key_bindings=key_bindings,
        validator=Validator.from_callable(validate_deployment_type),
        validate_while_typing=False,
    )

    print_options("GPU types:", MACHINE_TYPES)
    gpu_type = prompt(
    ...
    if is_custom_runtime:
        ...
        rich.print("\nRequirements file found: %s" % file_name)
        is_found_file_used = False
        if file_name_full:
        ...
        if not is_found_file_used or not file_name_full:
            file_name_prompt = prompt(
                "Select your dependency management file: ",
                default="%s" % "requirements.txt",
            )
        ...
        rich.print(
            f"\n{DEFAULT_RUNTIME_FILE_NAME} file generated successfully! Also pre filled `python_packages` . feel free to modify the file"
        )
        rich.print(
            f"for more information on runtime file please refer our docs: [link={RUNTIME_DOCS_URL}]{RUNTIME_DOCS_URL}[/link]"
        )
        rich.print(
            "you can also use [blue]`inferless runtime upload`[/blue] command to upload runtime\n"
        )

    # Prompts for non-serverless options
    if deployment_type == "CONTAINER":
        config["configuration"]["custom_runtime_id"] = ""
        config["configuration"]["custom_runtime_name"] = ""
    ...
    config["configuration"]["is_serverless"] = (
        deployment_type == "CONTAINER" and False or True
    )
    ...
    rich.print(
        f"\n{input_file_name} and {output_file_name} files generated successfully! Also pre filled jsons. feel free to modify the files"
    )
    rich.print(
