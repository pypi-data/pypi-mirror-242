...
def deploy_local(config_file_name, redeploy):
    is_yaml_present = is_inferless_yaml_present()
    ...
    with Progress(
        ...
        if not redeploy and config.get("model_import_id"):
            rich.print(
                f"[red]model_import_id already exists in {config_file_name}[/red]"
            )
            raise typer.Abort(1)

        if redeploy and not config.get("model_import_id"):
            rich.print(
                f"[red]model_import_id not found in {config_file_name}[/red]. To deploy run command [blue]`inferless deploy`[/blue]"
            )
            raise typer.Abort(1)

        if redeploy and config.get("model_import_id"):
            response = get_model_import_details(config.get("model_import_id"))
            if config.get("name") != response.get("model_import").get("name"):
                rich.print(
        ...
        if "model_import" in details:
            ...
            if (
                not config["configuration"]["is_serverless"]
                and config["configuration"]["custom_runtime_name"]
