...
<keyword> - Workspace name keyword to filter by (use "all" for no filter). Default is "all".
<filename> - CSV filename to save the output data to. Default is "report.csv".
<start_date> - Start date for Run lookups (use "all" for no filter). Default is "all".
<end_date> - End date for Run lookups (use "all" for no filter). Default is "all".
<mode> - Execution mode ("simple" or "advanced"). Default is "simple".
...
def get_run_details(
    ...
    while page is not None:
        ...
        if (
            calculate_time(
                raw_response["data"][0]["attributes"]["status-timestamps"][
                    "planning-at"]
