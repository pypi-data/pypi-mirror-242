...
def process_dataframe(row, index,interface, item_id):
    ...
    pci = row['nr-rrc.pdcch-DMRS-ScramblingID']
...
def process_e1dataframe(row, index,interface,item_id):
    #print("Processing BearerContextSetupRequest")
...
def getDirection(message, interface):
    #direction = f'static/intfconfig/{interface}_proc.json'
    direction = os.path.asbpath(os.path.join(os.path.dirname(__file__),'..','intfconfig',f'{interface}_proc.json'))
    #direction = pkg_resouces.resouce_filenae(f'drawranflow.servicelogic.intfconfig',f'{interface}.json')
...
def create_message_object(row, identifier, interface):
    try:
        ...
        logging.debug(f" creating message with src Node: {srcNode}, dstNode: {dstNode}, Message : {row['_ws.col.info']}, FrameNumner: {row['frame.number']}, Identifier_id: {identifier}")
