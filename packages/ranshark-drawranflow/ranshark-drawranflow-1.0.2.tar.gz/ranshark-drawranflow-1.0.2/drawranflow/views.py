...
def update_stats_by_id(file_id):
    ...
    identifiers = Identifiers.objects.filter(uploadedFiles_id=file_id)
    if not upload_file.is_analysis_complete:
        with transaction.atomic():
            ...
            update_cumulative_stats_for_category(file_id, 'InitialCtxt')
            update_cumulative_stats_for_category(file_id, 'RRC')
...
def update_cumulative_stats_for_category(upload_file_id, category):
    ...
    identifier_stats_summary = IdentifiersStats.objects.filter(
        uploadedFiles_id=upload_file_id, category=category
    ).aggregate(
...
def show_stats(request,id):
    try:
        ...
        identifier_stat_list = []
        for identifier in identifiers:
            identifier_stats = IdentifiersStats.objects.filter(identifier_id=identifier.id, category='RRC',uploadedFiles_id=upload_file_id)
        ...
        print("identifier_stat_list",identifier_stat_list,"---",cumulative_stat_list)
