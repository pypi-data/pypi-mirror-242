...
from octavia_lib.api.drivers import data_models as provider_dm
...
POOL_LISTENER_PROPERTIES = ['listener_id', 'protocol_port', 'protocol']
...
def get_pdm_lb(dl, pdm, create):
    ...
    if isinstance(pdm, provider_dm.LoadBalancer) and create:
        ...
        lb_cdm = lb_repo.get(session, **{'id': lb_id})
...
def get_pool_members(pool_id, create, new_pdm, deleted_id):
    session = db_api.get_session()
    ...
    for member in pdms:
        ...
        member_subnet = n_driver.get_subnet(member.subnet_id)
        m_dict['subnet_id'] = member_subnet.id
        m_dict['subnet_cidr'] = member_subnet.cidr
        m_dict['network_id'] = member_subnet.network_id
...
def get_pool_listeners(pool_id, create, new_pdm, deleted_id):
    session = db_api.get_session()
    listeners_repo = repositories.ListenerRepository()
    pdms = _get_pdms(
        session, listeners_repo,
    ...
    listeners = []
    for listener in pdms:
        l_dict = {}
        for prop in POOL_LISTENER_PROPERTIES:
            l_dict[prop] = getattr(
                listener, prop, PROPERTY_DEFAULTS.get(prop))
        l_dict['id'] = l_dict.pop('listener_id')
        listeners.append(l_dict)
...
def _get_healthmonitor(dl, healthmonitor_id):
    ...
    return _get_pdm(
        session, hm_repo, {'id': healthmonitor_id},
...
def get_lb_and_graph(pdm, create=False, delete=False):
    ...
    lb_pdm = get_pdm_lb(dl, pdm, create)
    ...
    for l in listeners:
        ...
        default_pool_members = get_pool_members(
