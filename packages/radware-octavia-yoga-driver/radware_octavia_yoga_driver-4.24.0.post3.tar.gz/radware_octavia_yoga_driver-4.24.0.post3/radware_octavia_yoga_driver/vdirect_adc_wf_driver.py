...
from . import config as rad_config
from . import vdirect_client
...
class VDirectADCWorkflowDriver():
    ...
    def create_lb_workflow(self, lb_id, params, tenant_id=None, feedback=None, feedback_kwargs=None):
        ...
        self.lb_create_payload.update(params)
        network_id = params['vip_network_id']
        ...
        parameters = dict(self.lb_create_payload, service_params=service,
                          ...
                          __tenants=tenants)
        print('EVG:create_lb_workflow:parameters')
        print(parameters)
    ...
    def validate_hm(hm):
        if hm.timeout > hm.delay:
            raise ConfigurationConflict(
                'Invalid health monitor settings. The timeout value is greater than the delay value.')
        if hm.timeout > 600:
            raise ConfigurationConflict(
                'Not supported configuration. Invalid health monitor settings.'
                'The timeout value is greater than maximum value supported by provider - 600.')
        if hm.delay > 600:
            raise ConfigurationConflict(
                'Not supported configuration. Invalid health monitor settings.'
                'The delay value is greater than maximum value supported by provider - 600.')

    @staticmethod
    def validate_member(member):
        if member.weight > 48:
            raise ConfigurationConflict(
                'Pool member weight higher than 48 '
