...
from .agent import Player, FixChameleonPlayer, ChameleonPlayer, FixUndercoverPlayer, PGMPlayer, FixAirportPlayer
from .environments import Environment, TimeStep, load_environment
from .backends import Human
from .config import ArenaConfig
from ..prompts.chameleon_prompt import chameleon_global_prompt_pgm
from ..prompts.undercover_prompt import undercover_clue_demo, undercover_view_demo, global_prompt_undercover_pgm
...
class Arena:
    ...
    def reset_with_enviroment_topic(self, game_setting):
        self.environment.game_setting = game_setting
        self.reset()

    def step(self, request_msg=None) -> TimeStep:
        """
        ...
        # print(self.environment.get_observation())
        ...
        for i in range(self.invalid_actions_retry):  # try to take an action for a few times
            if isinstance(player, FixChameleonPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase, test_start=self.environment.test_start)
            elif isinstance(player, ChameleonPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase, test_start=self.environment.test_start)  # take an action
            elif isinstance(player, FixUndercoverPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase, test_start=self.environment.test_start)  # take an action
            elif isinstance(player, FixAirportPlayer):
                action = player(observation, request_msg=request_msg, stage=self.environment._current_phase, test_start=self.environment.test_start)  # take an action
            else:
                action = player(observation, request_msg=request_msg, single_history=self.environment._single_history,test_start=self.environment.test_start)
    ...
    def run(self, num_steps: int = 1):
        ...
        for i in range(num_steps):
            if i == 0:
                request_msg = self.current_timestep["request_msg"]
            else:
                request_msg = timestep["request_msg"]
            # print(self.environment._current_phase, self.environment._next_player_idx)
            timestep = self.step(request_msg)
    ...
    def from_config(cls, config: Union[str, ArenaConfig]):
        ...
        player_names= [player_config["name"] for player_config in config.players]
        assert len(player_names) == len(set(player_names)), "Player names must be unique"

        # Create the environment and get backends for all players
        ...
        for player_config in config.players:
            #print("1: ", player_config["name"], player_config["backend"])
            if global_prompt is not None:
                player_config["global_prompt"] = global_prompt # default global_prompt
            fix_player = False
            pgm_player = False

            if "model" not in player_config["backend"]:
                model_name = env.player_backends[player_config["name"]]["model"]
            else:
                model_name = player_config["backend"]["model"] 
             
            # print(model_name)          
            if model_name.find("-pgm")>=0:
                pgm_player = True
            if model_name.endswith("-fix"): # whether there is any fix player.
                fix_player=True
            player_config["backend"]["model"] = model_name.replace("-pgm","")
            player_config["backend"]["model"] = player_config["backend"]["model"].replace("-fix","")
            # print(config.environment)
            # print("2: ", player_config["name"], player_config["backend"])
            if config.environment.env_type.startswith("chameleon"): 
                player_config["global_prompt"] = chameleon_global_prompt_pgm  # replace with new defined global prompt
                if fix_player:
                    player = FixChameleonPlayer.from_config(player_config)
                else:
                    player = ChameleonPlayer.from_config(player_config)
            elif config.environment.env_type.startswith("undercover"):
                # print(pgm_player, fix_player)
                player = Player.from_config(player_config)
                
                if pgm_player:
                    player_config["clue_demo"] = undercover_clue_demo 
                    player_config["view_demo"] = undercover_view_demo
                    player_config["global_prompt"] = global_prompt_undercover_pgm
                    player = PGMPlayer.from_config(player_config)
                
                if fix_player:
                    player = FixUndercoverPlayer.from_config(player_config)
            elif config.environment.env_type.startswith("airport_fee"):
                player = Player.from_config(player_config)
                if fix_player:
                    player = FixAirportPlayer.from_config(player_config)
            else:
                player = Player.from_config(player_config)
