...
from .agent import Player, FixChameleonPlayer
...
class Arena:
    ...
    def step(self) -> TimeStep:
        ...
        for i in range(self.invalid_actions_retry):  # try to take an action for a few times
            if isinstance(player, FixChameleonPlayer):
                action = player(observation, self.environment._current_phase)
            else:
    ...
    def from_config(cls, config: Union[str, ArenaConfig]):
        ...
        # Create players
        players = []
        print("in arena", env.player_backends)
        
        for player_name, player_config in zip(env.player_backends.keys(), config.players):
            # Add public_prompt to the player config
            if global_prompt is not None:
                player_config["global_prompt"] = global_prompt
            # Add player's backend config, used to change backend type and model.
            fix_player = False
            # print(env.player_backends[player_name])
            for k,v in env.player_backends[player_name].items():
                print(k,v)
                if v.endswith("-pgm"):
                    v = v.replace("-pgm","")
                if v.endswith("-fix"):
                    v = v.replace("-fix","")
                    fix_player=True
                player_config["backend"][k] = v
            
            # print(fix_player, player_config)
            if fix_player:
