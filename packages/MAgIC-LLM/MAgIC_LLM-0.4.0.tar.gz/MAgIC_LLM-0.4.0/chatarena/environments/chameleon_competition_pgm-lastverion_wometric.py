...
import numpy as np
import copy
from prompts.chameleon_pgm import *
...
class Chameleon_Competition_PGM(Environment):
    ...
    def __init__(self, player_names: List[str], topic_codes: Dict[str, List[str]] = None, competition=None, **kwargs):
        ...
        self._view_non_chameleon_templ = view_templ_active_non_chameleon_v2_wformat
        ...
        self._clue_templ_non_chameleon = clue_templ_active_v2
        self._clue_templ_chameleon = clue_templ_active_chameleon_v2
        with open("results/analysis_demo_two_role_simple.txt") as f:
           self._analysis_demo = f.read()
        # self._analysis_chame_demo = chameleon_demo
        # self._analysis_non_chame_demo = non_chameleon_demo
    ...
    def reset(self):
        ...
        # if self.pgm_chame_player:
        #     self._moderator_speak(f"To show how to analyze roles, I will give you an example:\n{self._analysis_chame_demo}", visible_to=self.pgm_chame_player)
        # if len(self.pgm_non_chame_players)>-0:
        #     self._moderator_speak(f"To show how to analyze roles, I will give you an example:\n{self._analysis_non_chame_demo}", visible_to=self.pgm_non_chame_players)
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        if self._current_phase == "give clues":
            ...
            if self._next_player_idx is not None:
                ...
                mod_text = mod_text.replace("<SECRET_CODE>", self.code)
                request_msg= mod_text
                # self._moderator_speak(mod_text, visible_to=[self.player_names[self._next_player_idx]], is_show=False)

            else:
                # If cannot find any PGM player from all the players, then go to give clue
            ...
            # print("Before")
            # self._pgm_list[self.player_names.index(player_name)].print() # print existing PGM
            # factor = self._text2change(action)
            # self.all_player_views.append(factor[self._next_player_idx])
            ...
            # decide whether the pgm is good or not
            ...
            if self._next_player_idx is not None:
                ...
                # Cannot find next pgm player, go to the clue phase
                self._next_player_idx = self._next_clue_player_idx
                if self._finish_clue:
                    self._current_phase = "accuse"
                    self._next_player_idx = 0
