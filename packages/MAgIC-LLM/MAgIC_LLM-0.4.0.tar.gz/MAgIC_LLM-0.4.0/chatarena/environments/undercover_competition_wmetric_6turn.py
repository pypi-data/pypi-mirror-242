...
class Undercover_Competition_WMetric(Environment):
    type_name = "undercover_competition_wmetric"
    ...
    def reset(self):
        ...
        init_timestep = TimeStep(observation=self.get_observation(),
                                 ...
                                 terminal=False, request_msg=request_msg)
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        if self._current_phase == "metric_consistency":
            message = Message(agent_name=player_name, content=action, turn=self._current_turn, is_show=False, is_consistency=True)
            self.message_pool.append_message(message)
            self._current_turn += 1
            ...
            if self._clue_turn+1 not in self.consisteny_dict:
                self.consisteny_dict[self._clue_turn+1] = {}
            self.consisteny_dict[self._clue_turn+1][self.player_names[self._next_player_idx]] = consistency_flag
        ...
        elif self._current_phase == "give clues":
            ...
            message = Message(agent_name=player_name, content=action, turn=self._current_turn, is_show=False, is_pgm=True)
            self.message_pool.append_message(message)
            self._current_turn += 1
            pgm_factor = self.parse_pgm(action)
            if self._clue_turn not in self.pgm_dict:
                self.pgm_dict[self._clue_turn] = {}
                self.pgm_metric_dict[self._clue_turn] = {} 

            self.pgm_dict[self._clue_turn][self.player_names[self._next_player_idx]] = pgm_factor
            ...
            if self._next_player_idx < len(self.player_names) - 1:
                ...
                factors =[d[1] for d in sorted(self.pgm_dict[self._clue_turn].items(), key=lambda k:k[0])]
                gold_pgm_metric, inter_pgm_metric = self.compute_pgm_metric(factors)
                self.pgm_metric_dict[self._clue_turn] = {"gold": gold_pgm_metric, "inter": inter_pgm_metric}
                self._next_player_idx = 0
                if self._next_clue_player_idx >= len(self.player_names):
                    if self._remain_clue_round == 1:
                        terminal=True
                    else:
                        print("go to the conisst 1")
