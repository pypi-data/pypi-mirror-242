...
class Undercover_Competition_PGM(Environment):
    ...
    def __init__(self, player_names: List[str], topic_codes: Dict[str, List[str]] = None, competition=None, **kwargs):
        ...
        self._clue_unclear = clue_templ_active_unclear_v1
        self._clue_undercover = clue_templ_active_undercover_v1
        self._clue_non_undercover = clue_templ_active_non_undercover_v1
    ...
    def reset(self):
        ...
        if self.is_pgm_player(0):
            self._moderator_speak(self._clue_unclear.replace("<PLAYER>", self.player_names[0]), visible_to=[self.player_names[0]])
    ...
    def _get_current_clues(self, player_name=None):
        msgs = self.get_observation()
        cur_clues = []
        for msg in msgs:
            if msg.agent_name.startswith("Player") and msg.visible_to == "all":
        ...
        for ri in range(round_num):
            clue_text_list.append(f"Round {ri}")
            clue_text_list += cur_clues[int(ri)*player_num:int(ri+1)*player_num]
        cur_clue_text = "\n".join(clue_text_list)
        # print(cur_clue_text)
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        if self._current_phase == "give clues":
            ...
            if self._next_player_idx < len(self.player_names) - 1: # enumerate all player to give clues
            ...
            else: # If a clue round ends, if there are any pgm player, start pgm analysis
                ...
                if self._next_player_idx is not None:
                    ...
                    # is_undercover = self._pgm_list[self._next_player_idx].is_undercover(self.threshold)
                    ...
                    # print("observations: ",  sself.get_observation())
                    mod_text = self._view_template
                    cur_clue_text = self._get_current_clues(self.player_names[self._next_player_idx])
                    mod_text = mod_text.replace("<CLUES>", cur_clue_text)
                    cur_code = self.undercover_code if self.player_names[self._next_player_idx] == self.undercover_name else self.non_undercover_code
                    other_players = [self.player_names[pi] for pi in range(len(self.player_names)) if pi != self._next_player_idx]
                    for pi, player in enumerate(other_players):
                        ques = self._view_template_question
                        ques = ques.replace("<OTHER_PLAYER>", player)
                        ques = ques.replace("<IDX>", str(3+pi))
                        mod_text += ques
                    mod_text = mod_text.replace("<PLAYER>", self.player_names[self._next_player_idx])
                    mod_text = mod_text.replace("<CODE>", cur_code)                    
                    print(f"===========analyze targe for {self.player_names[self._next_player_idx]} =========")
                    request_msg = mod_text
            ...
            print("Cur pgm player name: ",  player_name)
            cur_pgm = self._pgm_list[self.player_names.index(player_name)]

            # print("Before: PGM of ", player_name)
            # cur_pgm.print() 
            factor = self._text2changev3(action)
            cur_pgm.update(factor)
            # print(player_name, factor)
            # print("After: PGM of ", player_name)
            # cur_pgm.print()
            ...
            message = Message(agent_name=player_name, content=action, turn=self._current_turn, is_pgm=True, is_good_pgm=is_good_pgm,
                              visible_to=[player_name], is_show=False)
