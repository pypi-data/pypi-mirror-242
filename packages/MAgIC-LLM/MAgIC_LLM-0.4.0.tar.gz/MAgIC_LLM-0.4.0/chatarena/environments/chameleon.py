...
class Chameleon(Environment):
    ...
    def log_game(self, path):
        ...
        result = "chameleon" if self._win_group == 0 else "non-chameleon"

        with open(path, "w") as f:
            json.dump({"history": message_rows,"result": result}, f, indent=4)

    def get_win_group(self):
        # print(self._win_group)
        assert self._win_group >= 0
        return "chameleon" if self._win_group == 0 else "non-chameleon"
    ...
    def reset(self):
        ...
        # self.topic = "Countries"
        # self.code = "Canada"
        # self.chameleon_name = "Player 1"
