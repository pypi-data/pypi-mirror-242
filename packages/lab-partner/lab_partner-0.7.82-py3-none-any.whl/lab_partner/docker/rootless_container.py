...
class RootlessDockerContainer(object):
    ...
    def start_rootless_container(self, workspace_path: str, network_name: str) -> Tuple[DockerDaemonInfo, Dict[str, str]]:
        if self.is_rootless_container_running():
            logger.info(f'Rootless container {self.container_name}')
            return self._build_rootless_config()
        else:
        ...
        docker_daemon_info, cli_env = self._build_rootless_config()
        if not docker_daemon_info.network_exists('lab'):
            docker_daemon_info.create_network('lab')
        return docker_daemon_info, cli_env
    ...
    @staticmethod
    def _build_rootless_config() -> Tuple[DockerDaemonInfo, Dict[str, str]]:
        docker_daemon_info = DockerDaemonInfo.build_with_docker_host(ROOTLESS_DOCKER_URL)
        cli_env = {'DOCKER_HOST': ROOTLESS_DOCKER_URL}
        return docker_daemon_info, cli_env
