...
taskWizFile = os.path.realpath(__file__)
taskWizAIFolder = os.path.dirname(taskWizFile)
if os.getcwd() != taskWizAIFolder:
    os.chdir(taskWizAIFolder)
...
#venvDir = "venv"
#venvDirFullPath = os.path.join(taskWizAIFolder, venvDir)
...
#            # Run main.py
#            os.system(f"{activator} & {python} {taskWizFile}")
#        else:
#            # Activate virtual environment
...
#            # Run main.py
#            os.system(f"{python} {taskWizFile}")
#        venvActivated = True
#    except:
#        venvActivated = False
#    if venvActivated:
...
configFile = os.path.join(taskWizAIFolder, "config.py")
...
from taskwiz.utils.assistant import TaskWizAI
    ...
    with open(configFile, "w", encoding="utf-8") as fileObj:
        for name in dir(config):
            excludeConfigList = [
                ...
                # TaskWizAI methods shared from Class TaskWizAI
                "getFiles",
                "stopSpinning",
                "toggleMultiline",
                "print",
