...
class AzureContainer:
    def __init__(self, container_name: str, azure_storage_account: AzureStorageAccount):
        try:
            self.azure_storage_account: AzureStorageAccount = azure_storage_account
            self.container_name: str = container_name
            self.container_client: ContainerClient = self.azure_storage_account.blob_service_client.get_container_client(container_name)
    ...
    def get_all_directories_in_container(self) -> list[str] or None:
        ...
        """
        try:
            directories: list[str] = self.container_client.walk_blobs(name_starts_with='', delimiter='/')
    ...
    def generate_sas_token(self, permission: ContainerSasPermissions, expiry: datetime) -> str or None:
        ...
        try:
            ...
            account_name, account_key = self.azure_storage_account.get_details_from_connection_string()
    ...
    def generate_url(self, directory: str = "") -> str or None:
        """
        ...
        try:
            ...
            sas_url: str = ('https://' + self.azure_storage_account.account_name + '.blob.core.windows.net/' +
                            self.container_name + "/" + directory + '?' + sas_token)
            return sas_url
        except Exception as e:
            logging.error(f"Failed to generate url to container - {str(e)}")

    def is_container_exist(self) -> bool:
        """
    ...
    def delete_blobs_in_directory(self, directory_name: str) -> None:
        """
