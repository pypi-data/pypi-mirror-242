...
class AzureStorageAccount:
    ...
    def get_details_from_connection_string(self) -> tuple[str | None, str | None]:
        ...
        try:
            ...
            for part in parts:
                if part.startswith(Config.PROTOCOL_PREFIX):
                    continue  # Skip the protocol part
                elif part.startswith(Config.ACCOUNT_NAME_PREFIX):
                    account_name = part.split("=", 1)[Config.ACCOUNT_NAME_INDEX]
                elif part.startswith(Config.ACCOUNT_KEY_PREFIX):
                    account_key = part.split("=", 1)[Config.ACCOUNT_KEY_INDEX]
