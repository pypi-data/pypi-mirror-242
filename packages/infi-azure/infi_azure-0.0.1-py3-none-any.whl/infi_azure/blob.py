...
class AzureBlob:
    def __init__(self, blob_name: str, azure_container: AzureContainer):
        try:
            self.azure_container: AzureContainer = azure_container
            self.blob_client: BlobClient = self.azure_container.container_client.get_blob_client(blob_name)
    ...
    def is_blob_exist(self) -> bool:
        """
        Check if blob exist.
        """
        try:
            blob_list = list(self.azure_container.container_client.list_blobs(name_starts_with=self.blob_name))
    ...
    def is_empty_directory(self) -> bool:
        ...
        """
        try:
            blobs = self.azure_container.container_client.list_blobs(name_starts_with=self.blob_name+"/")
    ...
    def create_empty_directory(self) -> None:
        ...
        try:
            ...
            directory_client = self.azure_container.container_client.get_blob_client(directory_name)
    ...
    def blobs_count_in_directory(self) -> int or None:
        ...
        """
        try:
            if not self.is_blob_exist():
                return None
            blob_list = self.azure_container.container_client.list_blobs(name_starts_with=self.blob_name)
