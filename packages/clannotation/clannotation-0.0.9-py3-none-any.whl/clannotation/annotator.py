...
class Annotator:
    ...
    def patch_item(
        ...
        if type(route) is list:
            for route_to_check in route:
                annotation = (
                    self.get_annotations(f"{'/'.join(route_to_check)}") | item
                )
    ...
    def rebuild_route(route: str, new_identifier: str):
        ...
        elif path[1] == "journals":
            if type(new_identifier) is list:
                identifiers_list = []
                for identifier in new_identifier:
                    identifiers_list.append(["", "journals", str(identifier)])

                return identifiers_list

            new_path = ["", "journals", str(new_identifier)]
            path = new_path
