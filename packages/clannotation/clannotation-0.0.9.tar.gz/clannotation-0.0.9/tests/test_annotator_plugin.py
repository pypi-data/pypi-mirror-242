...
class TestAnnotatorPlugin(unittest.TestCase):
    ...
    def test_patch_item_with_list_route(self):
        annotation_plugin_instance = self.get_annotation_instance()

        route = "/journals/test-route2"
        routes = ["test-route1", "test-route2"]
        item = {"ID": "12345", "ISSN": route}
        identifier_key = "ISSN"
        expected_result = {
            "ID": "12345",
            "ISSN": route,
            "cr-labs-anno2": {"blah": "message"},
        }

        annotation_plugin_instance._valid_annotations = [
            "anno2.json",
        ]
