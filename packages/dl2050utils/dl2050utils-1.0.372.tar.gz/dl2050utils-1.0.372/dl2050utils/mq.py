...
class MQ():
    def __init__(self, log, db, qnames, cfg):
        ...
        self.cfg,self.url = cfg,f'amqp://{user}:{passwd}@mq:5672?heartbeat=0'
    ...
    async def publish(self, qname, email, payload):
        ...
        if jid is None:
            self.LOG(4, 0, label='MQ', label2='publish', msg='DB: failed to insert job')
            return None
...
class MQServer():
    ...
    def run(self, qname, cb):
        ...
        url = f'amqp://{user}:{passwd}@mq:5672?heartbeat=0'
        try:
            con = pika.BlockingConnection(pika.connection.URLParameters(url))  # log_level='critical'
            ch = con.channel()
