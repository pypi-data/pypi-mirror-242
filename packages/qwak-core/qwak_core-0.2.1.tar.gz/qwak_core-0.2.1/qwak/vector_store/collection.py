...
class Collection:
    ...
    @typechecked
    def fetch(self, vector_id: str) -> SearchResult:
        """
        ...
        result = self._vector_serving_client.fetch_vector(
            collection_name=self.name, vector_id=vector_id
        )
        if not result:
            raise QwakNotFoundException(
                f"Vector with id {vector_id} was not found in collection {self.name}"
            )
