...
def test_parse_python_env(requirements_file):
    for env_dict, expected_env, err in (
            ...
            ({'python': {}}, PythonEnv(), None),
            ({'python': {'version': '3.10.5'}}, PythonEnv(version='3.10.5'), None),
            ({'python': {'requirements-file': requirements_file}}, PythonEnv(requirements=requirements_file), None),
            ({'python': {'version': '3.9', 'requirements-file': requirements_file}}, PythonEnv(version='3.9', requirements=requirements_file), None),
            ({'python': {'version': '3.11.a'}}, None, 'unsupported python version: 3.11.a'),
            ({'python': {'version': '2.7'}}, None, 'unsupported python version: 2.7'),
            ({'python': {'requirements-file': 'not-existent.txt'}}, None, 'requirements file `not-existent.txt` was not found'),
    ):
        if err:
...
def test_python_script():
    def get_cfg(cmd: str, is_python: bool = True):
        return Config(
            ...
            s3_mounts=[],
            datasets=[],
            env=Environment(vars=None, docker_image=None, python=PythonEnv() if is_python else None),
            cloud_instance_type='c1.4',
            attach_project_disk=False,
            content='nevermind',
        )
