...
from yandex.cloud.datasphere.v2.project_pb2 import Project
...
@fixture
def expected_get_project_request(project_id) -> GetProjectRequest:
    return GetProjectRequest(project_id=project_id)
...
class RequestsData:
    ...
    community_id: str
    expected_get_project_request: GetProjectRequest
    oauth_token: str
...
def get_op(
    ...
    op.metadata.Pack(ExecuteProjectJobMetadata(job=Job(id=job_id, project_id=project_id)))
    return op


@fixture
def running_op(execute_op_id, job_id, project_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, job_id, project_id, expected_download_files, OperationStatus.RUNNING)


@fixture
def successful_op(execute_op_id, job_id, project_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, job_id, project_id, expected_download_files, OperationStatus.SUCCESS)


@fixture
def program_error_op(execute_op_id, job_id, project_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, job_id, project_id, expected_download_files, OperationStatus.SUCCESS,
                  program_error=True)


@fixture
def system_error_op(execute_op_id, job_id, project_id, expected_download_files) -> Operation:
...
def get_prj_stub(mocker):
    return mocker.patch('datasphere.client.ProjectServiceStub')()
...
def get_execute_mocks(mocker, code_data, files_data, requests_data, execute_op, get_op_side_effects: list):
    ...
    prj_stub.Get.return_value = Project(community_id=requests_data.community_id)
