...
def biobb_pdb_tofasta(input_file_path: str, output_file_path: str,  properties: dict = None, **kwargs) -> int:
    ...
    return Pdbtofasta(input_file_path=input_file_path,output_file_path=output_file_path,
                    properties=properties, **kwargs).launch()

def main():
    """Command line execution of this building block. Please check the command line documentation."""
    parser = argparse.ArgumentParser(description='Extracts the residue sequence in a PDB file to FASTA format.', formatter_class=lambda prog: argparse.RawTextHelpFormatter(prog, width=99999))
    ...
    required_args.add_argument('--output_file_path', required=True, help='Description for the output file path. Accepted formats: fasta.')

    args = parser.parse_args()
    args.config = args.config or "{}"
    properties = settings.ConfReader(config=args.config).get_prop_dic()

    biobb_pdb_tofasta(input_file_path=args.input_file_path, 
