...
class SDCN(nn.Module):
    ...
    def fit(self):
        ...
        for epoch in range(self.epochs):
            ...
            q_pred = (q.data.cpu().numpy().argmax(1)
                      )  # Get cluster result by Q-distribution
            z_pred = (pred.data.cpu().numpy().argmax(1)
                      )  # Get cluster result by Z-distribution
            p_pred = (p.data.cpu().numpy().argmax(1)
                      )  # Get cluster result by P-distribution
    ...
    def get_embedding(self):
        ...
        Returns:
            torch.Tensor
        """
        g = (self.graph.to("cuda:" + str(self.gpu))
             if torch.cuda.is_available() else self.graph)
