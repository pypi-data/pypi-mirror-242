...
class RedisPriorityQueue:
    ...
    def push(self, item, priority):
        ...
        if not self.redis.zscore(self.name, item_key):
            # 使用JSON格式将字典转换为字符串以便存储
            item_str = json.dumps(item)
            self.redis.zadd(self.name, {item_str: priority})
            print(f"键名：{self.name}, zadd {item_str: priority} successful")
            return True
        else:
            print(f"键名：{self.name}, zadd {item_str: priority} is exist id:{item_key}")
            return None
    def pop(self):
        """ 弹出具有最高优先级（最低分数）的项 """
        # 使用事务确保操作的原子性
