...
class SourceFileMissingError(FileNotFoundError):
    ...
    """
    def __init__(self, filename: t.Union[str, Path], *args, **kwargs):
        super().__init__(
...
class DisallowedScopeError(ValueError):
    ...
    """
    def __init__(self, line_no: t.List[int], *args, **kwargs):
        super().__init__(
...
class MissingMagicCommentsError(ValueError):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(
            "No magic CG_INSERT comments were found within the file.", *args,
            **kwargs
        )
...
class MagicCommentTransformer(cst.CSTTransformer):
    ...
    """
    # mypy has trouble finding cst.metadata.PositionProvider on gh actions,
    # it does however work when running mypy locally.
    # libcst own docs on metadata use this import
    ...
    def leave_EmptyLine(
        ...
        """
        if (
            isinstance(original_node, cst.EmptyLine) and
            original_node.comment is not None
        ):
            re_result = re.search(
            ...
            if re_result is not None:
                ...
                if self.class_count != 0 or self.func_count != 0:
                    pos = self.get_metadata(
                        # mypy has trouble finding cst.metadata.PositionProvider
                        # on gh actions, it does however work when running
                        # mypy locally.
                        # libcst own docs on metadata use this import
                        # https://libcst.readthedocs.io/en/latest/metadata.html
                        cst.metadata.
                        PositionProvider,  # type: ignore[attr-defined]
                        original_node
                    ).start
    ...
    def leave_ClassDef(
        self, original_node: cst.ClassDef, _: cst.ClassDef
    ) -> cst.ClassDef:
        self.class_count -= 1
        return original_node
...
def parse_file(filename: Path) -> cst.MetadataWrapper:
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        content = f.read()
        # mypy has trouble finding cst.metadata.MetadataWrapper on gh actions,
        # it does however work when running mypy locally.
        # libcst own docs on metadata use this import
        # https://libcst.readthedocs.io/en/latest/metadata.html
...
def main(argv: t.List[str]) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=
        'The relative path to the template file containing the magic comments.'
    )
