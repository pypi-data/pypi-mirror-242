...
class Datastore():
    ...
    def eject(self, period):
        ...
        if period == 'daily':
            ...
            print('Period Daily - Removable Datastore is ejected')

        elif period == 'weekly':
            if date.weekday() == 6:
                restart_proxmox_backup()
                subprocess.run(['eject', self.path], check=False)
                print('Period Weekly - Removable Datastore is ejected')
            else:
                print('Period Weekly - Removable Datastore is not ejected')

        elif period == 'monthly':
            if date.strftime("%d-%m-Y") == last_sunday_of_month(date).strftime("%d-%m-Y"):
                restart_proxmox_backup()
                subprocess.run(['eject', self.path], check=False)
                print('Period Monthly - Removable Datastore is ejected')
            else:
                print('Period Monthly - Removable Datastore is not ejected')

        else:
            print('Removable Datastore is not ejected')
