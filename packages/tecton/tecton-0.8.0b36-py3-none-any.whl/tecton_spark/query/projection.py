...
class DeriveValidityPeriodSparkNode(SparkExecNode):
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        aggregate_columns = [feature.output_feature_name for feature in aggregate_features]

        window = spark_window.Window.partitionBy(*join_keys).orderBy(self.timestamp_field)

        previous_value_columns = [
            F.lag(column).over(window).alias(f"_tecton_prev_{column}") for column in aggregate_columns
        ]
        df = df.select("*", *previous_value_columns)

        # Check if any aggregate column changed in value, set is_new=1 for rows with changes
        feature_value_update_conditions = [
            ~F.col(column).eqNullSafe(F.col(f"_tecton_prev_{column}")) for column in aggregate_columns
        ]
        df = df.withColumn(
        ...
        df = df.groupBy(*join_keys, *aggregate_columns, "_tecton_group").agg(
            F.min(valid_from()).alias(valid_from()),
            F.when(null_check, F.lit(None)).otherwise(F.max(valid_to())).alias(valid_to()),
        )

        df = self._remove_default_values(df, aggregate_features).drop("_tecton_group", self.timestamp_field)

        return df

    def _remove_default_values(
        self, df: pyspark.sql.DataFrame, aggregate_features: Tuple[feature_view__data_pb2.AggregateFeature, ...]
