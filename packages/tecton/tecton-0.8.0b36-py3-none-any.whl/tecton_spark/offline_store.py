...
def get_offline_store_writer(
    ...
    if case == "delta":
        ...
        return DeltaWriter(params, spark, version, partition_size)
    elif case == "parquet":
...
def get_offline_store_reader(
    ...
    if case == "delta":
        partition_size = partition_size_for_delta(fd).as_timedelta()
        return DeltaReader(spark, s3_path, partition_size)
    elif case == "parquet":
        partition_size = partition_size_for_parquet(fd).as_timedelta()
        partition_col = partition_col_for_parquet(fd)
...
class DeltaWriter(OfflineStoreWriter):
    ...
    def delete_time_range(
        ...
        start_time = core_time_utils.convert_timestamp_for_version(feature_start_time, self._version)
        end_time = core_time_utils.convert_timestamp_for_version(feature_end_time, self._version)
    ...
    def upsert_dataframe_in_tile(self, data_frame: DataFrame, time_range: pendulum.Period) -> None:
    ...
    def _in_range(self, time_range: pendulum.Period) -> str:
        ...
        return f"""
          {self._params.time_column} >= '{start_time_str}' AND {self._params.time_column} < '{end_time_str}'
          AND {TIME_PARTITION} BETWEEN '{start_partition_str}' AND '{end_partition_str}'
        """
...
class DeltaReader(OfflineStoreReader):
    def __init__(self, spark: SparkSession, path: str, partition_size: timedelta) -> None:
        self._spark = spark
        self._path = path
        self._partition_size = partition_size
