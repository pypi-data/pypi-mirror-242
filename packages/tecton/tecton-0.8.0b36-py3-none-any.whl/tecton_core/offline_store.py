...
class DeltaReader(OfflineStoreReader):
    ...
    def _storage_options(self) -> Dict[str, str]:
        if self._params.delta_table_uri.startswith("s3://"):
            ...
            # auth configurations supported by boto, which can be surprising to users used to boto's options. The
            # following does a fully-featured auth using boto, then passes derived credentials into the Rust process.
            #
