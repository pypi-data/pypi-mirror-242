...
class MaterializedFeatureView(FeatureView):
    ...
    def get_features_for_time_range(
        self,
        ...
        return querytree_api.get_features_for_time_range(
            dialect=dialect,
        ...
        if self._is_valid:
            ...
            if self._feature_definition.has_offset_window and spine is None:
                logger.warning(
                    ...
                    "aggregation will not return all features. Please use get_features_for_time_range() to return all features in"
...
class BatchFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        offline_store: Optional[configs.OfflineStoreTypes] = None,
        ...
        tecton_runtime_version: Optional[str] = None,
    ):
        ...
        if offline_store is None:
            offline_store = repo_config.get_batch_feature_view_defaults().offline_store

        if batch_compute is None:
            batch_compute = repo_config.get_batch_feature_view_defaults().batch_compute

        if tecton_runtime_version is None:
            tecton_runtime_version = repo_config.get_batch_feature_view_defaults().tecton_runtime_version
        ...
        args = _build_materialized_feature_view_args(
            ...
            options=options,
            tecton_runtime_version=tecton_runtime_version,
        )
...
def batch_feature_view(
    *,
    ...
    :param tecton_runtime_version: Version of `tecton` package used by your job cluster.
...
class StreamFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        if offline_store is None:
            offline_store = repo_config.get_stream_feature_view_defaults().offline_store

        if batch_compute is None:
            batch_compute = repo_config.get_stream_feature_view_defaults().batch_compute

        if tecton_runtime_version is None:
            tecton_runtime_version = repo_config.get_stream_feature_view_defaults().tecton_runtime_version
...
class FeatureTable(FeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        online: bool = False,
        offline: bool = False,
        offline_store: configs.DeltaConfig = None,
        online_store: Optional[configs.OnlineStoreTypes] = None,
        batch_compute: Optional[configs.ComputeConfigTypes] = None,
        ...
        if offline_store is None:
            offline_store = repo_config.get_feature_table_defaults().offline_store

        if batch_compute is None:
            batch_compute = repo_config.get_feature_table_defaults().batch_compute

        if tecton_runtime_version is None:
            tecton_runtime_version = repo_config.get_feature_table_defaults().tecton_runtime_version
...
def _build_materialized_feature_view_args(
    ...
    online: bool,
    offline: bool,
    offline_store: configs.OfflineStoreTypes,
    ...
    # TODO(TEC-17296): populate default value for tecton_runtime_version from repo config
