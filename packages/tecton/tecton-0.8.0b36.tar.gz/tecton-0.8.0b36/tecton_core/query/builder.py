...
def build_aggregated_time_range_validity_query(
    ...
    partial_aggs, spine = _build_partial_aggs_and_spine(
        dialect, compute_mode, fdw, from_source, feature_data_time_limits
    )
...
def build_aggregated_time_range_ghf_query(
    ...
    partial_aggs, spine = _build_partial_aggs_and_spine(
        dialect, compute_mode, fdw, from_source, feature_data_time_limits, aggregation_anchor_time=None
    )
...
def _build_partial_aggs_and_spine(
    dialect: Dialect,
    compute_mode: ComputeMode,
    fdw: feature_definition_wrapper.FeatureDefinitionWrapper,
    from_source: Optional[bool],
    ...
    if fdw.aggregation_secondary_key:
        ...
        spine = AggregationSecondaryKeyExplodeNode(
            dialect=dialect,
            compute_mode=compute_mode,
            input_node=RenameColsNode(
                dialect=dialect, compute_mode=compute_mode, input_node=partial_aggs, drop=cols_to_drop
            ).as_ref(),
            fdw=fdw,
        ).as_ref()
    else:
...
def wip_build_compaction_query(
    ...
    fv_pipeline = build_pipeline_querytree(dialect, compute_mode, fdw, for_stream=False, feature_data_time_limits=None)
    if fdw.is_temporal_aggregate:
        ...
        node = TakeLastRowNode.for_feature_definition(
            dialect=dialect, compute_mode=compute_mode, fdw=fdw, input_node=fv_pipeline
        )
