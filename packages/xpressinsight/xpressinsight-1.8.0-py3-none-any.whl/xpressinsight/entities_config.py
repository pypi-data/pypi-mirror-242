"""
    ...
    Copyright (c) 2023-2023 Fair Isaac Corporation. All rights reserved.
"""
...
from xpressinsight import EntityBase
from xpressinsight.entities import ENTITY_CLASS_NAMES, ScalarBase
...
class EntitiesConfig(ABC):
    """
    Abstract base class for entity container configuration. An entity container (such as `xi.AppConfig` or
    `xi.ScenarioData`) is used on classes that will contain Insight entity data, written to attributes identified by
    annotations (e.g. `xi.types.Scalar`).
    """
    ...
    def __get_old_syntax_entity_annotation_strings(annotations: Dict[str, Any]) -> Dict[str, str]:
        ...
        (i.e. postponed evaluation of annotations is activated).
        """
        old_syntax_entities = {}
...
class EntitiesContainer:
    """
    Abstract base class used for classes that contain scenario entities (such as `xi.AppBase` and
    `xi.ScenarioData`), providing a standard way to query entity definitions.

    The `xi.ScenarioData` decorator will automatically add the EntitiesContainer as a superclass of the class
    ...
    def get_entities_cfg(cls) -> EntitiesConfig:
        ...
        if not hasattr(cls, "_entities_cfg") or not cls._entities_cfg:
            raise AttributeError(
                "Cannot access the entities container configuration!\n"
                "    Please make sure that the class {cls.__name__} is decorated with the AppConfig or ScenarioData\n"
                "    decorator.")
