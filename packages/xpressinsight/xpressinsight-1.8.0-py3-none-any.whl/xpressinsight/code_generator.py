...
import sys
import shutil
from typing import TextIO, ValuesView, Set
...
class CodeGenerator:
    ...
    def __declaration_safe_sorted(cls, entities: List[EntityBase]) -> List[EntityBase]:
        ...
        #
        #
        entities_with_indexes = [e for e in entities if isinstance(e, Series) or isinstance(e, DataFrame)]
        entities_without_indexes = [e for e in entities if not(isinstance(e, Series) or isinstance(e, DataFrame))]
    ...
    def __init__(self, app_cls: Type[AppBase], file: TextIO):
        ...
        self.DeclaredEntities: List[EntityBase] = CodeGenerator.__declaration_safe_sorted(
...
model {}
    ...
    def write_app_config_annotations(self):
        for scen_type in self.app_cfg.scen_types:
            self.file.write(f"        @insight.scentypes.{validate_raw_ident(scen_type, 'scenario type identifier')}\n")
