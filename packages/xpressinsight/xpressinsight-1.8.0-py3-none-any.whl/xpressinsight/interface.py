...
class AppInterface(ABC):
    ...
    def get_scenario_data(self, scenario_path_or_id: str, scenario_data_class: Type[SCENARIO_DATA_CONTAINER], *,
        ...
        annotated using the `xi.data.` or `xi.types.` helper functions, and is decorated using the `xi.ScenarioData`
        or `xi.AppConfig` decorator. It's recommended to use `xi.ScenarioData` unless the class is the application
        class for the app being read from, as `xi.AppConfig` will perform additional validation that is only relevent
