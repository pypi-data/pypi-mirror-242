...
class APITestCase(TestCase):
    ...
    def remove_permission(self, codename, user=None):
        perm = Permission.objects.get(codename=codename)
        user = user or self.user
...
class TestUpdateFormSubmission(FormsAPITestCase):
    def setUp(self):
        super(TestUpdateFormSubmission, self).setUp()
    ...
    def test_attempting_update_without_permission_returns_403(self):
        submission = FormSubmissionFactory(
            form=self.form,
            raw_data={'fields': [{'name': 'first_name', 'value': 'bobby'}, {'name': 'last_name', 'value': 'stein'}]},
        ...
        resp = self.put('/submissions/%s' % submission.id, {'raw_data': [{'name': 'first_name', 'value': 'joe'}]})

        self.assertEqual(resp.status_code, 403)

    @factory.django.mute_signals(signals.form_submission_cleaned)
    def test_attempting_update_of_unowned_form_submission_returns_403(self):
        submission = FormSubmissionFactory(
            form=self.form,
            raw_data={'fields': [{'name': 'first_name', 'value': 'bobby'}, {'name': 'last_name', 'value': 'stein'}]},
            cleaned_data={'first_name': 'bobby', 'last_name': 'stein'},
            created_by=self.user
        )

        alternate_user = User.objects.create_user(username='baduser', email='baduser@test.com', password='imabaduser')
        self.add_permission('change_formsubmission', user=alternate_user)
        self.login_client({'username': alternate_user.username, 'password': 'imabaduser'})
    ...
    @factory.django.mute_signals(signals.form_submission_cleaned)
    def test_update_patches_object_partial(self):
        submission = FormSubmissionFactory(
            form=self.form,
            raw_data={'form': self.form.id,
                      'fields': [{'name': 'first_name', 'value': 'bobby'}, {'name': 'last_name', 'value': 'stein'}]},
            cleaned_data={'first_name': 'bobby', 'last_name': 'stein'},
            created_by=self.user
        )

        self.put('/submissions/%s' % submission.id, {'raw_data': [{'name': 'first_name', 'value': 'joe'}]})

        submission.refresh_from_db()

        self.assertDictEqual(
            submission.cleaned_data,
            {'first_name': 'joe', 'last_name': 'stein'}
        )
        self.assertEqual(submission.form.id, self.form.id)

    @factory.django.mute_signals(signals.form_submission_cleaned)
    @mock.patch('apollo.signals.form_submission_updated.send')
    def test_update_fires_signal_handler(self, mock_form_submission_updated_signal):
        submission = FormSubmissionFactory(
            form=self.form,
            raw_data={'form': self.form.id,
                      'fields': [{'name': 'first_name', 'value': 'bobby'}, {'name': 'last_name', 'value': 'stein'}]},
        ...
        self.assertTrue(mock_form_submission_updated_signal.called)
        self.assertEqual(mock_form_submission_updated_signal.call_count, 1)
