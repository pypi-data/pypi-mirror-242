    ...
    with track_activity(logger, "create_index_from_raw_embeddings", custom_dimensions={"num_documents": len(emb._document_embeddings)}) as activity_logger:
        ...
        def process_delete_results(results, start_time):
            if results == {}:
                ...
                activity_logger.info(f"Deleting documents succeeded", extra={"properties": {"duration": duration}})
                return True
            else:
        ...
        def process_upsert_results(results, batch_size, start_time):
            if results['upserted_count'] == batch_size:
                duration = time.time() - start_time
                activity_logger.info(f"Upserting documents succeeded", extra={"properties": {"num_docs_upserted": batch_size, "duration": duration}})
                return True
            else:
                logger.error(f"Failed to upsert all documents", extra={"properties": {"num_docs_upserted": results['upserted_count'], "duration": duration}})
                return False
        ...
        if len(deleted_ids) > 0:
            ...
            if not process_delete_results(results, start_time):
                logger.info(f"Retrying delete documents")
                results = pinecone_index_client.delete(deleted_ids)
                if not process_delete_results(results, start_time):
                    raise RuntimeError(f"Failed to delete documents")
            ...
            for doc_id, emb_doc in emb._document_embeddings.items():
                ...
                if len(batch) % batch_size == 0:
                    ...
                    if not process_upsert_results(results, len(batch), start_time):
                        logger.info(f"Retrying upsert documents since not all documents were upserted. "
                                    f"Documents that were upserted will simply be overwritten with the same values. "
                                    f"This retry will be idempotent.")
                        results = pinecone_index_client.upsert(batch)
                        if not process_upsert_results(results, len(batch), start_time):
                            raise RuntimeError(f"Failed to upsert documents")
                    batch = []
