...
from cengal.code_flow_control.smart_values.versions.v_0 import ResultCache, ResultExistence
...
class FIFOWithLengthControl(FIFO):
    def __init__(self, on_hold_limit=None, on_hold_data_size_limit=None,
        ...
        self._external_data_full_size = external_data_full_size or ResultExistence(False, 0)
    ...
    def put(self, data):
        ...
        self._data_full_size.result += data_size
        self._external_data_full_size.result += data_size
    ...
    def extend(self, iterable_data):
        ...
        while index < self._real_size:
            ...
            index += 1
        self._data_full_size.result += full_size_diff
        self._external_data_full_size.result += full_size_diff
    ...
    def get(self):
        ...
        self._external_deletable_data_full_size.result += result_len
        ...
        if (self._offset >= self._offset_limit) or (self._deletable_data_full_size > self._on_hold_data_size_limit):
            ...
            self._data_full_size.result -= self._deletable_data_full_size
            self._external_data_full_size.result -= self._deletable_data_full_size
    ...
    def _calculate_data_full_size(self):
        ...
        last_data_full_size = self._data_full_size.result
        diff_data_full_size = data_full_size - last_data_full_size
        self._data_full_size.set(data_full_size)
        self._external_data_full_size.result += diff_data_full_size
        
    def get_data_full_size(self):
        if not self._data_full_size:
            self._calculate_data_full_size()

        return self._data_full_size.result
    ...
    def remove(self):
        if not self._removed:
            self._external_data_full_size.result -= self._data_full_size.result
            self._external_deletable_data_full_size.result -= self._deletable_data_full_size
            self._removed = True
...
class FIFODequeWithLengthControl:
    ...
    def get(self):
        ...
        self._external_data_full_size.result -= result_len
    ...
    def remove(self):
        if not self._removed:
            self._external_data_full_size.result -= self._data_full_size
            self._removed = True
