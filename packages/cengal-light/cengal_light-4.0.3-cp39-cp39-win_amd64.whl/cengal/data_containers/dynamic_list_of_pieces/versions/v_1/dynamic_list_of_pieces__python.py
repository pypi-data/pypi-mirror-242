...
class DynamicListOfPiecesWithLengthControl(DynamicListOfPieces):
    def __init__(self, joiner=None, on_hold_limit=None, on_hold_data_size_limit=None,
                 external_data_length: ValueExistence=None,
        ...
        self._external_data_length = external_data_length or ValueExistence(False, 0)
    ...
    def add_piece_of_data(self, piece_of_data):
        ...
        piece_of_data_len = len(piece_of_data)
        self._data_length += piece_of_data_len
        self._external_data_length.value += piece_of_data_len
        self._useful_size += 1
        self._real_size += 1
    ...
    def get_data(self, size):
        ...
        if size < first_piece_len:
            result = self._data[pieces_qnt][:size]
            self._data_length -= size
            self._external_data_length.value -= size
            self._deletable_data_full_size += size
            self._external_deletable_data_full_size.value += size
            self._data[pieces_qnt] = self._data[pieces_qnt][size:]
            return result
    ...
    def remove(self):
        if not self._removed:
            self._external_data_length.value -= self._data_length
            self._external_deletable_data_full_size.value -= self._deletable_data_full_size
            self._removed = True
...
class DynamicListOfPiecesDequeWithLengthControl:
    def __init__(self, joiner=None, external_data_length: ValueExistence=None):
        self._removed = False
    ...
    def get_data(self, size):
        ...
        if size < first_piece_len:
            ...
            while self._real_size and (result_size < size):
                ...
                self._external_data_length.value -= another_piece_len
                self._real_size -= 1
            ...
            if result_size < size:
                ...
                self._external_data_length.value -= delta_size
    ...
    def get_data_pieces(self, size) -> List[bytes]:
        ...
        if size <= first_piece_len:
            ...
            self._external_data_length.value -= first_piece_len
            self._real_size -= 1
            return [self._data.popleft()]
        else:
            result_data = deque()
...
class DynamicListOfPiecesMemoryviewDequeWithLengthControl:
    def __init__(self, joiner=None, external_data_length: Optional[ValueExistence] = None, mem_buffer_size: Optional[ValueExistence] = None, output_fifo: Optional[FIFODequeWithLengthControl] = None):
        self._removed = False
        ...
        self.mem_buffer_size = mem_buffer_size or ValueExistence(False, 1024)
        self.mem_buffer = memoryview(bytearray(self.mem_buffer_size.value))
        self._external_data_length.value += self.mem_buffer_size.value
    ...
    def _allocate_new_mem_buffer(self):
        self._external_data_length.value -= len(self.mem_buffer.obj)
        self.mem_buffer = memoryview(bytearray(self.mem_buffer_size.value))
        self._external_data_length.value += self.mem_buffer_size.value
