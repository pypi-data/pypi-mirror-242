...
__all__ = ['StreamType', 'GateSecurityPolicy', 'StreamManagerIOCoreMemoryManagement', 'UdpStreamManager', 'UdpStreamReader', 'UdpStreamReaderProtocol', 'UdpStreamWriter']
...
class UdpStreamManager(StreamManagerAbstract):
    ...
    async def open_connection(self, host=None, port=None, *,
        ...
        The reader returned is a UdpStreamReader instance; the writer is a
        UdpStreamWriter instance.
        ...
        (If you want to customize the UdpStreamReader and/or
        ...
        reader = UdpStreamReader(self, message_protocol_settings, limit=limit, loop=loop)
        protocol = UdpStreamReaderProtocol(self, message_protocol_settings, reader, loop=loop)
        transport, _ = await loop.create_connection(
            lambda: protocol, host, port, **kwds)
        writer = UdpStreamWriter(transport, protocol, reader, loop)
    ...
    async def start_server(self, client_connected_cb, host=None, port=None, *,
        ...
        def factory():
            message_protocol_settings: MessageProtocolSettings = MessageProtocolSettings(protocol_greeting, message_size_len, max_message_size_len)
            reader =  UdpStreamReader(self, message_protocol_settings, limit=limit, loop=loop)
            protocol = UdpStreamReaderProtocol(self, message_protocol_settings, reader, client_connected_cb,
                                            loop=loop)
            return protocol
...
class UdpStreamReader(OriginalStreamReader, StreamReaderAbstract):
    def __init__(self, manager: UdpStreamManager, message_protocol_settings: MessageProtocolSettings, *args, **kwargs) -> None:
...
class UdpStreamReaderProtocol(OriginalStreamReaderProtocol, StreamReaderProtocolAbstract):
    def __init__(self, manager: UdpStreamManager, message_protocol_settings: MessageProtocolSettings, *args, **kwargs) -> None:
        self._stream_manager = manager
        self._message_protocol_settings: MessageProtocolSettings = message_protocol_settings
        super().__init__(*args, **kwargs)
    ...
    def connection_made(self, transport):
        ...
        if self._client_connected_cb is not None:
            self._stream_writer = UdpStreamWriter(transport, self,
                                               reader,
                                               self._loop)
...
class UdpStreamWriter(OriginalStreamWriter, StreamWriterAbstract):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._stream_manager: UdpStreamManager = self._protocol._stream_manager
