...
class DicomEmbedder:
    ...
    def from_table(
        ...
        frames = data.domain["frame"]
        frames = column_data_as_int(data, 'frame')
        file_path_frames = list(zip(file_paths, frames))
        embeddings_ = self._embedder.embedd_data(file_path_frames, callback=callback)
        return DicomEmbedder.prepare_output_data(data, embeddings_)

    def __enter__(self) -> "DicomEmbedder":
        return self
    ...
    def prepare_output_data(
        ...
        if np.any(embedded_images_bool):
            ...
            embedded_images = DicomEmbedder.construct_output_data_table(
...
if __name__ == "__main__":
    image_file_paths = ["/home/chris/Downloads/dicom/0002.DCM"]
    # with ImageEmbedder(model='inception-v3') as embedder:
    with DicomEmbedder(model="squeezenet") as embedder:
