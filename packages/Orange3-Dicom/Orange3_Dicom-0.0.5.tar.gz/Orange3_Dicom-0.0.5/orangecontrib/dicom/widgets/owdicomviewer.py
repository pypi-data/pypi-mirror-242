...
class OWDicomViewer(OWWidget):
    name = "DICOM Viewer"
    description = "View DICOM images referred to in the data."
    ...
    frame_attr: Optional[Variable] = settings.ContextSetting(None)
    ...
    def setData(self, data):
        ...
        if data is not None:
            ...
            self.frame_attr = self.title_model[2] if self.title_model else None
    ...
    def clear(self):
        ...
        #self.frame_model.set_domain(None)
    ...
    def setupModel(self):
        ...
        if self.data is not None:
            ...
            frames = column_data_as_int(self.data, 'frame')
            assert self.thumbnailView.count() == 0
            assert len(self.data) == len(urls)
            qnam = ImageLoader.networkAccessManagerInstance()
            items = []
            for i, (url, title, attr_val, frame) in enumerate(zip(urls, titles, im_attr_vals, frames)):
                if url.isEmpty():  # skip missing
                    continue
                future, deferrable = image_loader(url, qnam, frame)
                self.items.append(_ImageItem(i, url, future, attr_val, frame))
                items.append({
                    Qt.DisplayRole: title,
                    Qt.EditRole: title+"."+str(int(frame)),
...
def loader_local(
        url: QUrl, frame: int, future: 'Future[QImage]'
...
def image_loader(
        url: QUrl, nam: QNetworkAccessManager, frame: int
    ...
    if url.isValid() and url.isLocalFile():
        loader = loader_local(url, frame, future)
...
if __name__ == "__main__":
    ...
    WidgetPreview(OWDicomViewer).run(
        Table("/home/chris/code/orange/dicom.tab"))
