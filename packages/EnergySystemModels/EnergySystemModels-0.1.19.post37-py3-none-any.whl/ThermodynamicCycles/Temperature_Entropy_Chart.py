...
class Object:
    def __init__(self, fluid):
        self.fluid = fluid
        self.T_min = CP.PropsSI(fluid, 'Ttriple')-273.15-1  # Triple point temperature in Kelvin
        self.T_max = CP.PropsSI(fluid, 'Tcrit')-273.15+1    # Critical temperature in Kelvin
        self.S_min = None
        self.S_max = None
    ...
    def show(self, draw_arrows=False):
        ...
        for p in pressures:
            S = [CP.PropsSI('S', 'P', p, 'T', T+273.15, self.fluid) / 1000 for T in T_range]  # Convert S to kJ/kg-K
            plt.plot(S, T_range, 'k--', linewidth=0.5)  # T in °C
