...
class Object:
    ...
    def calculate(self):
      ...
      #  Test du calcul de point de Outlet à 90% d'humidité 7.5 g/kg, 12°C
        self.Outlet.w=self.HA_target
        # print("CC.Outlet.w consigne",self.Outlet.w)
        self.Inlet_HR=air_humide.HR(air_humide.func_Pv_sat(self.T_inlet),self.Inlet.w,self.Inlet.P)
        
      
        if self.Inlet.w>=self.HA_target and self.HA_target>=self.HA_sat: 
            
            if self.Inlet_HR<=90 :
        
                def syst(var): # définition du système
                    self.Outlet_Pvsat,self.T_Outlet, = var[0], var[1] # définition des variables
                    eq1 =self.Outlet_RH-air_humide.HR(self.Outlet_Pvsat,self.HA_target,self.Inlet.P)  #calcul de Pvsat
        ...
        elif self.Inlet.w<=self.HA_target and self.T_inlet>=self.T_target:
        ...
        #     self.Eff=(self.Inlet.w-self.Outlet.w)/(self.Inlet.w-self.HA_sat)
        #     self.FB=1-self.Eff
        ...
        #     #     self.Outlet.w=self.HA_sat+self.Eff*(self.HA_target-self.HA_sat)
        ...
        # elif self.Inlet.w>self.HA_sat and self.Inlet.w<=self.HA_target and self.T_inlet>=self.T_target:
        #     self.Eff=(self.T_target-self.T_sat)/(self.T_inlet-self.T_sat)
        #     self.FB=1-self.Eff
        #     self.Outlet.h=self.h_sat+self.Eff*(self.Inlet.h-self.h_sat)
        #     self.Outlet.w=self.HA_sat+self.Eff*(self.HA_target-self.HA_sat)
        ...
        # #         ''' Avec refroidissement sensible sans déshu '''
        # elif self.Inlet.w<=self.HA_sat and self.T_inlet>=self.T_target:
        #     self.Eff=(self.T_sat-self.T_target)/(self.T_inlet-self.T_sat)
        #     self.FB=1-self.Eff
        #     self.Outlet.h=air_humide_NB.Air4_Hs(self.T_target, self.Inlet.P, self.Inlet.w/1000)
        #     self.Outlet.w=self.Inlet.w
