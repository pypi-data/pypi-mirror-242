...
class Object:
    ...
    def show(self, draw_arrows=False):
        ...
        S_liquid = [CP.PropsSI('S', 'T', T+273.15, 'Q', 0, self.fluid) for T in T_range]  # Convert S to kJ/kg-K
        S_vapor = [CP.PropsSI('S', 'T', T+273.15, 'Q', 1, self.fluid)  for T in T_range]  # Convert S to kJ/kg-K
        ...
        for p in pressures:
            ...
            for T in T_range:
                try:
                    ...
                    entropy = CP.PropsSI('S', 'P', p, 'T', T_K, self.fluid)# Convert S to kJ/kg-K
                    S.append(entropy)
                except ValueError:
