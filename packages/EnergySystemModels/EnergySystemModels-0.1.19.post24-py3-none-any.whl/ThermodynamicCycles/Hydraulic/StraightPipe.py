...
class Object :
    def __init__(self):
        ...
        self.diff_P = None  # Modelica.Units.SI.Pressure diff_P dif de pression entre entree et sortie
    ...
    def calculate(self):
        ...
        self.V = self.Inlet.F_kgs / (self.rho * self.A)
        ...
        self.df = pd.DataFrame({'StraightPipe': [self.Timestamp,self.Inlet.fluid,self.Ti_degC,self.Inlet.F_kgs,self.Inlet.h,self.Outlet.h,self.A,self.V,self.Nbre_Re,self.delta_P], },
                      index = ['Timestamp','str_fluid','str_Ti_degC','str_Inlet.F_kgs','str_Inlet.h','str_Outlet.h','section du tube (m2)','Vitesse écoulement (m/s)','Nb Reynold','dP(Pa)' ])
