...
class ModelEvaluation:
    ...
    """
    def __init__(self, project_id, destination, credentials=None):
...
#             lst.append([f'roc_auc - {multi_class}', roc_auc_score(y_test, y_pred, multi_class=multi_class)])
    ...
    def historize_model_evaluation(self, soltn_nm, lst_mdls):
        ...
        print(f"{time.ctime()}, Start updating {self.destination} on mode '{c.write_disposition}' ")
        if self.credentials == None:
            ...
            client_bq = bigquery.Client(project=self.project_id)
        else:
            client_bq = bigquery.Client(project=self.project_id, credentials=self.credentials)
        ...
        print(f"{time.ctime()}, {self.destination} updated")
