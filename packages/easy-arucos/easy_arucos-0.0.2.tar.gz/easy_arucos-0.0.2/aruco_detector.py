...
class ArucoDetection():
    """ Aruco detector class. Used for:
        * Aruco detection
        * 
    """

    @staticmethod
    def detect(image: np.ndarray, dictionaries: List[cv2.aruco_Dictionary] = DICTIONARIES, marker_length: float = 0.02, matrix_coefficients: List[Tuple[float, float, float]] = MATRIX_COEFFICIENTS, distortion_coefficients: Tuple[float, float, float, float, float] = np.zeros((1, 5)), optimized: bool = False) -> List[Aruco]:
        """ 
    ...
    def augment_aruco(bbox: List[Tuple], image: np.ndarray, aug_image: np.ndarray) -> np.ndarray:
        """ Places a 2D-image on the surface of each detected aruco and returns the marked image."""
