...
def setup_gpu(gpu_argument, gpu_fraction, num_cpu) -> int:
    if isinstance(gpu_argument, list):
        gpu_argument = [re.sub("[^0-9]", "", num) for num in gpu_argument if re.sub("[^0-9]", "", num).isnumeric()]
        if len(gpu_argument) == 1:
            str_gpus = gpu_argument[0].strip().split(" ")
        else:
            str_gpus = [str(entry) for entry in gpu_argument]
        num_gpus = len(str_gpus)
        os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str_gpus)
    else:
        num_gpus = 1
        if gpu_argument != -1 and len(gpu_argument) > 0:
            str_gpus = str(gpu_argument)
            os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str_gpus)
    ...
    if num_cpu != -1:
        config_proto_args["intra_op_parallelism_threads"] = num_cpu
        config_proto_args["inter_op_parallelism_threads"] = num_cpu
...
def main(args=None):
    ...
    ###############################
    num_gpus = setup_gpu(args.gpu, args.gpu_fraction, args.num_cpu)
    ...
    if len(train_annot_pairs) == 0:
        print("Can't find any micrograph - annotation pairs. Check if the train_annot_folder and train_image_folder in your configuration file is correct.")
        sys.exit(1)
