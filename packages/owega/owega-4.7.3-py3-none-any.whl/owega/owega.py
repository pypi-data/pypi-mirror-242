...
def handle_temperature(temp_file, messages, ps, given="", temp_is_temp=False, tts_enabled=False):
	given = given.strip()
	try:
		baseConf["temperature"] = float(given)
	...
	new_temperature = pt.prompt(pt.ANSI(
		'\n' + clrtxt("magenta", " temperature ") + ': '
	)).strip()
	try:
		...
		info_print('Invalid input, keeping current temperature')
		pass
	return messages
...
def handle_top_p(temp_file, messages, ps, given="", temp_is_temp=False, tts_enabled=False):
	...
	new_top_p = pt.prompt(pt.ANSI(
		'\n' + clrtxt("magenta", " top_p ") + ': '
	)).strip()
	try:
		...
		info_print('Invalid input, keeping current top_p value')
		pass
	return messages
...
def handle_frequency(temp_file, messages, ps, given="", temp_is_temp=False, tts_enabled=False):
	given = given.strip()
	try:
		baseConf["frequency_penalty"] = float(given)
	...
	new_frequency_penalty = pt.prompt(pt.ANSI(
		'\n' + clrtxt("magenta", " frequency_penalty ") + ': '
	)).strip()
	try:
		...
		info_print('Invalid input, keeping current frequency penalty')
		pass
	return messages
...
def handle_presence(temp_file, messages, ps, given="", temp_is_temp=False, tts_enabled=False):
	given = given.strip()
	try:
		baseConf["presence_penalty"] = float(given)
	...
	new_presence_penalty = pt.prompt(pt.ANSI(
		'\n' + clrtxt("magenta", " presence_penalty ") + ': '
	)).strip()
	try:
		...
		info_print('Invalid input, keeping current presence penalty')
		pass
	return messages
