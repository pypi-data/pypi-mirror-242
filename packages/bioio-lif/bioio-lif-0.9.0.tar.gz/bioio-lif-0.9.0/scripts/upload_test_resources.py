...
from bioio_lif import __version__
...
def upload_test_resources(args: Args):
    ...
    try:
        ...
        resources_dir = (
            Path(__file__).parent.parent / "bioio_lif" / "tests" / "resources"
        ).resolve(strict=True)
        ...
        package_name = "bioio_lif/test_resources"
        ...
        # Upload
            ...
            if confirmation:
                pushed = package.push(
                    ...
                    message=f"Test resources for `bioio_lif` version: {__version__}.",
                )
