...
class Page():
    ...
    def encrypt_data(self, data_input, no_of_entries, nvs_obj):
        ...
        rel_addr = nvs_obj.page_num * \
            Page.PAGE_PARAMS['max_size'] + Page.FIRST_ENTRY_OFFSET
        ...
        for _ in range(0, no_of_entries):
            ...
            if addr_len > 2:
                ...
                tweak_val = tweak_tmp + \
                    (init_tweak_val * (tweak_len_needed - (len(tweak_tmp))))
            else:
            ...
            data_val = data_bytes + \
                (init_data_val * (data_len_needed - len(data_bytes)))
    ...
    def write_varlen_binary_data(
        ...
        while True:
            ...
            tailroom = (
                Page.PAGE_PARAMS['max_entries'] - self.entry_num - 1) * Page.SINGLE_ENTRY_SIZE
            assert tailroom >= 0, 'Page overflow!!'

            # Split the binary data into two and store a chunk of available
            # size onto curr page
            ...
            if not remaining_size:
                ...
                # compute crc of entry header
                entry_struct = curr_self.set_crc_header(entry_struct)

                # write last entry
                curr_self.write_entry_to_buf(entry_struct, 1, nvs_obj)
                break

        return entry_struct

    def write_single_page_entry(
            self,
            entry_struct,
            data,
            datalen,
    ...
    def write_varlen_data(self, key, data, encoding, ns_index, nvs_obj):
        ...
        if blob_limit_applies and datalen > max_blob_size:
            raise InputError(
                ' Input File: Size (%d) exceeds max allowed length `%s` bytes for key `%s`.' %
        ...
        if self.version == Page.VERSION2 and (
                encoding in ['hex2bin', 'binary', 'base64']):
...
class NVS():
    ...
    def create_new_page(self, version=None, is_rsrv_page=False):
        ...
        if self.pages:
            curr_page_state = struct.unpack(
                '<I', self.cur_page.page_buf[0:4])[0]
            if curr_page_state == Page.ACTIVE:
                page_state_full_seq = Page.FULL
                struct.pack_into(
                    '<I', self.cur_page.page_buf, 0, page_state_full_seq)
    ...
    def write_namespace(self, key):
        ...
        try:
            self.cur_page.write_primitive_data(
                key, self.namespace_idx, 'u8', 0, self)
    ...
    def write_entry(self, key, value, encoding):
        ...
        if encoding == 'hex2bin':
            ...
            if len(value) % 2 != 0:
                raise InputError(
                    '%s: Invalid data length. Should be multiple of 2.' %
                    key)
...
def write_entry(nvs_instance, key, datatype, encoding, value):
    ...
    :param encoding: Data encoding. Valid values are
        "u8", "i8", "u16", "i16", "u32", "i32", "u64", "i64", "string", "binary", "hex2bin" and "base64"
...
def set_target_filepath(outdir, filepath):
    ...
    if filepath:
        ...
        if not ext:
            ...
            sys.exit(
                'Error: `%s`. Only `%s` extension allowed.' %
                (filepath, bin_ext))
...
def encrypt(args):
    ...
    if (args.keygen is False) and (not args.inputkey):
        ...
        print(
            '\nWarning:',
            '--inputkey argument is given. --keyfile argument will be ignored...')
...
def generate_key(args):
    ...
    keys_outdir, output_keyfile = set_target_filepath(
        keys_outdir, args.keyfile)
    ...
    if args.key_protect_hmac:
        ...
        if args.kp_hmac_keygen:
            hmac_key_str = ''.join(random.choice('0123456789abcdef')
                                   for _ in range(64)).strip()
            ...
            if not args.kp_hmac_inputkey:
                raise RuntimeError(
                    'HMAC Key input file (HMAC-based encryption scheme) missing!')
        ...
        key = ''.join(random.choice('0123456789abcdef')
                      for _ in range(128)).strip()
        ...
        for row in reader:
            try:
                ...
                if len(row['key']) > max_key_len:
                    raise InputError(
                        'Length of key `%s` should be <= 15 characters.' %
                        row['key'])
                write_entry(
                    nvs_obj,
                    row['key'],
...
def main():
    parser = argparse.ArgumentParser(
        description=desc_format('ESP NVS partition generation utility'),
    ...
    parser_gen.add_argument('size', default=None, help=desc_format(
    ...
    parser_gen_key = subparser.add_parser(
        'generate-key',
    ...
    parser_gen_key.add_argument(
        ...
        default=os.getcwd(),
        help=desc_format('Output directory to store files created. (Default: current directory)'))
    parser_encr = subparser.add_parser(
        'encrypt',
    ...
    parser_encr.add_argument('input',
                             default=None,
                             help=desc_format('Path to CSV file to parse'))
    parser_encr.add_argument(
        'output',
        default=None,
        help=desc_format('Path to output NVS binary file'))
    parser_encr.add_argument('size', default=None, help=desc_format(
        'Size of NVS partition in bytes (must be multiple of 4096)'))
    parser_encr.add_argument(
        '--version',
        choices=[
