...
class Microservice(Construct):
    ...
    def update_user_data(
        self, base_user_data: _ec2.UserData, asg_name: str = None
    ):
        ...
        """
        self.auto_scaling_group.node.find_child(
            'LaunchConfig'
        ).add_property_override(
            property_path='UserData', value=cdk.Fn.base64(base_user_data)
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if is_network_load_balancer:
            nlb = Nlb(
                ...
                load_balancer_name='-'.join(
                    [app_name, main_component_name, 'nlb']
                ),
            ...
            alb = Alb(
                ...
                allow_ingress_from_everyone=True
                if allow_ingress_from_everyone.lower() == 'true'
                else False,
            )
            self.alb = alb.alb
        ...
        block_devices = _asg.BlockDevice(
            device_name='/dev/xvda', volume=volume
        )
