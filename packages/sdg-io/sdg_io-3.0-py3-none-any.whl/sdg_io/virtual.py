...
class VirtualIO(threading.Thread):
                ...
                while not vio.q_wr.empty():
                    ...
                    try:
                        ...
                        if self.dump:
                            ...
                            self.dump.flush()
                    except self.io.Exception:  # перенаправляем исключения в виртуальный интерфейс
                        vio.Exception = self.io.Exception
                        self.__exit = True
                        if self.log:
                            self.log.error(f"vio write Exception {vio.Exception}")
            # --- read ---
            try:
                msg = self.io.read(timeout=self.rxtimeout)
            except self.io.Exception:  # перенаправляем исключения в виртуальные интерфейсы
                for vio in allvio:
                    vio.Exception = self.io.Exception
                    self.__exit = True
                    if self.log:
                        self.log.error(f"vio read Exception {vio.Exception}")
            else:
                if msg:
                    if self.dump:
                    ...
                    else:  # адрес не совпадает, передаем во все безадресные каналы
    ...
    class VirtualIOchannel:
        ...
        def read(self, timeout=.01):
            if self.Exception:
                raise self.Exception("VirtualIO read exeption")
            try:
        ...
        def write(self, msg: bytes):
            if self.Exception:
                raise self.Exception("VirtualIO write exeption")
            self.q_wr.put(msg)
            if self.dump:
                self.dump.write("%d>%s\n" % (_dumptime(), dump_bytes(msg)))
                self.dump.flush()
