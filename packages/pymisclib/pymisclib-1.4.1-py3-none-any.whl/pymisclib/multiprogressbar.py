...
class SingleProgressBar:
    ...
    :param int total_steps: Steps required to complete progress.
    :param str prefix: Text before the bar.
    :param str suffix: Text after the bar.
    """
    total_steps: int = 100  # steps required to complete progress
    prefix: str = ''  # text before the bar
    suffix: str = ''  # text after the bar
    bar_segments: int = 25  # bar segments at 100%
    bar_color: BAR_COLORS = BAR_COLORS.Black
    decimals: int = 0  # Number of decimals to use in displaying the percent.
    _current_step: int = 0
    ...
    def color_string(self) -> str:
        ...
        :rtype str:
        """
        return BAR_COLORS.Black.value + self.prefix + \
            UC.box_drawings.light_vertical + \
            self.bar_color.value + \
            self.bar() + \
            BAR_COLORS.Black.value + \
            UC.box_drawings.light_vertical + \
            f' {self.percent_str()} {self.suffix}'
    ...
    def current_step(self) -> int:
        """Current step of the bar on the way towards :ref:`total_steps`."""
...
def demo_modern():
    ...
    def demo_modern_single():
        ...
        sb.bar_color = BAR_COLORS.Red
        for step in range(total_steps * 8 + 1):
            if step == (total_steps / 2) * 8:
                sb.bar_color = BAR_COLORS.Yellow
            elif step == total_steps * 8:
                sb.bar_color = BAR_COLORS.Green
    ...
    def demo_modern_multi():
        ...
        bars = [
            ...
            SingleProgressBar(total_steps=total_steps[3],
                              prefix='Proc3 ',
                              suffix='III',
                              bar_segments=total_steps[3] / 10,
                              bar_color=BAR_COLORS.BrightYellow),
