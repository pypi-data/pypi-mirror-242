...
async def present(
    ...
    Example:
        .. code-block:: sls
          ...
          idem_test_nsx_alb.infrastructure.service_engine_group_is_present:
              nsx_alb.nsx_alb.infrastructure.service_engine_group.present:
    ...
    if resource_id:
        before = await hub.exec.nsx_alb.infrastructure.service_engine_group.get(
            ctx,
            name=name,
        ...
        result["comment"].append(
            f"'nsx_alb.infrastructure.service_engine_group:{name}' already exists"
        )
    ...
    if result["old_state"]:
        ...
        if desired_state:
            for k, v in desired_state.items():
                if ("_ref" in k and isinstance(v, str)) and ("name=" in v):
                    before = (
                        await hub.exec.nsx_alb.infrastructure.service_engine_group.get(
                            ctx,
                            name=name,
                            resource_id=resource_id,
                        )
        ...
        if bool(changes.get("new")):
            if ctx.test:
                ...
                result["comment"] = (
                    f"Would update nsx_alb.infrastructure.service_engine_group '{name}'",
                )
                return result
            else:
                # Update the resource
                update_ret = await hub.exec.nsx_alb.infrastructure.service_engine_group.update(
                    ctx,
                    name=name,
                    resource_id=resource_id,
                    **{
                ...
                if result["result"]:
                    result["comment"].append(
                        f"Updated 'nsx_alb.infrastructure.service_engine_group:{name}'"
                    )
                else:
        ...
        if ctx.test:
            ...
            result["comment"] = (
                f"Would create nsx_alb.infrastructure.service_engine_group {name}",
            )
            return result
        else:
            create_ret = await hub.exec.nsx_alb.infrastructure.service_engine_group.create(
                ctx,
                name=name,
                **{
                    "resource_id": resource_id,
            ...
            if result["result"]:
                result["comment"].append(
                    f"Created 'nsx_alb.infrastructure.service_engine_group:{name}'"
                )
    ...
    after = await hub.exec.nsx_alb.infrastructure.service_engine_group.get(
        ctx,
        name=name,
        resource_id=resource_id,
    )
...
async def absent(hub, ctx, name: str, resource_id: str = None) -> Dict[str, Any]:
    ...
    Example:
        .. code-block:: sls
            ...
            idem_test_nsx_alb.infrastructure.service_engine_group_is_absent:
              nsx_alb.nsx_alb.infrastructure.service_engine_group.absent:
    ...
    if not resource_id:
        result["comment"].append(
            f"'nsx_alb.infrastructure.service_engine_group:{name}' already absent"
        )
        return result
    ...
    if before["ret"]:
        if ctx.test:
            result[
                "comment"
            ] = f"Would delete nsx_alb.infrastructure.service_engine_group:{name}"
            return result

        delete_ret = await hub.exec.nsx_alb.infrastructure.service_engine_group.delete(
            ctx,
            name=name,
            resource_id=resource_id,
        )
        ...
        if result["result"]:
            result["comment"].append(
                f"Deleted 'nsx_alb.infrastructure.service_engine_group:{name}'"
            )
        else:
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Example:
        ...
        .. code-block:: bash
            ...
            $ idem describe nsx_alb.infrastructure.service_engine_group
    """

    result = {}

    ret = await hub.exec.nsx_alb.infrastructure.service_engine_group.list(ctx)

    if not ret or not ret["result"]:
        hub.log.debug(
            f"Could not describe nsx_alb.infrastructure.service_engine_group {ret['comment']}"
        )
        return result
    ...
    for resource in ret["ret"]:
        ...
        result[resource_id] = {
            "nsx_alb.infrastructure.service_engine_group.present": [
