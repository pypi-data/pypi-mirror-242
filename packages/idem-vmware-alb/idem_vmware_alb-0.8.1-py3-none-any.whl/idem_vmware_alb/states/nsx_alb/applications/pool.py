...
async def present(
    ...
    Example:
        .. code-block:: sls
          ...
          idem_test_nsx_alb.applications.pool_is_present:
              nsx_alb.nsx_alb.applications.pool.present:
              - analytics_policy:
    ...
    if resource_id:
        before = await hub.exec.nsx_alb.applications.pool.get(
            ctx,
            name=name,
        ...
        result["comment"].append(f"'nsx_alb.applications.pool:{name}' already exists")
    ...
    if result["old_state"]:
        ...
        if bool(changes.get("new")):
            if ctx.test:
                ...
                result["comment"] = (
                    f"Would update nsx_alb.applications.pool '{name}'",
                )
                return result
            else:
                # Update the resource
                update_ret = await hub.exec.nsx_alb.applications.pool.update(
                    ctx,
                    name=name,
                    resource_id=resource_id,
                    **{
                ...
                if result["result"]:
                    result["comment"].append(
                        f"Updated 'nsx_alb.applications.pool:{name}'"
                    )
                else:
        ...
        if ctx.test:
            ...
            result["comment"] = (f"Would create nsx_alb.applications.pool {name}",)
            return result
        else:
            create_ret = await hub.exec.nsx_alb.applications.pool.create(
                ctx,
                name=name,
                **{
                    "resource_id": resource_id,
            ...
            if result["result"]:
                result["comment"].append(f"Created 'nsx_alb.applications.pool:{name}'")
    ...
    after = await hub.exec.nsx_alb.applications.pool.get(
        ctx,
        name=name,
        resource_id=resource_id,
    )
...
async def absent(
    ...
    Example:
        .. code-block:: sls
            ...
            idem_test_nsx_alb.applications.pool_is_absent:
              nsx_alb.nsx_alb.applications.pool.absent:
    ...
    if not resource_id:
        result["comment"].append(f"'nsx_alb.applications.pool:{name}' already absent")
        return result
    ...
    if before["ret"]:
        if ctx.test:
            result["comment"] = f"Would delete nsx_alb.applications.pool:{name}"
            return result

        delete_ret = await hub.exec.nsx_alb.applications.pool.delete(
            ctx,
            name=name,
            resource_id=resource_id,
        )
        ...
        if result["result"]:
            result["comment"].append(f"Deleted 'nsx_alb.applications.pool:{name}'")
        else:
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Example:
        ...
        .. code-block:: bash
            ...
            $ idem describe nsx_alb.applications.pool
    """

    result = {}

    ret = await hub.exec.nsx_alb.applications.pool.list(ctx)

    if not ret or not ret["result"]:
        hub.log.debug(f"Could not describe nsx_alb.applications.pool {ret['comment']}")
        return result

    for resource in ret["ret"]:
        resource_id = resource.get("resource_id")
        result[resource_id] = {
            "nsx_alb.applications.pool.present": [
