...
async def present(
    ...
    Example:
        .. code-block:: sls
          ...
          idem_test_nsx_alb.profiles.network_profile_is_present:
              nsx_alb.nsx_alb.profiles.network_profile.present:
              - configpb_attributes:
                  version: int
    ...
    if resource_id:
        before = await hub.exec.nsx_alb.profiles.network_profile.get(
            ctx,
            name=name,
        ...
        result["comment"].append(
            f"'nsx_alb.profiles.network_profile:{name}' already exists"
        )
    ...
    if result["old_state"]:
        ...
        if bool(changes.get("new")):
            if ctx.test:
                ...
                result["comment"] = (
                    f"Would update nsx_alb.profiles.network_profile '{name}'",
                )
                return result
            else:
                # Update the resource
                update_ret = await hub.exec.nsx_alb.profiles.network_profile.update(
                    ctx,
                    name=name,
                    resource_id=resource_id,
                    **{
                ...
                if result["result"]:
                    result["comment"].append(
                        f"Updated 'nsx_alb.profiles.network_profile:{name}'"
                    )
                else:
        ...
        if ctx.test:
            ...
            result["comment"] = (
                f"Would create nsx_alb.profiles.network_profile {name}",
            )
            return result
        else:
            create_ret = await hub.exec.nsx_alb.profiles.network_profile.create(
                ctx,
                name=name,
                **{
                    "resource_id": resource_id,
            ...
            if result["result"]:
                result["comment"].append(
                    f"Created 'nsx_alb.profiles.network_profile:{name}'"
                )
    ...
    after = await hub.exec.nsx_alb.profiles.network_profile.get(
        ctx,
        name=name,
        resource_id=resource_id,
    )
...
async def absent(hub, ctx, name: str, resource_id: str = None) -> Dict[str, Any]:
    ...
    Example:
        .. code-block:: sls
            ...
            idem_test_nsx_alb.profiles.network_profile_is_absent:
              nsx_alb.nsx_alb.profiles.network_profile.absent:
    ...
    if not resource_id:
        result["comment"].append(
            f"'nsx_alb.profiles.network_profile:{name}' already absent"
        )
        return result
    ...
    if before["ret"]:
        if ctx.test:
            result["comment"] = f"Would delete nsx_alb.profiles.network_profile:{name}"
            return result

        delete_ret = await hub.exec.nsx_alb.profiles.network_profile.delete(
            ctx,
            name=name,
            resource_id=resource_id,
        )
        ...
        if result["result"]:
            result["comment"].append(
                f"Deleted 'nsx_alb.profiles.network_profile:{name}'"
            )
        else:
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    Example:
        ...
        .. code-block:: bash
            ...
            $ idem describe nsx_alb.profiles.network_profile
    """

    result = {}

    ret = await hub.exec.nsx_alb.profiles.network_profile.list(ctx)

    if not ret or not ret["result"]:
        hub.log.debug(
            f"Could not describe nsx_alb.profiles.network_profile {ret['comment']}"
        )
        return result
    ...
    for resource in ret["ret"]:
        ...
        result[resource_id] = {
            "nsx_alb.profiles.network_profile.present": [
