...
def exec(*args):
    ...
    jbang_available = (subprocess.run(["which", "jbang"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode == 0) or \
                    ...
                    (subprocess.run(["which", "./jbang"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode == 0) or \
                    (subprocess.run(["which", "~/.jbang/bin/jbang"], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode == 0)
    ...
    if jbang_available:
        ...
        cmd_result = subprocess.run(f"jbang {arg_line}", capture_output=True, shell=True, text=True)
    elif curl_available and bash_available:
        log.debug(f"using curl + bash: {arg_line}")
        cmd_result = subprocess.run(f"curl -Ls https://sh.jbang.dev | bash -s - {arg_line}", capture_output=True, shell=True, text=True)
        ...
        cmd_result = subprocess.run(f'powershell -Command "%TEMP%/jbang.ps1 {arg_line}"', capture_output=True, shell=True, text=True)
    else:
    ...
    if cmd_result.returncode != 0:
        raise Exception(f"The command failed: 'jbang {arg_line}'. Code: {cmd_result.returncode}. Output: {cmd_result.stdout}. Error: {cmd_result.stderr}")
