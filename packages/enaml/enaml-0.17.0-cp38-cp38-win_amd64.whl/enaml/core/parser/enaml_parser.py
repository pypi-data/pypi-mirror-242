...
class EnamlParser(Parser):
    ...
    def child_def(self) -> Optional[enaml_ast.ChildDef]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.name()) and (self.expect(":")) and (c := self.child_def_body()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def operator_expr(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self._tmp_17()) and (b := self.py_expr()) and (self.expect("NEWLINE")):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
        ...
        self._reset(mark)
        if (a := self._tmp_18()) and (b := self.py_expr()) and (self.expect("NEWLINE")):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return (
        ...
        self._reset(mark)
        if (self.expect("<<")) and (a := self.block()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return enaml_ast.OperatorExpr(
                operator="<<",
    ...
    def decl_funcdef(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("async")) and (a := self.sync_decl_fundef()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def template_params(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (self.expect("*")) and (b := self.name()):
            return enaml_ast.TemplateParameters(
    ...
    def template_args(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (self.expect("*")) and (b := self.expression()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return enaml_ast.TemplateArguments(
                args=[],
    ...
    def statement_newline(self) -> Optional[list]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.compound_stmt()) and (self.expect("NEWLINE")):
            return [a]
        self._reset(mark)
    ...
    def simple_stmt(self) -> Optional[Any]:
        ...
        if assignment := self.assignment():
            return assignment
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "type")) and (
    ...
    def global_stmt(self) -> Optional[ast.Global]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("global")) and (a := self._gather_50()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Global(
    ...
    def nonlocal_stmt(self) -> Optional[ast.Nonlocal]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("nonlocal")) and (a := self._gather_52()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Nonlocal(
                names=[n.string for n in a],
    ...
    def import_from_as_name(self) -> Optional[ast.alias]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.name()) and (b := self._tmp_60(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.alias(
                name=a.string,
    ...
    def dotted_as_name(self) -> Optional[ast.alias]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.dotted_name()) and (b := self._tmp_63(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.alias(
                name=a,
    ...
    def parameters(self) -> Optional[ast.arguments]:
        ...
        if (
            ...
            and (b := self._loop0_70(),)
            and (c := self._loop0_71(),)
            and (d := self.star_etc(),)
        ):
            return self.check_version(
                (3, 8),
        ...
        if (
            (a := self._loop1_73())
            and (b := self._loop0_74(),)
            and (c := self.star_etc(),)
        ):
            return self.make_arguments(None, [], a, b, c)
        self._reset(mark)
        if (a := self._loop1_75()) and (b := self.star_etc(),):
    ...
    def slash_no_default(self) -> Optional[List[Tuple[ast.arg, None]]]:
        ...
        mark = self._mark()
        if (a := self._loop1_76()) and (self.expect("/")) and (self.expect(",")):
    ...
    def param_no_default(self) -> Optional[ast.arg]:
        ...
        mark = self._mark()
        if (a := self.param()) and (self.expect(",")) and (tc := self.type_comment(),):
    ...
    def signed_number(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (self.expect("-")) and (a := self.number()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.UnaryOp(
                op=ast.USub(),
    ...
    def imaginary_number(self) -> Optional[ast.Constant]:
        ...
        if imag := self.number():
            ...
            end_lineno, end_col_offset = tok.end
            return ast.Constant(
                value=self.ensure_imaginary(imag),
                lineno=start_lineno,
                col_offset=start_col_offset,
                end_lineno=end_lineno,
                end_col_offset=end_col_offset,
    ...
    def maybe_sequence_pattern(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (patterns := self._gather_105()) and (self.expect(","),):
            return patterns
        self._reset(mark)
        return None
    ...
    def items_pattern(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if _gather_107 := self._gather_107():
            return _gather_107
        self._reset(mark)
        return None
    ...
    def keyword_patterns(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if _gather_112 := self._gather_112():
            return _gather_112
        self._reset(mark)
        return None
    ...
    def type_param_seq(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_114()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def star_named_expressions(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_118()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def disjunction(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.conjunction()) and (b := self._loop1_120()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.BoolOp(
                op=ast.Or(),
    ...
    def conjunction(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.inversion()) and (b := self._loop1_121()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.BoolOp(
                op=ast.And(),
    ...
    def comparison(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (a := self.bitwise_or()) and (b := self._loop1_122()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Compare(
                left=a,
    ...
    def slices(self) -> Optional[Any]:
        ...
        if (a := self.slice()) and (self.negative_lookahead(self.expect, ",")):
            return a
        self._reset(mark)
        if (a := self._gather_123()) and (self.expect(","),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return (
                ast.Tuple(
    ...
    def atom(self) -> Optional[Any]:
        ...
        self._reset(mark)
        if (self.positive_lookahead(self.expect, "(")) and (
    ...
    def group(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (self.expect("(")) and (a := self._tmp_130()) and (self.expect(")")):
            return a
        self._reset(mark)
    ...
    def lambda_parameters(self) -> Optional[ast.arguments]:
        ...
        if (
            (a := self._loop1_134())
            and (b := self._loop0_135(),)
            and (c := self.lambda_star_etc(),)
        ):
            return self.make_arguments(None, [], a, b, c)
        self._reset(mark)
        if (a := self._loop1_136()) and (b := self.lambda_star_etc(),):
            return self.make_arguments(None, [], None, a, b)
        self._reset(mark)
    ...
    def lambda_slash_no_default(self) -> Optional[List[Tuple[ast.arg, None]]]:
        ...
        mark = self._mark()
        if (a := self._loop1_137()) and (self.expect("/")) and (self.expect(",")):
            return [(p, None) for p in a]
        self._reset(mark)
        if (
            (a := self._loop1_138())
    ...
    def fstring_full_format_spec(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect(":")) and (spec := self._loop0_146(),):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
    ...
    def fstring(self) -> Optional[Any]:
        ...
        if (
            (self.fstring_start())
            and (b := self._loop0_147(),)
            and (self.fstring_end())
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.JoinedStr(
    ...
    def strings(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if a := self._loop1_148():
            return (
                self.concatenate_strings(a)
                if sys.version_info >= (3, 12)
                else self.generate_ast_for_string(a)
            )
        self._reset(mark)
        return None
    ...
    def tuple(self) -> Optional[ast.Tuple]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("(")) and (a := self._tmp_149(),) and (self.expect(")")):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Tuple(
                elts=a or [],
    ...
    def double_starred_kvpairs(self) -> Optional[list]:
        ...
        mark = self._mark()
        if (a := self._gather_150()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def star_targets_list_seq(self) -> Optional[list]:
        ...
        mark = self._mark()
        if (a := self._gather_168()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def star_target(self) -> Optional[Any]:
        ...
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (self.expect("*")) and (a := self._tmp_171()):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast.Starred(
    ...
    def del_targets(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self._gather_172()) and (self.expect(","),):
            return a
        self._reset(mark)
        return None
    ...
    def invalid_arguments(self) -> Optional[NoReturn]:
        ...
        if (
            ...
            return self.raise_syntax_error_known_range(
                "expected argument value expression", a, b
            )
        self._reset(mark)
    ...
    def invalid_kwarg(self) -> Optional[NoReturn]:
        ...
        mark = self._mark()
        if (a := self._tmp_186()) and (b := self.expect("=")):
            return self.raise_syntax_error_known_range(
                f"cannot assign to {a.string}", a, b
            )
        self._reset(mark)
        if (
            (a := self.name())
    ...
    def invalid_assignment(self) -> Optional[NoReturn]:
        ...
        self._reset(mark)
        if (self._loop0_196(),) and (a := self.yield_expr()) and (self.expect("=")):
            return self.raise_syntax_error_known_location(
                "assignment to yield expression not possible", a
            )
        self._reset(mark)
        if (a := self.star_expressions()) and (self.augassign()) and (self._tmp_197()):
            return self.raise_syntax_error_known_location(
    ...
    def invalid_parameters(self) -> Optional[NoReturn]:
        ...
        self._reset(mark)
        if (self._tmp_201()) and (self._loop0_202(),) and (a := self.expect("/")):
        ...
        self._reset(mark)
        if (self._loop1_210()) and (self.expect("/")) and (a := self.expect("*")):
            return self.raise_syntax_error_known_location(
                "expected comma between / and *", a
            )
        self._reset(mark)
        return None
    ...
    def invalid_default(self) -> Optional[Any]:
        # invalid_default: '=' &(')' | ',')
        mark = self._mark()
        if (a := self.expect("=")) and (
            self.positive_lookahead(
                self._tmp_211,
            )
        ):
            return self.raise_syntax_error_known_location(
                "expected default value expression", a
            )
        self._reset(mark)
        return None
    ...
    def invalid_star_etc(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (a := self.expect("*")) and (self._tmp_212()):
            return self.raise_syntax_error_known_location(
                "named arguments must follow bare *", a
            )
        self._reset(mark)
        if (self.expect("*")) and (self.expect(",")) and (self.type_comment()):
        ...
        if (
            ...
            return self.raise_syntax_error_known_location(
                "* argument may appear only once", a
            )
        self._reset(mark)
        return None
    ...
    def invalid_lambda_parameters(self) -> Optional[NoReturn]:
        ...
        self._reset(mark)
        if (self._tmp_218()) and (self._loop0_219(),) and (a := self.expect("/")):
            return self.raise_syntax_error_known_location("/ may appear only once", a)
        self._reset(mark)
        if (
            self.call_invalid_rules
        ...
        if (
            ...
            return self.raise_syntax_error_known_location("/ must be ahead of *", a)
        self._reset(mark)
        if (self._loop1_228()) and (self.expect("/")) and (a := self.expect("*")):
            return self.raise_syntax_error_known_location(
                "expected comma between / and *", a
            )
        self._reset(mark)
    ...
    def invalid_lambda_star_etc(self) -> Optional[NoReturn]:
        ...
        mark = self._mark()
        if (self.expect("*")) and (self._tmp_230()):
    ...
    def invalid_except_stmt(self) -> Optional[None]:
        ...
        self._reset(mark)
        if (self.expect("except")) and (self.expect("*"),) and (self.expect("NEWLINE")):
            return self.raise_syntax_error("expected ':'")
        self._reset(mark)
        if (self.expect("except")) and (self.expect("*")) and (self._tmp_255()):
    ...
    def _tmp_3(self) -> Optional[Any]:
        # _tmp_3: ':' NAME
        mark = self._mark()
        if (self.expect(":")) and (z := self.name()):
            return z
        self._reset(mark)
        return None
    ...
    def _tmp_7(self) -> Optional[Any]:
        # _tmp_7: '(' ','.pragma_arg+ ')'
        mark = self._mark()
        if (self.expect("(")) and (b := self._gather_281()) and (self.expect(")")):
            return b
        self._reset(mark)
        return None
    ...
    def _tmp_8(self) -> Optional[Any]:
        # _tmp_8: ':' '.'.NAME+
        mark = self._mark()
        if (self.expect(":")) and (c := self._gather_283()):
            return c
        self._reset(mark)
        return None
    ...
    def _tmp_9(self) -> Optional[Any]:
        # _tmp_9: ':' dec_primary
        mark = self._mark()
        if (self.expect(":")) and (c := self.dec_primary()):
            return c
        self._reset(mark)
        return None
    ...
    def _tmp_11(self) -> Optional[Any]:
        # _tmp_11: ':' dec_primary
        mark = self._mark()
        if (self.expect(":")) and (d := self.dec_primary()):
            return d
        self._reset(mark)
        return None
    ...
    def _loop0_16(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(".")) and (elem := self.name()):
    ...
    def _loop0_24(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.template_param()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _tmp_25(self) -> Optional[Any]:
        # _tmp_25: ',' '*' NAME
        mark = self._mark()
        if (self.expect(",")) and (self.expect("*")) and (c := self.name()):
            return c
        self._reset(mark)
        return None
    ...
    def _tmp_26(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (self.expect(":")) and (z := self.template_ids()):
            return z
        self._reset(mark)
        return None
    ...
    def _loop0_28(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.template_argument()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _tmp_29(self) -> Optional[Any]:
        # _tmp_29: ',' '*' expression
        mark = self._mark()
        if (self.expect(",")) and (self.expect("*")) and (z := self.expression()):
            return z
        self._reset(mark)
        return None
    ...
    def _tmp_32(self) -> Optional[Any]:
        # _tmp_32: ',' '*' NAME
        mark = self._mark()
        if (self.expect(",")) and (self.expect("*")) and (z := self.name()):
            return z
        self._reset(mark)
        return None
    ...
    def _tmp_34(self) -> Optional[Any]:
        # _tmp_34: '.' '.'.NAME+
        mark = self._mark()
        if (self.expect(".")) and (z := self._gather_285()):
            return z
        self._reset(mark)
        return None
    ...
    def _loop0_37(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(";")) and (elem := self.simple_stmt()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_50(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.name()) is not None and (seq := self._loop0_51()) is not None:
            return [elem] + seq
        self._reset(mark)
        return None
    ...
    def _gather_52(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.name()) is not None and (seq := self._loop0_53()) is not None:
            return [elem] + seq
        self._reset(mark)
        return None
    ...
    def _loop0_59(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.import_from_as_name()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_62(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.dotted_as_name()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_86(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.with_item()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop1_95(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while except_star_block := self.except_star_block():
    ...
    def _loop0_100(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect("|")) and (elem := self.closed_pattern()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_106(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.maybe_star_pattern()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_108(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.key_value_pattern()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_111(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.pattern()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_113(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.keyword_pattern()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_115(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.type_param()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_114(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.type_param()) is not None and (
            seq := self._loop0_115()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_119(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.star_named_expression()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_124(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_294()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_123(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_294()) is not None and (
            seq := self._loop0_124()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_146(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while fstring_format_spec := self.fstring_format_spec():
    ...
    def _loop0_147(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while fstring_mid := self.fstring_mid():
    ...
    def _loop0_151(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.double_starred_kvpair()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_157(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_298()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_156(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_298()) is not None and (
            seq := self._loop0_157()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_160(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.kwarg_or_starred()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_169(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.star_target()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_173(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.del_target()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _loop0_175(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.expression()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _tmp_212(self) -> Optional[Any]:
        ...
        if literal := self.expect(")"):
            return literal
        self._reset(mark)
        if (literal := self.expect(",")) and (_tmp_303 := self._tmp_303()):
    ...
    def _loop0_223(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.lambda_param()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_222(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.lambda_param()) is not None and (
            seq := self._loop0_223()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _tmp_230(self) -> Optional[Any]:
        ...
        if literal := self.expect(":"):
            return literal
        self._reset(mark)
        if (literal := self.expect(",")) and (_tmp_304 := self._tmp_304()):
    ...
    def _loop0_237(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self.dotted_name()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_236(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.dotted_name()) is not None and (
            seq := self._loop0_237()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_239(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_305()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_238(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_305()) is not None and (
            seq := self._loop0_239()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_241(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_306()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_240(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_306()) is not None and (
            seq := self._loop0_241()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_243(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_307()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_242(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_307()) is not None and (
            seq := self._loop0_243()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _loop0_245(self) -> Optional[Any]:
        ...
        mark = self._mark()
        children = []
        while (self.expect(",")) and (elem := self._tmp_308()):
            children.append(elem)
            mark = self._mark()
        self._reset(mark)
        return children
    ...
    def _gather_244(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self._tmp_308()) is not None and (
            seq := self._loop0_245()
        ) is not None:
            return [elem] + seq
        self._reset(mark)
    ...
    def _tmp_252(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_309(),):
    ...
    def _tmp_258(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (positional_patterns := self.positional_patterns()) and (
            literal := self.expect(",")
        ):
    ...
    def _gather_283(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.name()) is not None and (seq := self._loop0_284()) is not None:
            return [elem] + seq
        self._reset(mark)
        return None
    ...
    def _gather_285(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (elem := self.name()) is not None and (seq := self._loop0_286()) is not None:
            return [elem] + seq
        self._reset(mark)
        return None
    ...
    def _tmp_292(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (self.expect("or")) and (c := self.conjunction()):
            return c
        self._reset(mark)
        return None
    ...
    def _tmp_293(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (self.expect("and")) and (c := self.inversion()):
            return c
        self._reset(mark)
        return None
    ...
    def _tmp_305(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_311(),):
            return [expression, opt]
        self._reset(mark)
        return None
    ...
    def _tmp_306(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expressions := self.expressions()) and (opt := self._tmp_312(),):
    ...
    def _tmp_307(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expression := self.expression()) and (opt := self._tmp_313(),):
            return [expression, opt]
        self._reset(mark)
        return None
    ...
    def _tmp_308(self) -> Optional[Any]:
        ...
        mark = self._mark()
        if (expressions := self.expressions()) and (opt := self._tmp_314(),):
            return [expressions, opt]
        self._reset(mark)
        return None
