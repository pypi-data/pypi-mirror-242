...
from garpix_utils.logs.services.logger_iso import LoggerIso, Action
...
class LogAdminMixin(admin.ModelAdmin, CreateLogMixin):
    ...
    def delete_model(self, request, obj):
        action = Action.user_delete.value if obj.__class__ == User else Action.user_reset.value
        log = self.log_delete(request, obj, action)
    ...
    def delete_queryset(self, request, queryset):
        ...
        logs = []
        for obj in queryset:
            logs.append(self.log_delete(request, obj, action))
