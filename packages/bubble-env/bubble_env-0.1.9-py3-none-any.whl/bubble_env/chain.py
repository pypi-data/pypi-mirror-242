...
from bubble_env.base.service import Service
from bubble_env.node import Node
from bubble_env.factory.factory import chain_factory
from bubble_env.genesis import Genesis
from bubble_env.utils.executor import concurrent_executor
...
class Chain(Service):
    ...
    def upload_bubble(self, bubble_file, nodes: List[Node] = None):
        """ 使用缓存上传bubble
        """
        nodes = nodes or self.nodes
        concurrent_executor(nodes, 'upload_bubble', bubble_file)

        logger.debug(f'Chain {self} upload bubble success!')
