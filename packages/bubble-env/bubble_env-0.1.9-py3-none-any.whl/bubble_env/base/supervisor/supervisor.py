...
class Supervisor(Process):
    ...
    def install(self):
        ...
        is_installed = self.host.ssh("which supervisord || echo needInstall", warn=False)
        if 'needInstall' in str(is_installed):
            self.host.ssh('apt update', sudo=True, warn=False)
            self.host.ssh('apt install -y --reinstall supervisor', sudo=True, warn=False)
            self.host.ssh('systemctl restart supervisor', sudo=True, warn=False)
            self._upload_config()
