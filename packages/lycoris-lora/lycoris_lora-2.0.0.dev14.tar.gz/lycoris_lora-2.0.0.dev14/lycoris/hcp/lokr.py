...
class LokrBlock(LycorisPluginBlock):
    ...
    def __init__(
        ...
        if decompose_both and self.dim< max(self.shape[0][0], self.shape[1][0])/2:
            ...
            self.use_w1 = True
            self.lokr_w1 = nn.Parameter(torch.empty(self.shape[0][0], self.shape[1][0]))  # a*c, 1-mode
        
        if self.module_type == 'conv':
            if self.dim>= max(self.shape[0][1], self.shape[1][1])/2:
                self.use_w2 = True
                self.lokr_w2 = nn.Parameter(torch.empty(self.shape[0][1], self.shape[1][1], *self.k_size))
            elif self.tucker:
                self.lokr_t2 = nn.Parameter(torch.empty(self.dim, self.dim, self.shape[2], self.shape[3]))
                self.lokr_w2_a = nn.Parameter(torch.empty(self.dim, self.shape[0][1])) # b, 1-mode
                self.lokr_w2_b = nn.Parameter(torch.empty(self.dim, self.shape[1][1])) # d, 2-mode
            else: # Conv2d not cp
                # bigger part. weight and LoRA. [b, dim] x [dim, d*k1*k2]
                self.lokr_w2_a = nn.Parameter(torch.empty(self.shape[0][1], self.dim))
                self.lokr_w2_b = nn.Parameter(torch.empty(self.dim, self.shape[1][1]*self.shape[2]*self.shape[3]))
            ...
            if self.dim < max(self.shape[0][1], self.shape[1][1])/2:
                ...
                self.use_w2 = True
                self.lokr_w2 = nn.Parameter(torch.empty(self.shape[0][1], self.shape[1][1]))
        else:
