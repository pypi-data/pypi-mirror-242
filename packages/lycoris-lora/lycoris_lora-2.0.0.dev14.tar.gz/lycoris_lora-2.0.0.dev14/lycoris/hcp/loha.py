...
class LohaBlock(LycorisPluginBlock):
    ...
    def __init__(
        ...
        if self.tucker:
            self.hada_t1 = nn.Parameter(torch.empty(self.dim, self.dim, self.shape[2], self.shape[3]))
            self.hada_w1_a = nn.Parameter(torch.empty(self.dim, self.shape[0])) # out_dim, 1-mode
            self.hada_w1_b = nn.Parameter(torch.empty(self.dim, self.shape[1])) # in_dim , 2-mode
            
            self.hada_t2 = nn.Parameter(torch.empty(self.dim, self.dim, self.shape[2], self.shape[3]))
            self.hada_w2_a = nn.Parameter(torch.empty(self.dim, self.shape[0])) # out_dim, 1-mode
            self.hada_w2_b = nn.Parameter(torch.empty(self.dim, self.shape[1])) # in_dim , 2-mode
        else:
