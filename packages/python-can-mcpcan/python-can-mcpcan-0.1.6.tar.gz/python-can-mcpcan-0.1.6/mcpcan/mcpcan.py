...
import can
from can import BusABC, Message, CanProtocol, typechecking
...
class MCPcanBus(BusABC):
    ...
    def _get_device(self):
        ...
        return self.serial_write_read(str_msg).strip().decode()
    ...
    def _recv_internal(self, timeout):
        ...
        try:
            ...
            if self.ser.in_waiting:
                rmsg = self.ser.read_until("\r").decode().strip()
            else:
                return None, True
        ...
        if rmsg[0] == "t":
            time_stamp = time() % 6000  # 2 bytes time stamp
            is_extended = False
            is_remote = False
