...
class ConversableAgent(Agent):
    ...
    async def a_receive(
        ...
        if request_reply is False or request_reply is None and self.reply_at_receive[sender] is False:
            return
        if isinstance(self, GroupChatManager):
            #  群聊，第一条消息会进入这里
            content = message if isinstance(message, str) else message.get('content')
            await run_manager[0].on_text(text=content)
        ...
        if not isinstance(self, GroupChatManager):
            ...
            await run_manager[0].on_text(text=reply, sender=self.name, receiver=receiver)
    ...
    async def a_check_termination_and_human_reply(
        ...
        **kwargs,
        ...
        if self.input:
            ...
            msg = {"message": reply, "sender": self.name, "receiver": {"is_bot": True}}
    ...
    async def a_execute_code_blocks(self, code_blocks, run_manager):
        ...
        for i, code_block in enumerate(code_blocks):
            ...
            await run_manager.on_text(
                text=f"<span style=\"color:red;\">~EXECUTING CODE BLOCK {i} (inferred language is {lang})...</span>",
            )
            if lang in ["bash", "shell", "sh"]:
                exitcode, logs, image = self.run_code(code, lang=lang, **self._code_execution_config)
            elif lang in ["python", "Python"]:
    ...
    async def a_execute_function(self, func_call, run_manager):
        ...
        if func is not None:
            ...
            if arguments is not None:
                ...
                await run_manager.on_text(
                    text=f"<span style=\"color:red;\">EXECUTING FUNCTION {func_name}...</span> ",
                )
                try:
                    content = func(**arguments)
                    is_exec_success = True
