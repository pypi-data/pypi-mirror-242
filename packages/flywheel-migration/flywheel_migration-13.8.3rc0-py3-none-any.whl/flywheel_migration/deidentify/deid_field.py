...
class DeIdField:
    ...
    def factory(cls, config, dry=False, mixin=None):
        ...
        if name and regex:
            raise ValueError(f"Field can not have name and regex defined. " f"Both found in {config}")
        if name:
            fieldname = name
        elif regex:
            fieldname = regex
            ...
            raise ValueError(f"Field element must defined either name or regex. " f"none found in {config}")
