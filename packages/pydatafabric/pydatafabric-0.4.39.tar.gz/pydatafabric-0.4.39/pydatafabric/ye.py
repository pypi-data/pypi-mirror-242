...
def get_spark(
    ...
    key = get_secrets(mount, path)["config"]
...
def send_email(subject, text, send_from, send_to, attachment=None, file_format=None):
    """
    ...
    if attachment:
        ...
        part.add_header(
            "Content-Disposition", f"attachment; filename={subject}.{file_format}"
        )
        msg.attach(part)
