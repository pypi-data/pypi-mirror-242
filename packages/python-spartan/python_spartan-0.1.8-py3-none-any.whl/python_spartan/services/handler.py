...
class HandlerService:
    ...
    def determine_source_stub(self):
        if self.subscribe and self.publish:
            ...
            return os.path.join(self.stub_folder, "sqs_subscribe.stub") if self.subscribe == "sqs" else os.path.join(self.stub_folder, "sns.stub")
        elif self.publish:
    ...
    def create_handler_file(self):
        try:
            if not os.path.exists(os.path.join(self.current_directory, self.destination_folder)):
                os.makedirs(os.path.join(self.current_directory, self.destination_folder))
            ...
            if os.path.exists(self.file_path):
                with open(self.file_path, "r") as dest_file:
                    ...
                    if "{{ sqs_listen }}" in dest_content and self.subscribe:
                        print("Destination already has SQS subscription placeholder. Consider updating manually.")
                        return

                    if "{{ sqs_publish }}" in dest_content and self.publish:
                        print("Destination already has SQS publishing placeholder. Consider updating manually.")
                        return
            ...
            handler_stub_content = self.insert_subscribe_publish_code(handler_stub_content)
    ...
    def insert_subscribe_publish_code(self, handler_stub_content):
        if self.subscribe or (self.subscribe and self.publish):
            handler_stub_content = self.insert_code_block(handler_stub_content, "sqs_listen.stub", "{{ sqs_listen }}")

        if self.publish or (self.subscribe and self.publish):
            handler_stub_content = self.insert_code_block(handler_stub_content, "sqs_trigger.stub", "{{ sqs_trigger }}")
