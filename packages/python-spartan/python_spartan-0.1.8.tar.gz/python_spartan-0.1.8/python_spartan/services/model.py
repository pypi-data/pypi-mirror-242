...
class ModelService:
    def __init__(self, name: str):
        ...
        self.destination_folder = "models"
        self.source_stub = os.path.join(self.home_directory, "stubs", "model", "default.stub")
        self.file_name = re.sub(r"\d", "", f"{self.name}.py").lower()
        self.file_path = os.path.join(
            self.current_directory, self.parent_folder, self.destination_folder, self.file_name
        )

    def create_model_file(self):
        if not os.path.exists(os.path.join(self.current_directory, self.parent_folder, self.destination_folder)):
            try:
                os.makedirs(os.path.join(self.current_directory, self.parent_folder, self.destination_folder))
        ...
        if not os.path.exists(self.file_path):
            ...
            updated_content = updated_content.replace("{{name}}", self.name.capitalize())
            ...
            try:
                ...
                print(f"An error occurred while writing the new model file: {e}")
        else:
