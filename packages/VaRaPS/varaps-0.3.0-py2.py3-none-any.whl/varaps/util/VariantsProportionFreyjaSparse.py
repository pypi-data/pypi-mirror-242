...
class VariantsProportionFreyjaSparse:
    ...
    def fit(self, freezeAlpha=False):
        if freezeAlpha:  # freeze alpha,,,optimize  using cvxpy
            dep = np.log(self.count_tot)
            dep = dep / np.max(dep)  # normalize depth scaling pre-optimization
            # set up and solve demixing problem
            A = np.array((self.pMutationsVariants.T * dep).T)
            b = np.array(self.mafH * dep)
            x = cp.Variable(A.shape[1])
            A_ = A - 2 * self.alpha * A + self.alpha
            cost = cp.norm(A_ @ x - b, 2)
            constraints = [sum(x) == 1, x >= 0]
            prob = cp.Problem(cp.Minimize(cost), constraints)
            prob.solve(verbose=False)
            self.solution = x.value
            self.errAlpha = self.alpha

        else:  # not a convex expression(when adding alpha to optimize) so can't use cvxpy this is way I will use scipy.optimize
            self.alpha = np.log(self.alpha / (1 - self.alpha))
            x = np.hstack(
                (
                    np.random.dirichlet(np.ones(self.pMutationsVariants.shape[1])),
                    [self.alpha],
                )
            )
            params = [
                self.count_mut,
                self.count_tot - self.count_mut,
                self.pMutationsVariants,
            ]
            res = minimize(
                self.kern,
                x0=x,
