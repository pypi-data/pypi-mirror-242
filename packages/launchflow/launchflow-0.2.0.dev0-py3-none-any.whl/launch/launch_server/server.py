...
class LaunchManager:
    def __init__(self, buildflow_app: BuildFlowApp):
        ...
        # {metric name: deque(10) of metric values}
        self.metrics = {}
            ...
            async def event_generator():
                ...
                while True:
                    if self.uvicorn_server.should_exit:
                        print("uvicorn server should exit")
                        break
                    # Pulls runtime state from the runtime server and forwards it to the relay
                    # server
                    try:
                        # TODO: This should use httpx so this request can run async
                        runtime_state_json = requests.get(
                            f"http://localhost:{self.runtime_server_port}/runtime/snapshot"
                        ).json()
    ...
    def run(self):
        ...
        monitor_task = loop.create_task(self.monitor_subprocess())
