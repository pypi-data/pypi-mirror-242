...
from .xray_config import DEFAULT_SECURITY, XrayConfig, TLS
            ...
            if self.uri.startswith(EConfigType.VMESS.value[1]):
                ...
                if not self.try_parse_new_vmess(config, allow_insecure):
                    if "?" in self.uri:
                        ...
                        vnext.users[0].security = DEFAULT_SECURITY if not vmess_qr_code.get("scy") else vmess_qr_code.get("scy")
                        vnext.users[0].alterId = 0 if not vmess_qr_code.get("aid") else int(vmess_qr_code.get("aid"))
                        ...
                        fingerprint = vmess_qr_code.get("fp") if vmess_qr_code.get("fp") is not None else (
                            stream_setting.tlsSettings.fingerprint) if stream_setting.tlsSettings is not None else None
                ...
                uri = urllib.parse.urlparse(Utils.fix_illegal_url(self.uri))
