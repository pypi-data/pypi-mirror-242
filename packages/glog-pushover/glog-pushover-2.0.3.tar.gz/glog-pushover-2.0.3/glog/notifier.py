...
class ConfigData(BaseModel):
    ...
    def check_service(cls, value):
        """
        This function checks that the service is either pushover or ntfy
        """
        if value not in ['pushover', 'ntfy']:
            raise ValueError('service must be either pushover or ntfy')
        return value
...
class Notifier():
    ...
    def __init__(self, config_data):
        ...
        elif self.config_data.service == 'ntfy':
    ...
    def send_ntfy_message(self):
        ...
        try:
            print(self.config_data.ntfy_host)
            requests.post(
                f'{self.config_data.ntfy_host}',
