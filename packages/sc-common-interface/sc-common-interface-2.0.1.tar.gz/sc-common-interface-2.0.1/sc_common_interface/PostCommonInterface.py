...
def search_oa_gift(data):
    ...
    # change_dict_into_hump("title_translations", gift_info)
    # change_dict_into_hump("currency_iso", gift_info["cost"])
    # change_dict_into_hump("currency_symbol", gift_info["cost"])
    # change_dict_into_hump("created_at", gift_info)
    # change_dict_into_hump("updated_at", gift_info)
    # change_dict_into_hump("unlimited_quantity", gift_info)
    # change_dict_into_hump("alt_translations", gift_info["medias"][0])
    # change_dict_into_hump("_id", gift_info["medias"][0])
    # change_dict_into_hump("favicon_large", gift_info["medias"][0]["images"])
    # change_dict_into_hump("favicon_small", gift_info["medias"][0]["images"])
    # change_dict_into_hump("transparent_large", gift_info["medias"][0]["images"])
    # change_dict_into_hump("transparent_thumb", gift_info["medias"][0]["images"])
    # change_dict_into_hump("transparent_xlarge", gift_info["medias"][0]["images"])
...
def get_page_post(data):
    ...
    if "page_size" in data:
        page_size = data
    params = {"page_size":page_size,"type":type,"since":since,"party_channel_id":page_id}
...
def send_post_comment(data):
    ...
    key = data["key"]
    object = "page"
    body = {"object": object, "entry": [{"id": page_id, "time": stamp, "changes": [{"field": "feed", "value": {
        "from": {"id": user_id, "name": name},
        "post": {"status_type": "added_video", "is_published": True, "updated_time": "2022-11-18T09:57:26+0000",
                 "permalink_url": "https://www.facebook.com/permalink.php?story_fbid=pfbid02jLK3e6YdFSXp2DmD7j7vtStLXoBzTi8rxKrp6jFhVMUTTEgz6qvZA8soR9Uwydd8l&id=107977035056574",
                 "promotion_status": "inactive", "id": post_id}, "message": keyword, "item": "comment",
        "verb": "add", "post_id": post_id, "comment_id": comment_id,
        "created_time": stamp, "parent_id": post_id}}]}]}
    if platform=="instagram".upper():
        object = platform.lower()
        comment_id = "1807300%d"%stamp
        body = {"object": object,"entry": [{"id": page_id, "time": stamp, "changes": [{"value": {"from":
        {"id": user_id, "username": name}, "media": {"id":post_id, "media_product_type": "FEED"}, "id":comment_id, "text":keyword}, "field": "comments"}]}]}
