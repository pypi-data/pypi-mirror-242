...
TIMEWORKLOAD_ANY_OF_SCHEMAS = ["List[WorkloadTime]", "float"]

class TimeWorkload(BaseModel):
    """
    TimeWorkload
    """
    ...
    # data type: List[WorkloadTime]
    anyof_schema_2_validator: Optional[List[WorkloadTime]] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[List[WorkloadTime], float]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: List[str] = Literal[TIMEWORKLOAD_ANY_OF_SCHEMAS]
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = TimeWorkload.model_construct()
        error_messages = []
        ...
        # validate data type: List[WorkloadTime]
        try:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in TimeWorkload with anyOf schemas: List[WorkloadTime], float. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into List[WorkloadTime]
        try:
            # validation
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TimeWorkload with anyOf schemas: List[WorkloadTime], float. Details: " + ", ".join(error_messages))
        else:
            return instance
