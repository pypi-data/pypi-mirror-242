...
cdef class atomic_shared_memory(atomic_object):
    ...
    cdef int s1(self, char operation_length)  except -1
    ...
    cdef void s2(self,  unsigned char[:] mv, unsigned char * reference, size_t length) except *
    cdef void s3(self,  const unsigned char[:] mv, unsigned char * reference, size_t length) except *
    ...
    cdef tuple s4(self, size_t start, size_t length)
