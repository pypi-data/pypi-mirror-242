...
from telegram.ext.dispatcher import block, Dispatcher, DispatcherHandlerStop
...
class TestDispatcher:
    ...
    def test_default_block_error_handler(self, dp, monkeypatch, block, expected_output):
        ...
        try:
            ...
            monkeypatch.setattr(dp, 'block', mock_async_err_handler)
    ...
    def test_default_block(self, monkeypatch, dp, block, expected_output):
        ...
        try:
            ...
            monkeypatch.setattr(dp, 'block', mock_block)
            dp.process_update(self.message_update)
    ...
    def test_block_multiple(self, bot, dp, dp2):
        ...
        dp.block(get_dispatcher_name, q1)
    ...
    def test_multiple_block_decorator(self, dp, dp2):
    ...
    def test_block_with_args(self, dp):
        dp.add_handler(
            MessageHandler(
                Filters.all, block(self.callback_if_not_update_queue), pass_update_queue=True
            )
        )
    ...
    def test_multiple_block_deprecation(self, dp):
        assert isinstance(dp, Dispatcher)
        ...
        with pytest.warns(TelegramDeprecationWarning, match='@block decorator'):
            dp.process_update(self.message_update)
    ...
    def test_block_no_error_handler(self, dp, caplog):
        def func():
    ...
    def test_async_handler_error_handler(self, dp):
        dp.add_handler(MessageHandler(Filters.all, self.callback_raise_error, block=True))
        dp.add_error_handler(self.error_handler)
    ...
    def test_async_handler_async_error_handler_context(self, cdp):
        cdp.add_handler(MessageHandler(Filters.all, self.callback_raise_error, block=True))
        cdp.add_error_handler(self.error_handler_context, block=True)
    ...
    def test_async_handler_error_handler_that_raises_error(self, dp, caplog):
        ...
        dp.add_error_handler(self.error_handler_raise_error, block=False)
        ...
        dp.add_handler(MessageHandler(Filters.all, self.callback_increase_count, block=True))
        dp.update_queue.put(self.message_update)
        sleep(0.1)
        assert self.count == 1
    ...
    def test_async_handler_async_error_handler_that_raises_error(self, dp, caplog):
        handler = MessageHandler(Filters.all, self.callback_raise_error, block=True)
        dp.add_handler(handler)
        dp.add_error_handler(self.error_handler_raise_error, block=True)
    ...
    def test_update_persistence_all_async(self, monkeypatch, dp):
        ...
        dp.bot.defaults = Defaults(block=True)
        try:
            for group in range(5):
                dp.add_handler(MessageHandler(Filters.text, dummy_callback), group=group)
    ...
    def test_update_persistence_one_sync(self, monkeypatch, dp, block):
        def update_persistence(*args, **kwargs):
            self.count += 1
        ...
        dp.add_handler(MessageHandler(Filters.text, dummy_callback, block=block), group=5)
    ...
    def test_update_persistence_defaults_async(self, monkeypatch, dp, block, expected):
        def update_persistence(*args, **kwargs):
            self.count += 1
