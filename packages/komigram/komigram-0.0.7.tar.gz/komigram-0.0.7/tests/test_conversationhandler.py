...
class TestConversationHandler:
    ...
    def test_promise_exception(self, dp, bot, user1, caplog):
        """
        Here we make sure that when a block handle raises an
    ...
    def test_conversation_handler_block_true(self, dp):
    ...
    def test_conversation_handler_block_false(self, dp):
        conv_handler = ConversationHandler(
            entry_points=[CommandHandler('start', self.start_end, block=True)],
            states=self.states,
            fallbacks=self.fallbacks,
        ...
        for handler in all_handlers:
            assert not handler.block.value
