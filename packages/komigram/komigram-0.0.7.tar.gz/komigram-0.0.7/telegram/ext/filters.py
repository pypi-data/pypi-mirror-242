...
class _ChatUserBaseFilter(MessageFilter, ABC):
    ...
    def __init__(
        self,
        chat_id: Optional[SLT[int]] = None,
        username: Optional[SLT[str]] = None,
    ...
    @abstractmethod
    def _get_chat_or_user(self, message: Message) -> Union[Chat, User, None]:
        ...
...
class Chat(_ChatUserBaseFilter):
    ...
    def _get_chat_or_user(self, message: Message) -> Optional[Chat]:
        return message.chat
...
class ForwardedFrom(_ChatUserBaseFilter):
    ...
    def _get_chat_or_user(self, message: Message) -> Union[User, Chat, None]:
...
class Mention(MessageFilter):
    ...
    def __init__(self, mentions: SLT[Union[int, str, User]]):
    ...
    @staticmethod
    def _fix_mention_username(mention: Union[int, str, User]) -> Union[int, str, User]:
        if not isinstance(mention, str):
            return mention
        return mention.lstrip("@")

    @classmethod
    def _check_mention(cls, message: Message, mention: Union[int, str, User]) -> bool:
        if not message.entities:
            return False
...
class User(_ChatUserBaseFilter):
    ...
    def _get_chat_or_user(self, message: Message) -> Optional[User]:
