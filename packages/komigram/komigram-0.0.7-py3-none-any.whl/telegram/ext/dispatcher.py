...
def block(
    ...
    Will run :attr:`telegram.ext.Dispatcher.block`.

    Using this decorator is only possible when only a single Dispatcher exist in the system.

    Note:
        DEPRECATED. Use :attr:`telegram.ext.Dispatcher.block` directly instead or the
        :attr:`Handler.block` parameter.

    Warning:
        If you're using ``@block`` you cannot rely on adding custom attributes to
    ...
    def async_func(*args: object, **kwargs: object) -> object:
        warnings.warn(
            'The @block decorator is deprecated. Use the `block` parameter of '
            'your Handler or `Dispatcher.block` instead.',
...
class Dispatcher(Generic[CCT, UD, CD, BD]):
    ...
    Args:
        ...
        workers (:obj:`int`, optional): Number of maximum concurrent worker threads for the
            ``@block`` decorator and :meth:`block`. Defaults to 4.
    ...
    Attributes:
        ...
        job_queue (:class:`telegram.ext.JobQueue`): Optional. The :class:`telegram.ext.JobQueue`
            instance to pass onto handler callbacks.
        workers (:obj:`int`, optional): Number of maximum concurrent worker threads for the
            ``@block`` decorator and :meth:`block`.
    ...
    def __init__(
        ...
        # only one instance of Dispatcher, it will be possible to use the `block` decorator.
    ...
    def _pooled(self) -> None:
        ...
        while 1:
            ...
            if not isinstance(promise, Promise):
                self.logger.debug(
                    "Closing block thread %s/%d", thr_name, len(self.__async_threads)
                )
                break
            ...
            # should happen only via the deprecated `@block` decorator or `Promises` created
    ...
    def block(
        ...
        Warning:
            * If you're using ``@block``/:meth:`block` you cannot rely on adding custom
              attributes to :class:`telegram.ext.CallbackContext`. See its docs for more info.
            * Calling a function through :meth:`block` from within an error handler can lead to
    ...
    def _block(
        ...
        **kwargs: object,
        ...
        # TODO: Remove error_handling parameter once we drop the @block decorator
    ...
    def process_update(self, update: object) -> None:
        ...
        Note:
            ...
            ``block=False``), :meth:`update_persistence` is called *once* after all handlers
    ...
    def add_error_handler(
        ...
        Args:
            ...
            block (:obj:`bool`, optional): Whether this handlers callback should be run
                asynchronously using :meth:`block`. Defaults to :obj:`False`.
