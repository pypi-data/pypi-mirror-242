...
class RattailWuttaConfig(WuttaConfig, RattailConfigMixin):
    ...
    * :class:`RattailConfigMixin`
    ...
    .. attribute:: prioritized_files

       Property which just calls
    ...
    def parse_list(self, value):
        """
        Convenience method around the :func:`parse_list()` function.

        Usage of this method is discouraged, at least until some more
        dust settles.  Not sure if this really belongs on the app
        handler or the config object...
        """
...
class RattailConfig(RattailConfigMixin):
    ...
    .. note::
       ...
       being phased out in favor of :class:`RattailConfiguration`.
       The real goal is to start using :class:`RattailWuttaConfig` but
       that is not well-tested yet.
...
def make_config(
    ...
    .. note::
       ...
       We currently are juggling 3 different styles of config classes.
       In all cases this function will now return a
       :class:`RattailConfigWrapper` instance, and the underlying
       config class will be handled automatically by that wrapper.
       See its docs for more info on how to pick the config style.
