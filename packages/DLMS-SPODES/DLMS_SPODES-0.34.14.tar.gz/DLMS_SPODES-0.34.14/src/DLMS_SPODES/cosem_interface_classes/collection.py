...
class Collection:
    ...
    def is_init_type(self) -> bool:
        ...
        if type_obj := self.get_object("0.0.96.1.1.255") is None:
            return False
        elif (type_value := type_obj.value) is None:
            return False
        else:
            self.set_server_type(type_value)
        if (ver_obj := self.get_object("0.0.0.2.1.255")) and (ver := ver_obj.value):
            pass
        elif (ver_obj := self.get_object("0.0.96.1.2.255")) and (ver := ver_obj.value):
            pass
        else:
            return False
        self.set_server_ver(0, AppVersion.from_str(ver.to_str()))
        if (man_obj := self.get_object("0.0.42.0.0.255")) and (man := man_obj.value) and (len(man)) == 12:
            pass
        elif (man_obj := self.get_object("0.0.96.1.0.255")) and (man := man_obj.value) and (len(man)) >= 3:
            pass
        else:
            self.set_manufacturer(bytes(man)[:3])
            return True
