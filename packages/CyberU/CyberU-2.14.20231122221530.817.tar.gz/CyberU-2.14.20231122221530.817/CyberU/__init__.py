...
sys.path.append('D:/Kaleidoscope/')
Logcount = 0
...
def manage_args(**merged_keys):
    def decorator(func):
        ...
        @wraps(func)
        def wrapper(*a, **b):
            # 最后还是要通过 *a, **b 的形式传参直接调用被装饰的函数
            a = list(a)
            res = {}
            # if len(a) and not len(_a):# 调用处没有写键而定义处有 *args
            # if len(a) > len(l0):
            #     res_a = a[len(l0):]
            #     a = a[:len(l0)]
            # 这只是用来容错的？
            ...
            # 调用处没有传参的而定义处具有默认值的，应当
            # 加入 all_args 进行参数传递
            for k, v in default_values.items():
                if not k in b:
                    b.update({k: v})

            # 记录 all_args 到 **b 中进行参数传递
            # 一般认为， all_args 可以有多种表现形式
...
def has_value(interval=2):
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            # 检查 kwargs 中是否包含 not_null，且其值不为 None
            if kwargs.get('not_null') is not None or kwargs.get('notnull') is not None:
                while True:
                    result = func(*args, **kwargs)
                    if result not in [None, "null", "None", [], False]:
                        return result
                    else:
    ...
    def decorator(func):
        def wrapper(*a, **c):
            ...
            if type(index) in [int]:  # 元组
                ...
                if c in [None, (), [], {}] or not index in list(c):  # **c 为空，或者键不存在
                    return func(*a, **c)
                if not (type(c[index] in [list, types.GeneratorType])):  # index 位置不是可迭代对象
                    return func(*a, **c)
                res = []
...
def consume(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if elapsed_time > 1:
            delog(f' {func.__name__} ：{int(elapsed_time)} s')
        return result
...
def simplinfo(num, author, title, disk=None):
    if disk == None:
        disk = diskname
    return json.dumps({str(num): {'disk': disk, 'author': author, 'title': title}},
                      ensure_ascii=False)
...
class Time():
    def __init__(self, *a, **b):
        ...
        # init， 默认设置为现在时间
        reset([self])
        year, month, day, hour, min, sec, mic = now().year, now().month, now(
        ).day, now().hour, now().minute, now().second, now().microsecond
        if b == {}:
            if len(a) in [1]:
                i = a[0]
                if type(i) in [Time]:
...
def save_value(variable=None, filename=None):
    """保存变量到指定的文件中"""
    import dill
    path = add_ext(varpath(filename), '.pkl')
...
def load_value(filename):
    """从指定的文件中加载变量"""
    import pickle
    f = file(add_ext(varpath(filename), '.pkl'), 'rb')
    return pickle.load(f.f)


def try_to_execute(s, t=0, silent=None):
    """
    尝试执行一段代码，异常不停止
    @param s: code
    @return:
...
execute = try_to_execute
execute_py = try_to_execute


@manage_args(path=['module'])
def execute_module(path=None, root=None, pool=None,
                   b=None, **leak):
    """
    执行一个python文件或者把它加入进程池
    ...
    if pool:
        ...
        try:
            ...
            warn('需要主程序在if __name__ == "__main__":中运行 run_module 函数。')


run_module = execute_module
...
@manage_args()
def arg_mutex(group1=None, group2=None,
              b=None,**leak):
    """
    检查两组变量的互斥关系。当第一组中任一变量不为空时，第二组所有变量必须全为空，反之亦然。

    @param group1: 第一组变量的列表
    @param group2: 第二组变量的列表
    @return: 如果满足互斥条件，则返回 True；否则返回 False
    """
    used1= any(var not in [None, False, ''] for var in group1)
    used2 = any(var not in [None, False, ''] for var in group2)
    if used1:
        if used2:
...
def used_arg(var=None, lis=None, strict=None):
    """
        在很多场景，我们碰到当传参时如果不是默认值 None ，
        就要进行参数类型判断和值判断，
        以确保传参是正常值以避免问题扩大和转移
    @param var:
    @param lis:
    @return:  参数不为空且符合预期类型/值
    """
    if var in [None,'']:
        return False
    if lis is None:
    ...
    for item in lis:
        if isinstance(item, type) and isinstance(var, item):
            return True
        elif isinstance(var, str) and var in lis:
            return True
    if strict:
        Exit(f'变量 {var} （{var} ）不符合类型断言的要求')


check_arg_type = check_type = used_var=\
    used_arg
...
def dicttourl(url_dict):
    ...
    query_params = {k: url_dict[k] for k in url_dict if not k.startswith('_')}
...
def get_lock(name=None, interval=1, max_wait=10):
    """
    主机范围
    @param name: 锁名
    @param interval: 检查间隔
    @return:
    """
    name = standarlizedFileName(name)
    while not createfile(path=lockpath(f'{name}'), silent=False):
        sleep(interval)
        max_wait -= 1
        if max_wait < 0:
            warn(f'{name}锁超时了。删除全部锁。')
...
def release_lock(name=None):
    ...
    """
    name = standarlizedFileName(name)
    deletedirandfile(lockpath(f'{name}'))


releaseloc = release_lock
...
def size(a, sum=0, bit=True, mode=None):
    """
...
# 文件系统读写
# region
def chdir(path=None, exist=True, create=False):
    """
...
def rmextension(fname):
    return extensionandname(fname)[0]
...
def look(path):
    ...
    # 自定义数据文件
    if type(path) in [pcsv, Csv, txt, rtxt, rjson, cache]:
        try_to_execute("""look(parentpath(path.path))""")
        return
...
def browser_path(s=''):
    return get_base_path(projectpath('browser/'), s)
...
def src_path(s=''):
    return get_base_path(projectpath('src/'), s)
...
def collectionpath(s=''):
    return get_base_path(selfpath('collections&files/'), s)


# 网页爬虫目录
def shotspath(s=''):
    return get_base_path(selfpath('collections&files//网页集锦'), s)
...
def jspath(s=''):
    if not '.js' in s:
        s += '.js'
    return get_base_path(codepath('js'), s)


# 可执行文件脚本目录
def exec_path(s=''):
    return get_base_path(projectpath('executable'), s)
...
def varpath(s=''):
    ...
    @return:
    """
    return get_base_path(projectpath('cache/var'), s)
...
def records_path(s=''):
    return get_base_path(projectpath('records'), s)


recordpath = records_path
record_path = records_path


def download_path(s=''):
    return get_base_path(cachepath('download'), s)
...
def lockpath(s=''):
    return get_base_path(cachepath('lock'), s)
...
def picpath(s=''):
    if not '.' in s[-5:]:
        s = s + '.png'
    return get_base_path(projectpath('pic'), s)
...
# 八爪鱼目录
def octopus_path(s=''):
    return get_base_path(projectpath('octopus'), s)
...
@manage_args(s1=['src','source','ori','original'],s2=['tar','des','target','desti','destination'])
...
class pcsv():
    ...
    def __init__(self, path, title=True, duplicate=False, ftype='csv', encoding='utf-8',
                 backup=True, silent=True, frequency=24,
                 dtype=None):
        """
    ...
    def backup(self, force=False):
        ...
        if not isfile(backuppath):
            ...
            return
        if force or createtime(self.path) > createtime(backuppath) + 3600 * self.frequency:
            copyfile(self.path, backuppath)
    ...
    def set(self, save=True):
        ...
        if hasattr(self, 'data'):
            ...
            self.data = self.data.drop_duplicates(subset=cols)
        if save:
            self.save()
    ...
    @consume
    def add(self, l, duplicate=None, save=True, isnew=True):
        """
        并行
        需要确保传入的值为纯数字，否则无法去重
        @param l:
    ...
    def rows(self, typ=list):
        ...
        """
        if typ == list:
            return self.get_data().values.tolist()
        if typ == dict:
            return self.get_data().to_dict(orient='records')
...
@listed()
def deletedirandfile(l, silent=True, rt=True):
    """
    删除文件和文件夹
    @param l: 可以是txt路径
    @param silent:
    ...
    import shutil
    # 删除txt里的文件
    if isfile(l) and l[-4:] in '.txt' and rt:
        f = txt(l)
        dlis = []
        for i in f.l:
            if i in ['\n', '']:
...
def createfile(path=None, encoding=None, silent=None):
    path = standarlizedPath(path)
    root = pathname(path)
    createpath(path)
    ...
    def add(self, i, silent=None):
        ...
        for k in i.split('\n'):
            ...
            if not self.l == []:
                ...
                file_operate('a', self.path, [k], encoding='utf-8')
            self.l.append(str(k))
            if not silent:
                delog(f'txt add {k}')
...
class RefreshTXT(txt):
    ...
    def get(self, silent=None):
        ...
        if len(self.l) < 1:
            return None
        self.l = Set(
            self.l + rtxt(self.path, encoding=self.encoding, silent=self.silent).l)
...
class cache(txt):
    ...
    def __init__(self, path, silent=None, json=True):
        super().__init__(path=path, silent=silent)
        self.silent = silent
        self.path = path
        self.json = json
        self.encoding = 'utf-8'
    ...
    def get(self, silent=False, interval=2):
        """
        删除。如果失误直接删除
        @param silent:
        @return:首条。
...
@manage_args(content=['message'], total_time=['t', 'duration'])
def desktop_inform(title='未命名', content='无内容', total_time=3,
                   b=None, **leak):
    from win10toast import ToastNotifier
    notifier = ToastNotifier()
    notifier.show_toast(title, content,
                        duration=total_time,
                        # icon_path=picpath('icon.ico'),
                        )


inform = notify = message = \
    desktop_inform
...
def useDisk(dname=None, d=None,
    ...
    def _use(d):
        ...
        if sepration:
            d += '/autom/'
        chdir(d, exist=False, create=True)
...
get_workspace = disk_path = diskpath = \
    os.getcwd
root = getDiskPath = \
    get_disk
...
def send_rpc(page=None,
    ...
    data = {
        ...
        "param": json.dumps(merge({"funcName": funcName}, param))
    }
...
def Elements(root=None, xpath=None, depth=7, silent=True,
    ...
    while True:
        ...
        if len(ret):
            if atr == None:  # 不是要属性要返回 element，直接返回
                return [obj.__setattr__(method, s) or obj for obj in ret]
            ...
            if atr not in ['text']:  # text  以外的属性
                ...
                try:
                    l = [i.text if i.text else i.get_attribute(
                        'text') for i in ret]
                    if strict:
            ...
            if depth >= 10:
                ...
                return Elements(
                    original_s, depth=depth + 1,
                    **(exclude(b, ['original_s', 'depth'])),
                )
...
def skip(l, s, method=By.XPATH, strict=False, t=99999):
    """
    简单等待，不做操作，等待人工操作
    t秒后或者元素消失
    :param l:页面
    ...
    if Element(l, s, depth=8, silent=True, method=method, strict=strict):
        warn(f'{s} detected. 等待({t})其消失中以继续。。。', traceback=False)
        alertpage([page])
        try:
...
@manage_args()
def requestdownload(path=None, url=None,
                    mode='wb', header={}, t=None,
                    b=None, **leak):
    """
...
def _browser(url='', mine=None,
    ...
    if manual_manage_webdriver:
        ...
        # selenium-pypi 官方地址 https://pypi.org/project/selenium/
        # webdriver downloads 在下面列出
        # 其中 chromecriver 地址 https://googlechromelabs.github.io/chrome-for-testing/known-good-versions-with-downloads.json
...
class Browser():
    ...
    def __init__(self, url=None, download_path=default_page_download_path,
                 silent=False, driver=None, mine=False, family=None,
                 mute=False,
                 method=None, _type=None,
                 b=None, **leak):
        """
        ...
        if not driver == None and type(driver) in [list]:
            self.driver = driver[0]
            return
        else:
            if method in ['js']:
                init_backend()
                if self.test_connect(depth=9):
                    self.driver = None
                    return
    ...
    def __del__(self):
        ...
        @return:
        """
        try_to_execute('self.quit()', silent=True)
    ...
    def forum(self, url=None, titletail=None, hostname=None,
        ...
        if redownload:
            ...
            pastcount = ''
        if saveuid:
            page.save(path=shotspath(f'{hostname}/{uid}_{title}{pastcount}/第1页/'), minsize=minsize,
    ...
    @manage_args(xpath=['s'])
    def click(self, xpath=None, strict=True, depth=9,
              b=None, **leak
              ) -> bool:
        """
        点击元素或是xpath
        ...
        if type(xpath) in [str]:
            if self.is_js():
                return self.send_rpc(f='click', p={'xpath': xpath})
    ...
    def skip(self, s, strict=False, click=False, t=200):
        """
        跳过元素
        @param s: xpath字符串
...
da      @return:
        ...
        if not click:
            return skip([self.driver], s, strict=strict, t=t)
        e = self.element(s, strict=False)
        if not e == None:
            self.click(e)

    def executejs(self, js=None, path=None):
        if not js == None:
            try:
    ...
    def nearend(self):
        ...
        """
        if self.is_js():
            return self.send_rpc(f='nearEnd')
    ...
    @manage_args()
    def Down(self, start=0, end=None,
...
if (element) {
    ...
    def fullscreen(self, path=None, scale=100, autodown=True, pause=1, clip=True, clipinterval=0.6,
                   cuttop=0, cutbottom=0, cutleft=0, cutright=0, maxheight=40000, _look=False,
                   overwrite=None):
        """
        ...
        if clip:
            ...
            buf = 60
            clipsize = self.Height() - Int(cuttop) - Int(
                cutbottom) - buf
            clipcount = 0
            while True:
                self.scroll(
    ...
    def save(self,
        ...
        return self._savepage(**(rmkey(b, 'self')))

    @useState
    def _savepage(self, path=None, video=False, minsize=(100, 100),
                  t=3, titletail=None, scale=100,
                  direct=False, look=None,
                  clicktoextend=None, autodown=True, check=False,
                  duplication=False, extrafunc=None,
                  pause=1, windowsize=None,
                  overwrite=True, redownload=True, savevideo=False, clip=True,
                  cuttop=0, cutbottom=0, cutleft=0, cutright=0, clipinterval=2,
                  top=None, bottom=None, left=None, right=None, maxheight=40000,
                  ):
        """
        保存整个网页，自定义参数
        ...
        if self.type == 'edge' and not self.silent:
            ...
            self.fullscreen(f'{path}/basic.png', scale=scale, autodown=autodown, pause=pause,
                            ...
                            clipinterval=clipinterval, maxheight=maxheight)
    ...
    @manage_args(xpath=['s'])
    def elements(self, xpath=None, depth=9,
                 silent=True, strict=True, root=None,
                 refresh=False, complete=True,
                 interval=1,
                 b=None, **leak
                 ) -> list:
        """
    ...
    @manage_args()
    def scroll(self, a=-1, xpath=None,
               e=None, h=None,
               b=None, **leak):
        """
        下滚或者滚动到元素
    ...
    def send_rpc(self, *a, depth=6,
        ...
        try:
            if depth > 10:
                warn(' send_rpc 失败。退出。 ', traceback=False)
                if not test:
                    Exit()
                return
            if not test and depth == 6 and probability((3,10)):
                self.test_connect(depth=-1)
            ret = send_rpc(page=self,
                           time_out=interval / (11 - depth),
                           *a, **(exclude(b, 'page')))
    ...
    def get(self, url=None, t=None,
            resethight=False, pre_wait=3, interval=3):
        """
    ...
    def title(self,b=None,**leak):
        ...
        if self.url() == '':
            Exit('尚未打开任何网页')
        try:
            ret = self.executejs('return document.title')
            if not ret in ['', None]:
    ...
    def screenshot(self, path=None,
                   ...
                   overwrite=True, pre_wait=True):
        """
        截图并保存。不下滚。
        @return:
        """
        if pre_wait:
            sleep(self.getscrollheight() / 1000 / download_speed)
        if path == None:
...
@consume
def download(url=None, path=None, t=999, silent=True, depth=0,
                  auto=None, redownload=None,
                  overwrite=False, page=None,
                  download_root=None,method=None,
                  b=None, **leak,
                  ):
    """
    ...
    # 参数处理
    #region
    if type(url) in [WebElement]:
        warn(f'传入了 WebElement 作为 url 下载。 xpath 为{url.xpath}', traceback=True)
        exit(-1)
    if type(url) in [selenium.webdriver.remote.webelement.WebElement]:
        warn(f'传入了 selenium WebElement 作为 url 下载。', traceback=True)
        exit(-1)
    from selenium import webdriver
    if t == 0:
        t = 15
    ...
    original_download_name = download_name
    previous_content = listfile(download_root)
    #endregion
    ...
    def _check_and_finish() -> bool:
        ...
        download_success = monitor_file_size(path=f'{download_root}/{download_name}.crdownload',
                                             max_time=t, silent=True, **exclude(b,[
                'silent','path','max_time','t'
            ]))
        ...
        for ii in listfile(download_root):
            ii = filename(ii)
            if download_name in ii and '.crdownload' in ii:
                return warn_and_download_again(s=f'{Int(t)} s后仍在下载。没有缓存文件存留（自动删除）  {url}',
                                               d=ii)
            if download_name == ii or '.' in ii and download_name == rmtail(ii, '.'):
                if '.htm' in ii:
                    return warn_and_download_again('下载了  .htm？？？',
                                                   d=ii)
                _targetname = targetname
                if not '.' in targetname and '.' in ii:  # 传参没有传扩展名但是浏览器下载自动加了
                    _targetname = targetname + \
                                  extension(ii, full=False, withdot=True)
                move(download_root + '/' + ii, targetroot + '/' + _targetname,
    ...
    try:
        ...
        if 'default_download_page' in globals() and not page == default_download_page:
            ...
            page.get(url)
        if tellstringsame(page.title(depth=10), '403 forbidden'):
        ...
        if type(e) in [ZeroDivisionError, ]:
            ...
            page.quit()
            return download(url, path, t=t, depth=depth + 1, silent=silent, auto=auto,
    ...
    sleep(1)  # 等待内容加载
    # delog('begin to download')
    i = 0
    while i < 10:
        # 什么？？？竟然要尝试10次，哈哈哈真是笑死我了
        # 提示： js 下载的核心思路是， get 到内容所在页面，再 createElement 下载到 default_path 。
        try:
    ...
    # 检查
    # region
    download_name = head(s=get_full_filename(download_root, download_name, recursive=True,
                                      loop=int(t / 2), silent=True),mark='.crdownload',strict=False)
    if not download_name:
        warn(f'js执行下载后查找失败，没有检测到下载文件\
         {Str(download_root) + Str(original_download_name)}\n 下载地址： {url}')
    return _check_and_finish()
    # endregion


@manage_args(page=['driver'], path=['target'])
def page_download(url=None, path=None, t=999, silent=True, depth=0,
                  auto=None, redownload=None,
                  overwrite=False, page=None,
                  download_root=None,
                  b=None, **leak,
    ...
    @return:
    """
    return download(method='page',**exclude(b,'method'))
...
download_list = Csv(projectpath('cache/download_list.csv'))
try:
