...
try:
    ...
    PROXY_CACHE_CLS: type[MutableMapping] = WeakValueDictionary
except ModuleNotFoundError:
    from xml.etree.ElementTree import SubElement, Element, ElementTree # type: ignore
    PROXY_CACHE_CLS = WeakKeyDictionary

from .helper import extract_name, items
from .undefined import undefined
from .xml import el_add, el_del, el_iterfind, el_set, resolve_prefix
...
class ElementProxy(ElementAttribProxy):
    ...
    def set(self, path=None, /, name=None, attrib=None, text=None, tail=None, merge=False):
        if path and name is None:
            name = extract_name(path)
        el = el_set(self._root, path, name=name, attrib=attrib, text=text, tail=tail, namespaces=NAMESPACES, merge=merge)
        if el is not None:
    ...
    def setfind(
        ...
        if find_text is undefined and find_tail is undefined:
            ...
            for el in self.iterfind(preds):
                if (find_text is undefined or el.text == find_text) and (find_tail is undefined or el.tail == find_tail):
                    break
            else:
                el = None
        if delete:
            if el is not None:
                self._root.remove(el._root)
        elif el is None:
            if auto_add:
                if find_attrib:
                    if attrib:
                        ...
                        attrib = find_attrib
                el = self.add(name, attrib=attrib, text=text, tail=tail)
        else:
            el_set(el, attrib=attrib, text=text, tail=tail, namespaces=NAMESPACES, merge=merge)
        return el
