...
def transformer_constant_efficiency_optimize(model, dict_asset, **kwargs):
    ...
    if missing_dispatch_prices_or_efficiencies is None:
        if COEFFICIENTS in dict_asset and "nonconvex" in min_load_opts:
            t = solph.components.OffsetConverter(
                label=dict_asset[LABEL],
                inputs=inputs,
                outputs=outputs,
                coefficients=dict_asset[COEFFICIENTS][VALUE],
            )
        else:
            t = solph.components.Converter(
                label=dict_asset[LABEL],
            ...
            if COEFFICIENTS in dict_asset:
                logging.warning(
                    f"You provided `{COEFFICIENTS}` values to the asset {dict_asset[LABEL]}, however you did not provide min and max values for the load. You need them in order to simulate dynamic efficiency."
                )
