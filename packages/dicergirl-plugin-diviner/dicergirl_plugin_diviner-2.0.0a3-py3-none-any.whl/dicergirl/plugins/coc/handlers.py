...
async def coc_handler(matcher: Matcher, event: Event):
    """COC 车卡指令"""
...
async def ticommandhandler(matcher: Matcher, event: Event):
    """COC 临时疯狂检定指令"""
    if not get_status(event) and not event.to_me:
        return
...
async def licommandhandler(matcher: Matcher, event: Event):
    """COC 总结疯狂检定指令"""
    if not get_status(event) and not event.to_me:
        return
...
async def sccommandhandler(matcher: Matcher, event: Event):
    """COC 疯狂检定指令"""
    if not get_status(event) and not event.to_me:
        return
