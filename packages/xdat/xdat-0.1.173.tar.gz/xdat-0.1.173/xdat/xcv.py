...
def process_video(video_in_path, video_out_path=None, frames_folder=None, plot_idx=None, max_frames=None, quality=90, pfunc=None, every=1, out_every=1):
    ...
    for idx in tqdm(range(frame_count)):
        ...
        if should_process and pfunc:
            last_image = pfunc(image, frame=idx)

        image = last_image

        if out and idx % out_every == 0:
            out.write(image)
