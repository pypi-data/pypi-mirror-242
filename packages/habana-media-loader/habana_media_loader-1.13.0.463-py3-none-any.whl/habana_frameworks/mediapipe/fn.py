    ...
    class operator():
        ...
        def __init__(self, **kwargs):
            ...
            if ("device" in kwargs.keys()):
                ...
                del kwargs["device"]
            self.schema = s.get_operator_schema(
...
def complex_operator_add(name: str):
    """
    Method to  add complex operator to fn.
    """
    class operator():
        """
        Class defining media operator.
        ...
        def __init__(self, **kwargs):
            ...
            if ("device" in kwargs.keys()):
                self.device = kwargs["device"]
                del kwargs["device"]
            self.schema = cs.get_operator_schema(
                type(self).__name__)
            self.num_outputs = self.schema.getNumOutputs()
            node_output_attr = node_output_attributes.copy()
            node_output_attr["outputType"] = self.schema.getDtype()
...
for op in operators:
    op_class = complex_operator_add(op)
    op_class.__module__ = module.__name__
    setattr(module, op, op_class)
