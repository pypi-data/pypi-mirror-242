...
class legacy:
    ...
    def run_hpu(self, pipeManager, input_list, input_type, rand_crp, is_crp_after_resize, vid_offset, host_buffer, bypass_host_buffers, out_dev_buffer_list):
        """
        ...
        if not (rand_crp is None):
            ...
            if (is_crp_after_resize == True) and (rand_crp.dtype == np.float32):
                raise RuntimeError(
