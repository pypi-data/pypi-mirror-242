...
class Operator(object):
    ...
    def check_existing_attributes(self, guid,
                                  params):
        if(guid != None and self.__guid != None):
            if(guid != self.__guid):
                raise ValueError(
                    "guid mismatch with existing op ", self.__name)
        if(params != self.__params):
            if(len(params.keys()) < len(self.__params.keys())):
                if(params.items() > self.__params.items()):
                    raise ValueError(
                        "params mismatch with existing op ", self.__name, self.__params, params)
...
class complex_operator_schema(operator_schema):
    ...
    def add_operator(self,
        ...
        """
        if name in self._ops_list:
            raise ValueError("Duplicate complex op being added ", name)
        else:
            self._ops_list.append(name)
        op = Operator(name, None, min_inputs, max_inputs,
                      input_keys, num_outputs, params, None, op_class, dtype)
        self._op_to_obj[name] = op
    ...
    def get_operator_schema(self, operator):
        ...
        if(operator in self._op_to_obj.keys()):
            ...
            raise RuntimeError(operator, " not implemented in complex op")
    ...
    def add_operator_cpu(self,
        ...
        :params dtype: defaults output dtypes of the operator.
        """
        raise RuntimeError("complex op doesnt respect seperate cpu ops")


schema = operator_schema()
complex_schema = complex_operator_schema()
