...
class Deserialization(BaseRule):
    ...
    def infilter(self, match_string, **kwargs):
        ...
        return super().infilter(match_string, **kwargs)
    ...
    def report_attack_without_finding(self, value, **kwargs):
        ...
        attack.set_response(ProtectResponse.BLOCKED)

        self._append_to_context(attack)

        raise contrast.SecurityException(self.name)
    ...
    def build_evaluation(self, value):
        ...
        """
        ia_result = InputAnalysisResult(None, None, None, None)
        ia_result.key = self.RULE_NAME
        ia_result.rule_id = self.RULE_NAME
        ia_result.input_type = InputType.UNKNOWN
