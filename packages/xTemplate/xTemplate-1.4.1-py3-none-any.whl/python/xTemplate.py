...
class DatabaseConnector:
    ...
    def connectPostgreSQL2(self,db,host,user,pw,port,schemas):
        try:
            # PostgreSQLConfigクラスを使用してデータベースに接続
    ...
    def connectMySQL(self,db):
        try:
            # PostgreSQLConfigクラスを使用してデータベースに接続
            connection = MySQLConfig(db, self.host, self.user, self.pw)
            return connection
        except (Exception, Error) as error:
            # データベースへの接続中にエラーが発生した場合はRuntimeErrorを発生させる
            raise RuntimeError(f"データベースへの接続エラー: {error}")
...
class EmailNotifier:
    ...
    def send_email2(self,subject,header,body,footer,recipients,recipients_cc,sender):
        try:
            # Email details
            if self.smtp_server != None:
                smtp_server = self.smtp_server
...
class Template:
    def __init__(self,config_file):
        ...
        if self.log:                                    # autoreset=Trueを設定して、Coloramaのテキストリセットを有効にします。
            ...
            logging.basicConfig(filename='record.log', level=logging.INFO, encoding="utf-8")
    ...
    def db_fetchall2(self,db,host,user,pw,port,schemas,query):
        try:
            # データベースへの接続
    ...
    def db_commit2(self,db,host,user,pw,port,schemas,query):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectPostgreSQL2(db,host,user,pw,port,schemas)
            # クエリの実行
    ...
    def db_commit_values2(self,db,host,user,pw,port,schemas,query,values):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectPostgreSQL2(db,host,user,pw,port,schemas)
            # クエリの実行
    ...
    def db_commit_many2(self,db,host,user,pw,port,schemas, query, values):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectPostgreSQL2(db,host,user,pw,port,schemas)
            # クエリの実行
    ...
    def mySQL_fetchall(self,db,query):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectMySQL(db)
            # クエリの実行
            return db_connection.fetchall(query)
    ...
    def mySQL_commit(self,db,query):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectMySQL(db)
            # クエリの実行
    ...
    def mySQL_commit_values(self,db,query,values):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectMySQL(db)
            # クエリの実行
    ...
    def mySQL_commit_many(self, db,query, values):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectMySQL(db)
            # クエリの実行
    ...
    def create_button(self, frame, text, command, style, image_key, row, column, rowspan=1, padx=(10, 0), pady=(1, 1), sticky="n"):
        ...
        button.config(image=self.image_references[image_key], compound=tk.LEFT)
        return button