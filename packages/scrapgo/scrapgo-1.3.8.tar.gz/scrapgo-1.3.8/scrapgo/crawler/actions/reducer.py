...
from scrapgo.lib import select_kwargs, filter_params, find_function, curl2requests, assingles, pluralize
...
class ReduceMixin:
    ...
    def dispatch_parser(self, action, response, extracted, soup, prev_results, results_cache, context):
        ...
        # if not isinstance(results, abc.Generator):
        #     if results is None:
        #         results = []
            
        #     if isinstance(results, tuple):
        #         if len(results) != 2:
        #             raise ParsedResultsError('Parsed results consist of two parts: results and context.')
        #         results, ctx = results
        #         context.update(ctx)
        ...
        #         if len(result) != 2:
        #             raise ParsedResultsError('Parsed results consist of two parts: results and context.')
        #         result, sub_context = result
        #         if not isinstance(context, abc.Mapping):
        #             raise ParsedResultsError('context must be a dictionary object')
    ...
    def dispatch_urlfilter(self, action, url, responsemap, prev_results, context):
        if not hasattr(action, 'urlfilter'):
            return True

        return self.dispatch(
            'urlfilter', action.urlfilter,
            url=url, responsemap=responsemap, prev=prev_results, context=context, **context
        )
