"""This module contains user-friendly datatypes for Ogmios objects.
    ...
    schema files. These types are useful for validating that data passed to and from the Ogmios
    server is properly formatted. However, the schema types are not setup for Pythonic use to
    work with. Therefore, we use intermediate datatypes for user-facing functions of this library.

    .. seealso::
        `cardano.json schema <https://ogmios.dev/cardano.json>`_
        `ogmios.json schema <https://ogmios.dev/ogmios.json>`_
...
class Utxo:
    ...
    def __init__(
        ...
        self._schematype = om.Utxo(
            __root__=[
                om.UtxoItem(
                    transaction=om.Transaction4(id=self.tx_id),
                    index=self.index,
                    address=om.Address(__root__=self.address),
                    value=om.Value(ada=self.value.get("ada")),
...
class GenesisConfiguration:
    """A class representing the genesis configuration of the blockchain.

    :param era: The era of the genesis configuration.
    :type era: Era
    :param genesis_key_hashes: The genesis key hashes.
    :type genesis_key_hashes: list
    :param genesis_delegations: The genesis delegations.
    :type genesis_delegations: list
    :param start_time: The start time of the blockchain.
    :type start_time: datetime
    :param initial_funds: The initial funds of the blockchain.
    :type initial_funds: list
    :param initial_vouchers: The initial vouchers of the blockchain.
    :type initial_vouchers: list
    :param security_parameter: The security parameter of the blockchain.
    :type security_parameter: int
    :param network_magic: The network magic of the blockchain.
    :type network_magic: int
    :param updatable_parameters: The updatable parameters of the blockchain.
    :type updatable_parameters: ProtocolParameters
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if era == Era.byron.value:
            ...
            self.era = era
            self.initial_parameters = InitialParameters(**kwargs.get("initial_parameters"))
            self._schematype = om.GenesisAlonzo(
                era=self.era, initialParameters=self.initial_parameters._schema_type
            )
...
class Lovelace:
    """A class representing an amount of lovelace.

    :param lovelace: The amount of lovelace.
    :type lovelace: int
    """

    def __init__(self, lovelace: int):
        self.lovelace = lovelace
        self._schematype = om.Lovelace(lovelace=self.lovelace)

    def __eq__(self, other):
        if isinstance(other, Lovelace):
            if self.lovelace == other.lovelace:
                return True
        return False

    def __str__(self):
        return f"{self.lovelace:,} lovelace"
...
class ProtocolParameters:
    ...
    :type minStakePoolCost: Lovelace
    ...
    def __init__(
        ...
        minFeeConstant: int | Lovelace,
        minUtxoDepositCoefficient: int,
        minUtxoDepositConstant: int | Lovelace,
        maxBlockBodySize: dict,
        maxBlockHeaderSize: dict,
        stakeCredentialDeposit: dict,
        stakePoolDeposit: int | Lovelace,
        stakePoolRetirementEpochBound: int,
        stakePoolPledgeInfluence: str,
        minStakePoolCost: int | Lovelace,
        ...
        self.min_fee_coefficient = minFeeCoefficient
        self.min_fee_constant = self._to_lovelace(minFeeConstant)
        self.min_utxo_deposit_coefficient = minUtxoDepositCoefficient
        self.min_utxo_deposit_constant = self._to_lovelace(minUtxoDepositConstant)
        self.max_block_body_size = maxBlockBodySize
        self.max_block_header_size = maxBlockHeaderSize
        self.max_transaction_size = maxTransactionSize
        self.max_value_size = maxValueSize
        self.extra_entropy = extraEntropy
        self.stake_credential_deposit = stakeCredentialDeposit
        self.stake_pool_deposit = self._to_lovelace(stakePoolDeposit)
        ...
        self.min_stake_pool_cost = self._to_lovelace(minStakePoolCost)
        ...
        self._schema_type = om.ProtocolParameters(
            ...
            stakeCredentialDeposit=self.stake_credential_deposit,
    ...
    def _to_lovelace(self, value: int | dict | Lovelace) -> Lovelace:
        match value:
            case Lovelace():
                return value
            case dict():
                return Lovelace(value.get("lovelace"))
            case int():
...
class InitialParameters:
    """A class representing the initial parameters of the blockchain.
    ...
    def __init__(
        ...
        self.max_value_size = maxValueSize
        self.script_execution_prices = scriptExecutionPrices
        self._schema_type = om.InitialParameters(
            minUtxoDepositCoefficient=self.min_utxo_deposit_coefficient,
            collateralPercentage=self.collateral_percentage,
            plutusCostModels=self.plutus_cost_models,
            maxCollateralInputs=self.max_collateral_inputs,
...
class GovernanceProtocolParameters:
    ...
    :type governanceActionDeposit: Lovelace
    :param delegateRepresentativeDeposit: The delegate representative deposit.
    :type delegateRepresentativeDeposit: Lovelace
