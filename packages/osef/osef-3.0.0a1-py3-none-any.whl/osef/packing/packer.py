...
def pack(frame: dict) -> bytes:
    ...
    """
    tree = _pack_to_tree(
        _types.OsefTypes.TIMESTAMPED_DATA, frame[OsefKeys.TIMESTAMPED_DATA.value]
    )
...
def _pack_value(value: Any, node_packer: _packers.PackerBase) -> bytes:
    ...
    try:
        ...
        osef_logger.error(
            f"Exception occurred while packing value for {node_packer}:\n"
