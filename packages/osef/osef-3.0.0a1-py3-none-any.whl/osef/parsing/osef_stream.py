...
def create_osef_stream(
    ...
    Instantiate an OsefStreamTcp is given path is tcp, else an OsefStreamFile
...
class OsefStreamTcp(OsefStream):
    ...
    def __init__(self, path: Union[str, pathlib.Path], auto_reconnect: bool = True):
        """
        Constructor
        :param path: TCP stream with form *tcp://hostname:port*
    ...
    def read(self, size: int = 4096) -> bytes:
        ...
        :raise OsefConnectionException: If no TCP socket.
    ...
    def connect(self) -> None:
        ...
        : raise OsefConnectionException if the connection failed.
        """
        try:
...
class OsefStreamFile(OsefStream):
    ...
    def connect(self) -> None:
        ...
        : raise OsefConnectionException if the file could not be opened.
        """
        try:
    ...
    def read(self, size: int = 4096) -> bytes:
        ...
        :param size: of binary value to be read
        :raise EOFError: if no value can be read or if it is empty.
        :raise OsefConnectionException: IOStream not opened.
        :return: Read binary value
        """
