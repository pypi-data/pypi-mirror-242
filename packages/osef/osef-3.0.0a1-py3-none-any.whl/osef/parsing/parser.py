...
def parse(
    ...
    def get_timestamp_ms(cur_frame_dict: dict) -> int:
        return cur_frame_dict[OsefKeys.TIMESTAMPED_DATA.value][
    ...
    while True:  # To reopen stream when Tlv is malformed (in auto reconnect mode)
        with create_osef_stream(path, auto_reconnect) as osef_stream:
            ...
            for _, tlv in iterator:
                ...
                if real_frequency and not isinstance(osef_stream, OsefStreamTcp):
                    _sleep(
                        first_frame_counter,
...
def parse_next_frame(
    ...
    :raise OsefParsingException if the next frame could not be parsed.
    :return: next tlv dictionary
