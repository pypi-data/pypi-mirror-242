...
class BluetoothClient:
    ...
    def rfcomm_port(self):
        stdoutdata = sp.getoutput("rfcomm")

        pattern = re.compile(r"rfcomm\d+: (\S+) channel (\d+) connected \[tty-attached]")

        # Split the output into lines and search for the target MAC address
        for line in stdoutdata.split('\n'):
            match = pattern.search(line)
            if match:
                mac_address, channel = match.groups()
    ...
    def bind_rfcomm(self):
        for port_number in range(0, 30):
            stdoutdata = sp.getoutput(f"sudo rfcomm bind dev/rcomm{port_number} {self.address}")
    ...
    def release_rfcomm(self):
        if self.rfcomm_port() != -1:
            stdoutdata = sp.getoutput(f"sudo rfcomm release dev/rcomm{self.rfcomm_port()}")
