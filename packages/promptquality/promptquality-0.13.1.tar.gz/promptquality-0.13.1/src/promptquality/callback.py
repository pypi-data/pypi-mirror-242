...
class GalileoPromptCallback(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        self.chain_inputs[run_id] = inputs
        ...
        for root_id in self.root_nodes:
            for node_id in self.root_child_mapping[root_id]:
                ...
                if row is None:
                    ...
                    continue
                # If the node is not a parent node, include it.
                if node_id not in self.parent_child_mapping:
                    rows_to_log.append(row)
                # If the node is a chain node, only include it if it has atleast 1 child.
                elif len(self.parent_child_mapping[node_id]) > 0:
                    rows_to_log.append(row)
                else:
