...
*
bind c
DS9CADET CADET -x $xpa_method  $param(CADET); -dec $decompose -shift $shift -th1 $threshold1 -th2 $threshold2 -sf $sf | $text
#DS9CADET CADET -x $xpa_method  $param(CADET); -dec $decompose -shift $shift -b $bootstrap -n $boot_n -th1 $threshold1 -th2 $threshold2 -sf $sf | $text
...
threshold1 entry {Threshold1} 0.4 {Volume calibrating threshold (float between 0 and 1).}
threshold2 entry {Threshold2} 0.6 {TP/FP calibrating threshold (float between 0 and 1).}
sf checkbox {Save files} 0 {Save raw CADET prediction and decomposed cavities.}
endparam
"""
