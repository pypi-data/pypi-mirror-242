...
class GoldPriceModel(object):
    ...
    def load_data(self, fred_api_key, num_months=60):
        ...
        self.current_price = gold.Close.values[-1]

        # load cpi and treasury yield
        fred = FredData(api_key='147c83da741e258846d44af26794a872')
        cpi = fred.CPI().rename('CPI')
        treasury = fred.Treasury10Y().rename('Treasury')
    ...
    def fit(self):
        ...
        X, y = self.df[['CPI', 'Treasury10Y']], self.df['Close']
    ...
    def display(self, return_figures=False):
        ...
        self.df['Pred'] = pred
        self.df['Pred'] = self.df.Pred.shift(1)
        fig2 = px.line(self.df.dropna(), y=['Pred', 'Close'])
