...
class FlowFactory():
    ...
    def create_udp_flow(
        ...
        name = flow_config.pop("name", "Unnamed flow")
        ...
        if l4s_analysis is None:
            ...
            raise InvalidInput(
                f"Flow {name!r}: L4S analysis must be either 'marking' or 'congestion'."
            )
        ...
        logging.info(
            "Created flow %s from %s to %s", name, source.name,
            destination.name
        )
    ...
    def create_conference_flows(
        ...
        #              conference_flow_config)
        name = conference_flow_config.pop("name", "Video_Conference")
        ...
        for flow_config, flow_name_suffix in \
                (conference_video_flow_config, f'{name}_Video'), \
                (conference_voice_flow_config, f'{name}_Audio'), \
                (conference_screenshare_flow_config, f'{name}_Screenshare'):
    ...
    def create_gaming_flow(
        ...
        if enable_latency:
            ...
            logging.info("Flow %r: Latency analyser created", name)
        else:
            flow.add_analyser(
                FrameLossAnalyser(
                    layer2_speed=layer2_speed, **gaming_analysis
            ...
            logging.info("Flow %r: Frame Loss analyser created", name)
...
def initialize_flows(
    ...
    if flow_type.lower() == "frame_blasting":
        ...
        raise ValueError(f'Unsupported Flow type: {flow_type!r}')
    return flows
