...
def run(
    ...
    port_flow_set_map = _initialize_ports(server, port_configs)
    # create and run scouting flows
    if enable_scouting_flows:
        for flow in flow_configurations:
            _run_scouting_flows(flow, port_flow_set_map)
    ...
    _initialize_flows(
        scenario, flow_configurations, layer2_speed, port_flow_set_map
    )
...
def _initialize_ports(
    ...
    :return: Mapping of initialized port to its *Flow Set*.
    ...
    port_flow_set_map: _PortGroupMap = defaultdict(list)
    for port_config in port_configs_list:
        ...
        for grp in port_groups:
            port_flow_set_map[grp].append(port)

    return port_flow_set_map


def _run_scouting_flows(
    flow_configuration: _FlowConfigurationList,
    port_flow_set_map: _PortGroupMap,
) -> None:
    ...
    for src_group in source_groups['port_group']:
        for dest_group in destination_groups['port_group']:
            ...
            port_pairs = _create_port_pairs(
                port_flow_set_map, src_group, dest_group
            )
...
def _initialize_port_flows(
    ...
    src_port_groups: _PortGroupList = flow_configuration.pop("source"
                                                             )["port_group"]
    dest_port_groups: _PortGroupList = flow_configuration.pop("destination"
                                                              )["port_group"]
...
def _create_port_pairs(
    port_flow_set_map, source_group, destination_group
) -> _PortPairSet:
    port_pairs: _PortPairSet = set()
    # create port pairs that have matching flow sets
    for src_port in port_flow_set_map[source_group]:
        for dest_port in port_flow_set_map[destination_group]:
