...
# Command: GET
@cloup.command(help="Get details about a URL")
@click.pass_obj
...
# Command: CHECK
@cloup.command(help="Checks the validity of a URL")
@bic_options.list()
...
def monitor(
    ...
    **kwargs,
    ...
    if isinstance(handler, HLSHandler) and handler.has_children():
        ...
        monitor_hls(handler, max, interval, silent)
...
def play(sub: int, player: str):
    ...
    if player == "CONFIG":
        player = ConfigProvider().get("player")
...
url.add_section(
    cloup.Section("Content Commands", [check, get, read, poll, monitor, play, profile])
)
