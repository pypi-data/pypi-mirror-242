...
class Grid(AbstractGrid, _ABC):
    ...
    def __init__(
        ...
        Args:
            blueprint (_Optional[type[Blueprint.AbstractGridBlueprint]], optional): The blueprint of the grid.
        Defaults to None.
        ...
        self.blueprint = blueprint if blueprint is not None else \
            Blueprint.TerrainGridBlueprint(
                    cell_size,
                    dimensions,
                    self.grid_id,
                    noise_scale,
                    noise_octaves,
                    noise_roughness
            ) if self._with_terrain else Blueprint.BaseGridBlueprint(
                    cell_size,
                    dimensions,
                    self.grid_id
            )
    ...
    def get_nearest_cell_with(
        ...
        Example:
            ...
            ```gird.get_nearest_cell_with('a00001', by_entry=True, qualfications = {'unit': 'any'}) returns the
    ...
    def _heuristic(self, cella, cellb):
        ...
        (x1, y1) = cell_a_coords
        (x2, y2) = cell_b_coords
        _logger.gridengine(f'Cell A coordinates: {cell_a_coords}')
        _logger.gridengine(f'Cell B coordinates: {cell_b_coords}')
        _logger.gridengine(f'Heuristic: {round(math.hypot(abs(x1 - x2), abs(y1 - y2)))}')
