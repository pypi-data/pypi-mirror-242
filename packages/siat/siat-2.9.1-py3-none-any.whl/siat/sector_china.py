...
def industry_scan_china(sw_level='F',indicator='Exp Ret%',start='MRY',end='default',printout='smart'):
    """
    ...
    """
    print("  Scanning Shenwan industry performance ...")
    ...
    idf['Sharpe Ratio']=idf['Exp Ret%'] / idf['Exp Ret Volatility%']
    idf['Sortino Ratio']=idf['Exp Ret%'] / idf['Exp Ret LPSD%']
    ...
    df_prt.rename(columns={'Industry Code':'行业代码','Industry Name':'行业名称', \
                           ...
                           'Sharpe Ratio':'夏普比率','Sortino Ratio':'索替诺比率'}, \
                  inplace=True)
    
    # 显示
    titletxt="申万行业业绩指标排行榜："+"层级/类别"+sw_level+'，共'+str(len(idf))+"个行业"
    print("\n***",titletxt,'\n')
