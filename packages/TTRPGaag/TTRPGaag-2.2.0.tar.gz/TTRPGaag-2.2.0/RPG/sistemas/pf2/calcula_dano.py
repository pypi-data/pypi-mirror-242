...
class CalculaDanoAtaque(CalculaDanoBase):
    ...
    def __str__(self):
        modificador = '' if self._mod_dano == 0 else f'+{self._mod_dano}' if self._mod_dano > 0 else f'{self._mod_dano}'
        fatal = f' (fatal d{self._fatal_dado})' if self._fatal_dado else ''
        deadly = f' (deadly d{self._deadly_dado})' if self._deadly_dado else ''

        extra = fatal + deadly

        return f"""{self.nome}: +{self._modificador_rolamento} vs. AC {self._numero_alvo}, {self._qtd_dados}d{self._face_dados}{modificador}{extra} = {self.dano_medio} ({self.dano_por_acao} por ação)"""
...
class CalculaDanoSave(CalculaDanoBase):
    ...
    def __str__(self):
        ...
        return f"""{self.nome}: {self._qtd_dados}d{self._face_dados}{modificador} DC {self._numero_alvo} vs. +{self._modificador_rolamento} = {self.dano_medio} ({self.dano_por_acao} por ação)"""
