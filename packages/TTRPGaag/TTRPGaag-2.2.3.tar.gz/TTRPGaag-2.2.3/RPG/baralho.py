...
class Baralho:
    def __init__(self, cartas: list):
        """
        Manipula uma lista de cartas_aplicadas, embaralhando seus elementos e permiti sacar, resetar para seu status original
    ...
    def embaralhar(self):
        ...
        while len(self._cartas) > 0:
            sacada = random.choice(self._cartas)
            novas_cartas.append(sacada)
            self._cartas.pop(self._cartas.index(sacada))
    ...
    def ver_cartas(self, qtd: int) -> list:
        ...
        """
        return self._cartas[0:qtd]

    def sacar(self, do_topo: bool = True):
        """
    ...
    def mover_cartas(self, qtd: int = 1, aleatorio: bool = True):
        ...
        """
        a_mover = list(self._cartas[0:qtd])
    ...
    def recuperar_cartas(self, cartas_descartadas: list, aleatorio: bool = False, embaralha: bool = True):
        """
        Coloca as cartas selecionadas da pilha de descartes de volta para a pilha de saque
        :param cartas_descartadas: uma lista com as cartas a serem devolvidas da pilha de descarte
        ...
        """
        while cartas_descartadas:
            sacada = self.pegar_do_descarte(cartas_descartadas[0])
            self.descer_carta(sacada, aleatorio, embaralha)


if __name__ == "__main__":
    baralho_teste = Baralho([carta for carta in range(1, 11)])
    print(baralho_teste)
