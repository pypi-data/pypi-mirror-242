...
class Model_Downloader:
    ...
    def get_file_urls(self, model):
        ...
        self.revision = soup.find('a', class_='rounded').text
        ...
        for tag in soup.find_all('a', attrs={'title': 'Download file'}):
            if tag.find('svg').next_sibling.strip() == '':
                direct_urls.append('https://huggingface.qichangzheng.net' + tag['href'])
            else:
                redirect_urls.append('https://huggingface.qichangzheng.net' + tag['href'])
        direct_urls.extend(self.get_redirect_urls(redirect_urls))
        return direct_urls

    def download(self, model_name, path):
        # Ensure the download directory exists
        urls = self.get_file_urls(model_name)
        path = path + '/models--' + '--'.join(model_name.split('/')) + f'/{self.revision}'
