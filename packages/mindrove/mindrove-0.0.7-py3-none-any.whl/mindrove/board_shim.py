...
class MindroveConfigMode(enum.Enum):
    """Enum to store all supported Board Ids"""
    
    EEG_MODE = str(b'\x00\x00\x00\x00\x00')  #:
    BEEP = str(b'\x01\x00\x00\x00\x00')  #:
    BOOP = str(b'\x02\x00\x00\x00\x00')  #:
    IMP_MODE = str(b'\x04\x00\x00\x00\x00')  #:
    TEST_MODE = str(b'\x10\x00\x00\x00\x00')  #:
...
class MindRoveError(Exception):
    ...
    def __init__(self, message: str, exit_code: int) -> None:
        detailed_message = '%s:%d %s' % (MindroveExitCodes(exit_code).name, exit_code, message)
        super(MindRoveError, self).__init__(detailed_message)
    ...
    def __init__(self, board_id: int, input_params: MindRoveInputParams) -> None:
        ...
        if board_id == BoardIds.STREAMING_BOARD.value or board_id == BoardIds.PLAYBACK_FILE_BOARD.value:
            try:
                ...
                raise MindRoveError('set master board id using params.other_info',
