...
class DeviceDataStruct(NamedTuple):
    ...
    Methods
    -------
    send(sending_map: Mapping[NetworkSysTypeEnum, Callable[[OutputMessage | List[OutputMessage]], None]]) -> None:
    ...
    def send(self, sending_map: Mapping[NetworkSysTypeEnum, Callable[[OutputMessage], None]]) -> None:  # TODO
        ...
        Parameters
        ----------
        sending_map : Mapping[NetworkSysTypeEnum, Callable[[OutputMessage | List[OutputMessage]], None]]
...
class DeviceDataStructBatch(NamedTuple):
    ...
    def send(self, sending_map: Mapping[NetworkSysTypeEnum, Callable[[OutputMessage | List[OutputMessage]], None]]) -> None:
        """
        This method preparing sending recipes for workers and send them to worker which defined by 'sending_map' param.
        ...
        logger.info(f'recipes DA: {da_output_batch}')
        logger.info(f'recipes NEW_LK: {new_lk_output_batch}')
        if da_output_batch and NetworkSysTypeEnum.OUTPUT_DATA_AGGREGATOR_DEVICE_DATA in sending_map:
            sending_map[NetworkSysTypeEnum.OUTPUT_DATA_AGGREGATOR_DEVICE_DATA](da_output_batch)
        if new_lk_output_batch and NetworkSysTypeEnum.OUTPUT_NEW_LK in sending_map:
            sending_map[NetworkSysTypeEnum.OUTPUT_NEW_LK](new_lk_output_batch)
                ...
                if self.send_always_to_data_logger:
                    for parsed_base_message in parsed_base_messages:
                        sending_recipes.append(DeviceDataStructSendingRecipe(
                            network=None,
                            message=parsed_base_message.copy(),
