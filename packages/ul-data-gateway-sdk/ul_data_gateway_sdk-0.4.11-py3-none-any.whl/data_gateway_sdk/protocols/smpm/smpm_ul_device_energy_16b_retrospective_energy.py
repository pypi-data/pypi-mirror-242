...
from data_aggregator_sdk.integration_message import IntegrationV0MessageConsumption, IntegrationV0MessageData, CounterType, ResourceType
...
class SmpmUlDeviceEnergy16BRetrospectiveEnergyData(Packet):
    ...
    def to_integration_data(self, received_at: datetime, device_tz: tzinfo, **kwargs: Any) -> List[IntegrationV0MessageData]:
        ...
        if not self.is_valid and self.period_ago == 0:
            integration_message_data = [
                IntegrationV0MessageData(
                    is_valid=self.is_valid,
                    dt=days_ago_calculation(received_at, device_tz, time(0), period_ago) if self.period_ago else received_at,
                    consumption=[
                        IntegrationV0MessageConsumption(
                            ...
                            journal_data_type=journal_data_type_for_end if self.period_ago else JournalDataType.CURRENT,
                        ),
                    ],
                ),
                IntegrationV0MessageData(
                    is_valid=self.is_valid,
                    dt=days_ago_calculation(received_at, device_tz, time(0), period_ago - delta),
                    consumption=[
                        IntegrationV0MessageConsumption(
                            tariff=tariff,
                            counter_type=CounterType.COMMON,
                ...
                IntegrationV0MessageData(
                    is_valid=self.is_valid,
                    dt=days_ago_calculation(received_at, device_tz, time(0), period_ago - delta * 2),
                    consumption=[
                        IntegrationV0MessageConsumption(
                            tariff=tariff,
                            counter_type=CounterType.COMMON,
                ...
                IntegrationV0MessageData(
                    is_valid=self.is_valid,
                    dt=days_ago_calculation(received_at, device_tz, time(0), period_ago - delta * 3),
                    consumption=[
                        IntegrationV0MessageConsumption(
                            tariff=tariff,
                            counter_type=CounterType.COMMON,
