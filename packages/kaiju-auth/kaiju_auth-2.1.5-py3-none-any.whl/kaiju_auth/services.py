...
class PermissionService(SQLService[str, Permission], ContextableService, PublicInterface):
    ...
    ) -> list[dict] | list[_TaggedPermissions]:
...
class GroupService(SQLService[str, Group], ContextableService, PublicInterface):
    ...
    async def get_permissions(self, id: str | Collection[str]) -> frozenset | list[_GroupPermissions]:
...
class UserService(SQLService[uuid.UUID, User], ContextableService, PublicInterface, UserInterface):
    ...
    async def register(self, username: str, email: str, password: str, settings: dict = None) -> User:
        ...
        return await self.get_user_and_permissions(settings['id'])
    ...
    async def auth(self, username: str, password: str) -> User | None:
        ...
        return await self.get_user_and_permissions(user['id'])
    ...
    async def get_user_and_permissions(self, id) -> dict:
...
class KeystoreService(ContextableService):
    ...
    def __init__(
        ...
        **kws,
        ...
        :param key_path: private RSA key location to a pem file (by default the key is auto-generated)
    ...
    async def init(self):
        ...
        self._task_renew_keys = self._scheduler.schedule_task(
            ...
            interval=self.ttl // 2,
            retries=10,
            name=f'{self.service_name}._update_public_key',
        )
    ...
    def encryption_key(self) -> (str, jwt.JWK):
        """Get a key pair."""
        return self._key.key_id, self._key

    async def get_public_key(self, kid: str = None) -> jwk.JWK | None:
        """Get a public key by its id."""
        if kid is None or kid == self._key.key_id:
            return self._pkey
    ...
    async def _load_encryption_key(self) -> None:
        ...
        if self.key_path:
            ...
            key = jwk.JWK.generate(kty=self.algorithm, size=self.key_size, kid=uuid.uuid4().hex)
    ...
    async def _update_public_key(self) -> None:
        """Update public key cache value."""
        self._deadline = time() + self.ttl
        await self._cache.set(self.ns.get_key(self._key.key_id), self._pkey, ttl=self.ttl)
...
class JWTService(ContextableService, TokenInterface):
    ...
    def _get_token_pair(self, claims) -> TokenInterface.TokenInfo | None:
    ...
    async def load_token(self, token: str) -> jwt.JWT | None:
        """Load a JWT token.
    ...
    def _generate_token(self, kid: str, key: jwt.JWK, data: dict, ttl: int) -> jwt.JWT:
        iat = int(time())
        exp = int(min(iat + ttl, self._keystore._deadline))  # noqa
        token = self._JWT(
