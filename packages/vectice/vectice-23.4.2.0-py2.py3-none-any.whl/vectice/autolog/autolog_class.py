...
if TYPE_CHECKING:
    ...
    TDataset = TypedDict("TDataset", {"variable": str, "dataframe": DataframeTypes})
...
class Autolog:
        ...
        for key in self._vectice_data.keys():
            ...
            if is_pandas:
                ...
                if isinstance(asset, DataFrame):
                    assets.append({"variable": key, "dataframe": asset})
            if is_pyspark:
                from pyspark.sql import DataFrame as SparkDF
    ...
    def _get_graph(self) -> Figure | SubplotBase | PltFigure | None:
        # catch tuple naming fig, axes
        check_plt = re.findall(r"fig,", self._cell_content)
        if check_plt and self._local_vars.get("fig"):
            self._vectice_data["fig"] = self._local_vars["fig"]
    ...
    def _log_dataset(self, dataset: TDataset, iteration: Iteration) -> None:
        ...
        no_resource_dataset = Dataset.clean(NoResource(dataframes=dataset["dataframe"]), name=dataset_name)
        if resource:
            # TODO Dataset type ?
            vec_dataset = Dataset.clean(FileResource(resource, dataframes=dataset["dataframe"]), name=dataset_name)
        else:
