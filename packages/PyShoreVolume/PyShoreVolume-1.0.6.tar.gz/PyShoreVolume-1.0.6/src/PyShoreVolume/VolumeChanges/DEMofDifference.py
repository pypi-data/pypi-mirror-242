...
def DEMofDifference(path, DODCRS, save_to_path, pixelsize, measurementerror):
        """
        ...
        measurementerror: measurement error of the data set
                ...
                if count < num:
                    ...
                    if old.shape == new.shape: 
                        ...
                        ###PLOT FUNC
                          ...
                          ax.set_title('DOD %s - %s' %(date1[6:9]+'/'+date1[4:6]+"/"+date1[0:4], date2[6:9]+'/'+date2[4:6]+"/"+date2[0:4]),fontsize=10)     
                    ...
                    DODdic[date1+'-'+date2] = {'Volumes': np.sum(vols),'LoD Vol':np.sum(lods), 'Date': datetime.strptime((date2[6:9]+'-'+date2[4:6]+'-'+date2[0:4]),'%d-%m-%Y')}
                else:
                        break
