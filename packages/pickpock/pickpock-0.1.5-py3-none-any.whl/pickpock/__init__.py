...
def init(browser="firefox", cookie_file=None, throttle=15):
    if not cookie_file:
...
def fetch_json(url):
    try:
        ...
        if not 'ItemModule' in tt_json:
            print(f'Response missing data section')
            return None
        time.sleep(random.randint(3, sleep))
        return tt_json
    except AttributeError:
        print(f'Missing response')
        return None
