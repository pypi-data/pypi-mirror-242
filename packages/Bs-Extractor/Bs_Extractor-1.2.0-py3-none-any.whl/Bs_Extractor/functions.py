...
def split_based_on_dates(text_list):
   ...
   r'(?<![.\d])(\b\d{2}/\d{2}/\d{4}\b'  # DD/MM/YYYY
    ...
    r'|\b\d{1,2}-[a-zA-Z]{3}-\d{2}\b)(?![.\d])'  # D or DD-MMM-YY
)
        ...
        if name_candidates:
            ...
            for i, ele in enumerate(lst):
                ...
                if 'balance carried forward' in ele.lower():
                    ...
                    print(f"next_words: {len(next_words)}")
...
def validate_name(eid_name, extracted_name):
            threshold = 65
            similarity = fuzz.partial_ratio(eid_name, extracted_name)
            print(similarity)
...
def extract_name_and_salary_from_gpt_resp(text):
    name_pattern = r'Name:\s*(.*)'  # Regex pattern for extracting name
    salary_pattern = r'Salary:\s*([\d.,]+)'  # Regex pattern for extracting salary
    ...
    if salary and float(salary.replace(',',''))<300.00:
        salary = None
