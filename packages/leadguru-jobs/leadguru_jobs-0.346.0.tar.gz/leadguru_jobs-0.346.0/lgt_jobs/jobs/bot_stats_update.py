...
from lgt_data.enums import SourceType, StatusConnection
...
class BotStatsUpdateJob(BaseBackgroundJob, ABC):
    ...
    def exec(self, data: BotStatsUpdateJobData):
        ...
        if bot.source.source_type == SourceType.DISCORD:
            ...
            bot.servers = [Server.from_dic(server) for server in client.get_servers()]
            ...
            user = users_repository.get(bot.user_id)
            discord_user: DiscordUser = user.get_discord_user(bot.user_name)
            discord_user.status = StatusConnection.COMPLETE
            users_repository.set(user.id, discord_users=[user.__dict__ for user in user.discord_users])
            return
