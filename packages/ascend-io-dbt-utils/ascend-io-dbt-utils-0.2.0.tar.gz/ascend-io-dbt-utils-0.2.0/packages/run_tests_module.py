...
from .manifest_utils import _get_tests_and_nodes
from .transform_utils import _create_transform
...
def deploy_tests_cmd(ctx, **kwargs):
    ...
    for node_to_be_tested in [node for node in nodes_with_test_data]:
        node_id = node_to_be_tested.split('.')[-1]
    ...
    for test, test_details in test_data.items():
        ...
        test_dependencies = [test.split('.')[-1] for test in test_details['depends_on']['nodes']]
...
def delete_tests_cmd(ctx, **kwargs):
    ...
    # Get test nodes
    test_data, _ = _get_tests_and_nodes(ctx.obj['manifest'])
    ...
    for node in node_ids:
        ...
        try:
            ...
            print(f"An error occurred while deleting transform {node}: {e}")
    ...
    try:
        ...
        print(f"An error occurred while deleting dbt_tests group: {e}")
...
def check_test_results_cmd(ctx, **kwargs):
    ...
    for test_node in nodes_to_be_tested:
        try:
            ...
            print(f"An error occurred while retrieving test {test_node} results: {e}")
