...
class Dataset:
    ...
    def complete_upload(
        ...
        Returns:
            None
        """
        transaction = self.__transaction_manager.get_transaction(
            transaction_id,
        ...
        iteration = 0
        while (
            transaction.expected_resource_count is not None
            and transaction.resource_count != transaction.expected_resource_count
        ):
            if time.time() - start_time > timeout:
                raise TimeoutError(
                    "Timeout waiting for upload to complete. "
                    f"Expected {transaction.expected_resource_count} files, found {transaction.resource_count}."
                )
            if iteration > 0:
                time.sleep(min((2**iteration) / 2, 16))
            transaction = self.__transaction_manager.get_transaction(
                transaction_id, resource_owner_id=self.__record.org_id
            )

        self.__transaction_manager.end_transaction(
            transaction_id,
