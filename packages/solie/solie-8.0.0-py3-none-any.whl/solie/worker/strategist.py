...
class Strategiest:
    def __init__(self):
        ...
        self.red_pixmap.load(
            f"{introduction.INSTALLED_PATH}/static/icon/traffic_light_red.png"
        )
        self.yellow_pixmap = QtGui.QPixmap()
        self.yellow_pixmap.load(
            f"{introduction.INSTALLED_PATH}/static/icon/traffic_light_yellow.png"
        )
        self.green_pixmap = QtGui.QPixmap()
        self.green_pixmap.load(
            f"{introduction.INSTALLED_PATH}/static/icon/traffic_light_green.png"
        )
    ...
    async def load(self, *args, **kwargs):
        ...
        try:
            ...
            filepath = (
                f"{introduction.INSTALLED_PATH}/static/sample_indicators_script.txt"
            )
            async with aiofiles.open(filepath, "r", encoding="utf8") as file:
                read_data = await file.read()
                first_strategy["indicators_script"] = read_data
            filepath = (
                f"{introduction.INSTALLED_PATH}/static/sample_decision_script.txt"
            )
            async with aiofiles.open(filepath, "r", encoding="utf8") as file:
                read_data = await file.read()
