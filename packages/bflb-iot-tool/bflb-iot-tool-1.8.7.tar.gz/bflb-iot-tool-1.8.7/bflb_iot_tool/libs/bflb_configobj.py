...
class Section(dict):
    ...
    def __setitem__(self, key, value, unrepr=False):
        ...
        if isinstance(value, Section):
            ...
            dict.__setitem__(self, key, Section(self, new_depth, self.main, indict=value,
                                                name=key))
        else:
            if key not in self:
    ...
    def __repr__(self):
        ...
        return '{%s}' % ', '.join([
            ('%s: %s' % (repr(key), repr(_getval(key)))) for key in (self.scalars + self.sections)
        ])
    ...
    def walk(self, function, raise_errors=True, call_on_sections=False, **keywargs):
        ...
        for i in range(len(self.sections)):
            ...
            out[entry] = self[entry].walk(function,
...
class ConfigObj(Section):
        ...
        while cur_index < maxline:
            ...
            if mat is not None:
                ...
                if cur_depth != sect_close.count(']'):
                    self._handle_error("Cannot compute the section depth", NestingError, infile,
                                       cur_index)
                    continue
                ...
                if cur_depth < this_section.depth:
                    ...
                    try:
                        ...
                        self._handle_error("Cannot compute nesting level", NestingError, infile,
                                           cur_index)
                        continue
            ...
            if mat is None:
                ...
                if value[:3] in ['"""', "'''"]:
                    try:
                        value, comment, cur_index = self._multiline(value, infile, cur_index,
                                                                    maxline)
                    except SyntaxError:
                        self._handle_error('Parse error in multiline value', ParseError, infile,
                                           cur_index)
                        continue
                    else:
                        if self.unrepr:
                    ...
                    if self.unrepr:
                        ...
                        try:
                            ...
                            self._handle_error('Parse error in value', ParseError, infile,
                                               cur_index)
                            continue
                #
    ...
    def _quote(self, value, multiline=True):
        ...
        * Obey list syntax for empty and single member lists.
        ...
        if check_for_single:
            ...
            elif ((value[0] not in wspace_plus) and (value[-1] not in wspace_plus) and
                  (',' not in value)):
                quot = noquot
            else:
    ...
    def _write_line(self, indent_string, entry, this_entry, comment):
        ...
        return '%s%s%s%s%s' % (indent_string,
                               ...
                               self._a_to_u(' = '), val, self._decode_element(comment))

    def _write_marker(self, indent_string, depth, entry, comment):
        """Write a section marker line"""
        return '%s%s%s%s%s' % (indent_string, self._a_to_u(
            '[' * depth), self._quote(self._decode_element(entry), multiline=False),
                               self._a_to_u(']' * depth), self._decode_element(comment))
    ...
    def validate(self, validator, preserve_errors=False, copy=False, section=None):
        ...
        for entry in configspec.scalars:
            ...
            ret_true, ret_false = validate_entry(entry, configspec[entry], val, missing, ret_true,
                                                 ret_false)
