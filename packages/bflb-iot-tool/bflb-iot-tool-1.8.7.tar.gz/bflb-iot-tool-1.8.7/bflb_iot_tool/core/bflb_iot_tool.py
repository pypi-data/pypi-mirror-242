...
#             shutil.rmtree(ota_path)
        ...
        if self.bl60x_mfg_ota_xz_gen(ota_path, chipname, chiptype, cpu_type,
                                     pt_parcel[pt_fw1len_name], hash_pk=hash_pk) == False:
...
#                 return bflb_utils.errorcode_msg()
            ...
            if config["input_path"]["publickey"]:
                with open(config["input_path"]["publickey"], 'r') as f:
                    pk = f.read()
                    pk_data = ecdsa.VerifyingKey.from_pem(pk).to_string()
                            ...
                            if temp_obj is not None and len(temp_obj) > 0:
                                ...
                                if name == "fw" or name == "fw_cpu0":
                                    ...
                                    if False == self.bl60x_mfg_ota_bin_gen(
                                            ...
                                            config["param"]["version"], config["param"]["ota"], hash_pk):
                                        bflb_utils.set_error_code("0082")
                                        return bflb_utils.errorcode_msg()
                                # 注释部分是mfg的单独处理
                                """elif name == "mfg":
        ...
        try:
            ...
            if act == "build":
                ...
                error = self.eflash_loader_t.efuse_flash_loader(args, self.eflash_loader_cfg,
                                                                self.eflash_loader_bin, callback, None, None,
                                                                privatekey_str=config["param"]["privatekey"],
            ...
            # generation partition table/boot2/fw bin files and rf files
                ...
                if act == "erase" or act == "erase_only":                    
                    ...
                    if not config["param"]["start"] and not config["param"]["end"]:
                        ...
                        if config["param"]["end"]:
                            options.extend(["--end=" + config["param"]["end"]])
#                 if "erase_only" in config["check_box"] and config["check_box"]["erase_only"] is True:
...
#                 return
    ...
    if args.build:
        ...
        if not args.firmware and not args.boot2 and not args.pt and not args.mfg and not args.media and not args.romfs and not args.dts:
            act = "erase_only"
        else:
            act = "erase"
    elif args.read:
