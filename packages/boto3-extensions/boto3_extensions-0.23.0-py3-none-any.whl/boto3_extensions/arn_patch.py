...
from boto3.resources.params import build_param_structure, get_data_member
from botocore import xform_name

logger = logging.getLogger(__name__)


class ServiceContext:  # pylint: disable=missing-class-docstring
    def __init__(
        self,
        service_name,
        service_model,
        ...
        if not self._session:
            ...
            if session is None or not isinstance(session, boto3.Session):
                raise Exception(  # pylint: disable=broad-exception-raised
                    "Unable to get session from stack frame."
                )
            self._session = session

    @property
    def service_name(self):  # pylint: disable=missing-function-docstring
        return self._service_name

    @property
    def service_model(self):  # pylint: disable=missing-function-docstring
        return self._service_model

    @property
    def service_waiter_model(self):  # pylint: disable=missing-function-docstring
        return self._service_waiter_model

    @property
    def resource_json_definitions(self):  # pylint: disable=missing-function-docstring
...
class PatchedResourceFactory(
    ...
    def load_from_definition(self, *args, **kwargs):
        ...
        cls.meta.session = service_context._session  # pylint: disable=protected-access
        attrs = {"meta": cls.meta}

        service_name = service_context.service_name
        resource_name = kwargs["resource_name"]
        api_version = None  # TODO: multiple arn versions?
        loader = service_context._session._loader  # pylint: disable=protected-access

        # Ensure the boto3-arn-patch data directory
        # is in the loader search path
        loader.search_paths.append(os.path.join(os.path.dirname(__file__), "data"))

        arn_model = loader.load_service_model(service_name, "arns-1", api_version)
    ...
    def _load_arn(self, attrs, meta, arn_model):  # pylint: disable=unused-argument
        ...
        if arn_config:
            ...
            if format_string:
                ...
                def construct_arn(self):
                    formatter = Formatter()
                    mapping = {}
                    keys = [k[1] for k in formatter.parse(format_string) if k[1]]
                    keys = set(keys)
                    ...
                    if isinstance(replace_args, list) and len(replace_args) == 2:
                        return format_string.format(**mapping).replace(
                            replace_args[0], replace_args[1]
                        )
                    ...
                    if self.meta.data is None:
                        if hasattr(self, "load"):
                            ...
                            raise ResourceLoadException(
                                "{0} has no load method".format(  # pylint: disable=consider-using-f-string
...
def _create_request_parameters(parent, request_model, params=None, index=None):
    ...
    for param in request_model.params:
        ...
        if source == "identifier":
            ...
            # Intercept requests to route53.HostedZone ResourceId and retrieve only the
            # resource id from the /hostedzone/<resourceid> which is otherwise returned
            if (
                parent.__class__.__name__ == "route53.HostedZone"
                and target == "ResourceId"
            ):
                value = getattr(parent, xform_name(param.name)).split("/")[-1]
            else:
            ...
            raise NotImplementedError(
                "Unsupported source type: {0}".format(  # pylint: disable=consider-using-f-string
                    source
                )
            )
...
def patch_session():  # pylint: disable=missing-function-docstring
    @property
    def account_id(self):
        """
        ...
        """
        try:
            return self._account_id  # pylint: disable=protected-access
        except AttributeError:
            # TODO: Error handling
            account_id = self.client("sts").get_caller_identity()["Account"]
            self._account_id = account_id  # pylint: disable=protected-access
        return self._account_id  # pylint: disable=protected-access
...
def patch_service_context():  # pylint: disable=missing-function-docstring
    logger.info("Patching ServiceContext")
    boto3.utils.ServiceContext = ServiceContext


def patch_create_request_parameters():  # pylint: disable=missing-function-docstring
...
def patch_resource_factory():  # pylint: disable=missing-function-docstring
    logger.info("Patching ResourceFactory")
    boto3.session.ResourceFactory = PatchedResourceFactory


def patch_resource_meta():  # pylint: disable=missing-function-docstring
