    ...
    def send_email(self,**kwargs):
        try:
            ...
            with smtplib.SMTP(kwargs['smtp_server'], kwargs['smtp_port']) as server:
                try:    
                    server.sendmail(kwargs['sender'], kwargs['recipients'], msg.as_string())
                    return 1
...
class Template:
    ...
    def db_fetchall(self,db_params,query):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectPostgreSQL(**db_params)
            # クエリの実行
    ...
    def db_commit(self,db,host,user,pw,port,schemas,query):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectPostgreSQL(db,host,user,pw,port,schemas)
            # クエリの実行
    ...
    def db_commit_values(self,connection,query,values):
        try:
            # データベースへの接続
            db_connection = self.database_connector.connectPostgreSQL(**connection)
            # クエリの実行
    ...
    def db_commit_many(self,params,query,values):
        try:
