...
def find_diverse_examples(experiment, experiment_type, k, topic_count=None):
...
def save_diverse_examples(experiment, experiment_type, ks, topic_counts = None, seed =None):
    if topic_counts:
    ...
    if topic_counts:
        ...
        for topic_count in topic_counts:
            df_diverse = find_diverse_examples(experiment, experiment_type, ks[0], topic_count)
            df_diverse["topic-count"] = topic_count
            df_diverse["seed"] = seed
            all_diverse_samples.append(df_diverse)
    for k in ks:
        df_diverse = find_diverse_examples(experiment, experiment_type, k, None)
        df_diverse["k"]=k
        all_diverse_samples.append(df_diverse)
