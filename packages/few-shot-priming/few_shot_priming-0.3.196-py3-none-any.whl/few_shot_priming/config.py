...
def get_diverse_example_path(experiment, experiment_type, topic_count=None):
    config = load_config()
    token = f"{experiment}-{experiment_type}"
    path = config["topic-similarity"]["diverse-examples"][token]
    if topic_count:
        return  path.replace("topic-count", topic_count)
    else:
        return path.replace("topic-count-topics", "")
...
def get_topic_similarity_logs_path():
    conf = load_config()
    return conf["topic-simialrity"] ["path-logs"]
