...
def generate_instructions(dataset, tokenizer, logger, dataset_training=None):
    mapper = {0:"CON", 1:"PRO"}
    token_limit = 2048 // (len(dataset) - 2)
    if dataset_training:
        examples = format_examples(dataset_training, tokenizer, mapper, token_limit, logger, alpaca_examples=True)
    ...
    for i, record in dataset.iterrows():
        ...
        if dataset_training:
            data["instruction"].append(examples)
...
def prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=None, fine_tune=False):
    ...
    for split in dataset:
        if fine_tune:
            ...
            if split!="training":
                generate_instructions(dataset[split], tokenizer, logger, dataset["training"])
    data = DatasetDict()
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, sampling_strategy=None, similarity=None, fine_tune=True):
    ...
    if sampling_strategy and similarity:
        ...
        tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name)
        dataset = load_splits(experiment, oversample=False)
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, tokenizer, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune)
        if validate:
            test_data = val_data
