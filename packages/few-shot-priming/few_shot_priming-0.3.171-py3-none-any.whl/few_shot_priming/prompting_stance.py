...
def format_examples(dataset, tokenizer, labels_text_map, token_limit, logger):
    ...
    for i, train_record in dataset.iterrows():
        ...
        log_message(logger, template + f"{id}:{token_limit}\tOn the topic '{topic}' the argument '{sentence}' has the stance {label[idx]}\t", level=logging.INFO)
        ...
        if len(tokens) > token_limit:
            ...
            if tokens_to_remove_from_sentence < len(sentence_tokens):
                sentence = tokenizer.decode(sentence_tokens[:-tokens_to_remove_from_sentence])
            else:
                sentence = ""
            template = f"On the topic '{topic}' the argument '{sentence}' has the stance {label[idx]}.\n"
...
def run_experiment_prompting(config=None, experiment="ibmsc", model=None, offline=False, validate=True, splits=None, logger=None, openai=False, sampling_strategy=None, similarity=None):
    ...
    for i, record in test_dataset.iterrows():
        ...
        if not sampling_strategy:
            ...
            print(training_dataset.shape[0])
        else:
            training_dataset = sample_diverse_examples(experiment, experiment_type, few_shot_size)
