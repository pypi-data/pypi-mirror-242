...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, sampling_strategy=None, similarity=None):
    ...
    if sampling_strategy and similarity:
        ...
        tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name)
        dataset = load_splits(experiment, oversample=False)
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, tokenizer, cutoff_len, few_shot_size=config["few-shot-size"])
        if validate:
            test_data = val_data
