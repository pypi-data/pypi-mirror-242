...
class SwitchOutput(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in SwitchOutput with oneOf schemas: AzureBlobStorageOutput, Broker, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in SwitchOutput with oneOf schemas: AzureBlobStorageOutput, Broker, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> SwitchOutput:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into SwitchOutput with oneOf schemas: AzureBlobStorageOutput, Broker, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into SwitchOutput with oneOf schemas: AzureBlobStorageOutput, Broker, CarrierOutput, SqlRaw, SyncResponse. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
