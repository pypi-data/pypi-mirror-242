...
class CwbookUrlsCrawler(BaseUrlsCrawler):
    def __init__(self):
        pass
        
    def is_target_list(self, url):
        is_list = False
        match = re.search('\.cloudfront\.net\/webapi/shopcategory\/getsalepagelist\/40358\/.+', url.lower())
        if match:
            is_list = True
        return is_list
    ...
    def scrape_list_to_urls(self, url, res):
        ...
        for item in json_data['Data']['SalePageList']:
            prd_urls.append({'url': 'https://shop.cwbook.com.tw/SalePage/Index/' + str(item['Id']), 
