...
class QUMO(Problem):
    ...
    def _normalize_solution(self, solution):
        ...
        for i in range(normalized_solution.size):
            if self._is_binary(i):
                ...
                elif normalized_solution[i] < 0.0:
                    normalized_solution[i] = 0.0
                else:
                    _logger.error(
                        "Solution vector contains zero value for binary variable at index %d; "
                        "aborting computation",
                        i,
                    )
                    raise ValueError("Zero value for binary variable")
    ...
    def evaluate(self, solution: Solution):
        ...
        objective = _quadratic_form(self.__quadratic, normalized)
