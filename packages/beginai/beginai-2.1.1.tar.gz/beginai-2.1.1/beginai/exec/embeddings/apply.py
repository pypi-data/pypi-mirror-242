...
class AlgorithmsApplier(object):
    ...
    def record_intervention_dates(self, filename, object_name, unique_identifier_column, intervention_date, file_separator=','):
        ...
        df = df.rename(columns={unique_identifier_column: 'internal_id',
                       intervention_date: self.INTERVENTION_TIMESTAMP})

        df[self.INTERVENTION_TIMESTAMP] = pd.to_datetime(
            df[self.INTERVENTION_TIMESTAMP], format="%d-%m-%Y")
        df[self.INTERVENTION_TIMESTAMP] = df[self.INTERVENTION_TIMESTAMP].apply(
            lambda x:  x.timestamp())
