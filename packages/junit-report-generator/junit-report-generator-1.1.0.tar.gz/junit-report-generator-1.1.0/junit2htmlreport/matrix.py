...
from .render import HTMLMatrix, HTMLMatrixJson, HTMLReport
...
class HtmlReportMatrixJson(ReportMatrix):
    """
    Render a matrix report as html
    """

    def __init__(self, outdir):
        super(HtmlReportMatrixJson, self).__init__()
        self.outdir = outdir
    ...
    def add_report(self, filename):
        ...
        Load a report
        """
        super(HtmlReportMatrixJson, self).add_report(filename)
        basename = os.path.basename(filename)
        # make the individual report too
        report = self.reports[basename].html()
        if self.outdir != "" and not os.path.exists(self.outdir):
    ...
    def short_outcome(self, outcome):
        if outcome == PASSED:
            return "ok"
        return super(HtmlReportMatrixJson, self).short_outcome(outcome)
    ...
    def summary(self):
        ...
        :return:
        """
        html_matrix = HTMLMatrixJson(self)
        return str(html_matrix)
