...
PARSER = ArgumentParser(prog="junit-report-generator")
...
PARSER.add_argument("--report-matrix-json", dest="html_matrix_json", type=str,
                    metavar="REPORTJSON",
                    help="Generate an HTML report matrix JSON")
...
def run(args):
    ...
    elif opts.html_matrix_json:
        util = matrix.HtmlReportMatrixJson(os.path.dirname(opts.html_matrix_json))
        for filename in inputs:
            print(filename)
            util.add_report(filename)
        output = util.summary()
        json_output = json.loads(output.replace("'", "\""))
        with open(opts.html_matrix_json, "w") as outfile:
