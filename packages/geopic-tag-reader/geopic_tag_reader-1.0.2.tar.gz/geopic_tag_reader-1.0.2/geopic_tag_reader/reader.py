...
def readPictureMetadata(picture: bytes) -> GeoPicTags:
    ...
    if d is None:
        d, llw = decodeDateTimeOriginal(data, "Exif.Image.DateTimeOriginal", "Exif.Photo.SubSecTimeOriginal")
        if len(llw) > 0:
            warnings.extend(llw)

    if d is None:
        d, llw = decodeDateTimeOriginal(data, "Exif.Photo.DateTimeOriginal", "Exif.Photo.SubSecTimeOriginal")
        if len(llw) > 0:
            warnings.extend(llw)
    ...
    # Type
    pic_type = data["Xmp.GPano.ProjectionType"] if isExifTagUsable(data, "Xmp.GPano.ProjectionType") else "flat"
