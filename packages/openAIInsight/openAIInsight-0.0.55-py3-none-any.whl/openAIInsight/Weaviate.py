...
class Weaviate:
    ...
    def getLookupData(self):
        ...
        try:
            ...
            logger.log(f"373 Final result:::{finalResult}")
            return finalResult
        except Exception as e:
        ...
        try:
            ...
            for key in queryJson:
                ...
                if len(key) > 0 and key.strip() != "":
                    ...
                    llm = ChatOpenAI(openai_api_key=self.openAI_apiKey, model_name=self.modelName, temperature=0)

                    response = llm.call_as_llm(f'''{qdocs} Question: {key}
                    Please answer the question with citation to the document in a sentence.
                    For every sentence you write, cite the document name and doc_id, page_content as mentioned in {docs} as {{doc_id}}, {{page_content}}
                    Return reponse strictly in valid dict with keys as {{answer}}, {{doc_id}} and {{page_content}}''')

                    logger.log(f"response::: \t{type(response)}\n\n{response}\n")

                    responseLst.append(response)
                else:
                    logger.log("Empty or blank value received ")
                    raise Exception("Empty or blank value received as Qusetion.")

            logger.log(f"responseLst::: \t{type(responseLst)}\n\n{responseLst}\n")
            return responseLst
            ...
            descr = str(e)
            errorXml = common.getErrorXml(descr, trace)
            logger.log(f"\n Exception in Lookup for Document type 'Q' for schemaName {docType_SchemaName}  \nerrorXml::: \n{errorXml}")
            return str(errorXml)