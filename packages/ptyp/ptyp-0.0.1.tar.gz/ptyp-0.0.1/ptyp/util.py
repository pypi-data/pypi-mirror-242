...
def typesusv(*types: Types) -> str:
    '''Underscore separated types'''
    return '_'.join(args2strs(*types))

# %% ../nbs/01_util.ipynb 19
def types2opts(*types: Types) -> tuple:
    '''If `None` in *types return `Optional[t]` for each other type'''
    if None in types: 
...
def classname(cls: type, clsname: StrBool = True) -> StrBool:
    '''Gets an optional classname for the given `cls`. 
