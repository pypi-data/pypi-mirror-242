...
class ResourcesChecker(AbstractChecker):
    ...
    def validate(
        ...
        for i in model_dataset.index:
            ...
            if sample.shape[0] > 4:
                ...
                journal_validation = JournalChecker()
                df_style.loc[i, "Наименование"] = str(c_act)
                df_volume.loc[i, "Наименование"] = str(c_act)
                for r in self.res:
                    value = model_dataset.loc[i, r]
    ...
    def common_validation(self, ksg_data: dict, val_data, plan_type: str):
                ...
                for r in w["workers"]:
                    ...
                    if val_sample.shape[0] > 3:
                        ...
                        if (res_volume >= q1) and (res_volume <= q99):
                            df_validation_table.loc[i, "Наименование работ"] = work_name
                            df_validation_table.loc[i, "Ресурсы"] = res_name
                            df_validation_table.loc[i, "Результат валидации"] = "green"
                            ...
                            df_validation_table.loc[i, "Наименование работ"] = work_name
                            df_validation_table.loc[i, "Ресурсы"] = res_name
                            df_validation_table.loc[i, "Результат валидации"] = "red"
                            key = work_name + " " + res_name
                            counts, bins, _ = plt.hist(
                                val_sample[res_name + "_res_fact"].values
                            )
                        ...
                        df_validation_table.loc[i, "Наименование работ"] = work_name
                        df_validation_table.loc[i, "Ресурсы"] = res_name
                        df_validation_table.loc[i, "Результат валидации"] = None
        elif plan_type == "s7":
            for w in ksg_data["schedule"]["works"]:
                work_name = w["display_name"]
                act_name = None
        ...
        not_perc = (
            (
                df_validation_table.loc[
                    df_validation_table["Результат валидации"] == None
                ].shape[0]
            )
        ...
        if not_perc == 100:
            ...
            norm_perc = (
                (not_non.loc[not_non["Результат валидации"] == "green"].shape[0])
                / not_non.shape[0]
            ) * 100
