...
class MschmAdapter:
    ...
    def delete_model(self, work_name):
        ...
        model = self.__execute_query(select(WorkModel).where(WorkModel.id_basic_work==work.id)).scalars().one()
    ...
    def save_s7_model(self, model):
        ...
        for name, data in model.items():
            ...
            if current is not None:
                self.__execute_query(delete(S7Model).where(S7Model.name==name), commit=True)
