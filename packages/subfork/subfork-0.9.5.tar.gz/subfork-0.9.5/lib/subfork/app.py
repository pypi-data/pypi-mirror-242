...
def create_app(client, template):
    ...
    template_data = util.read_template(template)
    ...
    log.info("template_folder: %s", template_folder)
...
def build_app(template):
    ...
    :param template: app template.yaml file.
    :returns: path to template.yaml build file.
    """
    log.info("building app: %s", template)
...
def run(client, template, host, port):
    """
    Starts a simple dev server process, and opens a webbrowser to
    the running dev server.

    :param client: Subfork client instance.
    :param template: file path to template.yaml file.
