...
class SinaraServer():
    ...
    def start(args):
        ...
        def get_server_logs(_instance, server_command):
            exit_code, output = docker_container_exec(_instance, server_command)
            return output
        ...
        def get_current_token(_instance):
            ...
            for cmd in commands:
                output = get_server_logs(_instance, cmd)
                stdout, sterr = output
                log_lines_stderr = sterr.decode('utf-8').split('\n')
                log_lines_stdout = stdout.decode('utf-8').split('\n')
                log_lines = [*log_lines_stderr, *log_lines_stdout]
