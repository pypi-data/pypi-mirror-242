...
class App:
    ...
    def run(self):
        ...
        if common.cmn_is_module:
            logger.start(f'{common.cmn_mod_name} is a module')
        else:
            logger.start(f'{common.cmn_mod_name} is not a module')
    ...
    def _check_mod_names(self):
        if not common.cmn_mod_name:
            logger.err('cmd_mod_name in tools/common.py not set')
        ...
        if common.cmn_mod_dir_name == '':
            logger.err('cmn_mod_dir_name in tools/common.py not set')
    ...
    def _check_doxyfile(self):
        ...
        with open(fname, 'r') as fp:
            while True:
                ...
                if m:
                    found_name = True
                    if common.cmn_is_module and m.group(1) != f'"{common.cmn_mod_name} Module"':
