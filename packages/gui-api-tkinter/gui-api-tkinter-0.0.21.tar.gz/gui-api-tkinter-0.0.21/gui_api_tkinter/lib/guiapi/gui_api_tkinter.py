...
class GuiApiTinker:
    ...
    def lbox_select(self, cmd):
        ...
        services.logger.info(f'lbox_select: current select: {wgt.curselection()}')
    ...
    def combobox_set(self, cmd):
        ...
        x = cmd['x']
        y = cmd['y']
        services.logger.info(f'combobox_set: {x} {y}')
        wgt = self._window.winfo_containing(x, y)
        # uncomment to debug
        # n = getattr(w, 'guiapi_name', '<unknown>')
        # services.logger.info(f'DBG combobox_set: on widget.name: {n}')
