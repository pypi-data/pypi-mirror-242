...
# 尝试获得字典项，如果不存在，则插入默认值
def get_or_put_dict_item(dict, key, default):
    if key not in dict:
        dict[key] = default
    return dict[key]
...
def parse_func(expr: Union[str, list], allow_no_bracket = False):
    ...
    # 解析函数名与参数
    #reg = rf'{reg_func_pure}'
    reg = r'([^\(]+)\((.*)\)' # ArgoFlowBoot中函数调用需求太复杂，在steps中调`流程.模板(参数)`，在workflowTemplateRef中调`流程(参数)`
    mat = re.match(reg, expr)
    if mat == None:
