...
class Server:
    ...
    async def __start(self, application: Application, port: int) -> aiohttp.web.AppRunner:
        """Start both servers"""
    ...
    async def start(self):
        ...
        if self.do_multicast:
            ...
            self.server_lists.append(await self.__start(self._multicast_server, self.multicast_port))

        self.server_lists.append(await self.__start(self._server, self.port))
        
    async def stop(self):
        """Stops the IPC server"""
        self.bot.dispatch("ipc_shutdown")
        for server in self.server_lists:
