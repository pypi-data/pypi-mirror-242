...
class CJ(Representor):
    ...
    def __format(request: Request, wstl_obj: Dict[str, Any], link_callback: Callable[[int, Link], None] = None) -> Dict[
        ...
        tvars = {}
        collection['items'] = tuple(item for item in _get_items(request, wstl, link_callback=link_callback, tvars=tvars))
        collection['links'] = tuple(link for link in _get_links(wstl.get('actions', []), tvars, link_callback=link_callback))
        ...
        collection['queries'] = tuple(query for query in _get_queries(wstl.get('actions', [])))
        if 'error' in wstl:
...
def _get_links(actions: List[Dict[str, Any]], tvars: Dict[str, Any] = None,
               link_callback: Callable[[int, Link], None] = None) -> Generator[dict[str, Any]]:
    """
    Get top-level links.
...
def _get_queries(actions: List[Dict[str, Any]]) -> Generator[dict[str, Any]]:
