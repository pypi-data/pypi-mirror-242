...
from collections.abc import Iterable, AsyncGenerator, Mapping, Callable
...
class MockS3(S3, MockDatabase):
    ...
    async def get_credentials_from_volume(self, request: Request, volume_id: Optional[str]) -> CredentialTypeVar | None:
        """
    ...
    async def get_accounts(self, request: Request, volume_ids: Iterable[str]) -> AsyncGenerator[tuple[AWSAccount, str], None, None]:
        for volume_id in volume_ids:
            yield self.get_account(request, volume_id), volume_id
...
class MockS3WithMockMongo(S3, MockMongo):
    ...
    async def get_file_system_and_credentials_from_volume(self, request: Request, volume_id: str | None) -> tuple[
        AWSFileSystem, CredentialTypeVar | None]:
        """
