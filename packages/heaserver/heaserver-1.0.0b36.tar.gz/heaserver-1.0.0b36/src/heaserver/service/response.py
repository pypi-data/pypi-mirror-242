...
async def get_multiple_choices(request: web.Request,
    ...
    The format of the links and desktop object in the response body depends on the the result of server-side content
    negotiation using the request's Accept header and the REST endpoint's supported content types. The default content
    type is Collection+JSON.
    ...
    otherwise correct request will result in a successful response with a 300 status code and an empty body or a
    body with just the options for opening the requested object, depending on the response body format negotiated
    between the client and server. To return an error status code, use the status_not_found() call or similar instead
    of this function.
