    ...
    async with session_get_with_retry(url, headers=headers_, params=query_params,
        ...
        if response_.status == 404:
            ...
            if result_len != 1:
                raise ValueError(f'Result from {url} has {result_len} values')
            if obj_ is None:
                obj_ = desktop_object_from_dict(result[0])
            else:
...
async def get_resource_url(app: web.Application, type_or_type_name: Union[str, Type[root.HEAObject]],
    ...
    :return: a URL string.
    :raises ValueError: if no resource URL was found in the registry or there was a problem accessing the registry
    service.
    """
    ...
    file_system_name_ = DEFAULT_FILE_SYSTEM if file_system_name is None else file_system_name
    type_name_ = type_or_type_name_to_type(type_or_type_name).get_type_name()
    result = component.get_resource_url(type_name_, file_system_name=file_system_name_, parameters=parameters, kwargs=kwargs) if component is not None else None
    if result is None:
        raise ValueError(f'No resource in the registry for type {type_or_type_name}, file system type {file_system_type_or_type_name}, and file system name {file_system_name_}')
    return result
...
async def get_property(app: web.Application, name: str,
    ...
    """
    Gets the Property with the given name from the HEA registry service.  It retries when connection errors occur.
    ...
    """
    return await get(app, URL(app[appproperty.HEA_REGISTRY]) / 'properties' / 'byname' / name,
                     Property,
                     client_session=client_session)
