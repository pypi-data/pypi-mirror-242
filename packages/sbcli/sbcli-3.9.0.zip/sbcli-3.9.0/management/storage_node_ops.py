...
def replace_node(kv_store, old_node_name, iface_name):
    ...
    if old_node is None:
        logger.error("Old node was not found in the cluster")
        exit(1)

    logger.info("Old node found: %s in state: %s", old_node.hostname, old_node.status)
    if old_node.status != StorageNode.STATUS_OFFLINE:
        logger.error("Node is not in offline state")
        exit(1)

    logger.info("Setting old node status to removed")
...
def get_device_capacity(device_id, history):
    ...
    for record in get_device_capacity_data(device_id, history):
        out.append({
            "Date": record['date'],
            "Drive Size": utils.humanbytes(record['drive_size']),
            "Provisioned": utils.humanbytes(record['max_size']),
            "Util": utils.humanbytes(record['util']),
...
def get_node_capacity(node_id, history):
    ...
    for line in data:
        out.append({
            "Name": line['name'],
            "Drive Size": utils.humanbytes(line["drive_size"]),
            "Provisioned": utils.humanbytes(line["max_size"]),
            "Util": utils.humanbytes(line["util"]),
