...
class BaseModel(PydanticBaseModel, Generic[T]):
    ...
    @classmethod
    def from_model(cls, model: Optional[T] = None) -> Optional[T]:
        if not model:
            return None
        return cls.from_orm(model)

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]] = None) -> Optional[T]:
        if not obj:
            return None
        return cls.parse_obj(obj)

    @classmethod
    def from_json(cls, json: Optional[str] = None) -> Optional[T]:
        if not json:
            return None
