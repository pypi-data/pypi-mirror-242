...
class MacalVm:
    ...
    def filter_record(self, result: list, record: dict, fields: dict):
        rrec = {}
        for field in fields.keys():
            if field not in record.keys():
                rrec[fields[field]] = AstNodetype.NIL
            else:
                rrec[fields[field]] = record[field]
        result.append(rrec)

    def null_record(self, result: list, fields: dict):
        rrec = {}
        for field in fields.keys():
            rrec[fields[field]] = AstNodetype.NIL
        result.append(rrec)
    ...
    def Execute(self):
        while True:
            #print(convertToHexAddr(self.rip.value), ":", self.memory[self.rip.value])
            ...
            if   opCode[0] == 9 : # PUSH
                ...
                while len(self.stack) > self.rsp.value and len(self.stack) > 2:
                    #print("reducing stack size to: ", self.rsp.value, " from: ", len(self.stack), " at: ", convertToHexAddr(self.rip.value-1))
            ...
            # Need to run a sensus in more applications to optimize the order.
                ...
                for var in self.scope.variables:
                    if var.stack_offset < len(self.stack):
                        ...
                        self.stack[var.scope_offset-self.rbp.value+var.stack_offset] = (var.node_type, var.value)
