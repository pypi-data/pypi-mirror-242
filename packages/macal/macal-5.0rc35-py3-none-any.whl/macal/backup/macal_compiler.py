...
class MacalCompiler:
    ...
    def compile_reserved_variables(self, scope: CompilerScope) -> None:
        ...
        if self.reserved_vars is None:
            return
        for var in self.reserved_vars:
            scope.SymbolTable.append((var.strip(), len(scope.SymbolTable)))
    ...
    def compile_block(self, block: AstNodeBlock, scope: CompilerScope, make_child: bool = False) -> None:
        self.emit_new_label("block", scope)
        for statement in block.statements:
            self.compile_statement(statement, scope)
        self.emit_new_label("block_end", scope)
    ...
    def emit_scope(self, scope: CompilerScope, label: Optional[int] = None) -> CompilerScope:
        ...
        if scope is None:
            ...
            if scope.break_label is not None:
                child.break_label = label
            if scope.continue_label is not None:
                child.continue_label = label
            if scope.exit_label is not None:
                child.exit_label = label
        return child

    def emit_scope_end(self, scope: CompilerScope) -> None:
        self.emitter.MOVRR(self.cs.rsp, self.cs.rbp)
        self.emitter.POP(self.cs.rbp)
        if scope.parent is not None and scope.parent.break_label is not None:
            self.emit_jump(Opcode.JMPBREAK, scope.parent.break_label, scope)
        else:
            self.emitter.RSTBREAK()
        if scope.parent is not None and scope.parent.continue_label is not None:
            self.emit_jump(Opcode.JMPCONTINUE, scope.parent.continue_label, scope)
        else:
            self.emitter.RSTCONTINUE()
        if scope.parent is not None and scope.parent.exit_label is not None:
            self.emit_jump(Opcode.JMPRETURN, scope.parent.exit_label, scope)
        else:
            self.emitter.RSTRETURN()
        scope.discard()
    ...
    def emit_jump(self, opcode: int, label: int, scope: CompilerScope) -> None:
        ...
        if opcode == Opcode.JMP:
            ...
            self.emitter.CONTINUE(label)
        elif opcode == Opcode.BREAK:
            self.emitter.BREAK(label)
        elif opcode == Opcode.RETURN:
            self.emitter.RETURN(label)
        elif opcode == Opcode.JMPCONTINUE:
            self.emitter.JMPCONTINUE(label)
        elif opcode == Opcode.JMPBREAK:
            self.emitter.JMPBREAK(label)
        elif opcode == Opcode.JMPRETURN:
            self.emitter.JMPRETURN(label)
        else:
            self.error(f"Unknown jump opcode {opcode} at {self.cs.rip.value}")

    # expression helper functions.

    def var_to_stack(self, var: MacalVariable, scope: CompilerScope) -> None:
        if var.arg:
    ...
    def compile_if_statement(self, statement: AstNodeIfStatement, scope: CompilerScope) -> None:
        ...
        if_end = scope.get_new_label("if_end")
        if_scope = self.emit_scope(scope, if_end)
        
        self.compile_expression(statement.condition, if_scope)
        label_for_JMPNZ_in_if = if_scope.get_new_label("label_jnz")
        self.emit_condition(label_for_JMPNZ_in_if, Opcode.JMPNZ, scope)
        self.compile_block(statement.block, if_scope)
        self.emit_jump(Opcode.JMP, if_end, if_scope)
        ...
        if statement.elif_block_list is not None:
            ...
            if count > 0:
                for elif_statement in statement.elif_block_list:
                    self.compile_elif_statement(elif_statement, if_end, if_scope)
        if statement.else_block is not None and len(statement.else_block.statements) > 0:
            self.compile_else_statement(statement.else_block, if_scope)
        scope.set_label_address(if_end, self.cs.rip.value)
        self.emit_scope_end(if_scope)
    ...
    def compile_foreach_statement(self, statement: AstNodeForeachStatement, scope: CompilerScope) -> None:
        ...
        foreach_scope.SymbolTable.append(("it", len(foreach_scope.SymbolTable)))
    ...
    def compile_continue_statement(self, statement: AstNodeContinueStatement, scope: CompilerScope) -> None:
        ...
        self.emit_jump(Opcode.CONTINUE, scope.continue_label, scope)
    ...
    def compile_break_statement(self, statement: AstNodeBreakStatement, scope: CompilerScope) -> None:
        ...
        self.emit_jump(Opcode.BREAK, scope.break_label, scope)
    ...
    def compile_return_statement(self, statement: AstNodeReturnStatement, scope: CompilerScope) -> None:
        ...
        self.emit_jump(Opcode.RETURN, scope.exit_label, scope)
    ...
    def compile_debug_variable_statment(self, statement: AstNodeDebugVariableStatement, scope: CompilerScope) -> None:
        var: MacalVariable = scope.get_variable(statement.expr.token_value)
        if var is None:
            self.error(f"Unknown variable {statement.expr.token_value} at {statement.line}:{statement.column} scope({scope.name})")        
        self.emitter.DBGVAR(name=statement.expr.token_value, stack_index=var.stack_index, stack_offset=var.stack_offset)
    ...
    def compile_indexed_variable_assignment(
        self, statement: AstNodeAssignmentStatement, var: MacalVariable, scope: CompilerScope
    ) -> None:
    ...
    def compile_assign_indexed(self, op: LexToken, var: MacalVariable, rhs: AstNodeExpression, scope: CompilerScope) -> None:
        if var.const:
    ...
    def compile_function_call(self, statement: AstNodeFunctionCallStatement, scope: CompilerScope) -> None:
        ...
        self.emitter.CALL(func.start_address, self.cs.rbp)
    ...
    def compile_variable_function_call(
        self, statement: AstNodeFunctionCallStatement, var: MacalVariable, scope: CompilerScope
    ) -> None:
    ...
    def compile_function_definition(self, statement: AstNodeFunctionDefinition, scope: CompilerScope) -> None:
        func = scope.define_function(statement.name, self.cs.rip.value + 1, self.cs.rsp.value, self.cs.rbp.value)  # +1 for the jump instruction.
        if func is None:
        ...
        for i, param in enumerate(statement.parameters):
            ...
            #self.emitter.DEBUGSTR(f"Parameter {param.name} at offset {offset}, rbp: {self.cs.rbp.value}, rsp: {self.cs.rsp.value}, scope: {function_scope.name}, {function_scope.rbp_offset}")
            #self.emitter.STACK()
            v = function_scope.define_variable(param.name, offset, self.cs.rsp.value)
            v.arg = True
        ...
        if statement.return_type is not None:
            ...
            rv = function_scope.define_variable("return_value", FUNCTION_CALL_RETURN_VALUE_STACK_OFFSET, self.cs.rsp.value)
            rv.arg = True
            #print(rv)
