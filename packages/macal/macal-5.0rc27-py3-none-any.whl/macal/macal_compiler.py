...
class MacalCompiler:
    def __init__(
        ...
        verbose: bool = False,
        debug: bool = False,
        reserved_vars: Optional[List[str]] = None,
    ) -> MacalCompiler:
    ...
    def compile_reserved_variables(self, scope: CompilerScope) -> None:
        ...
        if self.reserved_vars is None:
            return
        offset = self.cs.rsp.value - self.cs.rbp.value
    ...
    def var_to_stack(self, var: CTVariable, scope: CompilerScope) -> None:
        self.emitter.LOADR(self.cs.rax, self.cs.rbp, var.scope_offset-scope.rbp_offset+var.stack_offset)  # load rax, [rbp + stack_offset]
    ...
    def compile_foreach_statement(self, statement: AstNodeForeachStatement, scope: CompilerScope) -> None:
        ...
        it = foreach_scope.define_variable("it", self.cs.rsp.value - self.cs.rbp.value)
        ...
        self.emitter.LOADR(self.cs.rax, self.cs.rbp, 1)  # load rax, [rbp + 1] # rax is now the length
        self.emitter.LOADR(self.cs.rbx, self.cs.rbp, 2)  # load rbx, [rbp + 2] # rbx is now index
        self.emitter.CMPRR(self.cs.rax, self.cs.rbx)  # cmprr rax, rbx
        self.emit_jump(
            Opcode.JMPZ, foreach_end_label, scope.root
        )  # jmpz foreach_end_label if rax == rbx (we are at the end of the array)
        self.emitter.LOADR(self.cs.rax, self.cs.rbp, 0)  # load rax, [rbp + 0] # rax is now the array
        self.emitter.FEINDEX()  # rax = rax[rbx]      # rax is now the value for the variable it
        self.emitter.STORR(self.cs.rbp, it.scope_offset-foreach_scope.rbp_offset+it.stack_offset, self.cs.rax)  # it is now the value of the array at the current index
        ...
        self.emitter.LOADR(self.cs.rax, self.cs.rbp, 2)  # load rax, [rbp + 2] # rax is now the index
        self.emitter.INC(self.cs.rax)  # inc rax rax = rax + 1
        self.emitter.STORR(self.cs.rbp,2, self.cs.rax)  # stor [rbp + 2], rax increased the index
    ...
    def compile_halt_statement(self, statement: AstNodeHaltStatement, scope: CompilerScope) -> None:
        ...
        self.compile_expression(statement.expr, scope)
        self.emitter.POP(self.cs.rax)  # pop rax
        self.emitter.STORR(self.cs.rbp,-self.cs.rbp.value, self.cs.rax)  # stor [0], rax -> rax is the exit code
        self.emitter.HALT()
    ...
    def compile_assignment_statement(self, statement: AstNodeAssignmentStatement, scope: CompilerScope) -> None:
        ...
        if var is None:  # we go assign a new var.
            var = scope.define_variable(statement.name, self.cs.rsp.value - self.cs.rbp.value)
        ...
        if statement.op.token_type == AstNodetype.OPERATOR_ASSIGNMENT:
            if statement.append is True:
                ...
                self.emitter.LOADR(self.cs.rax, self.cs.rbp, var.scope_offset-scope.rbp_offset+var.stack_offset)  # load rax, [rbp + stack_offset] rax is now the variable.
                self.emitter.APPEND(self.cs.rax, reg)  # append rax, reg
                self.emitter.STORR(self.cs.rbp, var.scope_offset-scope.rbp_offset+var.stack_offset, self.cs.rax)  # stor [rbp + stack_offset], rax
...
# Select compiler
    ...
    def compile_fields(self, statement: AstNodeSelectStatement, source_data_register: BytecodeRegister, scope: CompilerScope):
        ...
        if len(statement.Fields) == 1 and statement.Fields[0].name == "*":
            return
        self.emitter.MOVRI(self.cs.rax, AstNodetype.RECORD, {key.name:key.altfieldname for key in statement.Fields})
