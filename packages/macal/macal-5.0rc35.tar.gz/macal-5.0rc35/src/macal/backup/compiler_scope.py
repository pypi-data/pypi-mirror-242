...
class CompilerScope:
    ...
    def define_variable(self, name: str, stack_index: int, stack_offset: int = 0) -> Optional[MacalVariable]:
    ...
    def get_variable(self, name: str) -> Optional[Union[MacalVariable, CTFunction]]:
    ...
    def find_variable(self, name: str) -> Optional[MacalVariable]:
        var = next((var for var in self.variables if var.name == name), None)
        if var is not None:
            return var
        if self.parent is not None and self.can_search_parent:
    ...
    def define_function(self, name: str, function_start_address: int, rsp: int, rbp: int) -> Optional[CTFunction]:
