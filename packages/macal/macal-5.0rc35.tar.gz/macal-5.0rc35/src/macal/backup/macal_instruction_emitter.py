...
class MacalInstructionEmitter:
    def __init__(self, memory, rip: BytecodeRegister, rsp: BytecodeRegister, rbp: BytecodeRegister, stack: deque) -> MacalInstructionEmitter:
        ...
        self.srsp: int = rsp.value
        self.srbp: int = rbp.value
    ...
    def POP(self, reg: BytecodeRegister, decrease_stackpointer: bool = True) -> None:
        self.memory[self.rip.value] = (10, reg.opcode)
        self.rip.value += 1
        if decrease_stackpointer:
            self.rsp.value -= 1
            self.srsp -= 1
        else:
            self.srsp -= 1
        ...
        if reg.name == "RBP":
            if decrease_stackpointer:
                ...
                self.srbp = self.rbp.value
            else:
                self.srbp = self.stack[self.srsp]
                print("biep")
            

    def MOVRR(self, reg1: BytecodeRegister, reg2: BytecodeRegister, decrease_stackpointer: bool = True) -> None:
    ...
    def CALL(self, addr: int, reg: BytecodeRegister) -> None:
        self.memory[self.rip.value] = (44, addr, reg.opcode)
        self.rip.value += 1
    ...
    def LOAD(self, reg: BytecodeRegister, stack_index: int) -> None:
        self.memory[self.rip.value] = (56, reg.opcode, stack_index)
        self.rip.value += 1

    def STOR(self, stack_index: int, reg: BytecodeRegister) -> None:
        self.memory[self.rip.value] = (57, stack_index, reg.opcode)
        self.rip.value += 1
    ...
    def DBGVAR(self, name: str, stack_index: int, stack_offset: int = 0):
        self.memory[self.rip.value] = (87, name, AstNodetype.INTEGER, stack_index, AstNodetype.INTEGER, stack_offset)
        if stack_offset == 0 and stack_index < 0:
            raise ValueError("stack_offset can't be 0 for stack_index < 0")
        self.rip.value += 1

    def BREAK(self, addr: int):
        self.memory[self.rip.value] = (88, addr)
        self.rip.value += 1

    def CONTINUE(self, addr: int):
        self.memory[self.rip.value] = (89, addr)
        self.rip.value += 1

    def RETURN(self, addr: int):
        self.memory[self.rip.value] = (90, addr)
        self.rip.value += 1
    
    def JMPBREAK(self, addr: int):
        self.memory[self.rip.value] = (91, addr)
        self.rip.value += 1

    def JMPCONTINUE(self, addr: int):
        self.memory[self.rip.value] = (92, addr)
        self.rip.value += 1

    def JMPRETURN(self, addr: int):
        self.memory[self.rip.value] = (93, addr)
        self.rip.value += 1

    def RSTBREAK(self):
        self.memory[self.rip.value] = (94,)
        self.rip.value += 1
    
    def RSTCONTINUE(self):
        self.memory[self.rip.value] = (95,)
        self.rip.value += 1

    def RSTRETURN(self):
        self.memory[self.rip.value] = (96,)
        self.rip.value += 1
