...
class MacalCompiler:
    ...
    def compile_debug_variable_statment(self, statement: AstNodeDebugVariableStatement, scope: CompilerScope) -> None:
        var = scope.get_variable(statement.expr.token_value)
        if var is None:
            self.error(f"Unknown variable {statement.expr.token_value} at {statement.line}:{statement.column} scope({scope.name})")        
        print("DEBUG VARIABLE", var.name, var.scope_offset, var.stack_offset)
        self.emitter.DBGVAR(name=statement.name, scope_offset=var.scope_offset, stack_offset=var.stack_offset, rbp_offset=scope.rbp_offset)
