    ...
    def Execute(self):
        while True:
            ...
            # Need to run a sensus in more applications to optimize the order.
                ...
                name = opCode[1]
                stack_offset = opCode[3]
                scope_offset = opCode[5]
                rbp_offset = opCode[7]
                print(f"VAR: {name}, stack_offset {stack_offset}, scope_offset {scope_offset}, rbp_offset {rbp_offset}")
                print(f"     var LOAD formula: RBP + scope_offset ({stack_offset}) - rbp_offset ({rbp_offset}) + stack_offset ({scope_offset}) = {self.rbp.value + stack_offset-rbp_offset+scope_offset}")
                print(f"     stack at that index: {self.stack[self.rbp.value + stack_offset-rbp_offset+scope_offset]}")
                ShowStack(self.stack, self.rsp, self.rbp)
            else: break
