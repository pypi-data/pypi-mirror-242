...
def GetDevicesLatency(org_id) -> None:
    ...
    try:
        ...
        LastErrorMessage = (
            f"Get Device Uplink Loss and Latency: {error.operation}: {error.status}, {error.message}, {error.reason}"
        )
    except Exception as ex:
...
def GetDeviceManagementInterface(serial) -> None:
    ...
    try:
        ...
        LastErrorMessage = (
            f"Get Device Management Interface Settings: {error.operation}: {error.status}, {error.message}, {error.reason}"
        )
    except Exception as ex:
...
def GetApplianceUplinkStatuses(org_id) -> None:
    ...
    try:
        ...
        LastErrorMessage = (
            f"Get Organization Appliance Uplink Statuses: {error.operation}: {error.status}, {error.message}, {error.reason}"
        )
    except Exception as ex:
...
def GetAppPerformance(serial) -> None:
    ...
    try:
        ...
        if error.status == 400:
            LastErrorMessage = (
                f'Get device appliance performance: {error.operation}: {error.status}, {serial} {error.message["errors"][0]}'
            )
        elif error.status != 404:
            LastErrorMessage = (
                f"Get device appliance performance: {error.operation}: {error.status}, {error.message}, {error.reason}"
            )
...
def GetApplianceUplinkUsageHistory(network_id) -> None:
    ...
    try:
        ...
        LastErrorMessage = (
            f"Get Appliance Uplink Usage History: {error.operation}: {error.status}, {error.message}, {error.reason}"
        )
    except Exception as ex:
...
def Output(*params) -> None:
    ...
    v = params[1]
    if v is not None and v != "" and v != {} and v != [] and v != "nil" and v != "NIL":
        print(params[0])
        print(v)
...
def Header(*params) -> None:
    ...
    serial = params[3]
    template = params[4]
    if model is None or model == "nil" or model == "NIL":
        model = ""
    if serial is None or serial == "nil" or serial == "NIL":
        serial = ""
    if template is None or template == "nil" or template == "NIL":
        template = ""
