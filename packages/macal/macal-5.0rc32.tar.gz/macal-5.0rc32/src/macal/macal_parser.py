...
from .ast_node_debug_registers_statement import AstNodeDebugRegistersStatement
from .ast_node_debug_stack_statement import AstNodeDebugStackStatement
...
class Parser:
    ...
    def parse(self) -> AstNodeExpression:
    ...
    def statement(self) -> AstNodeExpression:
        ...
        if self.current_token.token_type == AstNodetype.DEBUG_VARIABLE_STATEMENT:
            return self.debug_variable_statement()
        if self.current_token.token_type == AstNodetype.DEBUG_REGISTERS_STATEMENT:
            return self.debug_registers_statement()
        if self.current_token.token_type == AstNodetype.DEBUG_STACK_STATEMENT:
            return self.debug_stack_statement()
    ...
    def debug_variable_statement(self) -> AstNodeExpression:
        token = self.current_token
        self.expect(AstNodetype.DEBUG_VARIABLE_STATEMENT)
        node = AstNodeDebugVariableStatement(token)
        node.expr = self.parse_expression()
        if node.expr.expr_type != AstNodetype.VARIABLE_EXPRESSION:
            self.error(f"Invalid debug variable expression type {node.expr.expr_type.name.lower()} ({node.expr.line}, {node.expr.column})")
        self.expect(AstNodetype.SEMICOLON)
        return node
    
    def debug_registers_statement(self) -> AstNodeExpression:
        token = self.current_token
        self.expect(AstNodetype.DEBUG_REGISTERS_STATEMENT)
        node = AstNodeDebugRegistersStatement(token)
        self.expect(AstNodetype.SEMICOLON)
        return node
    
    def debug_stack_statement(self) -> AstNodeExpression:
        token = self.current_token
        self.expect(AstNodetype.DEBUG_STACK_STATEMENT)
        node = AstNodeDebugStackStatement(token)
        self.expect(AstNodetype.SEMICOLON)
        return node
    ...
    def type_statement(self) -> AstNodeExpression:
        ...
        return AstNodeTypeStatement(token, self.parse_expression())
