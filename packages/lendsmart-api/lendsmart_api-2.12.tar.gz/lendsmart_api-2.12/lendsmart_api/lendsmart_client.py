...
from lendsmart_api.objects import (
    PredictionWorkflow,
    LendingTerm,
    PredictionInference,
    PredictionSegment,
...
class Group:
    ...
    def raise_api_error(self, message="", status=200, json={}):
        """
        This function raises the api error ApiError
        """
        raise ApiError(message=message, status=status, json=json)

    def raise_unexpected_response(self, message="", status=200, json={}):
        """
        This function will raise the exception of type UnexpectedResponseError
        """
        raise UnexpectedResponseError(message=message, status=status, json=json)
...
class PredictionGroup(Group):
    def segment_create(self, label=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating Prediction Segment " "Client!",
                json=result,
            )
    ...
    def inference_create(self, label=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating Prediction Inference " "Client!",
                json=result,
            )
    ...
    def workflow_create(self, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating Longivew " "Client!", json=result
            )
    ...
    def workflow_describe(self, label=None):
        ...
        if not "items" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting PredictionWorkflow" "Client!",
                json=result,
            )
    ...
    def workflow_update_status(self, did=None, updated_at=None, param=None):
        ...
        result = self.client.put(
            "/prediction_workflows/{}/status?updated_at={}".format(did, updated_at),
            data=param,
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating PredictionWorkflow" "Client!",
                json=result,
            )
        c = PredictionWorkflow(self.client, result["id"], result)
        return c
...
class LendingTerms(Group):
    ...
    def lending_term_get(self, label=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting LendingTerm" "Client!", json=result
            )
        return result
...
class Documents(Group):
    ...
    def document_get(self, label=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting Document" "Client!", json=result
            )
    ...
    def document_update_status(self, did=None, updated_at=None, param=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating Document" "Client!", json=result
            )
        c = Document(self.client, result["id"], result)
        return c
    ...
    def document_pointer_create(self, data=None):
        ...
        result = self.client.post(
            "/" + self.client.get_namespace() + "/document_pointers", data=data
        )

        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating DocumentPointer" "Client!",
                json=result,
            )
...
class NotifiersGroup(Group):
    ...
    def notifier_create(self, label, data=None):
        ...
        notifier_url = (
            ...
            + "{}/{}".format(LENDSMART_NOTIFIER_ROOT, label)
        )
        result = self.client.post(notifier_url, data=data)

        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating notifier " "Client!", json=result
            )
...
class AccountSignGroup(Group):
    def account_sign_update(self, id, param=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting AccountSign" "Client!", json=result
            )
    ...
    def signature_get(self, label=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting Signature" "Client!", json=result
            )
...
class FreddieMacGroup(Group):
    def authenticate_freddiemac(self):
        ...
        if equals(path_or("", ["status"], result), "Failure"):
            raise UnexpectedResponseError(
                "Unexpected response when authentiation freddiemac" "Client!",
                json=result,
            )
        return result
    ...
    def run_lpa(self, loan_id, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when submitting loan to freddiemac" "Client!",
                json=result,
            )
        return result
    ...
    def poll_request_id(self, request_id, data=None):
        ...
        """
        result = self.client.post(
            FREDDIEMAC_RUN_LPA_ROOT + "/polls/" + request_id, data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when polling request id from freddiemac" "Client!",
                json=result,
            )
        return result
...
class TaskGroup(Group):
    def create_task(self, data=None):
        ...
        """
        result = self.client.post(
            "/" + self.client.get_namespace() + LENDSMART_TASK_CREATE_ROOT, data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating task" "Client!", json=result
            )

        # c = Task(self.client, result['id'], result)
        return result

    def update_task_by_namespace(self, data=None, push_event="yes"):
        """
        Updates the particular task
        """
        task_id = path_or("", ["id"], data)
        task_update_url = (
            "/"
            + self.client.get_namespace()
            + LENDSMART_TASK_CREATE_ROOT
            + "/"
            + task_id
            + "?push_event="
            + push_event
        )
        result = self.client.put(task_update_url, data=data)
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating task" "Client!", json=result
            )
        return result
    ...
    def update_provider_task(self, id, data=None):
        ...
        """
        result = self.client.put(
            LENDSMART_TASK_CREATE_ROOT + "/provider/" + id, data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating task" "Client!", json=result
    ...
    def get_task_by_provider_id(self, provider_id):
        ...
        result = self.client.get(
            LENDSMART_TASK_CREATE_ROOT + "/provider/" + provider_id
        )
        if self.is_not_found(result):
            return {}
        ...
        if not "id" in result or is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when describing task" "Client!", json=result
            )
        return result
...
class OpenidsGroup(Group):
    def create_openid(self, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating openid" "Client!", json=result
            )
    ...
    def update_openid(self, data=None):
        ...
        """
        result = self.client.put(
            LENDSMART_OPENID_CREATE_ROOT + path_or("", ["id"], data), data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating openid" "Client!", json=result
            )
        return result
    ...
    def get_openid_by_role_id(self, role_id, provider):
        ...
        """
        result = self.client.get(
            LENDSMART_OPENID_CREATE_ROOT + "/describe/" + role_id + "/" + provider
        )
        ...
        if not "items" in result or is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when describing openid" "Client!", json=result
            )
    ...
    def get_openid_by_account_id(self, account_id):
        ...
        """
        result = self.client.get(
            LENDSMART_OPENID_CREATE_ROOT + "/accounts/" + account_id
        )
        if self.is_not_found(result):
            return {}
...
class NamespaceGroup(Group):
    def create_namespace(self, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating namespace" "Client!", json=result
            )
    ...
    def update_namespace(self, data=None):
        ...
        """
        result = self.client.put(
            LENDSMART_NAMESPACE_ROOT + path_or("", ["id"], data), data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating namespace" "Client!", json=result
            )
        return result
    ...
    def get_namespace_by_id(self, namespace_id):
        ...
        if is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when describing namespace" "Client!", json=result
            )
        return result
...
class LoanappGroup(Group):
    def create_loanapp(self, data=None, params={}):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating loanapp" "Client!", json=result
            )

        return result

    def create_v2(self, data=None, params={}):
        """
        Creates a prime loanapp
        """
        api_url = f"/{self.client.get_namespace()}{LENDSMART_LOANAPP_ROOT}/v2"
        if params:
            api_url += f"?{urlencode(params)}"
        result = self.client.post(api_url, data=data)
        if not "id" in result:
    ...
    def update_loanapp(self, data=None):
        ...
        """
        result = self.client.put(
            LENDSMART_LOANAPP_ROOT + "/" + path_or("", ["id"], data), data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating loanapp" "Client!", json=result
            )
        return result
    ...
    def get_loanapp_by_id(self, loanapp_id):
        ...
        """
        result = self.client.get(
            f"/{self.client.get_namespace()}{LENDSMART_LOANAPP_ROOT}/{loanapp_id}"
        )
        if is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when describing loanapp" "Client!", json=result
            )
        return result
    ...
    def get_all_short_loanapp(self, data=None):
        ...
        """
        url = f"/{self.client.get_namespace()}{LENDSMART_LOANAPP_ROOT}/all/short"
        result = self.client.post(url, data=data)
        if is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when getting loanapp", json=result
            )

        return result

    def get_all_short_loanapp_v2(self, data=None):
        """
        fetch the loanapps with filters V2
        """
        url = f"/{self.client.get_namespace()}{LENDSMART_LOANAPP_ROOT}/v2/all/short"
        result = self.client.post(url, data=data)
        if is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when getting loanapp", json=result
...
class LoanappRoleGroup(Group):
    def create_loanapprole(self, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating loanapp role" "Client!", json=result
            )
        return result
...
class LoanstatusGroup(Group):
    def describe_loan_status(self, loanapp_id):
        ...
        if not "items" in result:
            raise UnexpectedResponseError(
                "Unexpected response when describe loanstatus" "Client!", json=result
            )
        return result
    ...
    def update_loan_status(self, namespace, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating loanstatus" "Client!", json=result
            )
        return result
    ...
    def get_loan_status_by_id(self, loan_status_id):
        ...
        if is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when get loanstatus" "Client!", json=result
            )
        return result
...
class LetterOfExplanationGroup(Group):
    def create_loe(self, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating letter of explanantion" "Client!",
                json=result,
            )
    ...
    def update_loe(self, data=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating letter of explanantion" "Client!",
                json=result,
            )
        return result
    ...
    def get_loe_by_loanid(self, loan_id):
        ...
        """
        result = self.client.get(
            LENDSMART_LETTER_OF_EXPLANATION_ROOT + "/describe/" + loan_id
        )
        if self.is_not_found(result):
            return {}
        ...
        if not "items" in result or is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when getting letter of explanantion" "Client!",
                json=result,
            )
...
class LoanDocumetGroup(Group):
    def update(self, id, param=None):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating LoanDocuments" "Client!", json=result
            )
...
class EventBridgeGroup(Group):
    def create(self, namespace, data=None):
        result = self.client.post(
            "/{}".format(namespace) + LENDSMART_EVENT_BRIDGE_CREATE_ROOT, data=data
        )
        if not "event" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating advisor profile" "Client!",
                json=result,
            )
        return result
...
class AdvisorProfileGroup(Group):
    ...
    def update(self, data=None):
        result = self.client.put(
            LENDSMART_ADVISOR_PROFILE_CREATE_ROOT + "/" + data["id"], data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating letter of explanantion" "Client!",
    ...
    def get_advisor_profile_by_email(self, email):
        result = self.client.get(
            LENDSMART_ADVISOR_PROFILE_CREATE_ROOT + "/email/" + email
        )
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting advisor profile by email id"
                "Client!",
                json=result,
            )
        return result
    ...
    def get_advisor_profile_by_user_name(self, name):
        ...
        """
        result = self.client.get(
            LENDSMART_ADVISOR_PROFILE_CREATE_ROOT + "/name/" + name
        )
        if self.is_not_found(result):
            return {}
        ...
        if not "id" in result or is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when describing advisor" "Client!", json=result
            )
...
class TeamMemberGroup(Group):
    def get_team_member_by_id(self, team_member_id):
        ...
        if not "id" in result or is_empty(result):
            raise UnexpectedResponseError(
                "Unexpected response when creating team member" "Client!", json=result
            )
        return result
    ...
    def update_team_member(self, data=None):
        print("data", data)
        result = self.client.put(
            LENDSMART_TEAM_MEMBER_ROOT + "/" + data["id"], data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating team member" "Client!", json=result
            )
        return result

    def create_team_member(self, data=None):
        result = self.client.post(
            "/" + self.client.get_namespace() + LENDSMART_TEAM_MEMBER_ROOT, data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating team member" "Client!", json=result
...
class AccountsGroup(Group):
    ...
    def account_get(self, label=None):
        ...
        # result = self.client.get('/{}/accounts/{}'.format(namespace, label))
        result = self.client.get("/accounts/{}".format(label))
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when getting Account" "Client!", json=result
            )
...
class OriginationLoanPerimissionGroup(Group):
    def create_originating_loan_permission(self, data):
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when get origination loan permission" "Client!",
                json=result,
            )
        return result
    ...
    def update_loan_permission(self, data):
        result = self.client.put(
            LENDSMART_ORIGINATION_LOAN_PERMISSIONS + "/" + data["id"], data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating origination loan permission"
                "Client!",
                json=result,
            )
        return result
    ...
    def get_originating_loan_permission_by_advisor_id(self, advisor_id):
        result = self.client.get(
            LENDSMART_ORIGINATION_LOAN_PERMISSIONS + "/advisor/" + advisor_id
        )
...
class BusinssGroup(Group):
    def describe_business(self, owner_id):
        ...
        if not "items" in result:
            raise UnexpectedResponseError(
                "Unexpected response when get business record" "Client!", json=result
            )
    ...
    def update_business(self, data):
        result = self.client.put(
            LENDSMART_BUSINESS_ROOT + "/" + path_or("", ["id"], data), data=data
        )
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when updating business" "Client!", json=result
            )
        return result
...
class DeclarationGroup(Group):
    """
    This class contains functions for declarations api
    """

    def describe(self, entity_id: str):
        """
        This function will describe the declaration by the provided
        id
        The id can be loan app id or loan role id
        ...
        if not entity_id:
            return self.raise_api_error(
                message="Entity id is empty, please provide the loan app id or loan app role id",
                status=400,
                json={},
            )

        result = self.client.get(LENDSMART_DECLARATIONS + "/describe/" + entity_id)

        if self.is_not_found(result):
            return {}

        if not "items" in result:
            return self.raise_unexpected_response(
                message="Unexpected response received from server, when decribing the declarations for enity id "
                + str(entity_id),
                status=500,
                json=result,
            )
...
class LendsmartClient:
    def __init__(self, service_account, base_url, user_agent=None):
        ...
        self.declaration = DeclarationGroup(self)
    ...
    def is_valid_404(self, response):
        """
        This function will check does the response contains
        a 404 status code.
        There are 2 posibilities to get 404.
        1. 404 from core api where the requested resource not found
        2. 404 from cluster says the url not found

        In this 2 cases we need to retry only for cluster 404
        so to find that , we are following a way , which is trying to
        convert the response to json.
        because the cluster 404 is not in json format.
        if the response is not json we return false so retry will happen.
        """
    ...
    @retry((ApiError, ConnectionError), tries=RETRY_COUNT, delay=RETRY_DELAY)
    def get_response(self, method, url, headers, body):
        ...
        if (
            ...
            and not self.is_valid_404(response)
            and RETRY_COUNT != self.retry_count
        ):
    ...
    def document_create(self, disk, label=None, description=None):
        ...
        if label is not None:
            params = {
                "object_meta": {"name": label["name"], "account": label["account"]},
        ...
        if not "id" in result:
            raise UnexpectedResponseError(
                "Unexpected response when creating an " "Document {}".format(disk)
            )
