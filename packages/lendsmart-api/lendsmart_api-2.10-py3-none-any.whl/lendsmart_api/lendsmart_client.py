...
class TaskGroup(Group):
    ...
    def update_task_by_namespace(self, data=None, push_event="yes"):
        ...
        task_update_url = "/" + self.client.get_namespace() + LENDSMART_TASK_CREATE_ROOT +"/" + task_id + "?push_event=" + push_event
...
class TeamMemberGroup(Group):
    ...
    def create_team_member(self, data=None):
        result = self.client.post('/'+ self.client.get_namespace() + LENDSMART_TEAM_MEMBER_ROOT,data=data)
        if not 'id' in result:
            raise UnexpectedResponseError('Unexpected response when creating team member'
                                          'Client!', json=result)
        return result
