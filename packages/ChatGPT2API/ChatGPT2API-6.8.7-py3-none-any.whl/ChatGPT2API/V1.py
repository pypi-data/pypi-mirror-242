...
class Chatbot:
    ...
    def __init__(
        ...
        if user_home is None:
            ...
            # mkdir ~/.config/ChatGPT2API
            if not Path(user_home, ".config").exists():
                Path(user_home, ".config").mkdir()
            if not Path(user_home, ".config", "ChatGPT2API").exists():
                Path(user_home, ".config", "ChatGPT2API").mkdir()
            self.cache_path = Path(user_home, ".config", "ChatGPT2API", "cache.json")
...
def configure() -> dict:
    ...
    if xdg_config_home := getenv("XDG_CONFIG_HOME"):
        config_files.append(Path(xdg_config_home, "ChatGPT2API/config.json"))
    if user_home := getenv("HOME"):
        config_files.append(Path(user_home, ".config/ChatGPT2API/config.json"))
    if windows_home := getenv("HOMEPATH"):
        config_files.append(Path(f"{windows_home}/.config/ChatGPT2API/config.json"))
