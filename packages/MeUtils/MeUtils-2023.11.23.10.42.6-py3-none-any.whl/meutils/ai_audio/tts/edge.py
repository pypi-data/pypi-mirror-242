...
# @WeChat       : meutils
# @Software     : PyCharm
# @Description  : 增加固定时长的生成，多进程

"""
for i, srtitem in enumerate(tqdm(pysrt.open('subtitle.srt'))):
    asyncio.run(tts(srtitem.text, filename=f"wav/{i:0>6}.wav"))
...
# @lru_cache()
async def tts(text, role: str = '云希', rate="+30%", filename: Optional[None] = None):
    ...
    communicate = edge_tts.Communicate(
        text or "文本为空", voice=SUPPORTED_VOICES.get(role, '陕西晓妮'),
        rate=rate
    )
