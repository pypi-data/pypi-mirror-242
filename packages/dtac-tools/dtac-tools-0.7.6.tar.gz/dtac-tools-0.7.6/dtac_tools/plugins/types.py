...
class EndpointResponse:
    ...
    def __setattr__(self, key, value):
        ...
        if key == '_api_endpoint_response':
            ...
            # Convert list to StringList
            string_list_value = {k: StringList(values=v) for k, v in value.items()}
            setattr(self._api_endpoint_response, f"set_{key}", string_list_value)
        elif key == 'value':
            ...
            if not isinstance(value, bytes):
                ...
                setattr(self._api_endpoint_response, f"set_{key}", serialized_value)
            else:
                setattr(self._api_endpoint_response, f"set_{key}", value)
        else:
            setattr(self._api_endpoint_response, f"set_{key}", value)
