...
class EndpointRequest:
    #
    # metadata: typing.Mapping[str, str] | None
    # headers: typing.Mapping[str, StringList] | None
    # parameters: typing.Mapping[str, StringList] | None
    # body: bytes | None

    def __init__(self, *args, **kwargs):
        self._api_endpoint_request = apiEndpointRequest(*args, **kwargs)
    ...
    def to_api_request(self):
        return self._api_endpoint_request

    @staticmethod
    def from_api_request(api_request: apiEndpointRequest):
...
class EndpointResponse:
    ...
    def __init__(self, *args, **kwargs):
        self._api_endpoint_response = apiEndpointResponse(*args, **kwargs)
    ...
    def __setattr__(self, key, value):
        #TODO: Remove me
        udp_sender = DebugSender(5678)
        udp_sender.write(f"Setting attribute {key} with value type: {type(value)}")
        udp_sender.close()
        print()
        if key == '_api_endpoint_response':
            ...
            # Convert list to StringList
            string_list_value = {k: StringList(values=v) for k, v in value.items()}
            setattr(self._api_endpoint_response, key, string_list_value)
        elif key == 'value':
            ...
            if not isinstance(value, bytes):
                ...
                setattr(self._api_endpoint_response, key, serialized_value)
            else:
    ...
    @staticmethod
    def from_api_response(api_response: apiEndpointResponse):
        # Create an SDK object from a gRPC object
