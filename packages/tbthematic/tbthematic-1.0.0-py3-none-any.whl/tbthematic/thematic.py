...
def tb_thematic_map(tiff_path,
    ...
    plot_colors = list()
    for unique_value in tiff_unique_list:
        plot_colors.append(all_colors[unique_value - 1])
    # 栅格图加载
    ...
    if quick_drawing:
        ...
        lon = np.linspace(img_info.overview_ulx, img_info.overview_lrx, img_info.overview_xsize)
        lat = np.linspace(img_info.overview_uly, img_info.overview_lry, img_info.overview_ysize)
        ax.contourf(lon, lat, arr_mask_nodata, extent=tiff_extent, transform=ccrs.PlateCarree(), cmap=cmap)
