...
class Teleport:
    def __init__(
        ...
        self.movement_vector_updater_script: Script | None = None
    ...
    def create_movement_vector_updater_script(self) -> Script:
...
class ShortLongSwitch(ABC):
    def __init__(self, key: str, short_press_max_delta: float = 0.3, context: dict | None = None):
        self._key = key
    ...
    def _on_press(self, _):
        ...
        self.on_press(self._context)

    def _on_release(self, _):
        if self._is_short_delta():
            self.on_short_press(self._context)
        else:
            self.on_long_press(self._context)
