...
def download_artifacts(
    ...
    fingerprints_url = f"{build.url}api/json?tree=fingerprint[fileName,hash]"
    log().debug("fetch artifact fingerprints from {fingerprints_url}")
    raw_fingerprints = [
        (fingerprint["fileName"], fingerprint["hash"])
        for fingerprint in client._session.get(fingerprints_url).json()["fingerprint"]
    ]

    # second: create new fingerprints from artifact names an fingerprint hashes, keeping their order
    artifact_hashes = {
        artifact: fp_hash
        for artifact, (_, fp_hash) in zip(sorted(build.artifacts), raw_fingerprints)
    }
