...
from nextpy.utils.source_reload import watch
...
def run_process_and_launch_url(run_command: list[str], killEvent: threading.Event):
    ...
    """
    config = get_config()
    watch("./" + get_config().app_name + "/", ".".join([config.app_name, config.app_name]))

    handle_updates(run_command, killEvent)

def handle_updates(run_command, killEvent: threading.Event):
    ...
    while True:
        ...
        for line in processes.stream_logs("Starting frontend", process):
            if killEvent.is_set():
                process.kill()
                console.error("Killing frontend")
                return
            ...
            if match:
                ...
                if first_run is False:
                    console.print("Changes were detected")
...
def run_frontend(root: Path, port: str, event: threading.Event):
    """Run the frontend.
    ...
    run_process_and_launch_url([prerequisites.get_package_manager(), "run", "dev"], event)  # type: ignore
...
def run_backend(
    ...
    #TODO: Handle the reloads for the backend API
    ...
    backendProcess = multiprocessing.Process(
        target=uvicorn.run,
        kwargs={ 
    ...
    while True:
        if event.is_set():
            console.error("Killing backend")
            backendProcess.kill()
            event.clear()
            return
        backendProcess.join(1.0)
