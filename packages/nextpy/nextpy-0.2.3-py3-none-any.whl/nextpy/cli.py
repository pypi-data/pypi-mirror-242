...
def _run(
    ...
    def exceptHandler(args):
        console.error(f"""
Thread {args.thread.getName()} had an exception
{args.exc_traceback}""")
        
        thread = args.thread
        console.error(f"{thread.getName()} killed")

        if thread.getName() == "Frontend":
            backendKillEvent.set()
        else:
            frontendKillEvent.set()
        sys.exit()

    threading.excepthook = exceptHandler

    frontendKillEvent = threading.Event()
    backendKillEvent = threading.Event()
    frontendKillEvent.clear()
    backendKillEvent.clear()

    t1 = threading.Thread(target=frontend_cmd, name="Frontend",args=[Path.cwd(), frontend_port, frontendKillEvent])
    t2 = threading.Thread(target=backend_cmd, name="Backend", args=[backend_host, backend_port, backendKillEvent])
...
def run(
    ...
    #trace.Trace().runfunc(_run, env, frontend, backend, frontend_port, backend_port, backend_host, loglevel)
...
def deploy(
    key: Optional[str] = typer.Option(
        None, "-k", "--deployment-key", help="The name of the deployment. Domain name safe characters only"
    ),
