...
class NumericComparisons(Comparison):
    ...
    @timeit("Euclidean and Cosine Distance")
    def generate_differenced_dataframe(self):
        ...
        differenced = pl.DataFrame([
                (c, dfn.compute_euclidean_distance(self.expected[c], self.tested[c]), dfn.compute_cosine_similarity(self.expected[c], self.tested[c])) for c in tqdm(self.columns_names)
            ],
        schema= ["Column Name", "Euclidean Distance", "Cosine Similarity"]
        )
    ...
    def compare(self):
        ...
        self.report['value_by_value']['report'] = tabulate([(column, distance, cosine, stringify_result(result)) for column, distance, cosine, result in self.differenced.rows()], headers=['Column Name', 'Euclidean Distance','Cosine Similarity','Result'], tablefmt='orgtbl')
        
        self.report['value_by_value']['explanation'] = "The numeric comparisons are done using the Euclidean distance. The Euclidean distance is a measure of the straight line distance between two points in a space.\nGiven two points P and Q with coordinates (p1, p2, ..., pn) and (q1, q2, ..., qn) respectively, the Euclidean distance d between P and Q is: d(P, Q) = (P-Q)ᵀ×(P-Q).\nThe cosine similarity is a measure of the similarity between two vectors of an inner product space that measures the cosine of the angle between them."
        
        if not self.report['value_by_value']['result']:
            self.report['value_by_value']['explanation'] += f"\nThe Euclidean distance between the expected and tested dataframes is not 0 for all columns.\nThis means that the expected and tested dataframes have different numeric values in the same column(s)."
        else:
            self.report['value_by_value']['explanation'] += f"\nThe Euclidean distance between the expected and tested dataframes is 0 for all columns.\nThis means that the expected and tested dataframes have the same numeric values for the same column(s)."
