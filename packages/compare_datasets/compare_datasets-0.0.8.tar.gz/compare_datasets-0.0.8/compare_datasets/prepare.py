...
class PrepareForComparison:
    ...
    def __init__( self, tested: pl.DataFrame, expected: pl.DataFrame, key=None, tolerance: float = 10e-6, verbose: bool = False) -> None:
    ...
    def matchRowCounts(self):
        ...
        if not self.key is None:
            ...
            self.expected_minus_tested = self.expected.with_columns(self.key).join(self.tested.with_columns(self.key), on=self.key, how='anti').select(self.__intersection__)
            self.tested_minus_expected = self.tested.with_columns(self.key).join(self.expected.with_columns(self.key), on=self.key, how='anti').select(self.__intersection__)                  
            self.expected = self.expected.with_columns(self.key).join(self.tested.with_columns(self.key), on=self.key, how='inner').select(self.__intersection__)
            self.tested = self.tested.with_columns(self.key).join(self.expected.with_columns(self.key), on=self.key, how='inner').select(self.__intersection__)                      
    ...
    def __str__(self):
        ...
        if self.row_mismatch:
            ...
            text.append("Rows in the expected dataframe but not in the tested dataframe. Please view the dataframe by <YOUR_COMPARE_OBJECT_NAME>.Eexpected_minus_tested:")
