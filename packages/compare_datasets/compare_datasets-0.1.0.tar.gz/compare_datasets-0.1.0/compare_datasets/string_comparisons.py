...
from compare_datasets_pure_python.structure import Comparison, stringify_result, timeit
...
class StringComparisons(Comparison):
    def __init__(self, prepared_data: PrepareForComparison, verbose=False):
        ...
        self.expected = prepared_data.expected.select(self.column_list['String Columns'])
    ...
    def generate_differenced_dataframe(self):
        ...
        return pl.DataFrame(
            [
                ...
                for c in tqdm(self.columns_names)
            ]
        )
    ...
    def compare(self):
        ...
        self.report['value_by_value']['report'] = tabulate([(column, distance, stringify_result(result)) for column, distance, result in zip(self.columns_names, levenshtein_distances, levenshtein_distances==0) ], headers=['Column Name', 'Total Levenshtein Distance', 'Result'], tablefmt='psql')
