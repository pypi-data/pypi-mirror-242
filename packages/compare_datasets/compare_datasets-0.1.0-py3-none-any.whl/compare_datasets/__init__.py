...
class Compare:
    def __init__ (self, tested, expected, key=None, verbose=False):
        self.data = PrepareForComparison(tested, expected, key, verbose=verbose)
        self.tested = self.data.tested
        self.expected = self.data.expected
        self.string_comparisons = StringComparisons(prepared_data=self.data, verbose=self.data.verbose)
        self.numeric_comparisons = NumericComparisons(prepared_data=self.data, verbose=self.data.verbose)
