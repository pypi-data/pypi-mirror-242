...
class PrepareForComparison:
    ...
    def matchRowCounts(self):
        if not self.key is None:
            ...
            common_rows = set(self.expected[self.key]).intersection(set(self.tested[self.key]))
            if number_unique_keys['expected'] != number_unique_keys['tested']:
                print("The number of unique keys in the expected and tested dataframes do not match. \nTaking the intersection of the keys in both the dataframes to perform comparison.")            
            ...
            if self.expected.shape[0] != self.tested.shape[0]:
                print( "The number of rows in the expected and tested dataframes do not match. \nTruncating the dataframes to the same number of rows." )
                ...
                print( f"Dataframes have been truncated to the same number of rows. Since no key is provided, the first {min_rows} of both the dataframes have been taken." )
