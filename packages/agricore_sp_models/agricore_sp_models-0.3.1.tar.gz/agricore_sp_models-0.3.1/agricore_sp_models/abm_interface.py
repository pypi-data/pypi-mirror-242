...
class ABMInterface:
    ...
    def fadnProductRelationAddURL(self, populationId):
        return urllib.parse.urljoin(self.baseUrl, '{0}/{1}/{2}'.format(self.populationPath, str(populationId), self.fadnProductRelationAddPath))
    ...
    def createFADNProductRelation(self, fadnProductRelation):
        headers = {'Accept': 'text/plain', 'Content-Type': 'application/json'}
        #print(message)
        response = requests.post(self.fadnProductRelationAddURL(fadnProductRelation.dict().get('populationId')), json.dumps(fadnProductRelation.dict(exclude={'id'})), headers=headers)
        if response.status_code ==201:
            if self.debug:
                print("Request successful")
            id = response.json()["id"]
