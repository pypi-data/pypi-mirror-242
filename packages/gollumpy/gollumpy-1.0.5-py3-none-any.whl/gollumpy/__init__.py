def gollum(args):
    ...
    df_acro = pd.read_csv(pkg_resources.resource_filename('gollumpy', 'resources/acro.short_arm.T2T.bed'), sep='\t', names=['Chromosome', 'Start', 'End'])
    samtools_command = ["samtools", "view", "-F", "2", "-q", "40", "--reference", args.fasta, args.cram, args.chr+':'+str(df_acro[df_acro['Chromosome']==args.chr]['End'].iloc[0])]
    logging.debug('samtools command for discordant extraction:\n'+' '.join(samtools_command))
    disc_extraction = subprocess.run(samtools_command, stdout=subprocess.PIPE, text=True) 
    ...
    if args.translocation:
        ...
        df_acro2 = df_acro[df_acro['Chromosome']==args.chr]
    gr_bed_guarracino = pr.PyRanges(df_acro2)
    def isPHR(row):
...
def main(args=None):
    ...
    parser.add_argument('--chr', default='chr22', help='chromosome for which to search for a ring (need to be acrocentric)')
    parser.add_argument('-s', '--sample-name', default='$CRAM', help='sample name, used for output files (default is the same as the cram name)')
    parser.add_argument('-f', '--fasta', help='path to the fasta used for the cram alignment', required=True)
    ...
    parser.add_argument('-t', '--translocation', dest='translocation', help='allow to look for mates on a different acrocentric chromosome', default=False, action='store_true')
    parser.add_argument('--e-value', default=0.01, help='e-value threshold for blat (keeping <=) (default 0.01)', type=float)
    parser.add_argument('--bit-score', default=150, help='bit score threshold for blat (keeping >=) (default 150)', type=int)
    parser.add_argument('--dbscan-eps', default=100, help='EPS value used by DBSCAN inside when running the clustering of the positions (default: 100)', type=int)
    parser.add_argument('--dbscan-min', default=5, help='Min number of reads inside a DBSCAN cluster (default: 5)', type=int)

    gollum(parser.parse_args(args))