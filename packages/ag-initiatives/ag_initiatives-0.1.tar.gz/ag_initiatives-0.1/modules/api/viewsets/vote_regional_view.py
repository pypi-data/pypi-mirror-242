...
from modules.api.filters import VoteRegionalFilter, VoteRegionalStatsFilter
from modules.api.serializers import (
    VoteRegionalSerializer,
    VoteRegionalDetailsSerializer,
    LocalitySerializer,
)
from modules.core.mixins.user_track_admin import TrackUserApiMixin
from modules.core.models import Locality, User
from modules.core.models.locality import Municipality
...
from modules.voting.models import (
    ...
    VoteRegional,
    # UserRegionalVote,
    VoteRegionalQuestion,
    VoteRegionalAnswer,
    LocalVotingGroup,
)

from modules.ecology.services.user_service import UserService as EcologyUserService
from modules.voting.models.user_municipal_vote import UserMunicipalVote
...
class VoteRegionalAPI(viewsets.ReadOnlyModelViewSet):
    queryset = (
        VoteRegional.objects.filter(is_published=True)
        .prefetch_related(
    ...
    serializer_class = VoteRegionalSerializer
    ...
    def get_serializer_class(self):
        if self.action == "retrieve":
            return VoteRegionalDetailsSerializer
        return super().get_serializer_class()

    def get_serializer_context(self):
        context = super(VoteRegionalAPI, self).get_serializer_context()
    ...
    def stats(self, request):
        self.filterset_class = VoteRegionalStatsFilter  # TODO проверить работает ли это
    ...
    def regional_vote(self, request, pk=None):
        instance: VoteRegional = VoteRegional.objects.filter(pk=pk).first()
    ...
    def _save_user_vote(
        ...
        #     user_vote_qs = UserRegionalVote.objects.filter(
        #         user=user, vote=instance, locality=locality
        #     )
        # else:
        user_vote_qs = UserMunicipalVote.objects.filter(user=user, vote_reg=instance)
        ...
        for question in instance.regional_questions.all():
            ...
            for answer in user_answers:
                ...
                if answer.type == AnswerType.OPTION:
                    ...
                    try:
                        answer_instance = question.regional_answers.get(pk=answer.value)
                    except VoteRegionalAnswer.DoesNotExist:
                        raise ValidationError(
                    ...
                    user_vote = UserMunicipalVote.objects.create(
                        user=user,
                        vote_reg=instance,
                        question_reg=question,
                        answer_reg=answer_instance,
