...
def List(message: str = "", choices=("cow", "dog", "horse", "camel"), separator: str = " ", spin_speed:int=0, colors=DEFAULT_COLORS, underline=False):
...
def Slider(message: str = "pick a letter: ", choices=("A", "B", "C", "D", "E"), spin_speed:int=-1, colors=DEFAULT_COLORS, wrap=True, underline=False):
...
def Checkbox(message: str = "pick an animal: ", choices=["cow", "dog", "horse", "camel"], separator: str = " ", min_choices=2, colors=DEFAULT_COLORS, alt_colors=ALT_COLORS, underline=False):
    ...
    def update():
        ...
        # tells the user how many more are selections needed
        if len(chosen_places) <= min_choices:

            # if they need 1 more
            if min_choices - len(chosen_places) == 1:
                end_message = COLORS.get("light red")[0] + " (1 more)" + STYLES["reset"]
            
            # if they need more than 1
            elif min_choices - len(chosen_places) > 1:
                end_message = COLORS.get("light red")[0] + f" ({min_choices-len(chosen_places)} more)" + STYLES["reset"]

            # if they don't need anymore
            elif min_choices - len(chosen_places) < 1:
                end_message = COLORS.get("light red")[0] + " (0 more)" + STYLES["reset"]

        time.sleep(DELAY_SECONDS)

        print(Q_MARK + message + output + end_message + (" "*8), end="\r")
