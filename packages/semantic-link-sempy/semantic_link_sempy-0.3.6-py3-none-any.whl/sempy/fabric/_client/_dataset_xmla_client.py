...
class DatasetXmlaClient(BaseDatasetClient):
    ...
    @log_xmla
    def _get_DAX(self, dax_string: str, batch_size: int = 100000, verbose: int = 0) -> pd.DataFrame:
        # = Chapter 1 =
        ...
        def dax_to_parquet():
            ...
            try:
                with self.get_adomd_connection() as adomd_connection:
                    fields = ParquetWriter.DAXToParquet(
                        dax_string,
                        temp_file_name,
                        batch_size,
                        adomd_connection,
    ...
    def _convert_dtypes(self, df: pd.DataFrame, fields) -> pd.DataFrame:
        ...
        for f in fields:
            if f.ClrType.Name == 'String':
                conversion_map[f.Name] = 'string'
            if f.ClrType.Name == 'Int64':
                conversion_map[f.Name] = 'Int64'
            elif f.ClrType.Name == 'Double':
                conversion_map[f.Name] = 'Float64'
            elif f.ClrType.Name == 'Decimal':
                conversion_map[f.Name] = 'Float64'
            elif f.ClrType.Name == 'Boolean':
                conversion_map[f.Name] = 'boolean'   # different from 'bool', which converts nulls to 'False'
