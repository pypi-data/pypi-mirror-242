...
class Trace():
    ...
    def __init__(
        ...
        self.trace_collector = TraceCollector(server, name, guid, stop_event)
    ...
    def set_filter(self, filter_predicate: Callable[..., bool]) -> None:
        ...
        self.trace_collector.set_FilterPredicate(Predicate[TraceEventArgs](filter_predicate))
    ...
    def start(self, delay: int = 3) -> None:
        ...
        """
        self.trace_collector.Start(delay)
    ...
    def stop(self, timeout: int = 5) -> pd.DataFrame:
        ...
        """
        stop_event_reached = self.trace_collector.Stop(timeout)
        if not stop_event_reached:
            warnings.warn(f"Did not reach specified stop event '{self.stop_event}' before stopping the trace.", UserWarning)
    ...
    def get_default_query_trace_schema() -> Dict[str, List[str]]:
        ...
        Default event classes: "QueryBegin", "QueryEnd", "VertiPaqSEQueryBegin", "VertiPaqSEQueryEnd", "VertiPaqSEQueryCacheMatch"
        ...
        dict
            Dictionary containing event types as keys and list of column names for that event as values.
        """
        base_cols = ["EventClass", "EventSubclass", "CurrentTime", "TextData", "SessionID"]
