...
def convert_db(df, relative_i_threshold=20, source='', source_info=''):
    ...
    for ik in tqdm(iks, desc='Collecting MS2 info'):
        ...
        precursor = df1['Precursor'].iloc[0]
        # 1. 获得所有该化合物信息
        all_s = []
        for i in range(len(df1)):
...
def pubchem_search(file, sleep_time=1):
    ...
    for i in tqdm(range(len(df)), desc='Searching in pubchem'):
        ...
        ik = df.loc[i, 'ik']
        if (name is np.nan) | (formula is np.nan) | (cas is np.nan) | (smile is np.nan):
            qurey_num += 1
            try:
    ...
    print(
        f'Totoal query number = {qurey_num}; Total response number = {response_num}, Response rate = {round(qurey_num / response_num * 100, 2)} %')
    return df
