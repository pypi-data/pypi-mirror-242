...
@LastEditTime: 2023-07-30 16:37:05
...
class StatConsoleModel():
    ...
    def __init__(self):
        ...
        """
        self.db_connect_key = "db_stat" if config.get_value("db_stat") else "db_cloudapp"
        self.redis_config_dict = config.get_value("redis_stat") if config.get_value("redis_stat") else None
                ...
                if len(stat_queue_list) > 0:
                    for stat_queue in stat_queue_list:
                        try:
                            ...
                            stat_log_model = StatLogModel(sub_table=FrameBaseModel().get_business_sub_table("stat_log_tb", {"app_id": stat_queue.app_id}), db_transaction=db_transaction)
                            is_add = True
