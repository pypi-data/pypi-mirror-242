...
class DeltaPositionMonitor:
    ...
    def process_breach(
        ...
        if target_strangle == self.strangle:
            ...
            self.handle_single_leg_shift(target_strangle)

    def handle_single_leg_shift(self, target_strangle: Strangle) -> None:
        pass
...
def filter_strangles_by_delta(
    ...
    filtered = {
        ...
        if not all([abs(delta) > 0.55 for delta in deltas[strangle]])
    }
