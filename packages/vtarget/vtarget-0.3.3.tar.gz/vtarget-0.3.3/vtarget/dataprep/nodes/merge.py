...
class Merge:
    ...
    def exec(self, flow_id: str, node_key: str, pin: dict[str, pd.DataFrame], settings: dict):
        ...
        if "iL" not in pin or "iR" not in pin:
            msg = app_message.messages["nodes"]["merge"]["input_port"](node_key)
        ...
        for col in left_columns:
            if col not in df_iL:
                msg = app_message.messages["nodes"]["merge"]["input_port_il"](node_key, col)
                bug_handler.default_on_error(flow_id, node_key, msg, console_level="error")
                return pout_struct

        for col in right_columns:
            if col not in df_iR:
                msg = app_message.messages["nodes"]["merge"]["input_port_iR"](node_key, col)
                bug_handler.default_on_error(flow_id, node_key, msg, console_level="error")
                return pout_struct
        ...
        try:
            ...
            msg = app_message.messages["nodes"]["exception"](node_key, str(e))
