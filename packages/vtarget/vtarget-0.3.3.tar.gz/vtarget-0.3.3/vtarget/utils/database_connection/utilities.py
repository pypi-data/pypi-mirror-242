...
class Utilities:
    ...
    def check_fields(self, data: dict, tier: str, node_key: str) -> tuple:
        """
        ...
        if not source:
            return False, app_message.messages["nodes"]["database_utilities"]["source_required"](node_key)
        ...
        if not (source in FIELD_VALIDATION_DATABASE and tier in FIELD_VALIDATION_DATABASE[source]):
            return False, app_message.messages["nodes"]["database_utilities"]["check_missing_source"](node_key)
        ...
        for field in fields:
            if not field in data:
                return False, app_message.messages["nodes"]["database_utilities"]["check_fields_to_connection"](node_key, field)

            value: any = data[field]
            if not value:
                return False, app_message.messages["nodes"]["database_utilities"]["check_empty_fields"](node_key, field)
        ...
        if "optional" in FIELD_VALIDATION_DATABASE[source][tier]:
            ...
            for _list in FIELD_VALIDATION_DATABASE[source][tier]["optional"]:
                ...
                if not flag:
                    ...
                    return False, app_message.messages["nodes"]["database_utilities"]["check_optional_fields"](node_key)
