...
class DatabaseWrite:
    def exec(self, flow_id, node_key, pin, settings):
        ...
        try:
            ...
            checked, msg = database_utilities.check_fields(settings, tier='write_data')
            if not checked:
            ...
            if source == 'postgresql' or source == 'mysql' or source == 'sqlite' or source == 'mariadb' or source == 'oracle':
                ...
                if save_type == 'replace': cursor.execute(f'TRUNCATE TABLE {table}')

                # Inserción
                cursor.executemany(
                    f"INSERT INTO {table} ({columns_name}) VALUES ({values})", params
                )
                cursor.commit()
                cursor.close()
                engine.close()
                ...
                with open(service_account_host) as file:
                    ...
                    df.to_gbq(f'{database}.{table}', project_id=project, if_exists=save_type, credentials=credentials)
                ...
                write_pandas(connection, df, table, project, database, overwrite= save_type == 'replace', auto_create_table=False)
                connection.close()
                ...
                if save_type == 'replace': collection.drop()
                collection.insert_many(df.to_dict('records'), ordered=True)
                client.close()

            else:
                msg = "(database_write) El recurso de conexión, no coincide con ninguno"
                return bug_handler.default_on_error(flow_id, node_key, msg, console_level="error")

        except Exception as error:
            msg = "(database_write) Exception:" + str(error)
            print(f'(database_write) Error al escribir una base de datos | {source}. Error: {error}')
            return bug_handler.default_on_error(flow_id, node_key, msg, str(error))
