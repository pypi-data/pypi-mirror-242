    ...
    def exec(self, flow_id: str, node_key: str, pin: dict[str, pd.DataFrame], settings: dict):
        ...
        if deploy_enabled:
            if "deploy_file_path" not in settings or not settings["deploy_file_path"]:
                msg = "(input) deploy mode está habilitado en el flujo, pero deploy_file_path no existe en la configuración del nodo"
        ...
        script.append("\n# INPUT")
        script.append(
            "df = pd.read_csv('{}', encoding='{}')".format(file_path, encoding)
        )
