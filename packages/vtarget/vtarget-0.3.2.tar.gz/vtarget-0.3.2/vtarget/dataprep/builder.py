class Builder:
    ...
    def set_nodes(self, data: dict):
        ...
        for ld in data["linkDataArray"]:
            ...
            # Agrego los hijos
            child = { "to": ld["to"], "topid": ld["topid"], "frompid": ld["frompid"], "to_id": self.nodes[ld["to"]]["idx"] }
    ...
    def analyzer(
        ...
        attemps: int = (
            0  # contador de intentos del while para procesar el flujo completo
        )
        ...
        while num_nodes_ok < len(self.nodes):
            ...
            for node_key, node_data in self.nodes.items():
                ...
                if (
                    disable_all_write_nodes
                    and (node_data["type"] in [NodeType.OUTPUTDATA.value, NodeType.EXCEL.value])
                ) or (
                    disable_all_email_nodes
                    and (node_data["type"] in [NodeType.EMAIL.value])
                ):
                ...
                if not node_data["parents"]:
                    ...
                    if node_data["type"] in [NodeType.INPUTDATA.value, NodeType.DFMAKER.value, NodeType.DATABASE.value, NodeType.SOURCE.value]:
                        is_loaded = True
                    else:
                ...
                if is_loaded:
                    ...
                    if (
                        ...
                        and (
                            ...
                            or (
                                ...
                                # and "ports_config" in data["nodeDataArray"][node_idx]["meta"] # y existe ports_config en el nodo
                                and node_settings["ports_config"] == json.dumps(data["nodeDataArray"][node_idx]["meta"]["ports_config"], sort_keys=True) # y ports_config no ha cambiado
                            )
                        )
                    ):
                    ...
                    # --------------------------------------------------
                        ...
                        # print(node_settings["config"])
                        # print(data["nodeDataArray"][node_idx]["meta"]["config"])
                        ...
                        if node_data["type"] not in [NodeType.INPUTDATA.value, NodeType.DFMAKER.value, NodeType.DATABASE.value, NodeType.SOURCE.value]:
                            for _, pin_name in enumerate(data["nodeDataArray"][node_idx]["meta"]["ports_map"]["pin"]):
                                if (
                                    "dtypes" in data["nodeDataArray"][node_idx]["meta"]["ports_map"]["pin"][pin_name]
                                    ...
                                    parent_cache = (
                                        cache_handler.cache[flow_id][parent_key]
                                        if flow_id in cache_handler.cache
                                        and parent_key in cache_handler.cache[flow_id]
                                        else dict()
                                    )
                        ...
                        if node_settings and (
                            (
                                ...
                                != json.dumps(data["nodeDataArray"][node_idx]["meta"]["config"], sort_keys=True)
                            ) # node config ha cambiado
                            or (
                                "ports_config" in node_settings # existe ports_config en cache
                                # and "ports_config" in data["nodeDataArray"][node_idx]["meta"]
                                and node_settings["ports_config"]
                                != json.dumps(data["nodeDataArray"][node_idx]["meta"]["ports_config"], sort_keys=True)
    ...
    def update_inputs_dtypes(
        self, node_name, node_key, current_dtypes, input_df: pd.DataFrame
    ):
        # if node_name == 'Merge':
        # print('\ncurrent_dtypes')
    ...
    def modify_node(self, flow_id: str, node_key: str, node: dict, port_name: str):
        ...
        try:
            cache_handler.load(flow_id, node_key, port_name)
            if (
                flow_id in cache_handler.cache
                and node_key in cache_handler.cache[flow_id]
            ):
                ...
                if "pout" in cached_node and port_name in cached_node["pout"]:
                    ...
                    head = utilities.get_head_of_df_as_list(
                        port_df, port_config, flow_id, node_key, port_name
                    )
                    ...
                    return {
                        ...
                        "node": node,
                        "success": False,
                        "error": f"The flow has not been run yet",
                    }
        except Exception as e:
            return {
                "flow_id": flow_id,
