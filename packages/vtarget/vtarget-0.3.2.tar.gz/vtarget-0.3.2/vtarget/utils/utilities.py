...
class Utilities:
    ...
    def get_table_config(self, meta: dict, port_name: str):
        ...
        ports_config: dict = (
            meta["ports_config"][port_name]
            if "ports_config" in meta and port_name in meta["ports_config"]
            else {}
        )
        # print(ports_config)
        return {
            "rows": ports_config["rows"]
            if "rows" in ports_config and ports_config["rows"] is not None
            else 50,
            "decimals": ports_config["decimals"]
            if "decimals" in ports_config and ports_config["decimals"] is not None
            else -1,
            "source": ports_config["source"]
            if "source" in ports_config and ports_config["source"]
            else "head",
            "sort_by": ports_config["sort_by"]
            if "sort_by" in ports_config and ports_config["sort_by"]
            else [],
        }
    ...
    def get_head_of_df_as_list(
        ...
        if "sort_by" in config and len(config["sort_by"]) > 0:
            ...
            cached_node = (
                cache_handler.cache[flow_id][node_key]
    ...
    def get_central_tendency_measures(self, df: pd.DataFrame):
        if df.empty:
            return {}
        info = df.describe(
            percentiles=[0.1, 0.25, 0.5, 0.75, 0.9], include="all", datetime_is_numeric=True
        ).T.reset_index()
    ...
    def get_interaction_data(self, df: pd.DataFrame, x_col: str, y_col: str, max_cats: int = 5):
        ...
        if (x_isNumeric and y_isNumeric): # ambos numericos
            return { "type": "scatter", "data": [{ "x": df[x_col].values.tolist(), "y": df[y_col].values.tolist(), "mode": "markers", "type": "scatter" }]}
        else: 
            ...
            if not x_isNumeric and not y_isNumeric: # ambos categoricos
                ...
                return { "type": "table", "data":  df_out.rename(columns={ "x_": "x", "y_": "y" }).to_dict("records")}
                ...
                df_out["type"] = "box"
                return { "type": "box", "data": df_out.rename(columns={ cat_key: "name" }).to_dict("records")}
