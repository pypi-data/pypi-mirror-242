...
class __Cache_Handler:
    ...
    def write(self, flow_id: str, node_key: str) -> None:
        ...
        try:
            ...
            for port_name in self.cache[flow_id][node_key]["pout"]:
                ...
                with open(
                    f"{TEMP_DIR}/cache/{flow_id}/{node_key}_{port_name}.tmp", "wb"
                ) as file:
            ...
            bug_handler.console(
                f'[Nodo]: "{node_key}" almacenado en cache (stg)', "info", flow_id
            )
    ...
    def load(self, flow_id: str, node_key: str, port_name: str) -> None:
        ...
        if (
            ...
            try:
                with open(
                    f"{TEMP_DIR}/cache/{flow_id}/{node_key}_{port_name}", "rb"
                ) as file:
                    self.cache[flow_id][node_key]["pout"][port_name] = pickle.load(
                        file
                    )
            except:
                traceback.print_exception(*sys.exc_info())
