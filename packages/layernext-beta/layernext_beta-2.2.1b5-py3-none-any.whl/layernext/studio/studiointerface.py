...
class StudioInterface:
    ...
    def attach_model_run_to_project_process(self, project_id: str, operation_id_array: list) -> dict:
        hed = {'Authorization': 'Basic ' + self.auth_token}
        url = f'{self.studio_url}/api/client/projects/modelRun/attach'
        payload = {
            "projectId": project_id,
            "operationIdArray": operation_id_array
        }
        try:
            response = requests.post(url=url, json=payload, headers=hed)
            status_code = response.status_code
            if status_code == 200:
                ...
                return {
                    'is_success': False,
                    'message': response.json().get('error', {}).get('details', 'Unprocessable')
                }
            else:
                return None
        # Handle connection error
