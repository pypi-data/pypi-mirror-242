    ...
    while flag:
        ...
        if document=='NotFound':
            ...
            if len(document['collection_name_registered'])==1:
                print('')
                print("ERROR occured while trying to find layernext annotation project details")
            ...
            if len(document['logs'])>0 and inference_flag:
                ...
                if document['logs'][0][:4]=='INFO':  
                    if initialize_progress_bar:
                        ...
                        pbar = tqdm(total=constraint, desc="Auto Annotation in progress ", ncols=100, unit="item") 
                    ...
                    if start_upload and initialize_upload_progress_bar and len(document["post_process_state"])==2:
                        ...
                        pbar_upload = tqdm(total=constraint_upload, desc="Annotations Upload in progress   ", ncols=100, unit="item") 
                    ...
                    new_status_message="Error Occured while Downloading the Annotation Project"
                    flag=False
...
def get_status_sam(url,unique_id,task,headers):
    ...
    while flag:
        ...
        if document=='NotFound':
            ...
            if state_start_end_flags[3][0]:
                print("Started Creating Inference EndPoint for Annotation Embedding Generation")
                state_start_end_flags[3][0]=False 
            if len(document['model_name_registered'])==1:
                print("Error occured while creating the inference endpoint")
                print(document['model_name_registered'][0])
            ...
            if len(document['logs'])>0 and inference_flag:
                ...
                if document['logs'][0][:4]=='INFO':  
                    if initialize_progress_bar:
                        constraint = document['number_of_files']
                        pbar = tqdm(total=constraint, desc="Auto Annotation Embedding Generation in progress ", ncols=100, unit="item") 
                        initialize_progress_bar = False
                    if len(document['inferenced_files'])>0 and not start_upload:
                        if len(document['inferenced_files'])<constraint:     
                            second_len = len(document['inferenced_files'])
                            ...
                            pbar.update(difference)
                            pbar.close()
                            print("Auto Annotation Embedding Generation Process is Complete!!!")
...
def main_annotation(url,payload,headers,unique_id,task, auto_annotation_operation): 
    ...
    if auto_annotation_operation == "sam-text": 
        ...
        status_thread = threading.Thread(target=get_status_sam, args=(url,unique_id,task,headers))
