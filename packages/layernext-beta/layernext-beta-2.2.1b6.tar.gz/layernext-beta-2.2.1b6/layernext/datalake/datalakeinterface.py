...
class DatalakeInterface:
    ...
    def get_file_id_and_key(self, payload):
        ...
        try:
            ...
            if status_code == 406:
                res = response.json()
                message = res['error']['message'] if isinstance(res, dict) \
                                                     and res.get('error') and isinstance(res['error'], dict) \
                                                     and res['error'].get('message') else ''

            if status_code != 204 and status_code != 200:
                return_obj = {"isSuccess": False}
                if status_code == 406 and message == REMOVED_OBJECT_UPLOAD:
                    print(
                        f"\nFile {payload['fileName']} previously removed from this collection and already attached to a another collection")

                    return_obj["object_removed"] = True
                return return_obj
    ...
    """
    Validates metadata fields to Data Lake
    """
