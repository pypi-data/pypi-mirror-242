...
class SweepEdgesLogarithmic(_SweepBase):
  """
  ...
  numbers of edges. All x shortest edges for the number of edges that maximises
  ...
  def __call__(self, sparse_distances, mst_mask, lens_mask, edge_penalty,
    ...
    min_value = 0 if self.start == 0 else np.log10(self.start * (n_edges - 1))
    exponents = np.linspace(min_value, np.log10(self.stop * (n_edges - 1)),
                            self.n_steps)
    evaluation_points = np.power(10, exponents).round().astype('int')
    evaluation_points = np.unique(evaluation_points)
