...
class SweepDistanceLinear(_SweepBase):
  """
  A helper class used to configure how STAD optimizes the network.

  This class computes a STAD network using a linear sweep between fixed distance
  values. All edges with a distance smaller than the distance that maximises
  ...
  def __call__(self, sparse_distances, mst_mask, lens_mask, edge_penalty,
    ...
    evaluation_points = np.linspace(self.start, self.stop, self.n_steps)
