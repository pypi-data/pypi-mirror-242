...
def run_module(id,  cpu, memory):
    if settings.ENVIRONMENT == "local":
        ...
        print(f'run moduel with task_def:{task_def}, subnets: {subnets}, cluster_name: {cluster_name}, id: {id} , with cpu: {cpu}, memory: {memory}')
        return "arn:aws:ecs:aws-region:xxxxxxxxxxxx:task/cluster/local"
    else:
        config = _get_env_config()
        task_def = config['taskdef']
        subnets = config['subnets']
...
def register_module(module_name, input_data):
    if settings.ENVIRONMENT == "local":
        print(f"register module {module_name} with input_data of {input_data}")
        return "randomuuid"
    else:
