...
class AttributeChoiceSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get("context", None):
            if kwargs["context"].get("request", None):
                language = (
                    kwargs["context"]["request"].language
                    if "language" in kwargs["context"]["request"].data
                    else None
                )
        ...
        if store:
            try:
                ...
                raise serializers.ValidationError(
                    f"AttributeChoice(PK={instance.pk}) doesnt have inventory at Store(PK={store.pk})"
                )
