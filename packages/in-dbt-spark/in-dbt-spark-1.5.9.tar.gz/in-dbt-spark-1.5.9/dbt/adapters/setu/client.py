...
class SetuClient:
    ...
    def create_session(
        ...
        session_initialization_config.spark_conf[
            "spark.yarn.queue"
        ...
        try:
            ...
            logger.error(e.response.text)  # type: ignore
            logger.exception(f"Failed while creating Spark Interactive session with error {e}")  # type: ignore
            raise
