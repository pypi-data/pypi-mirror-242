...
#define parent_pyobject(type, field, pointer)                                  \
  ((PyObject *)((char *)(pointer)-offsetof(type, field)))
...
static PyMethodDef boxobj_methods[] = {
    ...
    {"generate_random_points", (PyCFunction)boxobj_generate_random_points,
     METH_O, BOX_GRP_DOC},
    {"test_points", (PyCFunction)boxobj_test_points, METH_O,
     BOX_TEST_POINTS_DOC},
    {"split", (PyCFunctionWithKeywords)boxobj_split,
     METH_VARARGS | METH_KEYWORDS, BOX_SPLIT_DOC},
    {"check_intersection", (PyCFunction)boxobj_check_intersection, METH_O,
     BOX_CHECK_INTERSECTION_DOC},
    {NULL}};
...
static int boxobj_init(BoxObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kwds, "O&ddd|O&O&O&", kwlist, convert_to_dbl_vec, &cent, &xdim,
          &ydim, &zdim, convert_to_dbl_vec, &ex, convert_to_dbl_vec, &ey,
          convert_to_dbl_vec, &ez))
    return -1;
  ...
  box_dispose(&self->box);
  box_init(&self->box, (double *)PyArray_DATA(cent), (double *)PyArray_DATA(ex),
           (double *)PyArray_DATA(ey), (double *)PyArray_DATA(ez), xdim, ydim,
           zdim);
...
static PyObject *boxobj_generate_random_points(BoxObject *self,
                                               PyObject *npts) {
  ...
  int status =
      box_generate_random_points(&self->box, n, (double *)PyArray_DATA(points));
...
static PyObject *boxobj_split(BoxObject *self, PyObject *args, PyObject *kwds) {
  ...
  if (strcmp(dir, "auto") == 0)
    direct = BOX_SPLIT_AUTODIR;
...
static PyObject *surfobj_new(PyTypeObject *type, PyObject *args,
                             PyObject *kwds) {
  PyErr_SetString(PyExc_TypeError, "Can't instantiate abstract class Surface");
  return NULL;
}

static PyObject *surfobj_test_points(SurfaceObject *self, PyObject *points) {
  PyObject *pts;
  if (!convert_to_dbl_vec_array(points, &pts))
    return NULL;
...
static PyObject *surfobj_test_box(SurfaceObject *self, PyObject *box) {
  if (!PyObject_TypeCheck(box, &BoxType)) {
    PyErr_SetString(PyExc_ValueError, "Box instance is expected");
    return NULL;
  }
...
static PyMethodDef surfobj_methods[] = {
    ...
    {"test_points", (PyCFunction)surfobj_test_points, METH_O,
     SURF_TEST_POINTS_DOC},
    {NULL}};
...
static int sphereobj_init(SphereObject *self, PyObject *args, PyObject *kwds) {
  PyObject *center;
  double radius;
...
static int cylinderobj_init(CylinderObject *self, PyObject *args,
                            PyObject *kwds) {
  PyObject *point, *axis;
  double radius;
  if (!PyArg_ParseTuple(args, "O&O&d", convert_to_dbl_vec, &point,
                        convert_to_dbl_vec, &axis, &radius))
    return -1;

  cylinder_init(&self->surf, (double *)PyArray_DATA(point),
                (double *)PyArray_DATA(axis), radius);
  Py_DECREF(point);
  Py_DECREF(axis);
  return 0;
}

static int coneobj_init(ConeObject *self, PyObject *args, PyObject *kwds) {
  PyObject *apex, *axis;
  double ta;
  int sheet = 0;
  ...
  cone_init(&self->surf, (double *)PyArray_DATA(apex),
            (double *)PyArray_DATA(axis), ta, sheet);
  Py_DECREF(apex);
  Py_DECREF(axis);
  return 0;
}

static int torusobj_init(TorusObject *self, PyObject *args, PyObject *kwds) {
  PyObject *center, *axis;
  double r, a, b;
  if (!PyArg_ParseTuple(args, "O&O&ddd", convert_to_dbl_vec, &center,
                        convert_to_dbl_vec, &axis, &r, &a, &b))
    return -1;

  int status = torus_init(&self->surf, (double *)PyArray_DATA(center),
                          (double *)PyArray_DATA(axis), r, a, b);
  Py_DECREF(center);
  Py_DECREF(axis);
  return 0;
}

static int gqobj_init(GQuadraticObject *self, PyObject *args, PyObject *kwds) {
  PyObject *m, *v;
  double k, f;
  if (!PyArg_ParseTuple(args, "O&O&dd", convert_to_dbl_vec_array, &m,
                        convert_to_dbl_vec, &v, &k, &f))
    return -1;
...
static PyObject *planeobj_getoffset(PlaneObject *self, void *closure) {
...
static PyObject *sphereobj_getradius(SphereObject *self, void *closure) {
...
static PyObject *cylinderobj_getaxis(CylinderObject *self, void *closure) {
  npy_intp dims[] = {NDIM};
...
static PyObject *cylinderobj_getradius(CylinderObject *self, void *closure) {
  return Py_BuildValue("d", self->surf.radius);
}
...
static int rccobj_init(RCCObject *self, PyObject *args, PyObject *kwds) {
  ...
  Py_INCREF(top);
  Py_INCREF(bot);
  status = RCC_init(&self->surf, &((CylinderObject *)cyl)->surf,
                    &((PlaneObject *)top)->surf, &((PlaneObject *)bot)->surf);
...
static int mboxobj_init(BOXObject *self, PyObject *args, PyObject *kwds) {
  size_t arglen = PyTuple_Size(args);
...
static PyObject *coneobj_getaxis(ConeObject *self, void *closure) {
  npy_intp dims[] = {NDIM};
  PyObject *axis = PyArray_EMPTY(1, dims, NPY_DOUBLE, 0);
  double *data = (double *)PyArray_DATA(axis);
  for (int i = 0; i < NDIM; ++i)
...
static PyObject *coneobj_getta(ConeObject *self, void *closure) {
  return Py_BuildValue("d", self->surf.ta);
}

static PyObject *coneobj_getsheet(ConeObject *self, void *closure) {
...
static PyObject *torusobj_getaxis(TorusObject *self, void *closure) {
  npy_intp dims[] = {NDIM};
  PyObject *axis = PyArray_EMPTY(1, dims, NPY_DOUBLE, 0);
  double *data = (double *)PyArray_DATA(axis);
  for (int i = 0; i < NDIM; ++i)
...
static PyMemberDef torusobj_members[] = {
    {"_R", T_DOUBLE, offsetof(TorusObject, surf) + offsetof(Torus, radius),
     READONLY, "Torus's major radius."},
...
static PyObject *gqobj_get_v(GQuadraticObject *self, void *closure) {
  npy_intp dims[] = {NDIM};
...
static PyObject *gqobj_get_k(GQuadraticObject *self, void *closure) {
  return Py_BuildValue("d", self->surf.k);
}

static PyObject *gqobj_get_factor(GQuadraticObject *self, void *closure) {
...
static PyGetSetDef gqobj_getset[] = {
    ...
    {"_factor", (getter)gqobj_get_factor, NULL,
     "GQuadratic's normalisation factor", NULL},
    {NULL}};
...
static PyObject *shapeobj_getinvopc(ShapeObject *self, void *closure) {
  return Py_BuildValue("s", opcodes[invert_opc(self->shape.opc)]);
}

static PyObject *shapeobj_getargs(ShapeObject *self, void *closure) {
  PyObject *args = PyTuple_New(self->shape.alen);
  if (args == NULL)
    return NULL;
  if (self->shape.opc == COMPLEMENT || self->shape.opc == IDENTITY) {
    PyObject *pysurf =
        parent_pyobject(SurfaceObject, surf, self->shape.args.surface);
...
static PyGetSetDef shapeobj_getset[] = {
    ...
    {"invert_opc", (getter)shapeobj_getinvopc, NULL,
     "Inverted operation code of shape.", NULL},
    {"args", (getter)shapeobj_getargs, NULL, "Arguments of shape.", NULL},
    {NULL}};

static PyMethodDef shapeobj_methods[] = {
    {"test_box", (PyCFunctionWithKeywords)shapeobj_test_box,
     METH_VARARGS | METH_KEYWORDS,
     "Tests where the box is located with respect to the surface."},
    {"ultimate_test_box", (PyCFunctionWithKeywords)shapeobj_ultimate_test_box,
     METH_VARARGS | METH_KEYWORDS, ""},
    {"volume", (PyCFunctionWithKeywords)shapeobj_volume,
     METH_VARARGS | METH_KEYWORDS, ""},
    {"bounding_box", (PyCFunctionWithKeywords)shapeobj_bounding_box,
     METH_VARARGS | METH_KEYWORDS, ""},
    {"collect_statistics", (PyCFunction)shapeobj_collect_statistics,
     METH_VARARGS, ""},
  ...
  if (opc == IDENTITY || opc == COMPLEMENT) {
    ...
    if (alen <= 1) {
      PyErr_SetString(PyExc_ValueError,
                      "More than one shape object is expected");
      return -1;
    }
    PyObject *item;
...
static PyObject *shapeobj_ultimate_test_box(ShapeObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|Odb", kwlist, &box, &min_vol,
                                   &collect))
    return NULL;
  ...
  shape_reset_cache(&self->shape);
  int result = shape_ultimate_test_box(&self->shape, &((BoxObject *)box)->box,
                                       min_vol, collect);
  return Py_BuildValue("i", result);
}
...
static PyObject *shapeobj_collect_statistics(ShapeObject *self,
                                             PyObject *args) {
  PyObject *box;
  double min_vol;
...
static PyModuleDef geometry_module = {PyModuleDef_HEAD_INIT,
                                      "geometry",
...
PyMODINIT_FUNC PyInit_geometry(void) {
  ...
  *((double *)PyArray_DATA(ez) + 2) = 1.0;
  ...
  box_init(&global_box->box, (double *)PyArray_DATA(origin),
           (double *)PyArray_DATA(ex), (double *)PyArray_DATA(ey),
