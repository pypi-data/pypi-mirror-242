...
class Parser:
    ...
    def add_option(self, option: Option[Any], /) -> None:
        ...
        if option.short is not MISSING:
            if option.short in self.short_option_map:
                raise ValueError(
                    f"Short option {option.short!r} already exists for option "
                    f"{self.short_option_map[option.short]!r}. Cannot add "
                    f"option {option.name!r}."
                )
    ...
    def _parse_option(
        ...
        while value is MISSING:
            if option.cls is bool:
                ...
                raise ValueError(f"Option '{name!r}' requires an argument.")
        ...
        if minimum > 1:
            ...
            if maximum is not None and len(value) > maximum:
                raise ValueError(
                    f"Option '{name!r}' takes up to ({maximum}) arguments, "
                    f"got {len(value)}."
                )
            elif len(value) < minimum:
                raise ValueError(
                    f"Option '{name!r}' takes at least ({minimum}) arguments, "
                    f"got {len(value)}."
                )
    ...
    def _parse_short_option(
        ...
        if name not in command.short_option_map:
            raise ValueError(f"Unknown short option: {name!r}.")
    ...
    def _parse_argument(self, token: Token, ctx: dict[str, Any], /) -> None:
        ...
        if isinstance(command, (Parser, Group)):
            try:
                ...
                raise ValueError(f"Unknown command: {token.value!r}.")
