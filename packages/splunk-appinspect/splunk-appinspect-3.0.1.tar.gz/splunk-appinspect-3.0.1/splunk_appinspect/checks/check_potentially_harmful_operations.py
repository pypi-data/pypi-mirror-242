...
@splunk_appinspect.tags(Tags.SPLUNK_APPINSPECT, Tags.APPAPPROVAL, Tags.MANUAL)
@splunk_appinspect.cert_version(min="1.0.0")
def check_hard_coded_paths(app: "App", reporter: "Reporter") -> None:
    ...
    for result, match in results:
        ...
        if platform.system() == "Windows":
            reporter_output = (
                "{} will be checked for hard-coded paths during code review. File: {}, Line: {}."
            ...
            if app.is_text(Path(file_name)):
                reporter_output = ("Found possible hard-coded path '{}'.File: {}, Line: {}.").format(
                    match.group(), file_name, line_number
                )
                reporter.manual_check(reporter_output, file_name, line_number)
...
@splunk_appinspect.tags(Tags.SPLUNK_APPINSPECT, Tags.APPAPPROVAL, Tags.MANUAL)
@splunk_appinspect.cert_version(min="1.0.0")
def check_user_privileges(app: "App", reporter: "Reporter") -> None:
    ...
    matches = app.search_for_patterns(patterns, excluded_dirs=excluded_directories, excluded_types=excluded_types)
...
@splunk_appinspect.cert_version(min="1.0.0")
def check_fs_writes(reporter: "Reporter") -> None:
