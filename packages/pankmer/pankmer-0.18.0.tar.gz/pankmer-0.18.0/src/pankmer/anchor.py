...
def anchor_region(
    *pk_results,
    ...
    anchor_dicts = (
        result.get_regional_scores(anchor, {contig: [[start, end]]})
        for result in pk_results
    )
    if output_file:
        with (
...
def anchor_dicts_tuple(*pk_results, anchor, chromosomes):
    return tuple(
        result.get_regional_scores(anchor, {c: [] for c in chromosomes})
        for result in pk_results
    )
...
def anchor_heatmap(
    ...
    results = tuple(
        pk_results.get_regional_scores(
            ref, gff_to_dict(gff, n_features=n_features)
        )
        for ref, gff in zip(anchors, features)
    )
    # for anchor_dict in results:
    #     print(tuple(anchor_dict.values()))
    dfs = tuple(
        pd.DataFrame(
            {
                os.path.basename(ref.replace(".fasta", "").replace(".gz", "")): tuple(
                    sum(score)
