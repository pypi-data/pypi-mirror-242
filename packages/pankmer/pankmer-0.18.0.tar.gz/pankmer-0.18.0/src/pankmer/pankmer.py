...
def get_positional_kmers(fasta_path, upper, lower, contig_header="id"):
    ...
    with gzip_agnostic_open(fasta_path, "rt") as infile:
        for record in SeqIO.parse(infile, "fasta"):
            ...
            kmers_dict[names[contig_header]] = break_seq(
                seq_byte, upper, lower, KMER_MAX
            )
    return kmers_dict
...
class PKResults:
    def __init__(self, results_dir: str, threads: int = 1):
        ...
        if isfile(results_dir) and tarfile.is_tarfile(results_dir):
            ...
            kmer_size, genomes, positions, mem_blocks = retrieve_metadata(
                join(results_dir, "metadata.json")
            )
            self.number_of_genomes = len(genomes)
    ...
    def encode_kmer(self, kmer: str) -> int:
        ...
        if len(kmer_list) > 1:
            raise ValueError(
                f"encode_kmer should be used for one Kmer of size {self.kmer_size} only! "
                f"Please use get_sequence_kmerbits for a sequence larger than that."
            )
        return kmer_list[0]

    def get_sequence_kmerbits(
        self, seq: str, upper: int = None, lower: int = None
    ) -> list:
    ...
    def get_fasta_kmerbits(
        self, fasta_path: str, upper: int = None, lower: int = None
    ) -> set:
    ...
    def get_positional_fasta_kmerbits(
        self, fasta_path: str, contig_header: str = "id"
    ) -> dict:
    ...
    def get_sequences_scores(self, sequences: dict, cpu: int = None) -> dict:
        ...
        sequences_kmers = {
            name: self.get_sequence_kmerbits(seq) for name, seq in sequences.items()
        }
        combined_kmers = set()
        ...
        core_chunks = [[self.results_dir, {kmer: fillna_val for kmer in combined_kmers[i:i+chunk_size]}] for i in range(0, len(combined_kmers), chunk_size)]
        ...
        scored_kmers = {k: s for result in results for k, s in result.items()}
    ...
    def get_regional_scores(self, reference: str, regions: dict,
        ...
        for id, start, end in sequences.keys():
            ...
            sequences_reduced[id][(start, end)] = (
                [scores[0]] * math.ceil(kmer_flank) + scores
                + [scores[-1]] * math.floor(kmer_flank)
            )
        return sequences_reduced
    
    # def get_regional_conserv(self, reference : str, regions : dict) -> dict:
    #     """
    #     Get per k-mer conservation for each region in the reference specified by the 
    #     regions dictionary.
    ...
    #             conserv[id][region] = list(np.array(scores).sum(axis=1))
...
def run_index_wrapper(args):
    ...
    index_wrapper(
        ...
        index="",
        k=kmer_size(),
        fraction=1 - 10 ** (-1 * args.qual / (10 * kmer_size()))
        if args.qual
        else args.fraction,
...
def run_anchor_region(args):
    if args.output:
        anchor_region(*(PKResults(i) for i in args.index), anchor=args.anchor,
        ...
        for chrom, start, end, *values in anchor_region(
            *(PKResults(i) for i in args.index), anchor=args.anchor,
...
def run_anchor_genome(args):
    anchor_genome(*(PKResults(i) for i in args.index), output=args.output,
        anchor=args.anchor, chromosomes=args.chromosomes,
        output_table=args.table, groups=args.groups, title=args.title,
        x_label=args.x_label, legend=args.legend,
...
def main():
    ...
    anchor_reg_parser.add_argument(
        '-i', '--index', metavar='<index[.tar]>', type=str, required=True,
        nargs='+', help='index')
    anchor_reg_parser.add_argument(
        '-a', '--anchor', metavar='<anchor.fa.gz>', type=str, required=True,
        help='anchor genome')
    anchor_reg_parser.add_argument(
        '-c', '--coords', metavar='<chr:start-end>', type=str, required=True,
    ...
    anchor_genome_parser.add_argument(
        '-a', '--anchor', metavar='<anchor.fa.gz>', type=str,
