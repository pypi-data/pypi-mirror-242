...
def get_latest_valid_version(client):
    ...
    df['SemVer'] = df.Name.str.extract(r'(\d+\.\d+\.\d+)')
...
def setup_certs(**kwargs):
    ...
    _print(f'Getting pem files for [{lm_client.get_domain()}].', 2)
...
def setup_dlls(**kwargs):
    ...
    _print(f'Finding the latest valid version in [{lm_client.get_domain()}]', 4)
