...
class ConfigManager:
    ...
    title = colored('Lumipy domain config:', 'blue', attrs=['bold'])
    ...
    def delete(self, domain: str):
        ...
        if domain == self.domain:
            raise ValueError(
                ...
                f'Please switch to a different one before deleting.\n'
                'For example:\n    config.domain = "other"'
            )
    ...
    def creds(self, domain: Optional[str] = None) -> Dict[str, str]:
        ...
        """
        pats = self._read()
        if domain is None and len(pats) == 0:
            return {}
        ...
        return {'token': pats[domain][0], 'api_url': f'https://{domain}.lusid.com/honeycomb'}
    ...
    def domain(self) -> Union[None, str]:
        ...
        if len(lines) == 0:
            raise ValueError(
                'There are no active domain lines in the config. '
                f'Please set a new one with:\n    {cmd}'
            )
    ...
    def _list_lines(self, show_pats):
        ...
        for domain, (token, _) in pats.items():
            ...
            if not show_pats:
                ...
                front, back = token[:5], token[-7:]
                token = front + '-' * 20 + back

            end = colored('(active)', 'green') if current else ''
    ...
    def _check(self, domain):
        domain = domain.lower()
        pats = self._read()
        if domain not in pats:
            d = colored(domain, 'green')
            method = colored(f'config.add("{domain}", <PAT>)', 'green')
            raise ValueError(f'Domain {d} not found in config. You can add it with {method}.')
