...
USER_AGENT = 'requests.passer_web_client'
...
class PasserWebClient:
    ...
    @classmethod
    def from_apikey(cls, key: str, base_url: str, token_duration: float = 1): 
        """使用apikey创建客户端对象

        Args:
            key (str): _description_
            base_url (str): _description_
            token_duration (float, optional): _description_. Defaults to 1.

        Returns:
            _type_: _description_
        """
        assert not base_url.endswith('/')
        url = f"{base_url}/api/v1/api_key/token?key={key}&duration={token_duration}"
        headers = {
        'User-Agent': USER_AGENT,
        }
        response = requests.request("POST", url, headers=headers)
        assert response.status_code == 200, f'申请token失败: {response.text}'
        token = response.json()['authToken']['token']
        config = PasserWebConfig(token, base_url, try_get_user=False)
        # apikey 不一定能获取其所属用户的信息
        return cls(config)
    ...
    def import_dcm_to_collection(self, collection_id: str, file: BufferedReader) -> dict:
        """导入DICOM文件
        ...
        Returns:
            dict: _description_
        """
        res = self.post(f'/api/v1.1/dicom/instance?collectionId={collection_id}', 
                  data=file, headers={'Content-Type': 'application/octet-stream'})
        return res.json()
    ...
    def get_pipeline_info(self, pipeline_id: str)-> dict:
        """获取一个任务的信息
        Args:
            pipeline_id(str): 任务id
        Returns:
            pipeline: [dict]
        """
        api = f'/api/v1.1/task/pipeline/{pipeline_id}'
        res = self.get(api)
        return res.json()
    ...
    def post(self, endpoint, params={}, json={}, headers={}, data=None,
             with_token: bool = True) -> requests.Response:
        ...
        Args:
            ...
            with_token (bool, optional): 是否自动带上token
        ...
        res = requests.post(url, params=final_params, json=json, headers=final_headers, data=data)
        self._assert_200(res)
        return res
