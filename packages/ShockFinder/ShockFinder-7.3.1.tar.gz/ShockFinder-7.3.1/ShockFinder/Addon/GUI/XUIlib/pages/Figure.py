...
class page(page):
	...
	maxcross=20
	reset_funs=[]
	def low_pass_filter(self,signal, window_size):
	...
	def get_lineinfo(self,x,y,ylb,yco,yls,yargs):
		...
		lineinfo["y"]=self.low_pass_filter(lineinfo["y"],get_par("filterlevel",10)) if get_par("linefilter",False) else lineinfo["y"]
		return lineinfo
	def get_lineinfo3d(x,y,z,zlb,zco,zls,zargs):
		...
		def get_par(key,default):
			return lineinfo[key] if key in lineinfo.keys() else default
		lineinfo["z"]=self.low_pass_filter(lineinfo["z"],get_par("filterlevel",10)) if get_par("linefilter",False) else lineinfo["z"]
		return lineinfo
	...
	def FFT(self): #support 0d
		...
		def drawfft(event):
			...
			try:
				...
				if newargs!="":
					newargs=newargs[:-1]
				ft=self.low_pass_filter(ft,filv) if filt else ft
