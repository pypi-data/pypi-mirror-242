...
def get(Dataobj,quantity_name,args={},vargs={}):
	...
	quantities={
		"RMI_"+rho+"_"+velo:-1/fix_rho*Dataobj.quantities["Divergence_"+velo] if not np.all(fix_rho==0) else np.full(fix_rho.shape,0)
	}
...
def result(quantity_name=None,anafname=None):
	...
	return ["RMI_"+sstr[i]+"_"+sstr[i+1] for i in range(0,len(sstr),2)] #this function will return result types shown in GUI
