        ...
        def wrapper(*args, **kwargs):
            ...
            for r in res:
                ...
                if not keys:
                    keys = list(r.keys())
                values = [
                    proc_func(index, keys[index], convert_dt(index, value)) for (index, value) in enumerate(r)
                ]
                if num_cols is 1:
                    value = values[0]
                else:
        ...
        for k, v in self.filter_dict.items():
            ...
            if type(field.type) == sqlalchemy.Integer:
                try:
                  ...
                  type(field.type) == sqlalchemy.Numeric or
                  type(field.type) == sqlalchemy.DECIMAL):
                try:
                    v = float(v)
