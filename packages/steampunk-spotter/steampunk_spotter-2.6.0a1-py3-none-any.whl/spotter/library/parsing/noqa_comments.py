        ...
        if comment_token_item:
            if isinstance(comment_token_item, list):
                for comment_token in comment_token_item:
                    if isinstance(comment_token, yaml.CommentToken):
...
def _construct_noqa_from_dict_recursive(commented_map: Union[Dict[str, Any], yaml.CommentedMap]) -> List[SpotterNoqa]:
    """
    ...
    for v in commented_map.values():
        ...
        if isinstance(v, list):
            if isinstance(v, yaml.CommentedSeq):
                noqas += _construct_noqa_from_commented_item(v)

            for e in v:
                if isinstance(e, dict):
                    if isinstance(e, yaml.CommentedMap):
...
def match_comments_with_task(commented_map: yaml.CommentedMap) -> None:
    """
