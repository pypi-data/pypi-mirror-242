...
class SeleniumManagerBase:
    ...
    def run_actions(self):
        ...
        try:
            for _action in self.actions:
                ...
                _result = dict(action=_action, result=_result)
                yield _result
                if _callback := _action.get('callback'):
                    _callback_function_name = _callback.get('function')
                    if _callback_function := self.external_functions_mapping.get(_callback_function_name):
                        _callback_args = ','.join(_callback.get('args', []))
                        _callback_result = _callback_function(*eval(_callback_args))
                        yield dict(action=_action, callback=_callback_result)
                    else:
                        logging.warning(f"{_callback_function_name} not defined in external_functions_mapping variable")
        finally:
    ...
    def _get_env(self, search_value):
        _search_value = str(search_value)
        if (not _search_value.startswith(self.env_var_separator_start)
                and not _search_value.endswith(self.env_var_separator_end)):
            return search_value
        _search = _search_value.removeprefix(self.env_var_separator_start).removesuffix(self.env_var_separator_end).strip()
        if _value := self.environment.get(_search):
            return _value
        if _env_var := os.getenv(_search):
            return _env_var
        logging.warning(f"Environment variable '{_search}' not defined. Ignoring...")
        return search_value
...
class SeleniumManager(SeleniumManagerBase):
    ...
    def _external_function(self, action: dict):
        ...
        _function_args = ','.join(action.get('args', []))
        _function_return_variable = action.get('return')
        if _mapped_function := self.external_functions_mapping.get(_function_mapping_name):
            _result = _mapped_function.get('function')(*eval(_function_args))
            if _function_return_variable:
                _result_parts = _function_return_variable.split(',')
                if (isinstance(_result, list) or isinstance(_result, tuple)) and len(_result_parts) != len(_result):
                    logging.error(f"Function returns {len(_result)} elements and you only took {len(_result_parts)}")
                    return _result
                for _new_var_i, _new_var in enumerate(_result_parts):
