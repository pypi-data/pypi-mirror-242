...
def geom(args):
    ...
    if args.command in ['geomPrep', 'all']:
        calls.append({'fcn':orien.geomPrep, 'args':('geomPrep', args.force)})
        calls.append({'fcn':orien.submit, 'args':('setup', False)})
    
    # kinematics requires a geomPrep and several checkGeom calls
    if args.command in ['kinematics', 'all']:
        calls.append({'fcn':orien.geomPrep, 'args':('kinematics', args.force)})
        calls.append({'fcn':orien.submit, 'args':('setup', False)})
    ...
    for call in calls:
        ...
        if 'geomCheckRhs' in resp:
            for rh in resp['geomCheckRhs']:
                print('Checking kinematic output geometry for', rh.replace('_', ' '))
                orien = skolo.Orientation(args.project, args.run, rh, args.local)
                printResponse(orien.submit('setup'), args)
