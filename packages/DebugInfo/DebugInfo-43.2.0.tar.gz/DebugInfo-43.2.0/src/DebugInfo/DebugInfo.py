...
class 打印模板:
    ...
    def 准备表格(self, 对齐控制串: str = None, 列宽控制表: list[int] = None):
        ...
        :param 对齐控制串: 一个包含 l c r 的字符串，分别控制对应列的对齐方式，l: 左对齐, c: 居中对齐, r: 右对齐, 例如 'llcr'; 超出对齐控制串的列,由最后一个对齐控制字控制
...
class ssh接口类:
    ...
    @property
    def 可达(self) -> bool:
        if not self.__可达 and self.__测试次数 < 1 and self.有效:
...
class 交互接口类:
    ...
    def 发起选项交互(self,
        ...
        def 展示选项(操作说明: list[str] or str = None,
            ...
            # region 提示多选操作方式
            if 多选:
                操作说明信息.append(f'多选提示: 您可以使用 1,3,5 或者 1 2 s1 的样式选择多个选项')
                        ...
                        if 可用路径列表.总数 > 0:
                            ...
                            if 路径列表:
                                ...
                                if 可用路径列表.截断 or 截断标记:
                                    if 可用路径列表.截断:
                                        操作说明.append(红字(f'搜索结果未完全呈现, 共 >={搜索条数限制} 条'))
                                    else:
                                        操作说明.append(红字(f'搜索结果未完全呈现, 共 {可用路径列表.总数} 条'))
                            ...
                            if 文档列表:
                                ...
                                if 可用文档列表.截断 or 截断标记:
                                    if 可用文档列表.截断:
                                        ...
                                        操作说明.append(红字(f'搜索结果未完全呈现, 共 {可用文档列表.总数} 条'))
