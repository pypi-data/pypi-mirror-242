...
class RedisQueue(_RedisHelper):
    ...
    def package_task_v2(cls, work_tag, key, params, sub_key=None, task_type=TaskType.Normal, return_prefix=False,
        ...
        if return_prefix is True:
            v = ",".join([cls.part_handler.escape(cls._handle_package_sub_part(x)) for x in ps[:-2]])
        else:
            v = ",".join([cls.part_handler.escape(cls._handle_package_sub_part(x)) for x in ps])
        return v
    ...
    def _handle_package_sub_part(cls, data, action="package"):
        if action == "package":
            if isinstance(data, (tuple, list)):
                p = StringTool.join_decode([cls.sub_part_handler.escape(x) for x in data], "|")
            else:
                p = cls.sub_part_handler.escape("%s" % data)
            return p
        elif action == "unpack":
            s_data = data.split("|")
            return [cls.sub_part_handler.unescape(x) for x in s_data]
        return None

    @classmethod
    def unpack_task_v2(cls, task_info):
        data = dict()
        parts = [cls.part_handler.unescape(x) for x in task_info.split(",")]
        if len(parts) != 6:
...
class RedisWorker(RedisWorkerConfig, Worker):
    ...
    def _push_task(self, key, params, work_tag=None, sub_key=None, report_tag=None, is_report=False,
        ...
        if work_tag is None:
            ...
            queue_key = self.queue_prefix_key + "_" + work_tag
        task_info = RedisQueue.package_task_v2(
            work_tag, key, params, sub_key=sub_key, task_type=task_type,
            report_tag=report_tag, is_report=is_report,
            report_scene=report_scene, task_name=task_name)
    ...
    def get_task_item(self, item_index, hash_key=None, key=None,
                      sub_key=None):
        item_key = self._task_item_key(item_index, key, sub_key)
        if hash_key is None:
            ...
            _item = {}
            for key in item.keys():
                _item[self.bytes_2_str(key)] = StringData.unpack_data(
                    item[key])
            return _item
    ...
    def add_arguments(cls):
        cls.init_parser.add_argument("-b", "--heartbeat-value",
                                     dest="heartbeat_value",
                                     help="heartbeat value")
        cls.init_parser.add_argument("-c", "--worker-conf-path",
                                     "--redis_worker-conf-path",
                                     dest="conf_path",
                                     help="redis worker conf path")
        cls.init_parser.add_argument("-l", "--log-dir", dest="log_dir",
                                     help="worker log save dir")
        cls.init_parser.add_argument("-w", "--work-tag", dest="work_tag",
                                     help="work tag")
        cls.init_parser.add_argument("--debug", dest="debug",
                                     help="debug mode, print debug msg",
                                     action="store_true", default=False)
        # add version 2.1.0
        cls.init_parser.add_argument('-p', '--sys-path', dest='sys_paths',
                                     help='add path to sys.path',
                                     action='append', nargs='*')
        cls.test_parser.add_argument("-e", "--example-path",
                                     dest="example_path",
                                     help="run an example use this file")
        cls.test_parser.add_argument("-k", "--key", dest="key",
                                     help="task key")
        cls.test_parser.add_argument("-r", "--report-tag", dest="report_tag",
                                     help="report tag")
        cls.test_parser.add_argument("--report-scene", dest="report_scene",
                                     help="report scene")
        cls.test_parser.add_argument("-s", "--sub-key", dest="sub_key",
                                     help="task sub key")

        cls.work_parser.add_argument("-D", "--daemon", dest="daemon",
                                     help="work in daemon",
                                     action="store_true", default=False)
    ...
    def parse_args(cls, args=None):
        cls.add_arguments()
        p_args = cls.init_parser.parse_args(args)
        return p_args
