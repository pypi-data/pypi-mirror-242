...
class DAGTools(object):
    ...
    def _verify_pipeline_input_output(cls, p_params):
        tl = p_params["task_list"]
        output_keys = [k for k in p_params.keys() if k.startswith("output_")]
    ...
    def _verify_pipeline_item(cls, index, item):
        ...
        item_keys = item.keys()
        # 检查输出 ------------------------------------------------------------------------------------------------------
        output_keys = [x for x in item_keys if x.startswith("output_")]
...
class DAGWorker(RedisWorker):
    ...
    DEFAULT_WORK_TAG = constants.DEFAULT_PIPELINE_TAG
