...
from wiliot_deployment_tools.ag.wlt_types import *
...
def init_adva(adva=None, device:Literal['Tag', 'Bridge']='Tag'):
    ...
    adva = apply_adva_bitmask(adva, device)
    return adva
...
class BrgPktGenerator:
    ...
    def get_expected_coupled_mqtt(self, tag_idx:int=None):
        ...
        expected = {
            ...
            PAYLOAD: self.get_data(tag_idx)[16:]
        }
        return expected
    ...
    def get_expected_uncoupled_mqtt(self, tag_idx:int=None):
        ...
        expected_si = {
            TIMESTAMP: timestamp,
            PAYLOAD: self.get_si(tag_idx)[16:]
        }
...
p = BrgPktGenerator()
p.randomize_data_packet()
print(p.get_expected_uncoupled_mqtt())