...
def dispatch_function(this_filter, data_dict_input, extra=None):
    ...
    if match_item == "cdp_direction":
        ...
        # t = data_dict_input[this_filter["input"]]
        # # need to be fixed ********
        # keyname = "need to be fixed"
        # f = cdp_direction(extra.log, keyname, t)
        # data_dict_input[this_filter["output"]] = f
        pass

    elif match_item == "reduce":

        t = data_dict_input[this_filter["input"]]
        data_dict_input[this_filter["output"]] = t[1::2]
        ...
        tt = 1 / dt
        try:
            r = math.log(math.floor(tt / n), 2)
        except ValueError:
            r = 0

        print('Target samplerate       = ', n)
        print('Estimated samplerate    = ', tt)
        ...
        x1 = data_dict_input[this_filter["input"][0]]
        x2 = data_dict_input[this_filter["input"][1]]
        x1 = medfilt(x1, 3)
        x2 = medfilt(x2, 3)
        ...
        x0 = data_dict_input[this_filter["input"][0]]
        y0 = data_dict_input[this_filter["input"][1]]
        th = this_filter["threshold"]

        i = deblinker2(x0, y0, th)
        data_dict_input[this_filter["output"]] = i

    elif match_item == 'passthrough':

        f = data_dict_input[this_filter["input"]]
        ...
        f = data_dict_input[this_filter["input"][0]]
        data_dict_input[this_filter["output"]] = dshift(f)
        ...
        is_tracking = data_dict_input[this_filter["input"][1]]
        data_dict_input[this_filter["output"]] = tidy(f, n, thicken, np.logical_not(is_tracking))
        ...
        data_dict_input[this_filter["output"]] = medfilt(f, n)
        ...
        data_dict_input[this_filter["output"]] = replace_nan_by(data_dict_input, input_array, pointer)
