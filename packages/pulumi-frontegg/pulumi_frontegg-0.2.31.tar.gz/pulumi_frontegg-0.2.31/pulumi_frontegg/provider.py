...
class ProviderArgs:
    ...
    def _configure(
        ...
        if api_base_url is None and 'apiBaseUrl' in kwargs:
            api_base_url = kwargs['apiBaseUrl']
        if environment_id is None and 'environmentId' in kwargs:
            environment_id = kwargs['environmentId']
        if portal_base_url is None and 'portalBaseUrl' in kwargs:
            portal_base_url = kwargs['portalBaseUrl']

        _setter("client_id", client_id)
        _setter("secret_key", secret_key)
        if api_base_url is not None:
            _setter("api_base_url", api_base_url)
        if environment_id is not None:
            _setter("environment_id", environment_id)
        if portal_base_url is not None:
            _setter("portal_base_url", portal_base_url)
