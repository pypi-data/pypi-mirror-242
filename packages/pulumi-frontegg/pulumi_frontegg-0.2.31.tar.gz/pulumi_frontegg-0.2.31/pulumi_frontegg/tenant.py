...
class TenantArgs:
    ...
    def _configure(
        ...
        if application_uri is None and 'applicationUri' in kwargs:
            application_uri = kwargs['applicationUri']
        if selected_metadata is None and 'selectedMetadata' in kwargs:
            selected_metadata = kwargs['selectedMetadata']

        _setter("key", key)
        if application_uri is not None:
            _setter("application_uri", application_uri)
        if name is not None:
            _setter("name", name)
        if selected_metadata is not None:
            _setter("selected_metadata", selected_metadata)
