...
from stealthbot.output import is_slash_not_in_filename
...
from .json_finder import JsonFinder


class StealthDriver(webdriver.Chrome):
    ...
    def parse_sitemap(self, sitemap_url):
        ...
        elif isinstance(sitemap_url, list):
            sitemap_urls = sitemap_url
        elif isinstance(sitemap_url, tuple) and len(sitemap_url) > 1 and isinstance(sitemap_url[1], list):
            sitemap_urls = sitemap_url[1]
        else:
            print(f"Invalid sitemap URL format for: {sitemap_url}")
            return all_urls
        ...
        for url in sitemap_urls:
            try:
                ...
                sitemap_elements = re.findall(r'<loc>(.*?)</loc>', self.page_source)
                urls = [url for url in sitemap_elements if url.startswith('http')]
                print(f"Find {len(urls)} URL in sitemap {url}")
