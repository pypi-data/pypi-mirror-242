...
from stealthbot.get_chrome_version import get_driver_path

from .driver_about import AboutBrowser
from .anti_detect_driver import StealthDriver
from .user_agent import UserAgent, UA
...
def add_essential_options(options, profile, window_size, user_agent):
    ...
    if user_agent != UserAgent.REAL:
        if user_agent == None:
            if profile == None:
                user_agent = UA.get_random()
            else:
                user_agent = UA.get_hashed(profile)
        else:
...
def load_cookies(driver: StealthDriver, profile):
...
def create_selenium_driver(proxy, options, desired_capabilities, path, attempt_download=True):
    ...
    try:
        if proxy is not None:
            from .anti_detect_driver_selenium_wire import StealthDriverSeleniumWire

            selwireOptions = {'proxy': {'http': proxy, 'https': proxy}}

            driver = StealthDriverSeleniumWire(
