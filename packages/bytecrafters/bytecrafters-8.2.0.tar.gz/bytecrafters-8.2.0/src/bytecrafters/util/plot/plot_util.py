...
import logging
from config_util import get_plotfiles_params, get_freqplots_tooltip_params, get_badplot_params
...
def add_freqplots_tooltip(config, ax, dc_offset_suppr):
    logger = logging.getLogger(__name__)
    lg_enabled, lg_fontsz, lg_boxstyle, lg_vertalign, lg_bgcolor, lg_transp, lg_xpos, lg_ypos = get_freqplots_tooltip_params(config)
    if (lg_enabled):
