...
class PlotWindowWrapper:
    ...
    def __init__(self, multitab_mode, config, num_subplots, source_info):
        ...
        if (self.multitab_on):
            ...
            self.wnd_panel_rows = mt_max_rows # also good 2 rows 1 col and max 2 plots x tab
            self.wnd_panel_cols = mt_max_cols
            self.max_plots_x_tab = max_plotsxtab
            self.tab_grid_spec = 100 * self.wnd_panel_rows + 10 * self.wnd_panel_cols # 310
            max_th_grid = rows = (self.tab_grid_spec // 100) * ((self.tab_grid_spec % 100) // 10)
            if (self.max_plots_x_tab > max_th_grid):
                self.max_plots_x_tab = max_th_grid
            logger.debug('max_plots_x_tab reassessed: ' + str(self.max_plots_x_tab))
    ...
    #for multitab only
    def add_tabbed_subplot(self, title, is3D):
        ax = None
        if (self.curr_tab_figure is None) or (self.num_plots_in_curr_tab == self.max_plots_x_tab):
            tab_title = title if (self.max_plots_x_tab == 1) else None
        ...
        ax_compact_idx = self.tab_grid_spec + self.num_plots_in_curr_tab + 1

        ax = self.curr_tab_figure.add_subplot(ax_compact_idx, projection=('3d' if is3D else None)) # rows cols, nbr in total
        #if (is3D):
        #    ax = self.curr_tab_figure.add_subplot(ax_compact_idx, procjection='3cd') # rows cols, nbr in total
        #else:
        #    ax = self.curr_tab_figure.add_subplot(ax_compact_idx) # rows cols, nbr in total

        #self.num_total_tabbed_plots = self.num_total_tabbed_plots + 1
    ...
    def add_tab(self, title):
        ...
        #figure = Figure(constrained_layout=True, figsize=self.tab_figsize, dpi=self.tab_dpi)
    ...
    def nextsubplot_axes(self, title, is3D=False):
        logger = logging.getLogger(__name__)
        if (self.multitab_on):
            #figure = self.add_tab(title)
            #ax = figure.add_suXbplot(111) # rows cols, nbr in total
            ax = self.add_tabbed_subplot(title, is3D)
