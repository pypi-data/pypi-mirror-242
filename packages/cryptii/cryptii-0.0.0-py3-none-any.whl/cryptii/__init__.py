...
def mapper(line):
    ...
    yield year, int(temperature)

# Reducer function
def reducer(year, temperatures):
    min_temperature = min(temperatures)
    max_temperature = max(temperatures)
    yield year, min_temperature, max_temperature

# Read input from file and process
input_file_path = "weather_info.txt"
...
for line in data_lines:
    for year, min_temp, max_temp in reducer(*mapper(line)):
        if year in year_temperatures:
            year_temperatures[year].append((min_temp, max_temp))
        else:
            year_temperatures[year] = [(min_temp, max_temp)]

for year, temperature_data in year_temperatures.items():
    min_temperature = min(data[0] for data in temperature_data)
    max_temperature = max(data[1] for data in temperature_data)
    print(f"{year}\t{min_temperature}\t{max_temperature}")
'''
    return s
