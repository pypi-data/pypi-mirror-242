...
from pip_services4_mysql.persistence.IdentifiableJsonMySqlPersistence import IdentifiableJsonMySqlPersistence
...
class DummyJsonMySqlPersistence(IdentifiableJsonMySqlPersistence, IDummyPersistence):
    ...
    def get_count_by_filter(self, context, filter):
        filter = filter or FilterParams()
        key = filter.get_as_nullable_string('key')
