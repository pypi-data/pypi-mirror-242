...
class GenericCRUD:
    ...
    def _get_existing_duplicate_id(self, table_name: str, error: Exception) -> int:
        ...
        if not match:  # a different error
            raise error
        constraint_name, duplicate_value = match.group(2), match.group(1)
        query = """
        SELECT COLUMN_NAME 
    ...
    # Old name: update
    # TODO add @dispatch() to all insert and update methods - Usage of Python object oriented method overloading in insert() and update()
