...
Lmesh(lnpmed,9);
        ...
        with open('references/iron_REFERENCE.yaml', "r") as stream:
            iron_yaml = yaml.safe_load(stream)

        # assert
        self.assertEqual(data_iron, iron_yaml)
...
"""
        ...
        with open('references/cadata_REFERENCE.yaml', "r") as stream:
            cadata_yaml = yaml.safe_load(stream)

        # assert
        self.assertEqual(cadata, cadata_yaml)
...
"""
        ...
        with open('references/coil_REFERENCE.yaml', "r") as stream:
    ...
    def test_applyMultipoleSymmetry(self):
        ...
        with open('references/symmetry_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)
    ...
    def test_applyCoilTransformation(self):
        ...
        with open('references/transformation_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)
    ...
    def test_applySymmetryConditions(self):
        ...
        with open('references/symmetries_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)
    ...
    def test_applyTransformations(self):
        ...
        with open('references/transformations_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)
    ...
    def test_findConductorPositions(self):
        ...
        # Yaml.emitter.alt_null = 'Null'
        # Yaml.representer.add_representer(type(None), my_represent_none)
        # with open('references/block_output.yaml', 'w') as yaml_file:
        #     Yaml.dump(data.dict(), yaml_file)
        with open('references/block_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)
    ...
    def test_getCablePositions(self):
        ...
        cadata = pd.Cadata(
            ...
            quench={'NB3SNMP': pd.Quench(SCHeatCapa=3.0, CuHeatCapa=1.0, CuThermCond=1.0, CuElecRes=1.0,
                                         InsHeatCapa=2.0, InsThermCond=4.0, FillHeatCapa=3.0, He=0.0,
                                         comment="'matpro mat prop. Nb3Sn coils '")},
            cable={'CXF150HT5': pd.Cable(height=18.363, width_i=1.53, width_o=1.658, ns=40.0, transp=109.0, degrd=0.0,
                                         comment="'MQXF150, after HT, March2015 '")},
            conductor={'XF145HTH5': pd.Conductor(conductorType=None, cableGeom='CXF150HT5', strand='STRXF150',
        ...
        # Yaml.emitter.alt_null = 'Null'
        # Yaml.representer.add_representer(type(None), my_represent_none)
        # with open('references/cables_output.yaml', 'w') as yaml_file:
        #     Yaml.dump(data.dict(), yaml_file)
        with open('references/cables_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)
    ...
    def test_getWedgePositions(self):
        ...
        """
        # arrange
        coil = pd.CoilData(coils={1: pd.Pole(bore_center=pd.Coord(x=0.0, y=0.0), poles={1: pd.Layer(layers={
        ...
        # Yaml.emitter.alt_null = 'Null'
        # Yaml.representer.add_representer(type(None), my_represent_none)
        # with open('references/wedges_output.yaml', 'w') as yaml_file:
        #     Yaml.dump(data.dict(), yaml_file)
        with open('references/wedges_REFERENCE.yaml', "r") as stream:
            data_yaml = yaml.safe_load(stream)

        # assert
        self.assertEqual(data.dict(), data_yaml)
    ...
    def test_compare_half_turns_LEDET_ribbon(self):
        ...
        for name in magnet_names:
            ...
            roxie_parser = ParserRoxie()
            roxie_data = roxie_parser.getData(dir_data=path_data, dir_cadata=path_cadata,
                                                   dir_iron=None,
            ...
            for index, row in df.iterrows():
                ...
                rectangle_area = area_of_rectangle(x1, y1, x2, y2, x3, y3, x4, y4)
...
def area_of_rectangle(x1, y1, x2, y2, x3, y3, x4, y4):
