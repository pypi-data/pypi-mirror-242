...
class TestParserXYCE(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.path_XYCE = settings_dict['XYCE_path']
        print('path_XYCE:        {}'.format(self.path_XYCE))
    ...
    def test_write2XYCE_bare_minimum(self):
        ...
        file_name = os.path.join('output', 'netlist_bare_minimum_TEST.cir')
        ...
        circuit_data.Netlist.__setattr__('comment1', new_Component)
    ...
    def test_copy_additional_files_translated(self):
        ...
        # act
        pp = ParserXYCE(circuit_data=circuit_data, output_path=output_path)
        pp.copy_additional_files(flag_translate=flag_translate)
    ...
    def test_write2spice_manual(self):
        self.maxDiff = None
        # arrange - Define output file name. If file already exists, delete it
        file_name = os.path.join(os.getcwd(), 'output', 'netlist_TEST.cir')
        if os.path.isfile(file_name):
            os.remove(file_name)
            print('File {} already existed. It was deleted now.'.format(file_name))
        ...
        circuit_data.Netlist.__setattr__('L1', new_Component)
        new_Component = Component()
        ...
        circuit_data.Netlist.__setattr__('xD_PC_RQX', new_Component)
        new_Component = Component()
        ...
        circuit_data.Netlist.__setattr__('I_PC', new_Component)
        new_Component = Component()
        ...
        circuit_data.Netlist.__setattr__('E_ABM', new_Component)
        new_Component = Component()
        ...
        circuit_data.Netlist.__setattr__('V_control', new_Component)
        new_Component = Component()
        ...
        circuit_data.Netlist.__setattr__('xPC_minitrim', new_Component)
        new_Component = Component()
        ...
        circuit_data.Netlist.__setattr__('xPC_minitrim_2', new_Component)
        circuit_data.Analysis.analysis_type = 'transient'
    ...
    def test_write2yaml(self, file_name='IPQ_2magnets'):
        ...
        output_file_full_path = os.path.join('output', file_name + '.yaml')
    ...
    def test_read_write_netlist_consistency(self, circuit_name='IPQ_2magnets'):
        ...
        # Define output file name. If file already exists, delete it
        file_name_rewritten = os.path.join('output', circuit_name + '_REWRITTEN.cir')
        ...
        file_name_rewritten_yaml = os.path.join('output', circuit_name + '_modelData_REWRITTEN.yaml')
        # act
    ...
    def test_read_write_yaml_consistency(self, circuit_name='IPQ_2magnets'):
        ...
        file_name_rewritten_yaml = os.path.join('output', circuit_name + '_READ.yaml')
        file_name_rewritten_cir  = os.path.join('output', circuit_name + '_REWRITTEN_fromYAML.cir')
    ...
    def test_translate_library_PSPICE_2_XYCE(self):
        ...
        file_output = os.path.join('output','XYCE_test_library.lib')
        #Act
    ...
    def test_translate_stimulus(self):
        ...
        file_out1 = os.path.join('output', f'{stl_name1}.csv')
        file_out2 = os.path.join('output', f'{stl_name2}.csv')
        ...
        translate_stimulus(stl_name1, file_input, path_output = path_output)
        translate_stimulus(stl_name2, file_input, path_output = path_output)
    ...
    def test_translate_create_run_full_model(self, name='RB'):
        ...
        output_path = os.path.join('output', f'XYCE_test_{name}')
        ...
        pXYCE.write2XYCE(output_path_cir, verbose=verbose)
        pXYCE.copy_additional_files(flag_translate=True, verbose=verbose)
        pXYCE.resolve_paths_netlist(output_path_cir)

        # dXYCE = DriverXYCE(self.path_XYCE, output_path, verbose=verbose)
    ...
    def test_translate_create_run_full_model_multipleCircuits(self):
        ...
        for name in list_circuits:
            ...
            output_csd = os.path.join(output_path, f'{name}.csd')
            if not os.path.exists(output_path):
                os.mkdir(output_path)
            else:
                shutil.rmtree(output_path)
