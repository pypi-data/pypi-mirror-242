...
class MultipoleSolveTimeParametersAdaptive(BaseModel):
    ...
    rel_tol_time: float = None
    abs_tol_time: float = None
    norm_type: str = None
...
class MultipoleSolveThermal(BaseModel):
    ...
    """
    solved: str = None
    TSA_integration_order: int = None
...
class Pancake3DPosition(BaseModel):
    x: Optional[float] = Field(default=None, title='x coordinate', description='x coordinate of the position.')
    y: Optional[float] = Field(default=None, title='y coordinate', description='y coordinate of the position.')
    z: Optional[float] = Field(default=None, title='z coordinate', description='z coordinate of the position.')
    turnNumber: Optional[PositiveFloat] = Field(default=None, title='Turn Number', description='Winding turn number as a position input.')
    whichPancakeCoil: Optional[PositiveInt] = Field(default=None, title='Pancake Coil Number', description='The first pancake coil is 1, the second is 2, etc.')

class Pancake3DGeometryWinding(BaseModel):
    r_i: Optional[PositiveFloat] = Field(alias='innerRadius', title='Inner Radius', description='Inner radius of the winding.', default=None)
    t: Optional[PositiveFloat] = Field(alias='thickness', title='Winding Thickness', description='Thickness of the winding.', default=None)
    N: Optional[float] = Field(alias='numberOfTurns', ge=3, title='Number of Turns', description='Number of turns of the winding.', default=None)
    h: Optional[PositiveFloat] = Field(alias='height', title='Winding Height', description='Height/width of the winding.', default=None)
    name: Optional[str] = Field(default='winding', title='Winding Name', description='The The name to be used in the mesh..', examples=['winding', 'myWinding'])
    NofVolPerTurn: Optional[int] = Field(default=2, alias='numberOfVolumesPerTurn', ge=2, title='Number of Volumes Per Turn (Advanced Input)', description='The number of volumes per turn (CAD related, not physical).')
    theta_i: Optional[float] = Field(default=0.0, alias='startAngle', title='Start Angle', description='The start angle of the first pancake coil in radians.')

class Pancake3DGeometryInsulation(BaseModel):
    tsa: Optional[bool] = Field(alias='thinShellApproximation', title='Use Thin Shell Approximation', description='If True, the insulation will be modeled with 2D shell elements (thin shell approximation), and if False, the insulation will be modeled with 3D elements.', default=None)
    t: Optional[PositiveFloat] = Field(alias='thickness', title='Insulation Thickness', description='Thickness of the insulation.', default=None)
    name: Optional[str] = Field(default='insulation', title='Insulation Name', description='The name to be used in the mesh.', examples=['insulation', 'myInsulation'])

class Pancake3DGeometryTerminal(BaseModel):
    name: Optional[str] = Field(title='Terminal Name', description='The name to be used in the mesh.', examples=['innerTerminal', 'outerTeminal'], default=None)
    t: Optional[PositiveFloat] = Field(alias='thickness', title='Terminal Thickness', description="Thickness of the terminal's tube.", default=None)

class Pancake3DGeometryTerminals(BaseModel):
    i: Optional[Pancake3DGeometryTerminal] = Field(alias='inner', default=Pancake3DGeometryTerminal())
    o: Optional[Pancake3DGeometryTerminal] = Field(alias='outer', default=Pancake3DGeometryTerminal())
    firstName: Optional[str] = Field(default='firstTerminal', description='name of the first terminal')
    lastName: Optional[str] = Field(default='lastTerminal', description='name of the last terminal')

class Pancake3DGeometryAir(BaseModel):
    r: Optional[PositiveFloat] = Field(default=None, alias='radius', title='Air Radius', description='Radius of the air (for cylinder type air).')
    a: Optional[PositiveFloat] = Field(default=None, alias='sideLength', title='Air Side Length', description='Side length of the air (for cuboid type air).')
    margin: Optional[PositiveFloat] = Field(alias='axialMargin', title='Axial Margin of the Air', description='Axial margin between the ends of the air and first/last pancake coils.', default=None)
    name: Optional[str] = Field(default='air', title='Air Name', description='The name to be used in the mesh.', examples=['air', 'myAir'])
    type: Optional[Literal['cylinder', 'cuboid']] = Field(default='cylinder', title='Air Type', description='Air can be a cylinder or cuboid.')
    shellTransformation: Optional[bool] = Field(default=False, alias='shellTransformation', title='Use Shell Transformation', description='Generate outer shell air to apply shell transformation if True (GetDP related, not physical)')
    shellTransformationMultiplier: Optional[float] = Field(default=1.2, gt=1, alias='shellTransformationMultiplier', title='Shell Transformation Multiplier (Advanced Input)', description="multiply the air's outer dimension by this value to get the shell's outer dimension")
    cutName: Optional[str] = Field(default='Air-Cut', title='Air Cut Name', description='name of the cut (cochain) to be used in the mesh', examples=['Air-Cut', 'myAirCut'])
    shellVolumeName: Optional[str] = Field(default='air-Shell', title='Air Shell Volume Name', description='name of the shell volume to be used in the mesh', examples=['air-Shell', 'myAirShell'])
    fragment: Optional[bool] = Field(default=False, alias='generateGapAirWithFragment', title='Generate Gap Air with Fragment (Advanced Input)', description='generate the gap air with gmsh/model/occ/fragment if true (CAD related, not physical)')

class Pancake3DMeshWinding(BaseModel):
    axne: Optional[Union[List[PositiveInt], PositiveInt]] = Field(alias='axialNumberOfElements', title='Axial Number of Elements', description='The number of axial elements for the whole height of the coil. It can be either a list of integers to specify the value for each pancake coil separately or an integer to use the same setting for each pancake coil.', default=None)
    ane: Optional[Union[List[PositiveInt], PositiveInt]] = Field(alias='azimuthalNumberOfElementsPerTurn', title='Azimuthal Number of Elements Per Turn', description='The number of azimuthal elements per turn of the coil. It can be either a list of integers to specify the value for each pancake coil separately or an integer to use the same setting for each pancake coil.', default=None)
    rne: Optional[Union[List[PositiveInt], PositiveInt]] = Field(alias='radialNumberOfElementsPerTurn', title='Winding Radial Number of Elements Per Turn', description='The number of radial elements per tape of the winding. It can be either a list of integers to specify the value for each pancake coil separately or an integer to use the same setting for each pancake coil.', default=None)
    axbc: Optional[Union[List[PositiveFloat], PositiveFloat]] = Field(default=[1], alias='axialBumpCoefficients', title='Axial Bump Coefficients', description="If 1, it won't affect anything. If smaller than 1, elements will get finer in the axial direction at the ends of the coil. If greater than 1, elements will get coarser in the axial direction at the ends of the coil. It can be either a list of floats to specify the value for each pancake coil separately or a float to use the same setting for each pancake coil.")
    elementType: Optional[Union[List[Literal['tetrahedron', 'hexahedron', 'prism']], Literal['tetrahedron', 'hexahedron', 'prism']]] = Field(default=None, title='Element Type', description='The element type of windings and insulations. It can be either a tetrahedron, hexahedron, or a prism. It can be either a list of strings to specify the value for each pancake coil separately or a string to use the same setting for each pancake coil.')

class Pancake3DMeshInsulation(BaseModel):
    rne: Optional[List[PositiveInt]] = Field(alias='radialNumberOfElementsPerTurn', title='Insulation Radial Number of Elements Per Turn', description='The number of radial elements per tape of the insulation. It can be either a list of integers to specify the value for each pancake coil separately or an integer to use the same setting for each pancake coil.', default=None)

class Pancake3DMeshAirAndTerminals(BaseModel):
    structured: Optional[bool] = Field(default=False, title='Structure Mesh', description='If True, the mesh will be structured. If False, the mesh will be unstructured.')
    radialElementSize: Optional[PositiveFloat] = Field(default=1, title='Radial Element Size', description="If structured mesh is used, the radial element size can be set. It is the radial element size in terms of the winding's radial element size.")

class Pancake3DSolveAir(BaseModel):
    permeability: Optional[PositiveFloat] = Field(title='Permeability of Air', description='Permeability of air.', default=None)

class Pancake3DSolveIcVsLength(BaseModel):
    lengthValues: Optional[List[float]] = Field(title='Tape Length Values', description='Tape length values that corresponds to criticalCurrentValues.', default=None)
    criticalCurrentValues: Optional[List[float]] = Field(title='Critical Current Values', description='Critical current values that corresponds to lengthValues.', default=None)

class Pancake3DSolveCERNSuperConductorMaterial(BaseModel):
    name: Optional[Literal['HTSSuperPower']] = Field(title='Superconductor Material Name', description='Name of the superconductor material.', default=None)
    IcAtTinit: Optional[Union[PositiveFloat, str, Pancake3DSolveIcVsLength]] = Field(alias='criticalCurrentAtInitialTemperature', title='Critical Current at Initial Temperature', description='Critical current at initial temperature. The critical current value will change with temperature depending on the superconductor material.\n#Either the same critical current for the whole tape or the critical current with respect to the tape length can be specified. To specify the same critical current for the entire tape, just use a scalar. To specify critical current with respect to the tape length: a CSV file can be used, or lengthValues and criticalCurrentValues can be given as lists. The data will be linearly interpolated.\n#If a CSV file is to be used, the input should be the name of a CSV file (which is in the same folder as the input file) instead of a scalar. The first column of the CSV file will be the tape length, and the second column will be the critical current.', examples=[230, 'IcVSlength.csv'], default=None)
    nValue: Optional[PositiveFloat] = Field(default=30, alias='N-Value for E-J Power Law', description='N-value for E–J power law.')
    relativeThickness: Optional[float] = Field(default=None, le=1, title='Relative Thickness (only for winding)', description='Winding tapes generally consist of more than one material. Therefore, when materials are given as a list in winding, their relative thickness, (thickness of the material) / (thickness of the winding), should be specified.')
    jCriticalScalingNormalToWinding: Optional[PositiveFloat] = Field(default=1, title='Critical Current Scaling Normal to Winding', description='Critical current scaling normal to winding, i.e., along the c_axis.  We have Jc_cAxis = scalingFactor * Jc_abPlane. A factor of 1 means no scaling such that the HTS layer is isotropic.')
    electricFieldCriterion: Optional[PositiveFloat] = Field(default=0.0001, title='Electric Field Criterion', description='The electric field that defines the critical current density, i.e., the electric field at which the current density reaches the critical current density.')

class Pancake3DSolveCERNPlating(BaseModel):
    name: Optional[Literal['Copper']] = Field(title='Material Name', description='choices: Copper', default=None)
    relativeWidth: Optional[float] = Field(default=0, title='Relative width of plating', le=1, description='HTS 2G coated conductor are typically plated, usually  using copper. The relative width of the plating is the  width of the plating divided by the width of the tape.  0 means no plating.')

class Pancake3DSolveCERNNormalMaterial(BaseModel):
    name: Optional[Literal['Copper', 'Hastelloy', 'Silver', 'Indium', 'Stainless Steel']] = Field(title='Material Name', description='choices: Copper, Hastelloy, Silver, Indium, Stainless Steel', default=None)
    relativeThickness: Optional[float] = Field(default=None, le=1, title='Relative Thickness (only for winding)', description='Winding tapes generally consist of more than one material. Therefore, when materials are given as a list in winding, their relative thickness, (thickness of the material) / (thickness of the winding), should be specified.')
    rrr: Optional[PositiveFloat] = Field(default=100, alias='residualResistanceRatio', title='Residual Resistance Ratio', description='Residual-resistivity ratio (also known as Residual-resistance ratio or just RRR) is the ratio of the resistivity of a material at reference temperature and at 0 K.')
    rrrRefT: Optional[PositiveFloat] = Field(default=295, alias='residualResistanceRatioReferenceTemperature', title='Residual Resistance Ratio Reference Temperature', description='Reference temperature for residual resistance ratio')

class Pancake3DSolveMaterial(BaseModel):
    numberOfThinShellElements: Optional[PositiveInt] = Field(default=1, title='Number of Thin Shell Elements (Advanced Input)', description='Number of thin shell elements in the FE formulation (GetDP related, not physical and only used when TSA is set to True)')
    resistivity: Optional[PositiveFloat] = Field(default=None, title='Resistivity', description="A scalar value. If this is given, material properties won't be used for resistivity.")
    thermalConductivity: Optional[PositiveFloat] = Field(default=None, title='Thermal Conductivity', description="A scalar value. If this is given, material properties won't be used for thermal conductivity.")
    specificHeatCapacity: Optional[PositiveFloat] = Field(default=None, title='Specific Heat Capacity', description="A scalar value. If this is given, material properties won't be used for specific heat capacity.")
    material: Optional[Pancake3DSolveCERNNormalMaterial] = Field(default=Pancake3DSolveCERNNormalMaterial(), title='Material', description='Material from CERN material library.')

class Pancake3DSolveInsulationMaterial(Pancake3DSolveMaterial):
    resistivity: Optional[Union[PositiveFloat, Literal['perfectlyInsulating']]] = Field(default=None, title='Resistivity', description='A scalar value or "perfectlyInsulating". If "perfectlyInsulating" is given, the insulation will be perfectly insulating. If this value is given, material properties won\'t be used for resistivity.')

class Pancake3DSolveTerminalMaterialAndBoundaryCondition(Pancake3DSolveMaterial):
    cooling: Optional[Literal['adiabatic', 'fixedTemperature', 'cryocooler']] = Field(default='fixedTemperature', title='Cooling condition', description='Cooling condition of the terminal. It can be either adiabatic, fixed temperature, or cryocooler.')

class Pancake3DSolveWindingMaterial(BaseModel):
    resistivity: Optional[PositiveFloat] = Field(default=None, title='Resistivity', description="A scalar value. If this is given, material properties won't be used for resistivity.")
    thermalConductivity: Optional[PositiveFloat] = Field(default=None, title='Thermal Conductivity', description="A scalar value. If this is given, material properties won't be used for thermal conductivity.")
    specificHeatCapacity: Optional[PositiveFloat] = Field(default=None, title='Specific Heat Capacity', description="A scalar value. If this is given, material properties won't be used for specific heat capacity.")
    minimumPossibleResistivity: Optional[NonNegativeFloat] = Field(default=0, title='Minimum Possible Resistivity', description="The resistivity of the winding won't be lower than this value, no matter what.")
    maximumPossibleResistivity: Optional[PositiveFloat] = Field(default=1, title='Maximum Possible Resistivity', description="The resistivity of the winding won't be higher than this value, no matter what.")
    material: Optional[List[Union[Pancake3DSolveCERNNormalMaterial, Pancake3DSolveCERNSuperConductorMaterial]]] = Field(default=[Pancake3DSolveCERNNormalMaterial()], title='Winding Material', description='Because generally winding tapes consist of more than one material, materials should be given as a list of Material dictionaries.')
    plating: Optional[Pancake3DSolveCERNPlating] = Field(default=Pancake3DSolveCERNPlating(name="Copper", relativeWidth=0), title='Tape plating', description='The plating of the winding tape. Only copper is supported for now.By default, the tape is not plated.')

class Pancake3DSolveTolerance(BaseModel):
    quantity: Optional[Literal['solutionVector', 'totalResistiveHeating', 'voltageBetweenTerminals', 'resistiveHeating', 'temperature', 'magnitudeOfCurrentDensity', 'magnitudeOfMagneticField', 'axialComponentOfTheMagneticField', 'currentThroughCoil', 'magnitudeOfHeatFlux']] = Field(title='Quantity', description='Name of the quantity for tolerance.', default=None)
    relative: Optional[NonNegativeFloat] = Field(title='Relative Tolerance', description='Relative tolerance for the quantity.', default=None)
    absolute: Optional[NonNegativeFloat] = Field(title='Absolute Tolerance', description='Absolute tolerance for the quantity', default=None)
    position: Optional[Pancake3DPosition] = Field(default=Pancake3DPosition(), title='Probing Position of the Quantity', description='Probing position of the quantity for tolerance.')
    normType: Optional[Literal['L1Norm', 'MeanL1Norm', 'L2Norm', 'MeanL2Norm', 'LinfNorm']] = Field(default='L2Norm', alias='normType', title='Norm Type', description='Sometimes, tolerances return a vector instead of a scalar (ex, solutionVector). Then, the magnitude of the tolerance should be calculated with a method. Norm type selects this method.')

class Pancake3DSolveAdaptiveTimeLoopSettings(BaseModel):
    initialStep: Optional[PositiveFloat] = Field(default=None, alias='initialStep', title='Initial Step for Adaptive Time Stepping', description='Initial step for adaptive time stepping')
    minimumStep: Optional[PositiveFloat] = Field(default=None, alias='minimumStep', title='Minimum Step for Adaptive Time Stepping', description='The simulation will be aborted if a finer time step is required than this minimum step value.')
    maximumStep: Optional[PositiveFloat] = Field(default=None, alias='maximumStep', description="Bigger steps than this won't be allowed")
    tolerances: Optional[List[Pancake3DSolveTolerance]] = Field(default=[Pancake3DSolveTolerance()], title='Tolerances for Adaptive Time Stepping', description='Time steps will be adjusted so the predicted error of quantities below will stay in the tolerance range. Tolerances will decide how fine the time stepping will be.')
    integrationMethod: Optional[Literal['Euler', 'Gear_2', 'Gear_3', 'Gear_4', 'Gear_5', 'Gear_6']] = Field(default='Euler', alias='integrationMethod', title='Integration Method', description='Integration method for transient analysis')
    breakPoints: Optional[List[float]] = Field(default=[0], alias='breakPoints', title='Break Points for Adaptive Time Stepping', description='Make sure to solve the system for these times.')

class Pancake3DSolveFixedTimeLoopSettings(BaseModel):
    step: Optional[PositiveFloat] = Field(default=None, title='Step for Fixed Time Stepping', description='Time step for fixed time stepping.')

class Pancake3DSolveFixedLoopInterval(BaseModel):
    startTime: Optional[NonNegativeFloat] = Field(title='Start Time of the Interval', description='Start time of the interval.', default=None)
    endTime: Optional[NonNegativeFloat] = Field(title='End Time of the Interval', description='End time of the interval.', default=None)
    step: Optional[PositiveFloat] = Field(title='Step for the Interval', description='Time step for the interval', default=None)

class Pancake3DSolveTime(BaseModel):
    adaptive: Optional[Pancake3DSolveAdaptiveTimeLoopSettings] = Field(default=Pancake3DSolveAdaptiveTimeLoopSettings(), alias='adaptiveSteppingSettings', title='Adaptive Time Loop Settings', description='Adaptive time loop settings (only used if stepping type is adaptive).')
    fixed: Optional[Union[List[Pancake3DSolveFixedLoopInterval], Pancake3DSolveFixedTimeLoopSettings]] = Field(default=None, alias='fixedSteppingSettings', title='Fixed Time Loop Settings', description='Fixed time loop settings (only used if stepping type is fixed).')
    start: Optional[float] = Field(title='Start Time', description='Start time of the simulation.', default=None)
    end: Optional[float] = Field(title='End Time', description='End time of the simulation.', default=None)
    timeSteppingType: Optional[Literal['fixed', 'adaptive']] = Field(title='Time Stepping Type', description='Time stepping type.', default=None)
    extrapolationOrder: Optional[Literal[0, 1, 2, 3]] = Field(default=1, alias='extrapolationOrder', title='Extrapolation Order', description='Before solving for the next time steps, the previous solutions can be extrapolated for better convergence.')

class Pancake3DSolveNonlinearSolverSettings(BaseModel):
    tolerances: Optional[List[Pancake3DSolveTolerance]] = Field(default=[Pancake3DSolveTolerance()], title='Tolerances for Nonlinear Solver', description='The nonlinear solver will check the tolerance quantities to make sure it converged or not. If the quantities stay in the tolerance region after an iteration, it will conclude that the nonlinear solver converged.')
    maxIter: Optional[PositiveInt] = Field(default=100, alias='maximumNumberOfIterations', title='Maximum Number of Iterations', description='Maximum number of iterations allowed for the nonlinear solver.')
    relaxationFactor: Optional[float] = Field(default=0.7, gt=0, alias='relaxationFactor', title='Relaxation Factor', description='Calculated step changes of the solution vector will be multiplied with this value for better convergence.')

class Pancake3DSolveInitialConditions(BaseModel):
    T: Optional[PositiveFloat] = Field(alias='temperature', title='Initial Temperature', description='Initial temperature of the pancake coils.', default=None)

class Pancake3DSolveLocalDefect(BaseModel):
    value: Optional[NonNegativeFloat] = Field(default=None, alias='value', title='Value', description='Value of the local defect.')
    transitionDuration: Optional[PositiveFloat] = Field(default=None, title='Transition Duration', description='Transition duration of the local defect. If not given, the transition will be instantly.')
    startTurn: Optional[NonNegativeFloat] = Field(default=None, alias='startTurn', title='Start Turn', description='Start turn of the local defect.')
    endTurn: Optional[PositiveFloat] = Field(default=None, alias='endTurn', title='End Turn', description='End turn of the local defect.')
    startTime: Optional[NonNegativeFloat] = Field(default=None, alias='startTime', title='Start Time', description='Start time of the local defect.')

class Pancake3DSolveLocalDefects(BaseModel):
    jCritical: Optional[Pancake3DSolveLocalDefect] = Field(default=Pancake3DSolveLocalDefect(), alias='criticalCurrentDensity', title='Local Defect for Critical Current Density', description='Set critical current density locally.')

class Pancake3DSolveSaveQuantity(BaseModel):
    quantity: Optional[Literal['magneticField', 'magnitudeOfMagneticField', 'currentDensity', 'magnitudeOfCurrentDensity', 'resistiveHeating', 'temperature', 'voltageBetweenTerminals', 'currentThroughCoil', 'criticalCurrentDensity', 'heatFlux', 'resistivity']] = Field(title='Quantity', description='Name of the quantity to be saved.', default=None)
    timesToBeSaved: Optional[List[float]] = Field(default=None, title='Times to be Saved', description='List of times that wanted to be saved. If not given, all the time steps will be saved.')

class Pancake3DPostprocessTimeSeriesPlot(BaseModel):
    position: Optional[Pancake3DPosition] = Field(default=Pancake3DPosition(), title='Probing Position', description='Probing position of the quantity for time series plot.')
    quantity: Optional[Literal['magnitudeOfMagneticField', 'temperature', 'magnitudeOfCurrentDensity', 'resistiveHeating', 'axialComponentOfTheMagneticField', 'totalResistiveHeating', 'voltageBetweenTerminals', 'currentThroughCoil', 'magnitudeOfHeatFlux', 'resistivity', 'derivativeOfRhoWithRespectToJ']] = Field(default=None, title='Quantity', description='Name of the quantity to be plotted.')

class Pancake3DPostprocessMagneticFieldOnPlane(BaseModel):
    colormap: Optional[Literal['jet', 'ocean', 'gist_earth', 'terrain', 'gist_stern', 'gnuplot', 'gnuplot2', 'CMRmap', 'cubehelix', 'brg', 'gist_rainbow', 'rainbow', 'turbo', 'nipy_spectral', 'gist_ncar']] = Field(default='jet', title='Colormap', description='Colormap for the plot.')
    streamLines: Optional[bool] = Field(default=True, title='Stream Lines', description="If True, streamlines will be plotted. Note that magnetic field vectors may have components perpendicular to the plane, and streamlines will be drawn depending on the vectors' projection onto the plane.")
    interpolationMethod: Optional[Literal['nearest', 'linear', 'cubic']] = Field(default='linear', title='Interpolation Method', description='Interpolation type for the plot.\n#Because of the FEM basis function selections of FiQuS, each mesh element has a constant magnetic field vector. Therefore, for smooth 2D plots, interpolation can be used.\n#Types:\n#nearest: it will plot the nearest magnetic field value to the plotting point.\n#linear: it will do linear interpolation to the magnetic field values.\n#cubic: it will do cubic interpolation to the magnetic field values.')
    timesToBePlotted: Optional[List[float]] = Field(default=None, title='Times to be Plotted', description='List of times that wanted to be plotted. If not given, all the time steps will be plotted.')
    planeNormal: Optional[List[float]] = Field(default=[1, 0, 0], title='Plane Normal', description='Normal vector of the plane. The default is YZ-plane (1, 0, 0).')
    planeXAxisUnitVector: Optional[List[float]] = Field(default=[0, 1, 0], title='Plane X Axis', description="If an arbitrary plane is wanted to be plotted, the arbitrary plane's X axis unit vector must be specified. The dot product of the plane's X axis and the plane's normal vector must be zero.")

class Pancake3DGeometry(BaseModel):
    wi: Optional[Pancake3DGeometryWinding] = Field(alias='winding', title='Winding Geometry', description='This dictionary contains the winding geometry information.', default=Pancake3DGeometryWinding())
    ii: Optional[Pancake3DGeometryInsulation] = Field(alias='insulation', title='Insulation Geometry', description='This dictionary contains the insulation geometry information.', default=Pancake3DGeometryInsulation())
    ti: Optional[Pancake3DGeometryTerminals] = Field(alias='terminals', title='Terminals Geometry', description='This dictionary contains the terminals geometry information.', default=Pancake3DGeometryTerminals())
    ai: Optional[Pancake3DGeometryAir] = Field(alias='air', title='Air Geometry', description='This dictionary contains the air geometry information.', default=Pancake3DGeometryAir())
    N: Optional[PositiveInt] = Field(alias='numberOfPancakes', title='Number of Pancakes', description='Number of pancake coils stacked on top of each other.', default=None)
    gap: Optional[PositiveFloat] = Field(alias='gapBetweenPancakes', title='Gap Between Pancakes', description='Gap distance between the pancake coils.', default=None)
    dimTol: Optional[PositiveFloat] = Field(default=1e-08, alias='dimensionTolerance', description='dimension tolerance (CAD related, not physical)')
    pancakeBoundaryName: Optional[str] = Field(default='PancakeBoundary', description="name of the pancake's curves that touches the air to be used in the mesh")
    insulationBoundaryName: Optional[str] = Field(default='InsulationBoundary', description="name of the insulation's curves that touches the air to be used in the mesh (only for TSA)")

class Pancake3DMesh(BaseModel):
    wi: Optional[Pancake3DMeshWinding] = Field(alias='winding', title='Winding Mesh', description='This dictionary contains the winding mesh information.', default=Pancake3DMeshWinding())
    ii: Optional[Pancake3DMeshInsulation] = Field(alias='insulation', title='Insulation Mesh', description='This dictionary contains the insulation mesh information.', default=Pancake3DMeshInsulation())
    ti: Optional[Pancake3DMeshAirAndTerminals] = Field(default=Pancake3DMeshAirAndTerminals(), alias='terminals', title='Terminal Mesh', description='This dictionary contains the terminal mesh information.')
    ai: Optional[Pancake3DMeshAirAndTerminals] = Field(default=Pancake3DMeshAirAndTerminals(), alias='air', title='Air Mesh', description='This dictionary contains the air mesh information.')
    relSizeMin: Optional[PositiveFloat] = Field(alias='minimumElementSize', title='Minimum Element Size', description='The minimum mesh element size in terms of the largest mesh size in the winding. This mesh size will be used in the regions close the the winding, and then the mesh size will increate to maximum mesh element size as it gets away from the winding.', default=None)
    relSizeMax: Optional[PositiveFloat] = Field(alias='maximumElementSize', title='Maximum Element Size', description='The maximum mesh element size in terms of the largest mesh size in the winding. This mesh size will be used in the regions close the the winding, and then the mesh size will increate to maximum mesh element size as it gets away from the winding.', default=None)

class Pancake3DSolve(BaseModel):
    t: Optional[Pancake3DSolveTime] = Field(alias='time', title='Time Settings', description='All the time related settings for transient analysis.', default=Pancake3DSolveTime())
    nls: Optional[Pancake3DSolveNonlinearSolverSettings] = Field(alias='nonlinearSolver', title='Nonlinear Solver Settings', description='All the nonlinear solver related settings.', default=Pancake3DSolveNonlinearSolverSettings())
    wi: Optional[Pancake3DSolveWindingMaterial] = Field(alias='winding', title='Winding Properties', description='This dictionary contains the winding material properties.', default=Pancake3DSolveWindingMaterial())
    ii: Optional[Pancake3DSolveInsulationMaterial] = Field(alias='insulation', title='Insulation Properties', description='This dictionary contains the insulation material properties.', default=Pancake3DSolveInsulationMaterial())
    ti: Optional[Pancake3DSolveTerminalMaterialAndBoundaryCondition] = Field(alias='terminals', title='Terminals Properties', description='This dictionary contains the terminals material properties and cooling condition.', default=Pancake3DSolveTerminalMaterialAndBoundaryCondition())
    ai: Optional[Pancake3DSolveAir] = Field(alias='air', title='Air Properties', description='This dictionary contains the air material properties.', default=Pancake3DSolveAir())
    ic: Optional[Pancake3DSolveInitialConditions] = Field(alias='initialConditions', title='Initial Conditions', description='Initial conditions of the problem.', default=Pancake3DSolveInitialConditions())
    save: Optional[List[Pancake3DSolveSaveQuantity]] = Field(alias='quantitiesToBeSaved', default=[Pancake3DSolveSaveQuantity()], title='Quantities to be Saved', description='List of quantities to be saved.')
    type: Optional[Literal['electromagnetic', 'thermal', 'weaklyCoupled', 'stronglyCoupled']] = Field(title='Simulation Type', description='FiQuS/Pancake3D can solve only electromagnetics and thermal or electromagnetic and thermal coupled. In the weaklyCoupled setting, thermal and electromagnetics systems will be put into different matrices, whereas in the stronglyCoupled setting, they all will be combined into the same matrix. The solution should remain the same.', default=None)
    proTemplate: Optional[str] = Field(default='Pancake3D_template.pro', description='file name of the .pro template file')
    localDefects: Optional[Pancake3DSolveLocalDefects] = Field(default=Pancake3DSolveLocalDefects(), alias='localDefects', title='Local Defects', description='Local defects (like making a small part of the winding normal conductor at some time) can be introduced.')
    initFromPrevious: Optional[str] = Field(default='', title='Full path to res file to continue from', description='The simulation is continued from an existing .res file.  The .res file is from a previous computation on the same geometry and mesh. The .res file is taken from the folder Solution_<<initFromPrevious>>')
    isothermalInAxialDirection: Optional[bool] = Field(default=False, title='Equate DoF along axial direction', description='If True, the DoF along the axial direction will be equated. This means that the temperature will be the same along the axial direction reducing the number of DoF. This is only valid for the thermal analysis.')

class Pancake3DPostprocess(BaseModel):
    timeSeriesPlots: Optional[List[Pancake3DPostprocessTimeSeriesPlot]] = Field(default=[Pancake3DPostprocessTimeSeriesPlot()], title='Time Series Plots', description='Values can be plotted with respect to time.')
    magneticFieldOnCutPlane: Optional[Pancake3DPostprocessMagneticFieldOnPlane] = Field(default=Pancake3DPostprocessMagneticFieldOnPlane(), title='Magnetic Field on a Cut Plane', description='Color map of the magnetic field on the YZ plane can be plotted with streamlines.')

class Pancake3D(BaseModel):
    type: Optional[Literal['Pancake3D']] = Field(default=None)
    geometry: Optional[Pancake3DGeometry] = Field(default=Pancake3DGeometry(), title='Geometry', description='This dictionary contains the geometry information.')
    mesh: Optional[Pancake3DMesh] = Field(default=Pancake3DMesh(), title='Mesh', description='This dictionary contains the mesh information.')
    solve: Optional[Pancake3DSolve] = Field(default=Pancake3DSolve(), title='Solve', description='This dictionary contains the solve information.')
    postproc: Optional[Pancake3DPostprocess] = Field(default=Pancake3DPostprocess(), title='Postprocess', description='This dictionary contains the postprocess information.')
