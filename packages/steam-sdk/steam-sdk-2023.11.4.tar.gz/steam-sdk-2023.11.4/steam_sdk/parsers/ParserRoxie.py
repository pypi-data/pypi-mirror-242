...
class ParserRoxie:
                    ...
                    for j in fc.strip().split(';'):  # row content
                        ...
                        if j[:3] == 'dv ':  # design variable
                            ...
                            if '@' in statement[1]:  # then the angle is specified
                                ...
                                xExpr = arg[0].strip()
                                yExpr = arg[1].strip()
                                A = [evalContent("("+xExpr+")", all_global), evalContent("("+yExpr+")", all_global)]
                                A = [A[0] * cos(A[1]), A[0] * sin(A[1])]
                            elif statement[1].strip() in data.key_points:  # the point is copied from another one
                                A = [data.key_points[statement[1].strip()].x, data.key_points[statement[1].strip()].y]
                            else:  # otherwise coordinates
                                arg = statement[1].strip(' []').split(',')
                                xExpr = arg[0].strip()
                                yExpr = arg[1].strip()
                                A = [evalContent(scaling_str+"("+xExpr+")", all_global), evalContent(scaling_str+"("+yExpr+")", all_global)]
                            ...
                            if statement[1][:5] == 'Hyper':
                                ...
                                if lineType == 'Line':
                                    ...
                                    arg = [arg0, arg1] #scaling_str+"("+xExpr+")"
                                    if arguments[3].strip()[:2] == 'kp':
                                        ...
                                        D = 2 * evalContent(scaling_str+"("+val+")",
                                    ...
                                    arg = [evalContent(scaling_str+"("+xExpr+")", all_global),
                                           evalContent(scaling_str+"("+yExpr+")", all_global)]  # axes of the ellipse
                                    data.hyper_lines[state0] = \
    ...
    def findConductorPositions(self, block: pd.Block = None, conductor: pd.CondPar = None, verbose: bool = False):
        ...
        if self.block.type == 2:
            ...
            if abs(corner1[1]) > abs(corner2[1]):
                point_o12, point_i12 = corner1, corner2
            else:
                point_o12, point_i12 = corner2, corner1
            if abs(corner0[1]) > abs(corner3[1]):
                point_o03, point_i03 = corner0, corner3
            else:
                point_o03, point_i03 = corner3, corner0
            if arc_angle_between_point_and_abscissa(point_i12, bore_center) > arc_angle_between_point_and_abscissa(point_i03, bore_center):
                point_oH, point_iH = point_o12, point_i12
                point_oL, point_iL = point_o03, point_i03
            else:
                point_oH, point_iH = point_o03, point_i03
                point_oL, point_iL = point_o12, point_i12
            point_oH_ins, point_oL_ins, point_iH_ins, point_iL_ins = point_oH, point_oL, point_iH, point_iL
                    ...
                    for pole_nr, pole in coil.poles.items():
                        for layer_nr, layer in pole.layers.items():
                            for winding_nr, winding in layer.windings.items():
                                data_new.coil.coils[coil_nr].poles[pole_nr].layers[layer_nr].windings[
                                    winding_nr].blocks = dict()
                                for block_nr, block in winding.blocks.items():
                                    ...
                                    blocks_new_in_old = dict()  # Create new blocks in initial block
                                    ...
                                    for ht_nr, ht in block.half_turns.items():  # 14 hts
                                        nbr_ht = len(block.half_turns)
                                        for strand_group_nr, strand_group in ht.strand_groups.items():  # 8 strands
                                            nbr_strands = len(strand_group.strand_positions)
                                            # Here update block where hts are assigned
                                            if list_flag_ribbon[SumLayers]:
                                                for strand_nr, strand_position in strand_group.strand_positions.items():
                                                    ...
                                                    half_turn_count_latest = (block_nr - 1) * nbr_strands * nbr_ht
                                                    if strand_nr == 1:
                                                    ...
                                                    # plt.plot(x_coord_ih, y_coord_ih, "-o", color='red')
                                                    # plt.plot(x_coord_il, y_coord_il, "-o", color='red')
                                                    # plt.plot(x_coord_oh, y_coord_oh, "-o", color='red')
                                                    # plt.plot(x_coord_ol, y_coord_ol, "-o", color='red')
                                                    # plt.text(x_coord_oh, y_coord_oh, '{}'.format("oH"), color='k',
                                                    #          fontsize=14)
                                                    # plt.text(x_coord_il, y_coord_il, '{}'.format("iL"), color='k',
                                                    #          fontsize=14)
                                                    # plt.text(x_coord_ih, y_coord_ih, '{}'.format("iH"), color='k',
                                                    #          fontsize=14)
                                                    # plt.text(x_coord_ol, y_coord_ol, '{}'.format("oL"), color='k',
                                                    #          fontsize=14)
                                                    line = Line2D([x_coord_oh, x_coord_ol], [y_coord_oh, y_coord_ol],
                                                                  linestyle='-.', color='k')

                                                    plt.gca().add_line(line)
                                                    line = Line2D([x_coord_ih, x_coord_il], [y_coord_ih, y_coord_il],
                                                                  linestyle='--', color='g')

                                                    plt.gca().add_line(line)
                                                    line = Line2D([x_coord_il, x_coord_ol], [y_coord_il, y_coord_ol], linestyle='--', color='k')

                                                    plt.gca().add_line(line)
                                                    line = Line2D([x_coord_ih, x_coord_oh], [y_coord_ih, y_coord_oh], linestyle='--', color='k')
                                                    ...
                                                    # line = Line2D([ block_current.block_corners.iL.x,  block_current.block_corners.iH.x],
                                                    #               [block_current.block_corners.iL.y, block_current.block_corners.iH.y],
                                                    #               linestyle='-', color='k')
                                                    # plt.gca().add_line(line)
                                                    # line = Line2D([block_current.block_corners.oL.x,
                                                    #                block_current.block_corners.oH.x],
                                                    #               [block_current.block_corners.oL.y,
                                                    #                block_current.block_corners.oH.y],
                                                    #               linestyle='-', color='k')
                                                    # plt.gca().add_line(line)
                                                    # plt.show()
                                                    ...
                                                    block_current.half_turns[ht_nbr_new].corners.insulated = pd.Corner(
                                                        iH=pd.Coord(x=x_coord_ih-ih_ins_vector[0],
                                                                    y=y_coord_ih-ih_ins_vector[1]),
                                                        iL=pd.Coord(x=x_coord_il-il_ins_vector[0],
                                                                    y=y_coord_il-il_ins_vector[1]),
                                                        oL=pd.Coord(x=x_coord_ol-ol_ins_vector[0],
                                                                    y=y_coord_ol-ol_ins_vector[1]),
                                                        oH=pd.Coord(x=x_coord_oh-oh_ins_vector[0],
                                                                    y=y_coord_oh-oh_ins_vector[1]))
                                                ...
                                                block_current_key = SumLayers + 1
                                                if block_current_key not in blocks_new_in_old:
                                                    blocks_new_in_old[block_current_key] = pd.BlockData()

                                        init_ht_number = init_ht_number+1

                                    data_new.coil.coils[coil_nr].poles[pole_nr].layers[layer_nr].windings[winding_nr].blocks.update(blocks_new_in_old)
                                    for block_nr, block in blocks_new_in_old.items():
                                        data_new.coil.physical_order.append(
                                            pd.Order(coil=coil_nr, pole=pole_nr, layer=layer_nr,
                                                     winding=winding_nr, block=int(block_nr)))

                                    # Block corners when applicable
                                    for block_nr, block_curr in blocks_new_in_old.items():
                                        first_ht_nr = list(block_curr.half_turns)[0]
                                        last_ht_nr = list(block_curr.half_turns)[-1]

                                        p1 = (block_curr.half_turns[first_ht_nr].corners.bare.iH.x,
                                        block_curr.half_turns[first_ht_nr].corners.bare.iH.y)
                                        p2 = (block_curr.half_turns[last_ht_nr].corners.bare.iH.x,
                                        block_curr.half_turns[last_ht_nr].corners.bare.iH.y)
                                        center = (coil.bore_center.x, coil.bore_center.y)
                                ...
                                pass
                                # Assign block block_nbr SumLayers+1 and counter_halfturns+1
            elif not any(list_flag_ribbon): # All values are false
                pass
            else:
                raise ValueError("Mixed Ribbon and Rutherford cables are not supported!")
    ...
    def getData(self, dir_iron: Path = None, dir_data: Path = None, dir_cadata: Path = None,
        ...
        if dir_data and dir_cadata:
            ...
            #             ax.add_patch(
            #                 patches.Arc((center[0], center[1]), width=2 * radius, height=2 * radius, angle=0,
            ...
            #             arc = patches.Arc((center[0], center[1]), width=2 * line.arg1, height=2 * line.arg2,
            #                               angle=0,
