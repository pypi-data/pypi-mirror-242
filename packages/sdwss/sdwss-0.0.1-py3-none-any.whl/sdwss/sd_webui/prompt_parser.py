...
sdwss_hijacker_attribute = "__sdwss_hijacker"
prompt_parser_hijacker = hijacker.ModuleHijacker.install_or_get(
    module=prompt_parser,
    hijacker_attribute=sdwss_hijacker_attribute,
...
def encode_prompt(params: function_params.EncodePromptScheduleParams, prompt_encoder):
    ...
    if self.model.is_sdxl:
        return to_sdwss_cond_schedule(cond_schedule, key=self.current_cond_key)

    return to_sdwss_cond_schedule(cond_schedule)
...
def to_sdwss_cond_schedule(cond_schedules: List[prompt_parser.ScheduledPromptConditioning], key: Optional[str] = None) -> torch.Tensor:
