        ...
        async def call(
            ...
            *,
            ...
            if task is not None:
                ...
                # Invariant here is that the first time `call()` is invoked we
                # will wait for a response so we know that we have a response
                # at this point.
                assert task in self._calls
                assert task in self._responses
                return (self._calls[task], self._responses[task])

            # We'll run an `asyncio.Task` to make the call to
            # `React.Query` which will fill in `self._calls` and
            # `self._responses` and we need to wait until it has
            # filled those in in order to return them from this
            # method. This is the event that waits for that to have
            # occured.
            calls_and_responses_ready = asyncio.Event()
            ...
            async def query():
                ...
                while True:
                    ...
                    # Let the outer scope know that both `self._calls`
                    # and `self._responses` have been filled in with
                    # valid values.
                    calls_and_responses_ready.set()
                    ...
                    # `responses.cancel()` which will cancel the
                    # underlying gRPC generator.
                    async def loop():
                        assert task is not None
                    ...
                    try:
                        ...
                        # Continue trying to execute the query after
                        # backing off so we consume less resources
                        # (but at the tradeoff of possibly slower
                        # reactivity).
                        await backoff()
            ...
            await calls_and_responses_ready.wait()
