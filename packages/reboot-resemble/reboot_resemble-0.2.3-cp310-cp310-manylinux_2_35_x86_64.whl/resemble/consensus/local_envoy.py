...
class LocalEnvoy:
    ...
    async def _docker_run_envoy(self) -> str:
        ...
        # Check if we have an orphaned Envoy from a previous run and
        # stop it first.
        #
        # This may stop an envoy that is running because a user has an
        # `rsm dev` (or `rsm serve`, etc) running, e.g., in a
        # different terminal.
        while True:
            orphaned_container_id = await LocalEnvoy._async_check_output(
                'docker',
                'ps',
                '--filter',
            ...
            orphaned_container_id = orphaned_container_id.strip()
            ...
            try:
                # Just wait for the process to exit; we don't care about
                # the `returncode` and the container might also have
                # terminated on it's own before we called stop!
                if await docker_stop.wait() == 0:
                    break
            finally:
                # In the event we were cancelled or an exception was
                # raised make sure the process has been terminated so
                # we don't leave around any orphans (the exact thing
                # this code is trying to clean up from!)
