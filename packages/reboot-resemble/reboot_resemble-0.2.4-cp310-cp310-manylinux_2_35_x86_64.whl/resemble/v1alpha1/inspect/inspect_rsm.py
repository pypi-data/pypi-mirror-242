...
class InspectServicerMiddleware(Middleware):
    ...
    async def _abort_with_status(
        ...
        # of the actual type of 'aborted', i.e., 'aborted' here might
        # be the actual error type for this method or it may be
        # another error type, e.g., 'Foo.MethodError', because this
        # method called 'Foo.Method' but didn't catch the error. We
        # will always propagate the error and if the error detail is a
        # declared error for this method we will update the message
        # accordingly.
        if (
                error_type is not None and
                not isinstance(aborted, error_type)
                and error_type.is_declared_error(aborted.detail)
        ):
