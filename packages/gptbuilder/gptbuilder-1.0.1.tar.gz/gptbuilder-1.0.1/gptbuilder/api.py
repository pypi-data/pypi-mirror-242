...
class BatchAPI:
    ...
    async def _async_generate(self, idx, params):
        try:
    ...
    async def _generate_concurrently(self):
        ...
        tasks = [asyncio.create_task(self._async_generate(int(idx), params)) for idx, params in enumerate(params_list)]
        responses = await asyncio.gather(*tasks)
        outputs = copy.deepcopy(self.data)
        for response in responses:
            outputs[response[0]][self.response_key] = response[1]
        return outputs
