...
from aryahelpers.utils.genericutils import import_file, save_files, null_handler, merge_dicts
...
class MySQLHelpers(object):
    ...
    @staticmethod
    def _get_job_poster_info(db_key: str, user_id: int, job_id: int):
        """Get info (first name, last name, email etc.) for job poster"""
        user_name, email_id = "", ""
        query_str = "SELECT `First Name`, `Last Name`, `Email` FROM user WHERE UserId={};".format(user_id)
        query_res = MySQLManager.execute_query(query_str, (), **CONFIG[db_key]['arya'])
        if query_res:
            first_name = query_res[0]["First Name"]
            last_name = query_res[0]["Last Name"]
            email_id = query_res[0]["Email"]
            user_name = " ".join([first_name, last_name])
        return user_name, email_id
    ...
    def _get_job_metadata(db_key: str, job_id: int):
        ...
        user_name, email_id = MySQLHelpers._get_job_poster_info(db_key, user_id, job_id)
        job_metadata = {
            "JobCreatedBy": user_name,
            "JobCreatedEmail": email_id,
