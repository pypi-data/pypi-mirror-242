...
"""
    ...
    if RECLINE_INSTALLED:
        ...
        def flexcache_show(
            fields: List[Choices.define(["constituents_per_aggregate", "dr_cache", "global_file_locking_enabled", "name", "override_encryption", "path", "size", "use_tiered_aggregate", "uuid", "*"])]=None,
        ) -> ResourceTable:
            ...
            Args:
                ...
                name: FlexCache name
                override_encryption: If set to true, a plaintext FlexCache volume for an encrypted origin volume is created.
            ...
            if name is not None:
                kwargs["name"] = name
            if override_encryption is not None:
                kwargs["override_encryption"] = override_encryption
            if path is not None:
                kwargs["path"] = path
            if size is not None:
                kwargs["size"] = size
...
"""
    ...
    if RECLINE_INSTALLED:
        ...
        async def flexcache_create(
            ...
            if atime_scrub is not None:
                kwargs["atime_scrub"] = atime_scrub
            if cifs_change_notify is not None:
                kwargs["cifs_change_notify"] = cifs_change_notify
            if constituents_per_aggregate is not None:
                kwargs["constituents_per_aggregate"] = constituents_per_aggregate
            if dr_cache is not None:
                kwargs["dr_cache"] = dr_cache
    ...
    if RECLINE_INSTALLED:
        ...
        async def flexcache_modify(
            ...
            Args:
                ...
                override_encryption: If set to true, a plaintext FlexCache volume for an encrypted origin volume is created.
                query_override_encryption: If set to true, a plaintext FlexCache volume for an encrypted origin volume is created.
                path: The fully-qualified path in the owning SVM's namespace at which the FlexCache is mounted. The path is case insensitive and must be unique within a SVM's namespace. Path must begin with '/' and must not end with '/'. Only one FlexCache be mounted at any given junction path.
            ...
            if query_override_encryption is not None:
                kwargs["override_encryption"] = query_override_encryption
            ...
            if name is not None:
                changes["name"] = name
            if override_encryption is not None:
                changes["override_encryption"] = override_encryption
            if path is not None:
                changes["path"] = path
            if size is not None:
                changes["size"] = size
