...
Note the <i>create_inactive=true</i> paramater that is used to indicate that the configuration should be created but not enabled.
...
Example: abcdef"""
    ...
    r""" The configuration field of the azure_key_vault."""
...
Example: false"""
    ...
    """links,authentication_method,azure_reachability,client_id,configuration,ekmip_reachability,enabled,key_id,name,oauth_host,port,proxy_host,proxy_port,proxy_type,proxy_username,scope,state,svm.links,svm.name,svm.uuid,tenant_id,uuid,vault_host,verify_host,verify_ip,"""
    ...
    """client_certificate,client_id,client_secret,oauth_host,port,proxy_host,proxy_password,proxy_port,proxy_type,proxy_username,tenant_id,vault_host,verify_host,verify_ip,"""
    ...
    """client_certificate,client_id,client_secret,configuration,key_id,name,oauth_host,port,proxy_host,proxy_password,proxy_port,proxy_type,proxy_username,svm.name,svm.uuid,tenant_id,vault_host,"""
...
* [`DOC /security/azure-key-vaults`](#docs-security-security_azure-key-vaults)"""
    ...
    if RECLINE_INSTALLED:
        ...
        def azure_key_vault_show(
            fields: List[Choices.define(["authentication_method", "client_certificate", "client_id", "client_secret", "enabled", "key_id", "name", "oauth_host", "port", "proxy_host", "proxy_password", "proxy_port", "proxy_type", "proxy_username", "scope", "tenant_id", "uuid", "vault_host", "verify_host", "verify_ip", "*"])]=None,
        ) -> ResourceTable:
            ...
            if uuid is not None:
                kwargs["uuid"] = uuid
            if vault_host is not None:
                kwargs["vault_host"] = vault_host
...
* `configuration.name` - The configuration name is required if and only if the `create_inactive` flag is set to true.
### Optional properties:
* `port` - Authorization server and vault port number.
* `oauth_host` - Open authorization server host name.
* `vault_host` - AKV host subdomain.
    ...
    if RECLINE_INSTALLED:
        ...
        async def azure_key_vault_modify(
            ...
            Args:
                ...
                port: Authorization server and vault port number.
                query_port: Authorization server and vault port number.
                proxy_host: Proxy host.
                ...
                vault_host: AKV host subdomain.
                query_vault_host: AKV host subdomain.
                verify_host: Verify the identity of the AKV host name.
                query_verify_host: Verify the identity of the AKV host name.
                verify_ip: Verify the identity of the AKV IP address.
                query_verify_ip: Verify the identity of the AKV IP address.
            """
            ...
            if query_vault_host is not None:
                kwargs["vault_host"] = query_vault_host
