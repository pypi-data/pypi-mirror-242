...
class DynamoDBAthenaSynchronizer:
    def __init__(
        ...
        s3_bucket,
        athena_table_name,
        athena_database_name: str | None = None,
        columns: list[str] | None = None,
        dtypes: dict[str, str] | None = None,
        index_col: str | None = None,
        filter_expression=None,
        expression_attribute_values=None,
        **read_additional_kwargs: Any,
    ) -> None:
        self.table_name = table_name
        self.athena_table_name = athena_table_name
        if athena_database_name is None:
            if wr.config.database:
                athena_database_name = wr.config.database
            else:
        ...
        self.read_additional_kwargs = read_additional_kwargs
        self.table_snapshot = self.sync_table()

    @staticmethod
    def __parse_dates__(df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def sync_table(
        ...
        with warnings.catch_warnings():
            ...
            table = wr.dynamodb.read_items(
                table_name=self.table_name,
                allow_full_scan=True,
                filter_expression=self.filter_expression,
                expression_attribute_values=self.expression_attribute_values,
                **self.read_additional_kwargs,
            )
        table = self.__parse_dates__(table)
