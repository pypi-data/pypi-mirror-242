...
class FlavorFinder:
    ...
    def wgcheck(self):
        try:
            ...
            if os.path.exists(path):
                print("[bold green1]:small_Blue_Diamond: WireGuard already installed [bold green1]")
                return True
            else:
                wgcmd1 = subprocess.run(
                    self.INSTALLWG, shell=True, text=True, check=True, capture_output=True
                )
    ...
    def flavor_installer(self):
        if self.getdistro() == "ubuntu":
            if self.wgcheck():
                reconfigurestate = Prompt.ask("[bold green1]:small_Blue_Diamond: Do you wish to proceed with the reconfiguration? (Y/N) [bold green1]")
