...
from mfire.mask.gridage import Shape
...
class CardinalMasks:
    ...
    Args:
        geo: A geometric shape representing the area to be cut.
    ...
    def __init__(self, geo: Shape, area_id: str):
        ...
        self.inputs = {
            ...
            "Sud-Est": [
                ("change_me", "change_me", (max_lon, min_lat)),
                "le ",
            ],
            "Sud-Ouest": [
                ("change_me", "change_me", (min_lon, min_lat)),
                "le ",
            ],
            "Nord-Est": [
                ("change_me", "change_me", (max_lon, max_lat)),
                "le ",
            ],
            "Nord-Ouest": [
                ("change_me", "change_me", (min_lon, max_lat)),
                "le ",
            ],
        }
    ...
    def test_area(self, sub: Shape) -> bool:
        """
        ...
        Args:
            sub (Shape): Cut
            min_area (float): Minimum percentage of the geographical area
                (compared to the original area)
            max_area (float): Maximum percentage of the geographical area
                (compared to the original area)

        Returns:
            bool: True if the new area is valid, False otherwise.
        """
        geo_t = (
            self.geo
            if self.geo.geometryType() == "Polygon"
            else self.geo.buffer(self.BUFFER_SIZE)
        )
        sub_t = sub if sub.geometryType() == "Polygon" else sub.buffer(self.BUFFER_SIZE)
    ...
    def get_rect_mask(
        ...
        Returns:
            (Polygon) bounding rectangle transformed
        """
