...
class EventComposite(BaseComposite):
    ...
    def _compute_representative_and_extreme_values(
        self, field: xr.DataArray, kind: Literal["plain", "mountain"]
    ):
    ...
    def compute_plain_and_mountain(self) -> xr.DataArray:
        ...
        if self.plain is not None:
            ...
            self._compute_representative_and_extreme_values(plain_field, "plain")
        ...
        if self.mountain is not None:
            ...
            self._compute_representative_and_extreme_values(mountain_field, "mountain")
    ...
    def compute_downstream_aggregation(self, risk_da: xr.DataArray) -> xr.DataArray:
        ...
        if self.aggregation is not None:
            # If we are in the case of upstream aggregation, we know if the risk is
            # active or not
            self._values_ds["occurrence_event"] = new_risk_da > 0
            self._values_ds["occurrence_event"].attrs = {}
