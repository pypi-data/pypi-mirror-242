...
class BlocksBuilder:
    ...
    def _flag_max_and_long_type3_blocks(self, pd_summary: PandasWindSummary) -> None:
        """Flag WindBlocks with wf_max and with a duration >= _block_duration_min."""
        self._reset_ind()

        # Get max wind force
        wf_max: np.float64 = pd_summary.find_type_3_wf_max()

        flag_set: Optional[bool] = False

        # Set flag as True to WindBlock with wf_max
        for self._ind in range(self._ind_max + 1):
            block_cur: WindBlock = self._blocks[self._ind]

            # Get the wind force max of the current WindBlock
            ...
            if wf_max_cur == wf_max:
                ...
                block_cur.wind_type == WindType.TYPE_3
                and block_cur.duration >= self._t3_block_duration_min
            ):
                self._blocks[self._ind].flag = True
                flag_set = True
    ...
    def _try_to_merge_current_flagged_block_with_previous_blocks(
        ...
        if self._blocks[self._ind - 1].duration >= self._period_between_t3_blocks_bound:
            ...
            while ind >= 0 and self._blocks[ind].flag is False:
                self._blocks[self._ind - 1] = self._blocks[self._ind - 1].merge(
                    self._blocks[ind]
                )
                self._remove_block_from_ind(ind)
                self._ind -= 1
                ind = self._ind - 2
        ...
        self._blocks[self._ind] = self._blocks[self._ind].merge(
            self._blocks[self._ind - 2]
        )

        self._remove_block_from_ind(self._ind - 1)
        self._remove_block_from_ind(self._ind - 2)
        self._ind -= 2
        return True
    ...
    def _merge_blocks(self):
        ...
        # Merge last unflagged WindBlocks if necessary
        if self._blocks[self._ind_max].flag is False:
            self._reset_ind(self._ind_max - 1)

            while self._ind >= 0 and self._blocks[self._ind].flag is False:
                self._blocks[self._ind] = self._blocks[self._ind].merge(
        ...
        for i, block in enumerate(self._blocks):
            ...
            if block.flag is False:
                ...
                wind_force_finder: WindForcePeriodFinder = WindForcePeriodFinder(
                    data_wf, pd_summary, valid_times
                )
    ...
    def run(
        ...
        self._flag_max_and_long_type3_blocks(pd_summary)
