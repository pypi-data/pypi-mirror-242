...
class ImageUtils:
    ...
    @staticmethod
    def colourise_heatmaps(saliency_maps, image_width=224, image_height=224):
        ...
        for i, saliency_map in enumerate(saliency_maps):
            ...
            heatmap_image = Image.fromarray(np.uint8(saliency_map*255), 'L').resize((image_width,image_height), resample=Image.BICUBIC)
    ...
    @staticmethod
    def super_imposed_image(saliency_maps, original_image, image_width=224, image_height=224, alpha=0.8):
        
        original_image = original_image.resize((image_width, image_height))
        original_image = Image.blend(original_image.convert("RGB"), saliency_maps.convert("RGB"), alpha=alpha)
