...
Examples:
    ...
    >>> import os
    >>> from pall_predictor import Pall_process
    
    >>> def pred(x):

        >>> input_file, output_path, indice, sep = x
    ...
    >>> if __name__ == '__main__':
        ...
        >>> Pall_process(pred, 5).run(input_file, output_file, indice, sep)
...
class Pall_process():
    def __init__(self, funx, process):
        ...
        Example:
            ...
            Pall_process(funx, 5).run(input_file, output_file, indice, sep)
        """
        self.funx = funx
    ...
    def run(self, input_file, output_file, indice, sep):
        """
        执行主函数
        Args:
            input_file:需要预测的文件
        ...
        file_list = [(x, self.tmp_save_res, indice, sep) for x in file_list]
    
        self._pool(self.funx, file_list, self.process)
    
        self.concat_res(self.tmp_save_res, output_file, sep)
        
        os.system('rm -rf '+ self.file_split_path)
