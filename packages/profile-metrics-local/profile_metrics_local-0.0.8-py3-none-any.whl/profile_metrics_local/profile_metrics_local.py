...
class ProfileMetricsLocal(GenericCRUD):
    ...
    def __init__(self):
        super().__init__(schema_name=SCHEMA_NAME, default_table_name=TABLE_NAME,
                         default_id_column_name=DEFAULT_ID_COLUMN_NAME)
    
    def insert_profile_metrics(self, profile_id: int, profile_metrics_type: int, value: int) -> int:
    ...
    def update_by_profile_metrics_id(self, profile_metrics_id: int, profile_id: int, profile_metrics_type: int, value: int) -> None:
        ...
        data_json = {'profile_id': profile_id, 'profile_metrics_type': profile_metrics_type, 'value': value}
        profile_metrics_id = self.update_by_id(
            table_name=None, id_column_name=DEFAULT_ID_COLUMN_NAME, id_column_value=profile_metrics_id,
            data_json=data_json)
        logger.end()

    def delete_by_profile_metrics_id(self, profile_metrics_id: int) -> None:
        logger.start(object={'profile_metrics_id': profile_metrics_id})
        self.delete_by_id(table_name=TABLE_NAME, id_column_name=DEFAULT_ID_COLUMN_NAME,
                                  id_column_value=profile_metrics_id)
        logger.end()

    def select_one_dict_by_profile_metrics_id_id(self, profile_metrics_id: int) -> Dict[str, int]:
        logger.start(object={'profile_metrics_id': profile_metrics_id})
        select_clause_value = 'profile_id, profile_metrics_type, value'
        profile_metrics_dict = self.select_one_dict_by_id(
