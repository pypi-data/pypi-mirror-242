...
class ExportElasticSearchTC(testlib.AutoPopulateTest):
    ...
    def test_es_hooks_create(self, index, exists, create):
        with self.admin_access.cnx() as cnx:
            cnx.create_entity(self.tested_etype, name="Jean Valjean", age=14)
            cnx.commit()
            index.assert_called()
    ...
    def test_es_hooks_modify(self, index, exists, create):
        with self.admin_access.cnx() as cnx:
            entity = cnx.create_entity(self.tested_etype, name="Jean Valjean")
            cnx.commit()
            index.reset_mock()
            entity.cw_set(name="Victor Hugo")
            cnx.commit()
            index.assert_called()
