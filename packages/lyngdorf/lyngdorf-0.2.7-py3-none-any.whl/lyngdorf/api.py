...
class LyngdorfApi:
    ...
    async def _async_establish_connection(self) -> None:
        ...
        _LOGGER.info("%s: establishing connection", self._host)
        ...
        try:
            ...
            _LOGGER.debug("%s: Timeout exception on connect", self._host)
            raise asyncio.TimeoutError(f"TimeoutException: {err}", "connect") from err
        except ConnectionRefusedError as err:
            _LOGGER.debug("%s: Connection refused on connect", self._host, exc_info=True)
        ...
        self._writeSetup()
        _LOGGER.debug("%s: connection complete", self._host)
    ...
    def _monitor(self) -> None:
        ...
        if time_since_response > _MONITOR_INTERVAL * 4:
            _LOGGER.info(
                "%s: Keep alive failed, disconnecting and reconnecting", self._host
            )
        ...
        while self._connection_enabled and not self.healthy:
            ...
            async with self._connect_lock:
                try:
                    ...
                    _LOGGER.info("%s: Lyngdorf reconnected", self._host)
                    return
    ...
    def _writeCommand(self, command):
        ...
        _LOGGER.debug("%s send: '!%s'", self._host, command)
    ...
    def _process_event(self, message: str) -> None:
        ...
        _LOGGER.debug("%s recv: '%s'", self._host, message)
        # print("\r"+message+"\r")
        self._last_message_time = time.monotonic()
