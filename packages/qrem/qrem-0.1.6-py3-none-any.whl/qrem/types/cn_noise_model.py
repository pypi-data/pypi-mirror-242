...
#TODO: TUZJAN, prepare full local test
#TODO: Find a way of exporting to jason dictionary, in which key is a tuple. This is needed to export CNModelData to json, e.g. noise matrices are stored in a dictionary with tupels as keys.  
class CNModelData(DataStructureBase):
  

    """
    The class contains parameters of Clusters and Neighbors CN noise model (currently it does not support neighbors). 
    
    It is used to:

    1) store results of CN noise model reconstruction algorithm (noise characterization routine)

    2) Mitigate redout errors on marginal probability distributions (error mitigation routine)  


    Parameters
    ----------
    clusters_tuple : a tuple of tuples 
        Stores information about clusters of qubits. Eeach tuple contains indices of qubits belonging to a cluster  e.g. ((0,2),(1,3))   

    noise_matrices: dictionary (key - tuple, value - array)
        Stores noise matrices associated to clusters. Key in the dictionary is a tuple with indices of qubits belonging to a cluster. 
    ...
    inverse_noise_matrices : dictionary (key - tuple, value - array)
        Stores noise matrices associated to clusters. Key in the dictionary is a tuple with indices of qubits belonging to a cluster. 
        Corresponding value is an array storing inverse noise matrix associated with the cluster. Eg. (0:2) : [[0.8,0,0,0], [0.2,1,0.1,0],[0,0,0.9,0],[0,0,0,1]].
        This property is filled automatically when noise_matrices property is set via set_noise_matrices_dictionary method.

    qubit_in_cluster_membership: dictionary (key: tuple, value: tuple)
        Stores information about qubits membership in clusters. Key in the dictionary is a tuple constructed form a qubit index. Corresponding value is a cluster to which the qubit belongs.
...
if __name__=='__main__':
   #set clusters tuple
    qubits_in_cluster_1=(0,3)
    qubits_in_cluster_2=(1,2,4)

    #generation noise matrice for clusters with built in function probability.random_stochastic_matrix
    noise_matrix_1= probability.random_stochastic_matrix(2**2)
    noise_matrix_2= probability.random_stochastic_matrix(2**3)
    
    noise_matrices_dictionary_test = {qubits_in_cluster_1: noise_matrix_1, qubits_in_cluster_2: noise_matrix_2}

    #create noise model 
    test_model= CNModelData(number_of_qubits=5)
    test_model.set_noise_model(noise_matrices_dictionary=noise_matrices_dictionary_test) 
