...
class GlobalNoiseMatrixCreator:
    ...
    def __init__(self,
        ...
        for cluster_index in range(len(self._clusters_list)):
            key_cluster = anf.get_qubits_keystring(self._clusters_list[cluster_index])
            clusters_labels_list.append(key_cluster)
            neighbors_labels_list.append(anf.get_qubits_keystring(self._neighborhoods[key_cluster]))
    ...
    # #PPcomment: why not version from core/ancillary functions? MERGE
    # @staticmethod
    # def get_qubits_keystring(list_of_qubits):
    ...
    def compute_matrix_element(self,
       ...
       :param output_state: bitstring denoting OUTPUT classical state
        ...
        for cluster_index in range(len(self._clusters_list)):
            ...
            # print(neighbors_now,neighbors_input_state_now)
    ...
    def compute_global_noise_matrix_old(self,
        ...
        if neighbors_of_clusters is None:
            if self._neighborhoods is None:
                ...
                neighbors_of_clusters = [self._neighborhoods[anf.get_qubits_keystring(clust)] for clust in
                                         clusters_list]

        lambdas = [self._noise_matrices_dictionary[anf.get_qubits_keystring(clust)] for clust in
                   clusters_list]
            ...
            for output_state_integer in range(d):
                ...
                for cluster_index in range(len(lambdas)):
                    ...
                    if neighbours_now is not None:
                        ...
                        neighbors_string = anf.get_qubits_keystring(neighbours_now)
