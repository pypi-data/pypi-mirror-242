    ...
    def receive_layout(self, ltpage):        
        def render(item, page_number):
            if isinstance(item, LTPage) or isinstance(item, LTTextBox):
                ...
                # if page_number == 23 and (child_str == 'communication between the main server and agent server' or child_str == 'invention is not limited to the disclosed embodiments, but'):
                #     import pdb; pdb.set_trace()
                # if page_number == 23 and '55' in child_str:
                #     import pdb; pdb.set_trace()
                if child_str:
                    ...
                    if (not self.default_to_page_width or off_by_order_of_magnitude) and self.lttextline_bbox_min_x0_by_page is not None and self.lttextline_bbox_max_x1_by_page is not None:
