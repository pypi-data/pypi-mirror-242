...
def Dao(entity):
    ...
    def decorator(cls):
        ...
        # 新增生成表方法
        setattr(cls, generate_sql_create_table.__name__, generate_sql_create_table)
        # 新增插入方法
...
def Sql(static_sql_statement: str):
    ...
    Example2:
        ...
        dao = DaoStudent()
        result1 = dao.get_student_name(student_id=1,
                                       sql_statement=Dao.generate_sql('some args'))  # 通过隐藏的sql_statement参数传递sql语句。
        result2 = dao.get_student(student_id=1)
        print(result1, result2)  # 将以列表形式返回查询结果，其中每条记录的数据格式都是具名元组（与数据类类似）
        # result1: [namedtuple('Record', 'name'), ...]  # 查询部分字段时，仅以查询的字段生成相应的具名元组。
        # result2: [namedtuple('Record', 'name score student_id'), ...]  查询全部字段时，具名元组与定义的数据类完全一致。
    ...
    def decorator(func):  # noqa
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if isinstance(sql_statement, str):
                ValueError(
                    f"传入的sql语句应该是 'str' 类型，但得到的是 {type(sql_statement).__name__}")
...
def Insert(func):  # noqa
    ...
    Example:
        ...
        dao = DaoStudent()
        student_bob = Student(name='Bob', score=95.5)
        dao.insert(entity=student_bob)  # 将整条记录以数据类的形式插入数据库，避免了使用多个参数逐个匹配的麻烦。

    !Note:
        由于插入部分的结构完全固定，因此对数据访问类使用Dao装饰器后，会自动内置insert(entity)方法，无需额外定义（建议），可直接调用，但缺点
        是无法获取代码提示。
    ...
    def wrapper(self, entity):
        ...
        fields = [field_name for field_name in fields
                  ...
                  not in _parse_constraints(attr_value=getattr(self.entity, field_name, None))]
