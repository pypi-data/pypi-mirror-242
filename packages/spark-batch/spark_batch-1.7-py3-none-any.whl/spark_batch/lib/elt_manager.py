...
class EltManager:
    ...
    # 함수가 반복 실행을 지원하지 않는 문제로 삭제 예정
    def ingest_full_df_depreciated(self, source_object, target_object, source_df, target_customSchema=None, cleansing_conditions=None, append=False) :
    ...
    def ingest_full(self, source_object, target_object, source_customSchema=None, target_customSchema=None, count=True, offset=0, cleansing_conditions=None, delemeter=None) :    
        ...
        valid = offset == target_df.count() + tot_cleaned_count
           
        self.logger.info(f"ETL/FL Done : [ {targetInfo} / {valid} ({offset}, {target_df.count()}, {tot_cleaned_count}) / {timer.elapsed():.2f} ]")
