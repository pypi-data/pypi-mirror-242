...
def attempt_fix_dsl(requirement: dict, failing_dsl: str, instruct: str = "", total_retries=6) -> str:
    ...
    fix_block = stream(
        ...
        notebook_io.add_last_message_log("bot", ROLE),
        condition(
    ...
    if fix_dsl_chain.error:
        print(f"error executing dsl fixes: {fix_dsl_chain.error}, {fix_dsl_chain.stacktrace}, {fix_dsl_chain.memory.messages}")

    if fix_dsl_chain.memory.state.success_dsl != "":
        return fix_dsl_chain.memory.state.success_dsl
    else:
        print(fix_dsl_chain.memory.state.failed_attempts)
...
def generate_method_from_requirement_node(requirement: dict, instruc: str = "") -> str:
    ...
    chain = initial_chain | oai_chat_complete("gpt-3.5-turbo-16k")
    ...
    try:
        ...
        try:
            ...
            raise ValueError("fail to generate a valid function dsl from requirement") 
...
def run_feeds_on_requirements(requirements_yaml: str) -> List[Dict]:
    ...
    for i, func in enumerate(function_calls):
        try:
            ...
            # prettify_log("SYSTEM", f"generated output for {func}", "blue")
...
def run_edit_with_id():
    def _inner(memory: Memory):
        ...
        # prettify_log("SYSTEM", f"making copy edit for feed with ID: {_id}","cyan")
        old_content_msg = f"""
        ...
        notebook_io.add_message_to_log(f"generated edits, if you are satisfied- hit :confirm", ROLE)
...
def welcome_feed_chat_msg():
    ...
    """
    notebook_io.add_message_to_log(welcome_txt, ROLE)
...
def analyses_success_chat_msg():
    ...
    """
    notebook_io.add_message_to_log(txt, ROLE)
...
def run_feed_chat(path=None, requirements=""):
    ...
    return initial_chain | notebook_io.add_last_message_chat("user", ROLE) | convo_loop