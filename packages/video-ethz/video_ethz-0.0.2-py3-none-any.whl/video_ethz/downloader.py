...
def unsilence_file(filepath: str):
    ...
    print(
        f"Estimated time savings overall: {overall_time_saving[0]}s corresponding to {overall_time_saving[1]}%."
    )
...
def download_latest_episodes(
    ...
    start = max(first - 1, 0)
    for i in trange(start, start + n_lectures, desc=f"Downloading {n_lectures} latest episodes"):
        episode = episodes[i]
