...
logger, messages_collection, users_collection = None, None, None


def setup_env():
    global logger, messages_collection, users_collection
    try:
        logging.basicConfig(
...
with gr.Blocks(title="Puyuan Model Hub") as demo:
    ...
    demo.queue().launch(
        server_port=int(os.getenv("DASHBOARD_PORT")),
        auth=[(os.getenv("DASHBOARD_USER"), os.getenv("DASHBOARD_PASSWORD"))],
        root_path="/dash/",
    )
