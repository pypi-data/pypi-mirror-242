                ...
                if session:
                    if not self.client:
                        async with asyncio.Lock():
                            self.client = httpx.AsyncClient(proxies=proxies, verify=verify, http2=http2,
                                                            event_hooks={'response': [self.log_response]})
                            try:
                                list(self.client._mounts.values())[0]._pool._ssl_context = self.ssl_context
                            except:
                                pass
                    else:
                        self.client.proxies = proxies
                    resp = await self.client.request(method=method, url=url, content=content, data=data, files=files,
