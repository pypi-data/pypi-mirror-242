...
BAUDRATE = 9600
DELAY = 0.5  # fixed buffer timeout
BUFFER_SIZE = 9  # fixed buffer size
ADDRESS = 1  # default PS address ID number
...
class PsController(PsAbstract):
    ...
    def __init__(self, baudrate=BAUDRATE, delay=DELAY, address=ADDRESS):
        """
        constructor

        Args:
            baudrate (int, optional): COMs baud rate. Defaults to 9600.
            delay (float, optional): delay time between serial interface
                                     commands. Defaults to 0.1.
