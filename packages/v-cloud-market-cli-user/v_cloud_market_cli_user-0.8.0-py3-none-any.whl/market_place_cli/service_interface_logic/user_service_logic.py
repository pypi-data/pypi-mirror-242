...
class UserServiceLogic:
    ...
    def service_status_logic(self, serviceList: list, nonce: int):
        ...
        try:
            while True:
                ...
                choice = get_table_choice(self.console, w, has_next=False, extra=choices)
                if choice == 'm':
                    idx = self.get_target_index(data_list)
    ...
    def get_target_index(self, data_list: dict):
        ...
        return get_table_index(self.console, data_list, '[bright_green]Please enter the Service INDEX')
    ...
    def _get_page_choices(self, status: str):
        ...
        if status == 'ServiceRunning':
            extra = {'d': '[D]User Service Detail', 'a': '[A]User Service API Access', 't': '[T]Tail Service Log', 'b': '[B]Domain Name', 's': '[S]Service Status', 'r': '[R]Restart Service', 'f': '[F]Refund Service', 'g': '[G]Renew Service', 'h': '[H]Update Image', 'c': '[C]Container Config', 'u': '[U]Update Config', 'v': '[V]Update Deployment'}
