...
def render():
    ...
    while True:
        ...
        if keys[K_w]:
            camz += speed  # Move forward
        elif keys[K_s]:
            camz -= speed  # Move backward

        if keys[K_a]:
            camx += speed  # Move left
        elif keys[K_d]:
            camx -= speed  # Move right

        if keys[K_q]:
            camy += speed  # Move up
        elif keys[K_e]:
            camy -= speed  # Move down
        if keys[K_LEFT]:
            angle -= 5  # Move up
        elif keys[K_RIGHT]:
            angle += 5  # Move down
        ...
        # Calculate rotation matrix manually based on camera position and angle
        rotation_matrix = [
        ...
        # Move back to the negative of the camera position
        glTranslatef(-camx, camy, -camz)
