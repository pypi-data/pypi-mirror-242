...
def auto_exec(test, config_file, output_dir):
    if test:
        print("cofrun test done")
        logging(f"{config_file} test done")
        return
    ...
    if output_dir is not None:
        ...
        # mkdir
        log_dir = os.path.join(output_dir, extract_gpu_info(gpu_name)+formatted_time)
        os.mkdir(log_dir)
        # write config file
        ...
        with open(f'{log_dir}/log', "w") as fp:
            process = subprocess.Popen(['bash','cof_workspace/launch-all.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            ...
            while process.stderr.readable():
                ...
                fp.write(line)
            process.wait()            
            logging(f"process return code: {process.returncode}")
            process.kill()
    else:
        subprocess.run(['bash','cof_workspace/launch-all.sh'])

def auto_gen(config_file, template_file):
    ...
    cluster_ip_list = config_table["CLUSTER_IPS"].split(" ")
...
exit 0'
    ...
    for host_idx,host_ip in enumerate(cluster_ip_list[:NNODES]):
        script_content = autogen_from_template(config_table, template_file, host_idx)
...
def process_file(test, config_file, template_file, output_dir):
    assert auto_gen(config_file, template_file)==0
    start = time.time()
    auto_exec(test, config_file, output_dir)
    print(f"{config_file} done execution time: {time.time()-start:.2f}s")
    logging(f"{config_file} done execution time: {time.time()-start:.2f}s")

def process_batched_files(args):
    assert os.path.exists(args.input), args.input+" not exist"
    config_file_list = []
    for _, _, files in os.walk(args.input):
        for file in files:
            config_file_list.append(os.path.join(args.input, file))
    ...
    if args.list:
        ...
        candidate_config_file_list = [config_file_list[each] for each in parse_str_range(args.range)] if args.range is not None else config_file_list
        for id, config_file in enumerate(candidate_config_file_list):
            ...
            process_file(args.test, config_file, args.template, args.output)

def task_setup(args):
    logging(f"task setup ...")
    if args.input is not None:
        process_batched_files(args)
    else:
        ...
        if check_inner_list(args.file):
            args.input = 'cof-gen/'
            process_batched_files(args)
        else:
            process_file(args.test, args.file, args.template, args.output)
...
def main():
    ...
    for arg in vars(args):
        args_str += f"{arg.ljust(max_arg_length)}: {getattr(args, arg)}\n"
    logging(args_str)
