...
from fga.plugin import FGADestinationPlugin, ContextPlugin, Plugin, EnrichmentPlugin
...
class FGA:
    ...
    def track(self, event: BaseEvent):
        ...
        if isinstance(event, Event):
            self.configuration.server_url = constants.SERVER_URL[constants.EVENT]
        elif isinstance(event, User):
            self.configuration.server_url = constants.SERVER_URL[constants.USER]
        else:
    ...
    def user_identify(self, identify_obj: Identify, event_options: EventOptions, event_properties: Optional[dict] = None):
        if not identify_obj.is_valid():
            self.configuration.logger.error("Empty identify properties")
        else:
            event = User(event_properties=event_properties, user_properties=identify_obj.user_properties)
            event.load_event_options(event_options)
            self.track(event)
