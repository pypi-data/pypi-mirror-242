...
def test_show_changes_as_tables(repository: Repository, capfd):
    ...
    # Assert
    console_output = capfd.readouterr().out
    assert_that(console_output).contains('''## BGPPeering Changes

action    id    bgppeeraddr    bgppeerid    device    entity
--------  ----  -------------  -----------  --------  --------
DELETE    None  None           None         None      None
''')
    assert_that(console_output).contains('''## Device Changes
...
--------  ----  --------------------------------  ---------  --------------  -------------------------  -----  --------  ---------------  ---------  -------  ------  ---------  -----  -------  --------------------------------------------------------------
UPDATE     137  [-Computing Center (039)-]+AAAA+  +BBBB+     None            Computing Center Contacts      0       039  1225 KINCAID ST             OR       EUGENE  US         97401           === BEGIN AUTOMATION MSG ===  > WARNING: Any details entered b
''')
...
def test_show_changes_as_tables_select_columns(repository: Repository, capfd):
    ...
    assert_that(console_output).contains('''## Closet Changes
...
def test_status_report_when_failed_action(repository: Repository):
    ...
    netdot.defaults.ERROR_PICKLE_FILENAME = 'netdot.pickle'
    netdot.defaults.SAVE_AS_FILE_ON_ERROR = True
    # Try to save bogus changes will raise an exception:
    assert_that(repository.save_changes).raises(exceptions.NetdotError).when_called_with()

    # Act
    report = repository.proposed_changes.status_report()

    # Assert
    assert_that(netdot.defaults.ERROR_PICKLE_FILENAME).exists()
    ...
    assert_that(report).matches(r'''Failed Action\(s\):
...
---> Failed with exception: 400 .*''')
    ...
    # Cleanup
    site.delete(confirm=False)
    os.remove(netdot.defaults.ERROR_PICKLE_FILENAME)
