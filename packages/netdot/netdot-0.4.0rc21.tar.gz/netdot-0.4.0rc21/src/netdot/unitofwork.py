...
import netdot
from netdot import NetdotAPIDataclass, actions, defaults, utils
...
class UnitOfWork:
    ...
    def save_as_pickle(self, filename: str = defaults.ERROR_PICKLE_FILENAME) -> str:
...
'''
    ...
    def status_report(self, one_line=defaults.TERSE):
        """Show a report of all actions that have been completed, and all actions that remain to be completed.

        Args:
            one_line (bool, optional): Whether to truncate each line according to your terminal width. Defaults to defaults.TERSE.

        Returns:
            str: ALL actions as pretty string (including 'failed action')
        """
        message = f'''
Completed Actions:

{self.dry_run_of_actions(self._completed_actions, one_line=one_line, completed=True)}

Remaining Actions:

{self.dry_run(one_line=one_line)}
...
'''
    ...
    def changes_as_tables(self, terse=defaults.TERSE, select_cols=None, display_full_objects=defaults.DISPLAY_FULL_OBJECTS) -> str:
        """Return ASCII table(s) representing all of the changes to be made (grouped into tables based on Netdot Data Types).
        
        Args:
            terse (bool, optional): Whether to truncate data in each column. Defaults to defaults.TERSE
        ...
        for dataclass in sorted(
            ...
            data_as_table = self._tabulate_planned_changes(
                dataclass, columns or dataclass.table_header(), column_width, display_full_objects
            )
            if data_as_table:
    ...
    def dry_run(self, one_line=True) -> str:
        ...
        return self.dry_run_of_actions(self._actions, one_line=one_line)

    @staticmethod
    def dry_run_of_actions(actions, one_line=True, completed=False, empty_message='None, yet...') -> str:
        ...
        Returns:
            str: Each action in actions, printed in a nicely enumerated plain english list.
        """
        if not actions:
    ...
    def _tabulate_planned_changes(
        self, dataclass: CSVDataclass, selected_columns, maxcolwidths=None, display_full_objects=defaults.DISPLAY_FULL_OBJECTS
    ):
        table_data = self.with_data_type(dataclass)
        if not table_data:
            return

        rows = [row.as_table_row(selected_columns, display_full_objects=display_full_objects) for row in table_data.as_list()]
        return tabulate(
            rows,
    ...
    def save_changes(self, netdot_repo: "netdot.Repository"):
        ...
        if netdot_repo._dry_run:
            raise RuntimeError('Repository cannot be in dry run mode. Use disable_propose_changes() or build a new Repository with (dry_run=False).')
        save_functions = {
