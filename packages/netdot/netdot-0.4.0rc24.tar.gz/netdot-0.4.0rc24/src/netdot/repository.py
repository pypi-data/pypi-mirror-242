...
from . import client, defaults, exceptions, utils
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            threads (int, optional): How many threads can be used when making GET requests? Defaults to `defaults.THREADS`.
        """
    ...
    def save_changes(self, confirm=True):
        ...
        if self._lock.acquire(blocking=False):
            try:
                ...
                if defaults.SAVE_AS_FILE_ON_ERROR:
                    try:
                        # Save proposed changes to file for recovery
                        self.proposed_changes.save_as_pickle(
                            defaults.ERROR_PICKLE_FILENAME
                        )
                        logger.exception(
                            f'Saved proposed changes to file "{defaults.ERROR_PICKLE_FILENAME}" for recovery.'
                        )
                    except Exception:
                        logger.exception(
                            f'Unable to save changes to file "{defaults.ERROR_PICKLE_FILENAME}"'
                        )
                raise exceptions.NetdotError(
...
Failed to: {self.proposed_changes.failed_action_msg()}."""
    ...
    #
    #
    def show_all_changes(self, one_line=defaults.TERSE):
        # See UnitOfWork.status_report for more details
        print(self.proposed_changes.status_report(one_line))
    ...
    #
    #
    def enable_trace_downloads(self, threshold_bytes: defaults.TRACE_THRESHOLD):
        # See client.Client.enable_trace_downloads for more details
        self.connection.enable_trace_downloads(threshold_bytes)
    ...
    def _connect(cls, _input=input, _getpass=getpass, **kwargs):  # pragma: no cover
        netdot_url = (
            _input(f'What is the URL of the NetDot server? [{defaults.SERVER_URL}]: ')
    ...
    def get_all(self, *args, **url_params) -> List[dataclass]:
        if args:
            if len(args) == 1 and args[0].lower() == 'all':
                ...
                raise ValueError(f'{get_all_function_name} only accepts keyword arguments (or the special positional argument "all")')
