...
from . import defaults, exceptions, parse, trace, utils, validate
...
class Client(Client_v1):
    ...
    def __init__(self, *args, times_to_retry=3, trace_downloads=defaults.TRACE_DOWNLOADS, trace_threshold_bytes=defaults.TRACE_THRESHOLD, **kwargs):
        self._retries = times_to_retry
        self._trace_downloads = trace_downloads
        self._download_tracer: trace.DownloadTracer
        if trace_downloads:
            self.enable_trace_downloads(trace_threshold_bytes)
    ...
    def enable_trace_downloads(self, threshold_bytes=defaults.TRACE_THRESHOLD):
        ...
        Args:
            threshold_bytes (bool, optional): How many bytes to wait between log messages. Defaults to defaults.TRACE_THRESHOLD.
        """
        self._trace_downloads = True
        self._download_tracer = trace.DownloadTracer('Netdot', threshold_bytes)
