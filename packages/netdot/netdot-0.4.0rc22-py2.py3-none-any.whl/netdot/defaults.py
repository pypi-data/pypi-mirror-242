...
def build_NETDOT_CLI_ENV_VARs_parser():
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help='Raise an exception if there is an error parsing any server response (from Netdot). Else, log a warning and continue. (These are generally warnings that should be fixed by a netdot python package developer)', # TODO add a link to "submit an issue" so people can report these errors back... Or maybe just do some telemetry to report these errors back to the netdot python package developer?
    )
    parser.add_argument(
        '--warn-missing-fields',
