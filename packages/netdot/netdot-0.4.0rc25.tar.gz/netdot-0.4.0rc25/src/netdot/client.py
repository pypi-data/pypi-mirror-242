...
class Client_v1(AbstractContextManager):
    ...
    def __init__(self, server, username, password, verify_ssl=(not config.SKIP_SSL), timeout=config.TIMEOUT):
...
class Client(Client_v1):
    ...
    def __init__(self, *args, times_to_retry=3, trace_downloads=config.TRACE_DOWNLOADS, trace_threshold_bytes=config.TRACE_THRESHOLD, **kwargs):
