...
class NetdotAPIDataclass(CSVDataclass):
    ...
    #
    id: int = None
    _auto_update_lock: bool = dataclasses.field(default=threading.Lock(), repr=False)
    ...
    def _display(self, display_full_objects=config.DISPLAY_FULL_OBJECTS):
        ...
        args_str = ', '.join(
            [f'{attr}={value}' for attr, value in display_attributes.items()]
        )
    ...
    def as_table_row(self, select_columns=None, display_full_objects=config.DISPLAY_FULL_OBJECTS) -> Tuple[str, ...]:
        table_row = list()
    ...
    def _infer_xlink_class_map(cls):
        ...
        """
        # TODO, pull from the Union fields themselves instead, similar to from_DTO method.
        ...
        for field, string_parser in cls._infer_base_parsers().items():
            if typing_inspect.is_union_type(string_parser):
                for arg in typing_inspect.get_args(string_parser):
                    if str(arg).startswith("ForwardRef('netdot."):
    ...
    def from_DTO(
        ...
        raise_parse_errors=config.RAISE_FIELD_PARSE_ERRORS,
        warn_missing_fields=config.WARN_MISSING_FIELDS,
    ) -> 'NetdotAPIDataclass':
