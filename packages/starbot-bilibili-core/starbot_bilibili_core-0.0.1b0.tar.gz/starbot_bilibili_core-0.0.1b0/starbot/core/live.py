...
class LiveRoom:
    ...
    async def disconnect(self) -> NoReturn:
        ...
        executor.dispatch(event, LiveEvent.DisconnectedEvent, self.room_id)

    def __add_event_listener(self) -> NoReturn:
        """
        添加心跳响应超时和连接成功监听器
        """
        executor.remove_event_listener("RAW", "CONNECTED", self.room_id)
        executor.remove_event_listener("RAW", "TIMEOUT", self.room_id)

        @executor.on("RAW", "CONNECTED", self.room_id)
        async def on_connected():
            ...
            executor.dispatch(event, LiveEvent.ConnectedEvent, self.room_id)

        @executor.on("RAW", "TIMEOUT", self.room_id)
        async def on_timeout():
            source = RoomUserInfo(self.face, self.uid, self.uname, self.room_id)
            event = TimeoutEvent(source)
            executor.dispatch(event, LiveEvent.TimeoutEvent, self.room_id)
        ...
        for info in data:
            ...
            if info["datapack_type"] == DataPackType.VERIFY_SUCCESS_RESPONSE.value:
                if info["data"]["code"] == 0:
                    ...
                    executor.dispatch(callback_info, "RAW", "CONNECTED", self.room_id)
                ...
                executor.dispatch(callback_info, "RAW", "VIEW", self.room_id)
                ...
                executor.dispatch(callback_info, "RAW", callback_info["type"], self.room_id)
            else:
    ...
    async def __heartbeat(self, ws: ClientWebSocketResponse) -> NoReturn:
        ...
        while True:
            if self.__heartbeat_timer == 0 or self.__heartbeat_timer == -15:
                ...
                self.status = ConnectionStatus.ERROR
                executor.dispatch(None, "RAW", "TIMEOUT", self.room_id)
                break
...
class LiveEventListener:
    ...
    async def add(self, uid: Optional[int] = None, room_id: Optional[int] = None) -> int:
        ...
        @executor.on("RAW", "LIVE", room.room_id)
        async def live_on(raw):
            ...
            executor.dispatch(event, LiveEvent.LiveOnEvent, room.room_id)

        @executor.on("RAW", "PREPARING", room.room_id)
        async def live_off():
            """
            原始下播事件
            ...
            executor.dispatch(event, LiveEvent.LiveOffEvent, room.room_id)

        @executor.on("RAW", "DANMU_MSG", room.room_id)
        async def on_danmu(raw):
            ...
            executor.dispatch(event, event_type, room.room_id)

        @executor.on("RAW", "SEND_GIFT", room.room_id)
        async def on_gift(raw):
            ...
            if total_coin == 0 or discount_price == 0:
                ...
                executor.dispatch(event, LiveEvent.SilverGiftEvent, room.room_id)
                return

            if base["blind_gift"] is None:
                event = GoldGiftEvent(source, sender, fans_medal, honor_level, gift_id, gift_name, count, price)
                executor.dispatch(event, LiveEvent.GoldGiftEvent, room.room_id)
                return
            ...
            executor.dispatch(event, LiveEvent.BlindGiftEvent, room.room_id)

        @executor.on("RAW", "SUPER_CHAT_MESSAGE", room.room_id)
        async def on_super_chat(raw):
            ...
            executor.dispatch(event, LiveEvent.SuperChatEvent, room.room_id)

        @executor.on("RAW", "GUARD_BUY", room.room_id)
        ...
        @executor.on("RAW", "INTERACT_WORD", room.room_id)
        ...
        @executor.on("RAW", "WATCHED_CHANGE", room.room_id)
        async def on_watched_update(raw):
            ...
            executor.dispatch(event, LiveEvent.WatchedUpdateEvent, room.room_id)

        @executor.on("RAW", "LIKE_INFO_V3_CLICK", room.room_id)
        async def on_like_click(raw):
            ...
            executor.dispatch(event, LiveEvent.LikeClickEvent, room.room_id)

        @executor.on("RAW", "LIKE_INFO_V3_UPDATE", room.room_id)
        async def on_like_update(raw):
            ...
            executor.dispatch(event, LiveEvent.LikeUpdateEvent, room.room_id)
    ...
    async def remove(self, uid: int) -> NoReturn:
        ...
        executor.remove_event_listener("RAW", "CONNECTED", room.room_id)
        executor.remove_event_listener("RAW", "TIMEOUT", room.room_id)
        executor.remove_event_listener("RAW", "LIVE", room.room_id)
        executor.remove_event_listener("RAW", "PREPARING", room.room_id)
        executor.remove_event_listener("RAW", "DANMU_MSG", room.room_id)
        executor.remove_event_listener("RAW", "SEND_GIFT", room.room_id)
        executor.remove_event_listener("RAW", "SUPER_CHAT_MESSAGE", room.room_id)
        executor.remove_event_listener("RAW", "GUARD_BUY", room.room_id)
        executor.remove_event_listener("RAW", "INTERACT_WORD", room.room_id)
        executor.remove_event_listener("RAW", "WATCHED_CHANGE", room.room_id)
        executor.remove_event_listener("RAW", "LIKE_INFO_V3_CLICK", room.room_id)
        executor.remove_event_listener("RAW", "LIKE_INFO_V3_UPDATE", room.room_id)
