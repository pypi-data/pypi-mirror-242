...
class SetLockGains(Action):
    ...
    *locks : list, optional
    ...
    def _do(self, state):
        err_sigs = [lck.error_signal for lck in self.locks]
        err_sig_names = [sig.name for sig in err_sigs]

        gain_matrix = state.apply(self.sensing_action)
        for idx, _ in enumerate(self.locks):
            ...
            if "_Q" in err_sig:
                ...
                gain = val.real
            self.locks[idx].gain = -self.gain_scale / gain

    def _requests(self, model, memo, first=True):
        if len(self.locks) == 0:
            self.locks = tuple(lck for lck in model.locks if not lck.disabled)
        else:
