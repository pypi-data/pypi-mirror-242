...
cdef sortkey(x) noexcept:
    return int(x.to)

cdef status_string(int status) noexcept:
...
cdef class CCSMatrix:
    ...
    cpdef _declare_submatrix(self, SuiteSparse_long _from, SuiteSparse_long _to,
                        unicode name, callback=None, type_='m') noexcept:
        """
    ...
    cpdef set_rhs(self, SuiteSparse_long index, complex_t value, unsigned rhs_index=0) noexcept:
    ...
    cdef np.ndarray get_numpy_array_view(self, SuiteSparse_long _from, SuiteSparse_long _to, complex_t** start_ptr, SuiteSparse_long* from_rhs_index) noexcept:
        """
    ...
    cpdef clear_rhs(self, unsigned rhs_index=0) noexcept:
        """
    ...
    cpdef factor(self) noexcept:
        raise NotImplementedError()

    cpdef refactor(self) noexcept:
        raise NotImplementedError()
