...
cdef class KnmConnectorWorkspace(ConnectorWorkspace):
    ...
    cdef initialise_knm_workspaces(KnmConnectorWorkspace self) noexcept:
        cdef:
    ...
    cdef update_map_data(KnmConnectorWorkspace self) noexcept:
        cdef:
            KnmInfo *info
...
@cython.wraparound(False)
@cython.initializedcheck(False)
cdef object c_optical_quantum_noise_knm(ConnectorWorkspace cws) noexcept:
    cdef:
