...
cdef class MirrorWorkspace(KnmConnectorWorkspace):
    ...
    cpdef compile_abcd_cy_exprs(self) noexcept:
        cdef:
...
cdef object c_mirror_carrier_fill(ConnectorWorkspace cws) noexcept:
    r"""
...
cdef object c_mirror_signal_opt_fill(ConnectorWorkspace cws) noexcept:
    cdef:
        MirrorWorkspace ws = <MirrorWorkspace>cws
        double t = sqrt(ws.mv.T)
        double r = sqrt(ws.mv.R)
...
cdef object c_mirror_signal_mech_fill(ConnectorWorkspace cws) noexcept:
    cdef:
        MirrorWorkspace ws = <MirrorWorkspace>cws
        double t = sqrt(ws.mv.T)
        double r = sqrt(ws.mv.R)
...
cdef void get_carrier_vectors(MirrorWorkspace ws, int carrier_index, DenseZVector *c_p1_i, DenseZVector *c_p2_i, DenseZVector *c_p1_o, DenseZVector *c_p2_o) noexcept:
    assert(c_p1_i)
    assert(c_p2_i)
    assert(c_p1_o)
    assert(c_p2_o)
...
cdef object c_mirror_fill_qnoise(ConnectorWorkspace cws) noexcept:
    r"""
