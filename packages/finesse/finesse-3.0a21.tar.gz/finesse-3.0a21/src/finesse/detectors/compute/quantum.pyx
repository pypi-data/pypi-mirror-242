...
cdef class QND0Workspace(DetectorWorkspace):
    ...
    cpdef fill_selection_vector(self) noexcept:
        cdef:
            Py_ssize_t i, j
            Py_ssize_t s_idx
    ...
    cdef fill_source_selection_vector(self, sources, complex_t fill_value) noexcept:
        cdef:
            Py_ssize_t i, j
            Py_ssize_t node_id
            Py_ssize_t idx
    ...
    cpdef get_source_selection_vector(self) noexcept:
        return self.source_s

    cpdef set_covariance_matrix(self, complex_t[::1] v, unicode name) noexcept:
        # TODO: Should this be a copy?
        self.cov = v

qnd0_output = OutputFuncWrapper.make_from_ptr(c_qnd0_output)
cdef object c_qnd0_output(DetectorWorkspace self) noexcept:
    cdef:
...
cdef class QNDNWorkspace(DetectorWorkspace):
    ...
    cdef fill_carrier_qnoise_contributions(self) noexcept:
        cdef:
            Py_ssize_t i, j, k, nf
            Py_ssize_t base_idx
            frequency_info_t f
...
cdef object c_qndN_output(DetectorWorkspace self) noexcept:
    cdef:
...
cdef object c_qshot0_output(DetectorWorkspace self) noexcept:
    cdef:
...
cdef object c_qshotN_output(DetectorWorkspace self) noexcept:
    cdef:
...
cdef class QuantumNoiseDetectorWorkspace(DetectorWorkspace):
    ...
    cdef fill_carrier_qnoise_contributions(QuantumNoiseDetectorWorkspace self) noexcept:
        cdef:
            Py_ssize_t i, j, k
            Py_ssize_t car_idx
            frequency_info_t f
    ...
    cdef fill_qnoised_selection_vector(QuantumNoiseDetectorWorkspace self) noexcept:
        cdef:
            Py_ssize_t i, j, k
            Py_ssize_t s_idx
            frequency_info_t fc, fs
    ...
    cdef c_qnoised_output(self) noexcept:
