...
class OptimiseRFReadoutPhaseDC(Action):
    ...
    DegreeOfFreedom or driven Parameter in the model. The phases are optimised by
    calculating the DC response of the readouts. This Action changes the state of the
    model by varying the readout demodulation phases. If no arguments are given it will
    try to automatically optimise any lock element in the model that is using an RF
    readout with respect to the lock feedback parameter.

    Parameters
    ----------
    args
        Pairs of DegreesOfFreedom or Parameter and ReadoutRF elements, or pairs
    ...
    def _setup(self, model):
        ...
        for i, name in enumerate(self.readout_output_names):
            ...
            if isinstance(obj, ReadoutDetectorOutput):
                ...
                default = list(obj.outputs.__dict__.values())[0]
                deprecation_warning(
