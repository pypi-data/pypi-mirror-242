...
class IntegrateAction(TableAction):
    ...
    def do(self):
        ...
        self._save_current_state_and_set_cell_values(table, area, rmse, model, is_valid)

        return True

    def _save_current_state_and_set_cell_values(
        self, table, area, rmse, model, is_valid
    ):
        self.memory = table[self.data_row_idx]
        app = self.appendum + self.postfix
        set_value = partial(table._set_value, [self.data_row_idx])
...
class ChromatogramIntegrateAction(IntegrateAction):
    ...
    def do(self):
        ...
        chromatogram_cols = ["rtmin_chromatogram", "rtmax_chromatogram", "chromatogram"]

        area = rmse = model = is_valid = None
        if integrator and all(row[f + postfix] is not None for f in chromatogram_cols):
            chromatogram = row["chromatogram" + postfix]

            model = integrator.fit_chromatogram(self.rtmin, self.rtmax, chromatogram)
            area = model.area
            rmse = model.rmse
            is_valid = model.is_valid
