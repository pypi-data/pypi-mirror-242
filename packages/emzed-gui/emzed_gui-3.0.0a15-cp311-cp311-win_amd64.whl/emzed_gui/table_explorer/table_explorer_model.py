...
class TableModel(QAbstractTableModel):
    ...
    def set_row_header_colors(self, widget_row_indices, color_row_headers):
        ...
        first = min(self.colors_selected_rows.keys())
        last = max(self.colors_selected_rows.keys())
    ...
    def chromatogram_col_names(self):
        return ["rtmin_chromatogram", "rtmax_chromatogram", "chromatogram"]
    ...
    def has_peaks(self):
        return self.check_for_any(self.eic_col_names()) or self.check_for_any(
            self.chromatogram_col_names()
        )

    def hasEIC(self):
        return self.check_for_any(["eic"])
    ...
    def get_chromatograms(self, data_row_idx, postfixes=None):
        ...
        for postfix in supported_postfixes(self.table, names):
            if postfixes is not None and postfix not in postfixes:
                continue
            rtmin, rtmax, chromatogram = (row[key + postfix] for key in names)
            peak_id = row.get("id" + postfix, "")
            if all(
                (
                    rtmin is not None,
...
class MutableTableModel(TableModel):
    ...
    def integrate(self, label, method, rtmin, rtmax):
        ...
        for data_row_index in self.selected_data_rows:
            widget_row_idx = self.dataRowToWidgetRow[data_row_index]
            self.runAction(
                action_class,
                data_row_index,
                postfix,
            ...
            widget_row_indices.append(widget_row_idx)
