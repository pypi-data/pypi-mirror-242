...
def Test():
    ...
    #Ret=gpf.IndexBCC("./Examples/BuildBCC/list.txt")
    #Ret=gpf.RunBCC("a的{}Freq")
    
def Test1():
    gpf = GPF()
    gpf.SetMax()   
    Ret=gpf.CallBCC("a的{}Freq")
    print(Ret)
...
def Test3():
    gpf = GPF()
    In=open("./Examples/BuildBCC/Corpus.txt","r")
    Out=open("./Examples/BuildBCC/Segment_0Ex.txt","w")
    Num=0
    for S in In:
        Ret=gpf.Segment(S.strip())
        print(Num,Ret,file=Out);
        if Num%10000 == 0:
            print(Num,Ret)
        Num+=1 
    In.close()
