...
class GPF:
    ...
    def Segment(self,text):
        ...
        self.library_gpf.GPF_CRF.argtypes = [c_void_p, c_char_p]
        self.library_gpf.GPF_CRF.restype  = c_int
        str_len=self.library_gpf.GPF_CRF(self.hHandleGPF,self.RetBuff)
        ret = string_at(self.RetBuff, str_len)
        return ret.decode()

    def POS(self,text):
        Ret=self.Segment(text)
        self.library_gpf.GPF_POS.argtypes = [c_char_p, c_char_p]
        self.library_gpf.GPF_POS.restype  = c_int
        str_len=self.library_gpf.GPF_POS(Ret.encode(),self.RetBuff)
        ret = string_at(self.RetBuff, str_len)
        return ret.decode()
