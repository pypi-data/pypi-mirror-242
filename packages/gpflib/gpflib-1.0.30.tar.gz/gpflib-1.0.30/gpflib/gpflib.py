...
class GPF:
    ...
    def CallStructure(self, sentence, name):
        return self.CallService(sentence, name)

    def RunService(self, sentence, name):
        return self.CallService(sentence, name)

    def GetStructure(self, sentence, name):
        return self.CallService(sentence, name)
    ...
    def Seg(self,text):
        ...
        self.library_gpf.GPF_GridSeg.argtypes = [c_void_p, c_char_p]
        self.library_gpf.GPF_GridSeg.restype  = c_int
        str_len=self.library_gpf.GPF_GridSeg(self.hHandleGPF,self.RetBuff)
        ret = string_at(self.RetBuff, str_len)
        return ret.decode()
    ...
    def Segment(self,text,table=""):
        ...
        if table == "":
            ...
            str_len=self.library_gpf.GPF_Seg(self.hHandleCRFPOS,text.encode(),self.RetBuff)
            ret = string_at(self.RetBuff, str_len)
        else:
            self.SetGridText(text)
            self.library_gpf.GPF_GridSegUser.argtypes = [c_void_p, c_char_p, c_char_p]
            self.library_gpf.GPF_GridSegUser.restype  = c_int
            str_len=self.library_gpf.GridSegUser(self.hHandleGPF,table.encode(),self.RetBuff)
            ret = string_at(self.RetBuff, str_len)
        return ret.decode()  

    def POS(self,text,table=""):
        Ret=self.Segment(text,table)
        self.library_gpf.GPF_POS.argtypes = [c_void_p, c_char_p, c_char_p]
        self.library_gpf.GPF_POS.restype  = c_int
        str_len=self.library_gpf.GPF_POS(self.hHandleCRFPOS,Ret.encode(),self.RetBuff)
        ret = string_at(self.RetBuff, str_len)
        return ret.decode()
