...
initial_mapping_options = ["random", "periodic_random", "periodic_fixed", "random_diagonal", "fcfs", 
						"1d_random_distribution", "2d_random_distribution"]
...
# "1d_random_distribution": the quantum chip is horizontally equally separated (--> area) as much as the number of the algorithm qubits.
# 							each algorithm qubit is randomly arranged in each area
# "2d_random_distribution" : the quantum chip is horizontally and vertically equally separated (--> 2 x n) 
# 							each algorithm qubit is randomly arranged in each area
...
def initialize_qubit_mapping(algorithm_qubits, qchip_size, **kwargs):
	...
	print(algorithm_qubits, qchip_size)
	...
	if number_algorithm_qubits > qchip_size:
		...
		elif mapping_option in ["1d_random_distribution", "2d_random_distribution"]:
			
			# qchip_architecture : 양자칩 dimension (m x n)
			qchip_architecture = kwargs.get("qchip_dimension")

			print(qchip_architecture)
			if mapping_option == "1d_random_distribution":
				# 양자칩 width (n) 을 area 로 나누고, 각 영역별로.. random 하게 배치..
				area_width = int(qchip_architecture.get("width") / number_algorithm_qubits)
				area_height = qchip_architecture.get("height")
				...
				if area_width:
					...
					for i in range(number_algorithm_qubits-1):
						for r in range(area_height):
							for k in range(area_width):
								qubits_in_area[i].append(r*qchip_architecture["width"] + i*area_width + k)

					included_qubits = (number_algorithm_qubits-1) * area_width
					for r in range(area_height):
						for k in range(included_qubits, qchip_architecture.get("width")):
							qubits_in_area[number_algorithm_qubits-1].append(r*qchip_architecture["width"] + k)

				else:
					raise Exception("The column width is not suitable for this option")
			
			else:
				# 양자칩을 2 라인으로 나뉘고, 각 영역별로 random 하게 배치..
				number_areas = number_algorithm_qubits
				# 나눌 영역을 짝수개로...
				if number_areas%2 == 1: number_areas+=1

				# rows 구분자 인덱스
				rows_separator = int(qchip_architecture["height"]/2)
				cols_separator = int(qchip_architecture["width"]*2/number_areas)

				rows = {0: (0, rows_separator), 1: (rows_separator, qchip_architecture["height"])}
				cols = {i : (i*cols_separator, (i+1)*cols_separator) for i in range(int(number_areas/2)-1)}
				cols[int(number_areas/2)-1] = ((int(number_areas/2)-1)*cols_separator, qchip_architecture["width"])

				if rows_separator and cols_separator:
					# qubits_in_area 부터 설정
					qubits_in_area = collections.defaultdict(list)

					# area 에 대해서...
					for i in range(number_areas):
						row_idx = int(i*2/number_areas)
						col_idx = int((i)%(number_areas/2))

						for r in range(rows[row_idx][0], rows[row_idx][1]):
							for k in range(cols[col_idx][0], cols[col_idx][1]):
								qubits_in_area[i].append(r*qchip_architecture["width"] + k)

				else:
					raise Exception("The area width or height is not suitable for this option ")
...
if __name__ == "__main__":
	...
	ret = initialize_qubit_mapping(algorithm_qubits, 30, option="1d_random_distribution", qchip_dimension={"height": 6, "width": 5})
	ic(ret)