...
def read_simdb(path, file):
    ...
    '''
    widgets = ['Iterating through APSIM output database now: ',
               progressbar.BouncingBar(marker='', left='|', right='|', fill=' ', fill_left=True)]
    bar = progressbar.ProgressBar().start()
    if file.endswith('.db') == False:
        print('*** Warning: \n The file name: {0}  is not a data base file did you forget to add .db extension?'.format(
            file))
    else:
        pth = opj(path, file)
...
def runAPSIM(path, filename, apsimxlocal=apsimloc, readdb=True, select_report=None, ):
    ...
    try:
        ...
        if filename == '':
            ...
            print(
                '*** Warning: \n The file name: {0}  is not an apsimx file did you forget to add .apsimx extension?'.format(
                    filename))
        else:
