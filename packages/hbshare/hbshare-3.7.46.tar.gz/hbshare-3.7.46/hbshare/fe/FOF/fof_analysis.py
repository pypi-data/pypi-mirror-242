...
class FOF_analysis:
    ...
    def valuation_table_analysis(self,product,end_date,start_date=None,get_nav_from_gzb=False):
        ...
        orginal_code=['1ZLZG3', '1BYTZ2', '1AAAA7', '1BXZG2', '1BLKS1', '2QHGH1', '1HYZG5', '1HFL32', '2HFLH1',
                      ...
                      'FLWJN1','xfcxf2','GYLW1A','SFCHCA','YTCZN7','GFJZ12','1HCHF1','66391B','246040','23582A','HBCT2A','HXQJ1F','Y4405C']
        maped_code=['SLP211', 'SNZ338', 'SQS281', 'STR792', 'SED697', 'SY0256', 'S83614', 'SNX384', 'SLK497',
                    ...
                    'P02494','P12095','SJ5324','S63506','SS0299','S62866','SQG573','S66391','M06228','S23582','M06228','SH5035','SY4405']
        ...
        if(len(wrong_jjdm)>0):
            ...
            hold_df=pd.merge(hold_df,wrong_jjdm[['科目代码','投资基金代码_x']],how='left',on='科目代码')
            hold_df.loc[hold_df['投资基金代码_x'].notnull(),'科目代码']=hold_df[hold_df['投资基金代码_x'].notnull()]['投资基金代码_x']
            hold_df.drop('投资基金代码_x',axis=1,inplace=True)

            wrong_jjdm=wrong_jjdm[wrong_jjdm['投资基金代码_x'].isnull()]
        if(len(wrong_jjdm)>0):
        ...
        def remvoe_duplicate_ticker(hold_df):
            ...
            if(len(error_list)>0):
                ...
                error_list[['quant_x', 'value_increased_x']] = error_list[['quant', 'value_increased']].fillna(0)
                error_list=error_list.groupby(['jjdm', 'date']).sum()[['quant_x','value_increased_x']].reset_index()
                ...
                hold_df.drop(['quant_x', 'value_increased_x'],axis=1,inplace=True)
    ...
    def fund_attribution_new(self,hold_df,gl_df):
        ...
        for jjdm in hold_df['jjdm'].unique():
            ...
            # tempdf=hold_df[hold_df['jjdm']==jjdm] SEU546 SLP211
    ...
    def generate_index_enhance_index(start_date,end_date):
        ...
        quant_pool_in = \
        pd.read_excel(r"E:\GitFolder\docs\To虚怀\业绩统计(fof&池）\基金池\数据源\基金池202308\量化基金池202308.xlsx"
...
if __name__ == '__main__':
    ...
    #['大类配置','易米大类配置','冲精选N1']
    for product in ['冲精选N1']:

        #
        fa = FOF_analysis(dir=r"E:\FOF分析\{}\交易流水.xlsx"
                          .format(product), end_date='20230930')
        # fa.valuation_table_analysis(product=product,start_date='20150101',end_date='20230930')
        fa.holding_analysis(dir=r"E:\FOF分析\{}\基金持仓数据.xlsx".format(product),
                             start_date='20200301',end_date='20230930')
