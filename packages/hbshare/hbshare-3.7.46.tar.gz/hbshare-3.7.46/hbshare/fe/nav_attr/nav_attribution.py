...
class StyleAttribution:
    ...
    def _verify_input_param(self):
        ...
        if self.fund_type not in ['mutual', 'private','index']:
            msg = "fund_type not in ['mutual', 'private','index'], check your input"
            logger.error(msg)
    ...
    def _compute_style_attribution_xz(self, date, df):
        ...
        perf_attr.loc['alpha', 'return_attr'] = perf_attr.loc['alpha']['factor_exposure']
    ...
    def _style_analysis_xz(self):
        ...
        #增加误差项
        perf_attr_df.loc[len(perf_attr_df)] = ['误差', 0,
                                               real_return-perf_attr_df['return_attr'].sum(), date]
