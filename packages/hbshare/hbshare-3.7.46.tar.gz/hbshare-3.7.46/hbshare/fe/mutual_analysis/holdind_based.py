...
class General_holding:
    ...
    def adjusted_hldquant(self,jjdm_list,start_date,end_date):
        ...
        sql_script = "SELECT JYRQ,SFZM FROM funddb.JYRL WHERE JYRQ >= {} and JYRQ <= {} and SFJJ=0 and SFZM=1".format(
        ...
        for m in range(int(np.ceil(len(zqdm_list)/500))):
            ...
            select ZQDM ,JYRQ ,SPJG from FUNDDB.ZGJY where  DRJJ is not null and JYRQ in ({0}) and ZQDM in ({1})
             """.format(util.list_sql_condition(calander),
                        util.list_sql_condition(zqdm_list[m*500:(m+1)*500]))
            spjg=hbdb.db2df(sql, db='readonly')
            if(len(spjg)>0):
                price_df.append(spjg.drop('ROW_ID', axis=1))
