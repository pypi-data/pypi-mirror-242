...
class QuantitativeReport:
    ...
    def get_fund_stock_change(self):
        ...
        fund_holding_disp['SEC_SHORT_NAME'] = fund_holding_disp.apply(lambda x: x['SEC_SHORT_NAME'] + '（香港）' if len(x['TICKER_SYMBOL']) < 6 else x['SEC_SHORT_NAME'], axis=1)
    ...
    def get_quantitative_report(self):
        ...
        document.save('{0}{1}（{2}）定量分析报告.docx'.format(self.file_path, self.target_fund_name.replace('/', ''), self.target_fund_code))
