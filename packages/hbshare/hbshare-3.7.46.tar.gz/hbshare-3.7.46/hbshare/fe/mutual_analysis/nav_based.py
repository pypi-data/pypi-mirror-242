...
def prv_pool_report(date,style_date):
    ...
    jzrq['预警']=''
    # jzrq.loc[(jzrq['最近3个月最大回撤']>=max_draw_back.loc['75%']['2103'])|
    #          (jzrq['最近6个月最大回撤']>=max_draw_back.loc['75%']['2106'])|
    #          (jzrq['最近一年最大回撤']>=max_draw_back.loc['75%']['2201']),'预警']='业绩波动较大,'
            ...
            for file in filenames:
                if(glr[0:2] in file):
                    hld_file=pd.read_excel(holding_folder+"\\"+file
                                           ,sheet_name='原始数据')
            # print(file)
            hld_file=hld_file[(hld_file['交易日期']==hld_file['交易日期'].max())&(hld_file['股票代码'].notnull())]
    ...
    for col in ['前三大个股占比', '前五大个股占比', '前十大个股占比']:
...
if __name__ == '__main__':
    ...
    #mutual_fund_industry_exp().get_the_industry_exp('20231101','20200601','20230630')
    ...
    prv_pool_report('20231110','20230930')
    ...
    # nbs.style_exp_simpleols_foroutsideuser_prv('20230928')
