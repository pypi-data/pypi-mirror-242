...
class CliGen0Python(CliGenBase):
    ...
    def gen_parser(self, data=None, fp=sys.stdout, base_indent=4):
        def gen_parsed_data(data, path, parent=None):
            ...
            known_keywords = ['help', 'description', 'epilog', 'aliases']
            # print(f'{kws=}')
            for k in kws:
                if k not in known_keywords:
                    print(f'unhandled keyword {k!r} with value {kws[k]!r} for PData')
            for attr in known_keywords:
                if attr in kws:
            ...
            for subparser in sub_parsers:
                ...
                if not isinstance(sp_name, str):
                    # assume tuple/ruamel.yaml.comments.CommentedKeySeq
                    aliases = sp_name[1:]
    ...
    def generate_help(self, pd, fp=sys.stdout, base_indent=4):
        ...
        for oa in pd.all_options(local_first=True):
            if oa.typ == 'str':
                if oa.metavar is not None:
                    target = ' ' + oa.metavar.upper()
                else:
        ...
        for arg in pd.arguments:
            ...
            if self._add_defaults is not None and arg.typ not in ['bool']:
                ...
                h += self._add_defaults
            h = self.help_format(h, arg)
            # h = h % (dict(
            #              default=f'{{pd.{arg.target}!r}}',
            #              metavar=f'{{pd.{arg.metavar}!r}}',
            #          ))
        ...
        for oa in pd.all_options(local_first=True):
            ...
            if ops:
                ...
                if self._add_defaults is not None and oa.typ not in ['bool']:
                    # don't specify a type even if we know ao.typ
                    h += self._add_defaults # % (dict(default=f'{{pd.{oa.target}!r}}'))  # NOQA
                h = self.help_format(h, oa)
    ...
    def help_format(self, s: str, oa: Option) -> str:
        return s % dict(
            default=f'{{pd.{oa.target}!r}}',
            metavar=f'{oa.metavar}',
        )
    ...
    def gen_one_parser(self, parsed_data, fp=sys.stdout, base_indent=4):
        ...
        for sp in pd.subparsers:
            if sp.aliases:
                print(f"{bi}{bi}elif token in {[sp.name] + sp.aliases!r}:", file=fp)
            else:
...
#                # if not hassatr
    ...
    def gen_option_code(self, oa, short, bi, ei, fp):
        ...
        if oa.typ == 'int' and oa.action == 'count':
            ...
            print(f"{bi}{ei}self.{oa.target} = {oa.const if oa.const is not None else True}", file=fp)  # NOQA
        elif oa.typ == 'str':
            if short:
    ...
    def get_kw(self, data, pdata: PData = None):
        ...
        for elem in data:
            ...
            if tag is None:
                ...
                continue
            if not top_level and tag in ['!DefaultSubCommand', '!DefaultSubparser']:
                parent = pdata.parent
                assert parent is not None
                if pdata.parent.default_subparser not in [None, pdata]:
                    print(
                        f'cannot have !DefaultSubCommand for both "{pdata.parent.default_subparser.name}" and "{pdata.name}"',  # NOQA
                    )
                    sys.exit(1)
