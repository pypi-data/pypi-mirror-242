    ...
    def send_account_status_report(self):
        ...
        if option_position.shape[0] > 0:
            ...
            for exp_date in exp_date_list:
                ...
                option_summary = option_position[option_position[position_summary.EXP_DATE]==exp_date][['account name',position_summary.SYMBOL, position_summary.STRATEGY, position_summary.QUANTITY, position_summary.OPEN_PRICE, position_summary.LAST_PRICE,  position_summary.LAST_STOCK_PRICE, position_summary.BREAKEVEN_L, position_summary.BREAKEVEN_H, position_summary.GAIN, position_summary.LAST_WIN_PROB, position_summary.MAX_LOSS, position_summary.MAX_PROFIT,  position_summary.PL, 'days to earning']]
                #option_summary[position_summary.OPEN_PRICE] = option_summary.apply(lambda x: round(x[position_summary.OPEN_PRICE],3) if np.isnan(x[position_summary.OPEN_PRICE]) != True else np.nan, axis = 1)  
                #option_summary[position_summary.LAST_PRICE] = option_summary.apply(lambda x: round(x[position_summary.LAST_PRICE],3) if np.isnan(x[position_summary.LAST_PRICE]) != True else np.nan, axis = 1)  
                option_summary[position_summary.LAST_STOCK_PRICE] = option_summary.apply(lambda x: round(x[position_summary.LAST_STOCK_PRICE],3) if np.isnan(x[position_summary.LAST_STOCK_PRICE]) != True else np.nan, axis = 1)  
                option_summary[position_summary.GAIN] = option_summary.apply(lambda x: round(x[position_summary.GAIN],3) if np.isnan(x[position_summary.GAIN]) != True else np.nan, axis = 1)  
                option_summary[position_summary.MAX_RISK] = option_summary.apply(lambda x: x[position_summary.MAX_LOSS] *  x[position_summary.QUANTITY] * 100, axis = 1) 
                option_summary[position_summary.MAX_PROFIT] = option_summary.apply(lambda x: round(x[position_summary.MAX_PROFIT] *  x[position_summary.QUANTITY] * 100,2), axis = 1) 
                max_risk = option_summary[position_summary.MAX_RISK].sum()
                option_summary.sort_values(position_summary.GAIN, inplace=True)
                display = option_summary[[position_summary.SYMBOL, position_summary.STRATEGY, position_summary.QUANTITY, position_summary.MAX_PROFIT, position_summary.MAX_RISK, position_summary.PL, position_summary.GAIN, 'days to earning', position_summary.LAST_WIN_PROB, position_summary.LAST_STOCK_PRICE, position_summary.BREAKEVEN_L, position_summary.BREAKEVEN_H]]
                display.columns  = [sub.replace('_', ' ') for sub in display.columns]                
                mail_body += '<h4>Expired Date: %s Days to Expire %d Postiion # %d Sum of Max Risk $%.2f</h4>' % (str(exp_date), days_to_expire, option_summary.shape[0], max_risk) + display.to_html(index=False)
...
if __name__ == '__main__':
    ...
    me = user(site=mysite, user_name='stester')
