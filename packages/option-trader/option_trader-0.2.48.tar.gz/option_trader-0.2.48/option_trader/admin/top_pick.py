...
class top_pick_mgr():
    ...
    def refresh_top_picks(self, symbol_list):                 
        df = get_watchlist_trade_targets(symbol_list, st.ALL_STRATEGY)
        symbol_list = df[position_summary.SYMBOL].unique()
        for symbol in symbol_list:
            dd = df[df[position_summary.SYMBOL]==symbol]    
            exp_date_list = dd[position_summary.EXP_DATE].unique()
            for exp_date in exp_date_list:
                de = dd[dd[position_summary.EXP_DATE]==exp_date]            
                strategy_list = dd[position_summary.STRATEGY].unique()
...
if __name__ == '__main__':
    ...
    tp.refresh_top_picks(['AAPL', 'MSFT', 'NVDA'])
