...
"""
# pylint: disable=inconsistent-return-statements, disable=line-too-long
import os
import time
        ...
        def func_with_retries(*args, **kwargs):
            ...
            while _tries > 0:
                try:
                    ...
                    # get logger message
                    _delay = float(os.environ.get('API_WAIT_TIME', _delay))
                    if _tries == 1:
...
def request_handler(
    wait: float = 0, backoff_factor: float = 0.01, backoff_method: str = "random"
):
    """
    Decorator - managing API failures
