...
class GaussianInputController:
    ...
    def generate_gaussian_input(self):
        ...
        if utils.file_exists_in_subdir('temp', 'molecule.pdb'):
            xyz_data, charge, multiplicity = utils.pdb_to_xyz_data('temp/molecule.pdb')
            if xyz_data is None:
                self.view.result_label.setText("Error - Unable to process 'molecule.pdb'.")
                return
        else:
            xyz_data, charge, multiplicity = utils.smiles_to_xyz(smiles_string)
...
class ViewerWindow(QWidget):
    ...
    def convertAndSendSmiles(self):
        pdb_data = utils.smiles_to_pdb(self.smilesString)
        if pdb_data:
...
def main():
    ...
    atexit.register(utils.cleanup_file)
    sys.exit(app.exec())
