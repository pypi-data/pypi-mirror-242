...
class KineticForms():
    ...
    def __init__(self, connection):
        if isinstance(connection, dict):
            ...
            try:
                ...
                return {"error_code": "5501",
                        "error_msg": "Connection Vault " + str(connection_vault_path) + " not found.",
                        "data": {}}
    ...
    def connect(self):
        try:
            ...
            return {"error_code": "5501", "error_msg": "Connection Vault " + str(connection_vault_path) + " not found.",
                    "data": {}}
        except json.JSONDecodeError as e:
            return {"error_code": "5502", "error_msg": "Error decoding JSON" + str(e), "data": {}}
    ...
    def touch(self, table_name, input_dict):
        ...
        sql = "select count(*) as c from " + str(table_name) + " where id = '" + str(sha256) + "'"
        c = self.sql0(sql)
        count = c['data']['c']

        if c['data']['c'] > 0:
            return {"error_code": "0", "error_message": "", "data": { "id": sha256, "count": count }}
        else:
            sql = "insert into " + str(table_name) + " values ('" + str(sha256) + "')"
            print(sql)
            self.execute(sql)
