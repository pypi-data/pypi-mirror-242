...
class FakeClient:
    ...
    def getTransformationRuns(self, condDict):
        ...
        if condDict["TransformationID"] == self.transID:
            ...
            if not runs and self.transFiles:
                ...
                if not res["OK"]:
                    return res
                runs = list({meta["RunNumber"] for meta in res["Value"]["Successful"].values()})
            for run in runs:
    ...
    def prepareForPlugin(self, lfns):
        ...
        print(f"Preparing the plugin input data ({len(lfns)} files)")
        ...
        for lfnChunk in breakListIntoChunks(lfns, 200):
            ...
            if res["OK"]:
                ...
                print(f"Error getting replicas of {len(lfns)} files:", res["Message"])
        print(f"Obtained replicas of {len(lfns)} files in {time.time() - startTime:.3f} seconds")
...
def main():
    ...
    if defaultOptions:
        ...
        res = Operations().getOptionsDict(f"TransformationPlugins/{plugin}")
        if res["OK"]:
            gLogger.notice(f"Parameters for plugin {plugin} (*<param> means it is a generic parameter)")
            options = res["Value"]
            ...
            for opt in [opt for opt in sorted(options) if "SEs" in opt]:
                gLogger.notice(f"\t{opt} : {options[opt]}")
            # Other options
    ...
    if (
        allFiles
        or not plugin
        or plugin == "DestroyDataset"
        or pluginScript.getOption("Productions")
        or transType == "Processing"
    ):
        visible = "All"
    bkQueryDict = {}
    checkReplica = True
    ...
    if requestedLFNs:
        print(f"{len(requestedLFNs)} requested LFNs")
    else:
    ...
    if requestedLFNs:
        ...
        lfns = bkQuery.getLFNs(
            printSEUsage=(
                (transType == "Removal" or not plugin)
        ...
        if not checkReplica:
            ...
            print(f"{len(lfns)} files in directories:")
            directories = {}
            import os
    ...
    print(f"\nNow testing the {transType.lower()} plugin {plugin}")
    ...
    oplugin = TransformationPlugin(
        plugin, transClient=fakeClient, dataManager=DataManager(), bkClient=BookkeepingClient()
    )
    pluginParams["TransformationID"] = transID
    ...
    startTime = time.time()
    res = oplugin.run()
    print(f"Plugin took {time.time() - startTime:.1f} seconds")
    print("")
    if res["OK"]:
        ...
        for task in res["Value"]:
            ...
            if len(task[1]) == 1:
                ...
                if previousTask["Tasks"]:
                    if task[0] == previousTask["SEs"] and location == previousTask["Location"]:
                        ...
                        if previousTask["First"] == i - 1:
                            ...
                            print(
                                "%d:%d (%d tasks)" % (previousTask["First"], i - 1, i - previousTask["First"]), end=" "
                            )
                            "- Target SEs:", previousTask["SEs"], "- 1 file", " - Current locations:", previousTask[
                                "Location"
                            ]
                ...
                if previousTask["Tasks"]:
                    ...
                    printFinalSEs(transType, previousTask["Location"], previousTask["SEs"])
                    previousTask = {"First": 0, "SEs": None, "Location": None, "Tasks": 0}
                print(i, "- Target SEs:", task[0], f"- {len(task[1])} files", " - Current locations:", location)
