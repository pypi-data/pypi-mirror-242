...
class Fun:
    ...
    def generate_variables_data(bid, cat_id, types, status, page, page_id, location_codes, keys, moduleIds, sort):
        b = Fun.generate_context_data(cat_id, types, status, page)
        global_data = {
    ...
    @staticmethod
    def generate_context_data(cat_id, types, status, page, str_page=1):
        common_data = Fun.generate_common_data(cat_id, types, int(time.time() * 1000))
        if types:
            str_page = "1"
        context_map = {
            "beikeItems": {**common_data, "statusOrders": [f"{status}"], "page": f"{page}"},
            "foldedItems": {**common_data, "statusOrders": ["2"], "pageSize": "4", "page": str_page},
            "recommend": {**common_data, "statusOrders": [f"{status}"], },
            "items": {**common_data, "statusOrders": [f"{status}"], "page": f"{page}"}
        }
        return context_map

    @staticmethod
    def generate_common_data(cat_id, types, ti):
        path = "paimai2021_tz"
        ...
        if types:
            ...
            data = {
                ...
                "path": f"{path}",
                "keywordSourceInfo": {}, "appendMap": {"sid": f"6261471648_{ti}"}
            }
        else:
            # 拍卖
            data = {
