import os
from abstract_gui import AbstractBrowser,text_to_key,make_component,ensure_nested_list,expandable,RightClickManager,get_screen_dimensions
...
instructions_keys = ["instructions",]+list(InstructionManager().default_instructions.keys())
...
def get_feedback()->list:
    ...
    sub_sub_layout = []
    sub_layout = []
    for i,title in enumerate(instructions_keys+['other']):
...
def get_responsive_column(contents):
    return make_component("Column", contents, **expandable(vertical_scroll=True))

  # Add other components as needed
def get_file_preview_layout():
    listbox = make_component("Listbox", [], size=(50, 10), key=text_to_key(text='files list', section='responses'), enable_events=True)
    file_preview_text = make_component("Text", "", size=(50, 10), key=text_to_key(text='file preview', section='responses'))
    return [[listbox], [file_preview_text]]

def get_response_tab_layout():
    collate_responses_checkbox = make_component("Checkbox", 'Collate Responses', key=text_to_key("collate responses", section='responses'), tooltip="Check this to collate responses from selected files", enable_events=True)
    format_json_checkbox = make_component("Checkbox", 'Format JSON', default=True, key=text_to_key("format json to string", section='responses'), tooltip="Enable to format JSON responses for readability", enable_events=True)
    file_options_frame = make_component("Frame", 'File Options', [[collate_responses_checkbox], [format_json_checkbox]], tooltip="Settings for file processing")
    response_tab_layout = [[file_options_frame]]  # Add more components as needed
    return response_tab_layout

def utilities()->object:
    layout = []
    file_preview_layout = get_file_preview_layout()
    responsive_column = get_responsive_column([response_tab, file_preview_layout])
    collate_responses_checkbox = make_component("Checkbox", 'Collate Responses', key=text_to_key("collate responses", section='responses'), tooltip="Check this to collate responses from selected files", enable_events=True)
    format_json_checkbox = make_component("Checkbox", 'Format JSON', default=True, key=text_to_key("format json to string", section='responses'), tooltip="Enable to format JSON responses for readability", enable_events=True)
    file_options_frame = make_component("Frame", 'File Options', [[collate_responses_checkbox], [format_json_checkbox]], tooltip="Settings for file processing")
    ...
    layout.append(make_component("Tab",'RESPONSES', abstract_browser_layout(section='responses',extra_component = make_component("Frame",'file options',ensure_nested_list([[file_preview_layout],
                                                                                                                                                                            [responsive_column],
                                                                                                                                                                            [file_options_frame]]))),key=text_to_key(text='response tab'),**expandable(size=(50, 100)))),
...
def abstract_browser_layout(section:str=None,extra_component:(list or object)=None)->object:
