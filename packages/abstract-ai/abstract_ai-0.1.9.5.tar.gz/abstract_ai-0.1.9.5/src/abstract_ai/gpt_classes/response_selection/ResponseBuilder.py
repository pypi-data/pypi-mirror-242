...
class ResponseManager:
    ...
    def post_request(self):
        ...
        """
        prompt = self.prompt_mgr.create_prompt()  # Assumes create_prompt() method generates the required prompt
        endpoint = self.api_mgr.endpoint  # Assumes api_mgr has the endpoint information
        headers = self.api_mgr.headers  # Assumes api_mgr manages headers, including API key
    ...
    def process_response(self, response):
        ...
        self.save_manager.save(response_json, self.title, self.directory, self.model_mgr.selected_model_name)
    ...
    def get_all_instruction(self, data: dict) -> None:
        ...
        for response_key, value in data.items():
            if response_key in self.abort_js:
                ...
                if response_key == "abort" and value:
                    self.break_current = True
                continue

            if response_key == "token_adjustment":
                self.token_adjustment = int(value) if value else 0
                continue

            if value == '':
                value = None

            if response_key in ["suggestions", "notation", "generate_title"]:
                setattr(self, response_key, value)

            if response_key in ["prompt_as_previous", "additional_responses", "request_chunks"]:
                bool_value = bool(value)
                setattr(self, response_key, bool_value)
    ...
    def send_query(self):
        ...
        self.break_current = self.abort_js.get("abort", False)
