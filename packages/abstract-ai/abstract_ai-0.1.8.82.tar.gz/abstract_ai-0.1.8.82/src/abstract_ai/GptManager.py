...
class GptManager:
    ...
    def __init__(self)->None:
 	...
 	"""
        ...
        self.window_name = self.window_mgr.add_window(window_name="Chat GPT Console",title="Chat GPT Console",layout=get_total_layout(),window_height=0.9,window_width=0.9)
...
# Updates model manager with latest model name from GUI
    ...
    def model_event_check(self)->None:
        ...
        if self.event in "-MODEL-":
            self.update_model_mgr()
            self.prompt_mgr.update_request_and_prompt_data(model_mgr=self.model_mgr)
        else:
            return False
        return True
