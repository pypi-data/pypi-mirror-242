...
class InstructionManager:
    def __init__(self)->None:
        self.default_instructions = {"api_response": {"instruction": "place response to prompt here", "example": "this is the reply to your request"},
                                "additional_responses": {"instruction": "if needed, marking true initiates a loop that continues to send the current chunks prompt until the module returns a False value. This option also enables a module to have access to previous notations",
                                                         "example": "false"},
                                "generate_title": {"instruction": "please generate a title for this query and place it in this value. To maintain continuity, the generated title for a given sequence is shared with subsequent queries, use undrscored for spaces.",
                                                   "example": "title_for_the_query"},
                                "notation": {"instruction": "this value is reserved entirely for the modules to communicate with one another thruought the query itteratons. These notations can be used to preserve relevant information or context that should be carried over to subsequent prompts.", "example": "selecting additional responses because completion tokens were not adequate."},
                                "suggestions": {"instruction": "A parameter that allows the module to provide suggestions for improving efficiency in future prompt sequences. These suggestions will be reviewed by the user after the entire prompt sequence is fulfilled.",
                                                "example": "consider ...."},
                                "abort": {"instruction": "if you cannot fullfil the request, respond with this value as true. be sure that you leave a detailed reason as to why the query stream was aborted in suggestions",
                                          "example": "False"},
                                "prompt_as_previous": {"instruction": "this is a user end declaration; if this is visible that means that the request portion of the prompt will change such that the previous response data will be available if needed",
                                                       "example": "True"},
                                "request_chunks": {"instruction": "you may request that the previous chunk data be prompted again, if selected, the query itterate once more with the previous chunk included in the prompt.",
                                                   "example": "False"},
                                "token_adjustment": {"instruction": "generally the token size of incoming queries is going to be static, because of this, if it will provide better results to increase or decrease the future allotment, place a number -100 up to 100 for a percentage adjustment for the remainder of the chunks and data",
                                                     "example": "0"},
                                }
        self.instructions = []
