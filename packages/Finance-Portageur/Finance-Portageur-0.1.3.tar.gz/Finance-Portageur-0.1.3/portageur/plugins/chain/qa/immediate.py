...
text_mapping = {
    ...
    "agent_system_prefix":"Answer the following questions as best you can. Please pur numbers in markdown table if possible. You have access to the following tools:"
}
...
class Immediate(object):
    def __init__(self, batch_size, text_content_description=None,
                 agent_system_prefix=None,
                 metadata=None, trans_name=None, callback=None):
        self.text_splitter = RecursiveCharacterTextSplitter.from_tiktoken_encoder(
            encoding_name='cl100k_base',chunk_size=500, chunk_overlap=20)
        
        self.init_model()
        self.init_weaviate(batch_size=batch_size)
        ...
        self.tools = [
            {'name':f"documents_qa",
             ...
             'description':f"A tool to answer questions based on company sustainability documents."
                        ...
                        "EXAMPLE: 'the employee health policy of SHEL'"}]
    ...
    def init_model(self):
        ...
        self.gpt35_short = AzureChatOpenAI(temperature=0, max_tokens=450, deployment_name="gpt-35-turbo",
                                           model_name="gpt-35-turbo", request_timeout=60, max_retries=2,
                                           streaming=True)
        
        self.gpt4_short = AzureChatOpenAI(temperature=0, max_tokens=700, deployment_name="gpt-4",
                                          model_name="gpt-4", request_timeout=60, max_retries=2,
                                          streaming=True)
        

    def init_weaviate(self, batch_size): 
    ...
    def format_citation(self, answer, citation_list):
        ...
        for source_idx, source_doc in enumerate(citation_list):
            ...
            if '-table-' in source_doc:
                can_docs = self.master_vs.get(where_filter={"path": ["source"],
                    "operator": "Equal",
                    "valueText": source_doc
                })[0].page_content
            elif '-paragraph-' in source_doc:
                can_docs = self.master_vs.get(where_filter={
                    "path": ["source"],
                    "operator": "Equal",
                    "valueText": source_doc
                })[0].page_content
            ...
            if len(can_docs) > 0:
                ...
                text_elements.append(
                        {'content':doc_text, 'name':source_name}
                    )
