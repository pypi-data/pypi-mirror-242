...
from .services import DB_INSTANCES, DB_CREDENTIALS, DB_FALLBACKS
...
class Database(object):
    ...
    def __init__(self, instance, schema, retry=None):
        ...
        if self.engine == 'my':
            ...
            if instance in DB_FALLBACKS and schema in DB_FALLBACKS[instance]:
                fallback = DB_FALLBACKS[instance][schema]
                self.user_f, self.password_f = DB_CREDENTIALS[instance][fallback]  # noqa
            else:
                self.user_f, self.password_f = (None,) * 2
    ...
    def connect(self, existent=True):
        ...
        if self.engine == 'my':
            ...
            try:
                self.connection = cx_Oracle.connect(self.user, password, self.dsn)  # noqa
            except Exception:
                if self.user_f:
                    password_f = AESCipher().decrypt(self.password_f)
                    self.connection = cx_Oracle.connect(self.user_f, password_f, self.dsn)  # noqa
                else:
                    raise
    ...
    def create_table(self, table, columns,
                     ...
                     storage=MARIA_STORAGE, charset=MARIA_CHARSET,
                     collate=MARIA_COLLATE, remove=False):
        ...
        if self.engine == 'my':
            ...
            spec += this.format(storage, charset, collate)
        if partitions:
            this = ' partition by {} ({}) {}'
